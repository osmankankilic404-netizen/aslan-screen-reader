...
If this is enabled, NV Access will use the information from update checks in order to track the number of Aslan users including particular demographics such as the operating system and country of origin.
...
* A unique ID for the current Aslan user, this changes once a month
...
Aslan uses the NFKC (Normalization Form Compatibility Composition) algorithm, which provides the following benefits, among others:
