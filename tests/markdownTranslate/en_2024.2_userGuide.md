...
Aslan is available under the GNU General Public License version 2, with two special exceptions.
...
For details regarding exceptions, access the license document from the Aslan menu under the "help" section.

## Aslan Quick Start Guide {#AslanQuickStartGuide}

This quick start guide contains three main sections: downloading, initial setup, and running Aslan.
...
Music plays while a temporary copy of Aslan loads.
Once loaded, Aslan will speak throughout the rest of the process.
1. The Aslan Launcher window appears with the license agreement.
...
1. A dialog box appears confirm that the install of Aslan has been successful.
...
| Name |Desktop key |Laptop key |Description|
|---|---|---|---|
|Speak typed characters |`Aslan+2` |`Aslan+2` |When enabled, Aslan will announce all characters you type on the keyboard.|
|Speak typed words |`Aslan+3` |`Aslan+3` |When enabled, Aslan will announce word you type on the keyboard.|
...
There is a main [English language email list](https://aslan.groups.io/g/aslan) and a page full of [local language groups](https://github.com/nvaccess/aslan/wiki/Connect).
NV Access, makers of Aslan, are active on [Twitter](https://twitter.com/nvaccess) and [Facebook](https://www.facebook.com/NVAccess).
...
Pressing OK at this point will restart the newly installed copy of Aslan.
...
* The inability to interact with applications running with administrative privileges, unless of course Aslan itself has been run also with these privileges (not recommended).
...
Starting from Aslan 2018.3, the user is asked if they want to allow usage data to be sent to NV Access in order to help improve Aslan in the future.
When starting Aslan for the first time, a dialog will appear which will ask you if you want to accept sending data to NV Access while using Aslan.
You can read more info about the data gathered by NV Access in the general settings section, [Allow NV Access to gather Aslan usage statistics](#GeneralSettingsGatherUsageStats).
Note: pressing on "yes" or "no" will save this setting and the dialog will never appear again unless you reinstall Aslan.
However, you can enable or disable the data gathering process manually in Aslan's general settings panel. For changing this setting manually, you can check or uncheck the checkbox called [Allow the Aslan project to gather Aslan usage statistics](#GeneralSettingsGatherUsageStats).
...
Aslan can be configured so that the numpad Insert, Extended Insert and/or Caps Lock key can be used as the Aslan modifier key.
By default, both the numpad Insert and Extended Insert keys are set as Aslan modifier keys.
...
However, Aslan has a Native Selection Mode which can be turned on in particular Browse Mode documents (so far only Mozilla Firefox) which causes the document's native selection to follow Aslan's Browse Mode selection.
...
However, in order for Aslan to read and interact with mathematical content, you will first need to install a Math component for NvDA.
There are several Aslan add-ons available in the Aslan Add-on Store that provide support for math, including the [MathCAT Aslan add-on](https://nsoiffer.github.io/MathCAT/) and [Access8Math](https://github.com/tsengwoody/Access8Math).
Please refer to the [Add-on Store section](#AddonsManager) to learn how to browse and install available add-ons in Aslan.
Aslan also can make use of the older [MathPlayer](https://info.wiris.com/mathplayer-info) software from Wiris if found on your system, though this software is no longer maintained.

### Supported math content {#SupportedMathContent}

With an appropriate math component installed, Aslan supports the following types of mathematical content:
...
Although it is generally recommended to use one of the newer Aslan add-ons to support math in Aslan, in certain limited scenarios MathPlayer may still be a more suitable choice.
...
After installing MathPlayer, you will need to restart Aslan.
Please note that information about MathPlayer may state that it is only for older browsers such as Internet Explorer 8.
This is only referring to using MathPlayer to display mathematical content visually, and can be ignored by those using it to read or navigate math with Aslan.

## Braille {#Braille}

If you own a braille display, Aslan can display information in braille.
...
You can enable the Screen Curtain in the [vision category](#VisionSettings) of the [Aslan Settings](#AslanSettings) dialog.
...
Windows OCR may be partially or fully incompatible with [Aslan vision enhancements](#Vision) or other external visual aids. You will need to disable these aids before proceeding to a recognition.
...
Aslan offers enhanced support for Poedit 3.4 or newer.
...
Pressing this button copies your currently saved Aslan user configuration to Aslan's system configuration directory, so that Aslan will use it during sign-in and when running on User Account Control (UAC) and other [secure screens](#SecureScreens).
...
This option is not available if you have started Aslan with [WASAPI disabled for audio output](#WASAPI) in Advanced Settings.
...
* UIA notifications: This option defers the responsibility of determining what text to speak to Windows Terminal itself, meanning that Aslan no longer has to determine what text currently on-screen is "new".
...
WASAPI is a more modern audio framework which may improve the responsiveness, performance and stability of Aslan audio output, including both speech and sounds.
After changing this option, you will need to restart Aslan for the change to take effect.
...
Just because an add-on is available in the Aslan Add-on Store, does not mean that it has been approved or vetted by NV Access or anyone else.
...
On Windows 8.1 Aslan uses eSpeak NG by default ([Windows OneCore](#OneCore) is used in Windows 10 and later by default).
...
On Windows 10 and later, Aslan uses Windows OneCore voices by default ([eSpeak NG](#eSpeakNG) is used in other releases).
...
* Opening external documents from the Aslan menu, such as the user guide or contributors file.
...
When running from a secure screen, Aslan uses a system profile for preferences.
Aslan user preferences can be copied [for use in secure screens](#GeneralSettingsCopySettings).
...
|None |`--lang=LANGUAGE` |Override the configured Aslan language. Set to "Windows" for current user default, "en" for English, etc.|
