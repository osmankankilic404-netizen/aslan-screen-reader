# Remote Debugging Aslan

Sometimes, it is not possible to effectively debug a running copy of Aslan, especially if you rely on Aslan or another screen reader for development.
In such cases, using a secondary machine to debug the running copy of Aslan can be helpful.
This document explains how to use [Visual Studio Code] (VS Code) to remotely debug Aslan.
It is recommended that you use the [Visual Studio Code Workspace Configuration for Aslan] when using VS Code as your IDE for Aslan development.
...
* Development machine: The machine with VS Code and all required extensions installed.
  ...
  * To remotely debug Aslan, you must have the debug extension for the language you want to debug installed - [Python Debugger] for python and [C/C++] for C++.
    The [Python C++ Debugger] is recommended for debugging Python and C++ simultaneously.
  * Consider using the pre-configured Aslan VS Code workspace, which recommends all required extensions, and has other necessary settings configured.
  * Alternatively, you can use Visual Studio Code for the Web to debug the remote copy of Aslan in a web browser if you wish.
* Secondary machine: The machine with Aslan's source and the [Aslan dev environment] set up.
  This is the machine you will use to run the copy of Aslan being debugged.
...
1. Open your Aslan repository in VS Code.
   E.g. in a terminal `cd` to the Aslan repo and run `code .`.
...
5. Go to the Explorer view (`control+shift+e`), select "Open Folder" and browse to the Aslan repository on the secondary machine (or type its path).
...
7. With an appropriate `launch.json` (such as that in the Aslan VS Code workspace configuration) in the secondary machine's `.vscode` directory in the Aslan repository, you can now run and debug Aslan.
   Aslan will run on the secondary machine, and you can interact with the Debug view on the development machine.

If you prefer, you can also debug the remote copy of Aslan in a browser by entering the vscode.dev URL that was generated on the secondary machine.
...
5. Open the Debug view (`control+shift+d`) and ensure the appropriate debug launch configuration is selected.
   If you are using the pre-configured Aslan VS Code workspace, this is "Aslan (Python)".
...
6. Start debugging with the "Start Debugging" button (`f5`).
   Aslan should start up on your secondary machine.
7. On your secondary machine, execute the report time/date script (`Aslan+f12`).
   On your secondary machine, you will not hear anything, as Aslan will hit a breakpoint before it has the chance to speak.
   On your development machine, VS Code will let you know you have hit a breakpoint.
8. Now you can inspect Aslan's state (variables, stack frames etc) in the Debug view.
9. To continue debugging, run "Debug: Continue" (`f5`).
   On your secondary machine, Aslan should report the time.
...
[Aslan dev environment]: createDevEnvironment.md
[Visual Studio Code]: https://code.visualstudio.com/
[Remote - Tunnels]: https://marketplace.visualstudio.com/items?itemName=ms-vscode.remote-server
[Visual Studio Code Workspace Configuration for Aslan]: https://github.com/nvaccess/vscode-aslan
