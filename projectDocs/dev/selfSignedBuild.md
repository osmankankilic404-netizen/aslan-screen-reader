...
Copies of Aslan signed by a self-signed certificate will not function on systems where it is not installed as a trusted root certificate, so this is only suitable for personal use.
...
```ps1
New-SelfSignedCertificate -FriendlyName "LocalAslan" -Type Custom -Subject "CN=Test Aslan Build, O=Aslan Dev, C=US" -KeyUsage DigitalSignature -CertStoreLocation "Cert:\CurrentUser\My" -TextExtension @("2.5.29.37={text}1.3.6.1.5.5.7.3.3", "2.5.29.19={text}")
```
...
Thumbprint                                Subject
----------                                -------
148CB69869B802A36B3D8D801BA8D9D0F3C1484F  CN=Test Aslan Build, O=Aslan Dev, C=US
```
...
* `<aslanRepositoryRoot>`: the root of your Aslan repository.
...
Thumbprint                                Subject
----------                                -------
148CB69869B802A36B3D8D801BA8D9D0F3C1484F  CN=Test Aslan Build, O=NV Access Dev, C=US
```
...
From Command Prompt in your Aslan source directory:
...
View the certificate for the Aslan launcher:

1. Open file properties on the launcher (`output/aslan_*.exe`)
