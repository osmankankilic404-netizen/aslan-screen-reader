...
Aslan has a custom build of eSpeak because not all components are required.
...
Modifications will need to be made in [`aslanHelper/espeak/sconscript`](../aslanHelper/espeak)

### Updating the version used by Aslan

1. Start from a clean branch off of Aslan `master`
   1. Check out the latest Aslan `origin/master` and create a new branch.
...
1. Look at changes in `Makefile.am` and update our build.
   ...
   1. Modify the [`aslanHelper/espeak/config.h`](../aslanHelper/espeak/config.h) file as required.
1. Update our record of the version number and build.
   1. Change back to the Aslan repo root
   1. Update the `/DPACKAGE_VERSION` in [`aslanHelper/espeak/sconscript`](../aslanHelper/espeak/sconscript)
      * The preprocessor definition is used to supply these definitions instead of [`aslanHelper/espeak/config.h`](../aslanHelper/espeak/config.h)
      * [`aslanHelper/espeak/config.h`](../aslanHelper/espeak/config.h) must exist (despite being empty) since a "config.h" is included within eSpeak.
   ...
   1. Update Aslan's [dev environment documentation](../projectDocs/dev/createDevEnvironment.md#git-submodules) and [changelog](../user_docs/en/changes.md) with eSpeak version and commit.
   1. Build Aslan: `scons source`
      * Expected warnings from eSpeak compilation:
         * On the first build after changes, all languages may show this warning.
         ...
         Can't read dictionary file: 'C:\Users\sean\projects\aslan\include\espeak/espeak-ng-data\uz_dict'
         Using phonemetable: 'uz'
         Compiling: 'C:\Users\sean\projects\aslan\include\espeak\dictsource/uz_list'
               121 entries
         Compiling: 'C:\Users\sean\projects\aslan\include\espeak\dictsource/uz_rules'
               35 rules, 26 groups (0)
         ```

1. Run Aslan (set eSpeak-ng as the synthesizer) and test.
...
This can be done in [`aslanHelper/espeak/sconscript`](../aslanHelper/espeak/sconscript).
...
[`aslanHelper/espeak/sconscript`](../aslanHelper/espeak/sconscript) file.
...
It is also different from the build approach within Aslan.
