...
* Při aktualizaci doplňků Aslan oznámí verzi doplňku. (#5324)
...
* Pokud používáte laptop nebo tablet, Aslan oznámí, jestli se baterie nabíjí či nikoliv, a změny při otáčení obrazovky. (#4574, #4612)
...
* V režimu prohlížení ve Firefoxu a Chrome při pohybu klávesou tab nebo rychlé navigaci Aslan oznámí názvy skupin formulářových prvků. (#3321)
...
* Úpravy souborů podle verzí (uživatelská nastavení, hlasové slovníky) se neukládají, pokud Aslan běží jako dočasná verze. (#7688)
* Starší braillské řádky Braillino, Bookworm a Modular společnosti Handy Tech nejsou již podporovány ve výchozím nastavení Aslan. Pro jejich použití je třeba nainstalovat Handy Tech Universal Driver a doplněk Aslan. (#7590)

### Opravy

* Odkazy jsou nyní zobrazeny v braillu v aplikacích jako Microsoft Word. (#6780)
* Aslan již není nápadně pomalý, pokud je otebřeno více záložek v prohlížečích Firefox nebo Chrome. (#3138)
...
* Ve Windows 10 Creators Update je Firefox znovu přístupný po restartu Aslan. (#7269)
* Při restartu Aslan, pokud je Mozilla Firefox v zaměření, režim prohlížení bude znovu dostupný, i když možná budete muset přejít sem a tam klávesami alt+tab. (#5758)
* Matematický obsah je nyní přístupný v Google Chrome, aniž by musel být v systému nainstalovaný Mozilla Firefox. (#7308)
* Operační systém a další aplikace by měly být stabilnější hned po instalaci Aslan před restartem ve srovnání s předchozími verzemi Aslan. (#7563)
* Při rozpoznávání obsahu (např. Aslan+r) Aslan ohlásí chybu, pokud není objekt již dostupný. (#7567)
...
* In Browse mode for MSHTML (e.g. Internet Explorer) and Gecko (e.g. Firefox), the existance of long descriptions are now announced. It's also possible to open the long description in a new window by pressing Aslan+d. (#809)
...
-Aslan dokáže detekovat podtržený a přeškrtnutý text v editačních polích IAccessible2 (např. v aplikacích Mozilla). 
-Po pádu se Aslan automaticky restartuje. 

### Opravy

•V programu internet explorer a ostatních prvcích MSHTML Aslan nebude oznamovat odrážky a číslování v seznamech, pokud autor nastavil, že se tyto informace nemají zobrazovat (styl seznamu je none). (#1671) 
-Restart Aslan po zamrznutí (např. stiskem kláves Ctrl+Alt+N) nespustí znovu Aslan dřív, než se ukončí zamrznutá kopie.
...
* Aslan nyní ve webových prohlížečích oznamuje seřazené položky (vlastnost aria-sort). (#1500)
* Braillské znaky zapsané v kódu unicode jsou nyní správně zobrazovány na braillských řádcích. (#1505)
* Když se v Internet Exploreru a nebo jiných MSHTML prvcích přesune fokus do seskupení prvků uzavřeného do tagu fieldset, Aslan ohlásí název seskupení (vlastnost legend). (#535)
...
* V aplikaci Microsoft Word bude nyní při pohybu po tabulce pomocí klávesy Tab Aslan ohlašovat aktuální buňku. (#159)
* V dialogu ohlašování formátování dokumentu je možné nastavit oznamování souřadnic buněk v tabulkách. (#719)
* Aslan nyní dokáže detekovat formátování a barvu textu vypisovaného přímo na obrazovku.
* V seznamu správ v programech Outlook express, Windows mail a Windows live bude Aslan ohlašovat, že správa je nepřečtená a rovněž stav konverzací. (#868)
...
* Pro přesunutí textového kurzoru na čtecí kurzor nyní použijte dvojitý stisk klávesové zkratky pro přesun fokusu na prohlížený objekt (desktop Aslan+Shift+numerické mínus, laptop Aslan+Shift+Backspace) Tím se uvolnilo více kláves pro případné použití do budoucna. (#837)
...
* Uživatelské rozhraní a dokumentace Aslan místo označení virtuální prohlížeč používá označení režim prohlížení, protože termín virtuální prohlížeč většině uživatelů nic neříká. (#1509)
...
* Služba Aslan už nezastavuje a opětovně nnespouští Aslan na uživatelských vstupních plochách.
* Na systémech Windows XP a Windows Vista již Aslan nepoužívá UI automation, i když je tato funkce dostupná. Přestože UI automation vylepšuje přístupnost některých moderních aplikací, tak při jeho použití na těchto systémech docházelo k mrznutí, pádům a dalším problémům. (#1437)
...
* Aslan hlásí stav okna, pokud se pohybujete po prvcích uvnitř. Např. pokud se pohybujete uvnitř dokumentu, který se teprve načítá, Aslan ohlásí "zaneprázdněn".
* Pro některé položky v nabídkách, které mají podnabídky už není hlášeno "sbaleno". 
* Příkaz pro ohlášení formátování Aslan+f už neoznamuje formátování na pozici textového, ale na pozici čtecího kurzoru. Ve výchozím nastavení se čtecí kurzor pohybuje spolu s textovým kurzorem, takže většina uživatelů si změny ani nevšimne. Nicméně díky této změně je nyní možné zjistit informace o formátování při čtení čtecím kurzorem např. v režimu plošného prohlížení

### Opravy

* Jestliže byl formulářový režim zapnut pomocí kláves Aslan+mezerník, sbalení rozbalovacího políčka už nepřepíná automaticky zpět do režimu prohlížení. (#1386)
* V dokumentech Gecko a MSHTML je nyní text nacházející se na jednom řádku správně interpretován rovněž na jednom řádku. Ve starších verzích Aslan byl rozdělen do více řádků. (#1378)
...
* Pokud používáte rozložení kláves pro laptop, klávesová zkratka Aslan+delete správně oznámí rozměry prohlíženého objektu.
* V dokumentech MSHTML už správně funguje atribut aria-selected.
* Když se Aslan automaticky přepne z režimu prohlížení do formulářového režimu, je nejprve ohlášena informace o zaměřeném objektu. Např. pokud se fokus nachází na seznamu, bude hned po aktivaci formulářového režimu ohlášen seznam. (#1491)
...
* Pokud se fokus nachází na editačním poli jen pro čtení, Aslan to ohlásí. (#1436)
* V režimu prohlížení Aslan nyní správně zpracovává editační pole jen pro čtení.
* Při práci s prvkem, který má nastaven atribut aria-activedescendant (automatické dokončování v některých seznamech a pod.), už Aslan chybně nepřepíná z formulářového režimu do režimu prohlížení.
...
* Pokud v XFA PDF dokumentu dojde ke změně hodnoty rozbalovacího políčka, Aslan to ohlásí.
...
* Pokud ve výukovém režimu držíte stisknutou Aslan klávesu , již nedochází k hlášení, jako by klávesa působila sama na sebe, např. Aslan+Aslan. To je nesmysl a bylo to opraveno.
...
* Pokud je zobrazen dialog se zprávou, nemůžete vyvolat dialog s nastavením Aslan, protože v takovém případě okno s nastavením způsobovalo mrznutí programu. (#1451)
* V aplikaci Microsoft Excel již není patrná prodleva když rychle označujete nebo se pohybujete po buňkách.
* Opraveny občasné pády služby Aslan, které způsobovaly že Aslan nemluvilo na zabezpečených obrazovkách.
* Opraven problém s braillskými řádky, kdy změna způsobovala zmizení zobrazeného textu.
* Okno pro stahování v Internet Exploreru 9 je nyní možné používat s Aslan. (#1280)
* Bylo zabráněno nechtěným spuštěním více instancí Aslan současně. (#507)
* Na pomalých systémech již Aslan při běhu nezobrazuje své hlavní okno. (#726)
* Při spuštění WPF aplikace na systémech Windows XP již Aslan nepadá. (#1437)
...
* V aplikacích Outlook Express a Windows Live Mail v dialogu Použít Pravidla už Aslan chybně neoznačuje některé seznamy jako zaškrtávací políčka. (#576)
* U rozbalovacích políček už není chybně hlášena podnabídka.
* Aslan už dokáže číst příjemce v polích komu, kopie a skrytá kopie v aplikaci Microsoft Outlook. (#421)

= 2011.1.1 =
Tato verze opravuje některé bezpečnostní problémy a jiné závažné chyby ve verzi 2011.1.

== Opravy ==
* Položka darovat v nabídce Aslan není zobrazená na přihlašovací a ostatních zabezpečených obrazovkách Windows, protože je to bezpečnostní riziko. (#1419)
* Při běhu Aslan na přihlašovací nebo jiných zabezpečených obrazovkách Windows není možné používat funkce kopírovat, vyjmout a vložit, protože to představuje bezpečnostní riziko. (#1421)
* V programu Firefox 4 správně funguje klávesová zkratka Aslan+Ctrl+mezerník na přechod do nadřazeného virtuálního prohlížeče. Tímto způsobem je opět možné opustit vnořené objekty např. prvky Adobe flash. (#1429)
* Pokud je zapnuté ohlašování příkazových kláves, Aslan už nečte písmena stisknutá v kombinaci s klávesou Shift jako příkazové klávesy. (#1422)
* Pokud je zapnuté ohlašování příkazových kláves, Aslan nyní správně ohlašuje stisk mezerníku spolu s  jakoukoliv modifikační klávesou kromě shiftu jako příkaz. (#1424)
* Záznam logu do souboru je při běhu Aslan na přihlašovací a dalších zabezpečených obrazovkách Windows zakázaný, protože je to bezpečnostní riziko. (#1435)
...
* Pro některé objekty lze nastavit ohlašování barev. Automatické oznamování změny barvy je možné nastavit v dialogu Ohlašování formátování dokumentu. Aktuální barvu je rovněž možné zjistit použitím funkce informace o formátování (Aslan+F)
...
* Aslan nyní automaticky ohlašuje nový text  v programech mIRC, PuTTY, Tera Term a SecureCRT. (#936)
...
* Podpora pro globální zásuvné moduly. Globální zásuvné moduly mohou přidat nové funkce do Aslan, které jsou potom k dispozici v jakékoli aplikaci (#281)
* Jestliže je při psaní zapnuta klávesa Caps lock a zároveň je přistisknuta klávesa Shift, Aslan krátce pípne. Tuto funkci lze vypnout odškrtnutím nové volby v dialogu nastavení klávesnice. (#663)
...
* Abychom ulehčili práci případným dalšým vývojářům, lze nyní znovu načíst aplikační a globální zásuvné moduly bez nutnosti restartu Aslan. Použijte položku Znovu načíst zásuvné moduly z nabídky nástroje nebo klávesovou zkratku Aslan+Ctrl+F3. (#544)
* Nyní si Aslan pamatuje pozici ve virtuálním prohlížeči a při návratu na navštívenou stránku tuto pozici obnový. Toto platí, dokud neukončíte Aslan nebo prohlížeč. (#132)
...
* Když Aslan detekuje zamrznutí, i nadále zachytává stisk Aslan klávesy, přičemž ostatní klávesy propouští do systému. To zabrání nechtěnnému přepínání klávesy Caps lock. (#939)
* Pokud po vyvolání příkazu pro propouštění kláves stisknete nějakou kombinaci více kláves a nějaké další přistisknete, aslan je všechny bude propouštět operačnímu systému, dokud neuvolníte poslední stisknutou klávesu
* Pokud stisknete klávesu Aslan dvakrát rychle po sobě, aby byla propuštěna, tak každé další stisknutí této klávesy bude rovněž propuštěné do operačního systému.
...
* V dialozích řečových slovníků je obsah sloupců Rozlišovat velikost písmen a Regulární výraz zobrazován v jazyce nastaveném v programu Aslan. Doposud byli tyto informace dostupné jen v angličtině.
...
* Jestliže stisknete klávesu Aslan  dvakrát rychle po sobě a k tomu stisknete další klávesy, tak každé následné stisknutí klávesy Aslan nebude propuštěné do systému.
* Ve výukovém režimu jsou nyní čteny interpunkční znaky, i když je ohlašování interpunkce vypnuté. (#977)
* V dialogu nastavení klávesnice jsou názvy  rozložení klávesnice zobrazeny v jazyce nastaveném v programu Aslan a ne v angličtině. (#558)
...
* Tlačítko "Použít aktuální nastavení Aslan pro přihlašovací a další zabezpečené obrazovky" v dialogu obecné nastavení je možné použít hned po instalaci Aslan. Dříve, pokud nebyla před použitím tohoto tlačítka zobrazená zabezpečená obrazovka, Aslan oznámil, že kopírování bylo úspěšné, ale akce neměla žádný efekt. (#1194)
* V této verzi už není možné mít otevřených několik dialogů s nastavením Aslan současně. To opravuje problémy s nastaveními v otevřeném dialogu, která jsou závislá na nastaveních z druhého otevřeného dialogu (např. změna hlasového výstupu při otevřeném dialogu nastavení hlasu). (#603)
* V systémech se zapnutou kontrolou UAC už tlačítko "Použít aktuální nastavení Aslan pro přihlašovací a další zabezpečené obrazovky" po zobrazení dialogu UAC pracuje správně, i když jméno aktuálně přihlášeného uživatele systému obsahuje mezeru. (#918)
* V programu internet explorer a ostatních prvcích MSHTML Aslan použije URL odkazu v případě, že není k dispozici žádný text nebo název. (#633)
* Aslan už neignoruje fokus v nabídkách programu AOL Instant Messenger 7. (#655)
* Aslan nyní správně ohlašuje názvy chyb v dialogu kontrola pravopisu programu Microsoft word (např. není ve slovníku, pravopisná chyba, interpunkce). Doposud byly všechny chyby ohlašované jako pravopisná chyba. (#883)
* Psaní v programu microsoft word se zapnutým brailským řádkem by už nemělo způsobovat vkládání náhodného textu, a rovněž byla opravena chyba, která při stisku směrovacích tlačítek na braillském řádku způsobovala zamrznutí Aslan. (#1212) nicméně problémem zůstává fakt, že v programu Word 2003 a starších není možné číst text ve směru zprava do leva (např. arabský text) spolu s braillským řádkem. (#627)
* Po stisku klávesy delete v editačním poli je správně aktualizovaný text a pozice kurzoru na braillském řádku. (#947)
* Dynamické změny v gecko 2 dokumentech (např. Firefox 4) jsou nyní Programem Aslan správně zpracované. Doposud byli zpracované jen změny na první otevřené kartě. (Mozilla bug 610985)
* Aslan dokáže správně oznamovat seznam návrhů v dialogu kontrola pravopisu programu Microsoft word. (#704)
* V programu internet explorer a jiných prvcích MSHTML už Aslan ve virtuálním prohlížeči nezobrazuje cíle odkazů do této stránky , ale jsou správně skryté tak, jak by měli být. (#1326)
* Objektová navigace přes a uvnitř seskupení už není nesimetrická.
* Ve Firefoxu se již Aslan nezasekne ve vnořeném rámci pokud se jeho obsah načte dřív než obsah nadřazeného dokumentu.
* Aslan správně oznámí následující znak, pokud se pro vymazání aktuálního znaku použije numerický delete. (#286)
...
* Na braillských řádcích ALVA BC640/BC680 už Aslan neopomíná klávesy, které zůstávají stisknuté i po uvolnění jiných kláves.
* Adobe Reader už nepadá po opuštění dialogu "neotagovaný dokument" před tím, než začne samotné zpracování dokumentu. (#1218)
* Při znovu načtení konfigurace Aslan je nyní načten správný ovladač braillského řádku. (#1346)
* průvodce projektem v programu Visual Studio 2008 je opět správně přečtený. (#974)
* Aslan už neodmítá pracovat v aplikacích, jejichž spustitelný soubor obsahuje v názvu jiné než ascii znaky. (#1352)
* Při zapnutém zalamování v programu Akelpad Aslan Při čtení po řádcích už nečte na konci řádku první písmeno následujícího řádku.
* V editoru kódu programů Visual Studio 2005/2008 Aslan už nečte celý text po každém napsaném znaku. (#975)
* Byl Opraven problém, když při změně braillského řádku nebo při ukončení Aslan nebyly některé řádky správně smazané.
* Po spuštění Aslan už čas od času není informace o aktivním prvku oznamovaná dvakrát. (#1359)
...
* Aslan nyní umí ohlašovat formátování textových prvků i v aplikacích využívajících Java Access Bridge (včetně OpenOffice.org). (#358, #463)
* Při pohybu myší po buňkách v Microsoft Excelu bude nyní Aslan oznamovat jejich text.
...
* Aslan již dokáže zachytit většinu textu vypisovaného na obrazovku. (#40, #643)
...
* Při označení celého řádku nebo sloupce v programu Microsoft excel stiskem kláves Shift+Mezerník a Ctrl+Mezerník Aslan nyní správně ohlašuje nový výběr. (#759)
 ...
 * Aslan+numerická 7 přepíná do plošného prohlížení. Nastaví čtecí kurzor na aktuální pozici právě zaměřeného objektu. Takto můžete začít prohlížet obsah obrazovky či dokumentu, (jste-li zrovna uvnitř nějakého) prostřednictvím příkazů pro prohlížení textu.
 * Aslan+numerická 1 Zapne objektové prohlížení na aktuální pozici čtecího kurzoru. Je vyhledán objekt, jehož text právě čtete v režimu plošného prohlížení a čtecí kurzor je umístěn do tohoto objektu, aby bylo možno daný objekt prohlížet a pohybovat se po dalších objektech.
* Aktuální uživatelské nastavení Aslan lze nyní stiskem tlačítka v dialogu Obecné nastavení zkopírovat pro použití na přihlašovací a dalších zabezpečených obrazovkách systému windows.
* Podpora pro Mozilla Firefox 4.
* Podpora pro Microsoft Internet Explorer 9.

### Změny

* Příkazy pro přečtení celého prohlíženého objektu (Aslan+numerické Plus), následujícího objektu v návaznosti (Aslan+Shift+numerická 6) a předchozího objektu v návaznosti (Aslan+Shift+numerická 4) byly prozatím odstraněny kvůli chybovosti a také, aby byly tyto klávesy uvolněny pro případné pozdější přiřazení k novým funkcím.
...
* Pokud se fokus nachází uvnitř vnořené aplikace nebo ve virtuálním prohlížeči uvnitř jiného virtuálního prohlížeče (např. Flash), můžete nyní pro opuštění tohoto objektu a návrat do domovského dokumentu použít Aslan+Ctrl+mezerník. K tomuto účelu dříve sloužila kombinace pouze Aslan+mezerník, ta však nyní slouží výhradně pro přepínání mezi formulářovým režimem a režimem prohlížení.
...
* Ukládání konfigurace a změna některých nastavení je zablokovaná, pokud Aslan běží na přihlašovací obrazovce.
* Hlasový výstup eSpeak byl aktualizován na verzi 1.44.03.
* Pokud je Aslan již spuštěn, aktivace zástupce na ploše nebo stisk klávesové zkratky Ctrl+Alt+N zajistí restart Aslan.
...
* V Internet Exploreru a dalších prvcích MSHTML je nyní atribut ARIA role upřednostňován před jinými informacemi o typu, aby tak Aslan umožňoval mnohem přesnější a intuitivnější práci s prvky ARIA.
...
* Je-li při startu Aslan již spuštěn program Skype, není nutné ho restartovat, aby byl přístupný. To může platit i pro ostatní aplikace, které kontrolují systémovou značku přítomnosti odečítače.
* V aplikacích Microsoft Office už Aslan nepadá při pokusu o přečtení okna aplikace v popředí (Aslan+B) nebo při pohybu po některých objektech na panelu nástrojů. (#616)
...
* Aslan opět dokáže v některých problematických víceřádkových editačních polích programu Internet explorer 8 číst po řádcích . (#467)
* Windows Live Messenger 2009  se už hned po spuštění automaticky neukončuje, jestliže je spuštěn Aslan. (#677)
...
* Aslan nyní správně funguje v systému, kde byl z cesty  DLL search path (pomocí nastavení položky registry CWDIllegalInDllSearch na 0xFFFFFFFF) odstraněn aktuální adresář. Toto pro většinu uživatelů není důležité. (#907)
...
* Pokud pomocí navigačních kláves není možné přejít do další buňky v tabulce programu Microsoft word proto, že kurzor je na okraji tabulky, je hláška okraj tabulky přeložená do aktuálně nastaveného jazyka programu Aslan. Doposud byla tato informace vyslovována jen v angličtině. (#921)
...
* Spuštění Aslan už nebude selhávat na systémech, které nemají žádná výstupní zvuková zařízení. Aby mohl v takovém případě Aslan předávat uživateli informace, bude pochopitelně třeba použít braillský řádek nebo výstup bez mluvení v kombinaci s novým nástrojem prozobrazení řeči, který nahrazuje předchozí virtuální hlasový výstup Display. (#425)
* Do dialogu ohlašování formátování dokumentu byla přidána volba hlásit oblasti stránky, která určuje, zda má Aslan při pohybu po webových stránkách ohlašovat tzv. oblasti stránky. Z důvodů zajištění kompatibility s předchozí verzí je tato funkce standardně zapnutá.
* Při zapnutém ohlašování příkazových kláves bude Aslan hlásit i názvy tzv. multimediálních kláves (např. přehrát, zastavit přehrávání, domovská stránka atd) na mnoha klávesnicích. (#472)
* Aslan od této verze ohlašuje při stisku control+backspace právě smazané slovo u těch prvků, které to umožňují. (#491)
...
* Aslan lépe podporuje vnořené editovatelné dokumenty v Internet Exploreru. (#402)
* Nový příkaz (Aslan+shift+numerické mínus) umožňuje přesunout systémový fokus na právě prohlížený objekt.
...
* Nyní lze při instalaci Aslan zakázat vytvoření zástupce na ploše, a tedy i zkratkové klávesy. (#518)
* Aslan dovede pracovat s rozhraním Iaccessible2 i v 64 bitových aplikacích. (#479)
* Vylepšená podpora live regionů v aplikacích Mozilla. (#246)
* Bylo vyvinuto rozhraní, které umožňuje externím aplikacím pracovat s Aslan; např. přečíst text, přerušit řeč, zobrazit zprávu na braillském řádku atd.
...
* V programu Miranda IM nyní Aslan dokáže automaticky číst příchozí zprávy v oknech konverzace, pokud je zapnuto ohlašování dynamicky změněného obsahu (Aslan+5). Také byly přidány příkazy pro přečtení tří nejnovějších zpráv (Aslan+ctrl+číslo 1, 2, nebo 3).
...
* Syntezátor Display, virtuální hlasový výstup, byl nyní nahrazen nástrojem nazvaným zobrazení řeči. Lze jej zapnout zvolením položky zobrazení řeči z nabídky Aslan, menu nástroje. Může být aktivní nezávisle na tom, který hlasový syntezátor právě používáte. (#44)
...
* Nastavení, zda se má zobrazování na braillském řádku řídit systémovým či čtecím kurzorem (Aslan+ctrl+T), je nyní možné měnit také v dialogu braillské nastavení, a rovněž se ukládá do konfiguračního souboru.
...
* Spuštění Aslan už nebude selhávat v případě, kdy je zdánlivě dostupná podpora UI Automation, ale z nějakého důvodu se neinicializuje. (#483)
* Při přesunu fokusu v rámci buňky tabulky v dokumentech Mozilla už Aslan nebude číst celý řádek tabulky. (#482)
* Odstraněna dlouhá prodleva odezvy Aslan při rozbalování větví stromového seznamu, které obsahují velmi mnoho vnořených položek.
* Při vypisování SAPI 5 hlasů se Aslan nyní pokouší rozpoznat problémové hlasy a ty pak nezpřístupní v dialogu Nastavení hlasu  ani v kruhu nastavení hlasového výstupu. Pokud se v systému dříve vyskytoval byť jen jediný problémový hlas, Aslan někdy nedokázal načíst svůj SAPI 5 ovladač.
...
* Prázdné řádky v dokumentech Microsoft Wordu a v editovatelných prvcích Microsoft HTML jsou nyní náležitě reflektovány i na braillském řádku. Aslan v takových případech dříve zobrazoval aktuální větu, nikoli aktuální řádek. (#420)
* Několik bezpečnostních oprav v situaci, kdy Aslan běží při přihlašování do systému nebo na jiných zabezpečených obrazovkách. (#515)
...
* Ve virtuálním prohlížeči už Aslan nezobrazuje text obrázků uvnitř odkazů a ostatních kliknutelných objektů, které jsou označeny jako irelevantní pro odečítače obrazovky. (#423)
...
* Při práci s TeamTalkem 3 nebo TeamTalkem 4 Classic už Aslan neohlašuje změny indikátoru průběhu týkající se VU meteru v hlavním okně. Rovněž národní znaky jsou nyní správně čteny v okně příchozího chatu.
...
* Aslan už nebude chybně číst určitá čísla oddělená pomlčkou, např. 500-1000. (#547)
* Ve Windows XP už Aslan nebude způsobovat zamrzání Internet Exploreru při přepínání zaškrtávacích políček na stránce Windows Update. (#477)
...
* Aslan už neoznamuje, že dokument Firefoxu se stal zaneprázdněným (např. díky aktualizaci nebo obnovení), u dokumentů na pozadí. To také vedlo k mylnému ohlašování stavového řádku aplikace v popředí.
...
* Dočasné řešení nepříjemné chyby v některých SAPI 4 hlasových výstupech, která působila časté potíže  a pády Aslan. (#597)
...
K tomuto účelu byla vytvořena speciální verze výstupu Newfon pro Aslan. (#206)
...
* Při použití kláves pro změnu hlasitosti počítače Aslan už nepřerušuje řeč, takže změny jsou okamžitě slyšitelné. (#287)
...
* Jestliže se v systému Windows Vista zobrazí UAC dialog, což  je dialog, který vyvolává zapnutý nástroj pro řízení uživatelských účtů, nebo jestliže uživatel přejde stiskem kláves ctrl+alt+del na zabezpečenou pracovní plochu, aslan je schopen s těmito obrazovkami systému pracovat a plně je ozvučit. Jsou podporovány i veškeré zabezpečené obrazovky v systému Windows XP včetně ozvučení přihlašovací obrazovky. To je možné zapnout či vypnout v dialogu obecné nastavení. Pro práci se zabezpečenými obrazovkami systému má Aslan zvláštní konfigurační soubor. Díky tomu je možné nastavit jiné chování Aslan pro běžnou práci v systému a pro práci se zabezpečenými obrazovkami. Pozor! Toto se netýká přenosné verze Aslan. (#97)
* V okně příkazového řádku dokáže aslan ohlašovat text pod kurzorem.
* Podpora pro UI Automation prostřednictvím klientského rozhraní UI Automation v systému Windows 7 a další opravy vylepšující kompatibilitu Aslan s tímto operačním systémem.
* V dialogu obecné nastavení přibyla položka pro nastavení automatického spouštění Aslan po přihlášení do systému. Pozor! Toto se netýká přenosné verze Aslan.
...
* Dialog seznam odkazů, jenž lze ve virtuálním prohlížeči vyvolat stiskem kláves Aslan+F7, byl přejmenován na  seznam prvků, ve kterém je možné krom odkazů zobrazit i nadpisy a oblasti stránky. Nadpisy a oblasti stránky jsou zobrazeny jako stromový seznam. (#363)
...
* Přenosná verze Aslan nyní ukládá uživatelská nastavení do složky 'userConfig', která je umístěna ve složce programu Aslan. Uživatelská nastavení jsou tedy stejně jako u instalační verze Aslan oddělena od ostatních souborů programu.
...
* Dokumenty virtuálního prohlížeče jsou nyní načítány na pozadí, což umožňuje do jisté míry pracovat se systémem i během procesu načítání. Aslan oznámí, že se zpracovává dokument, pokud to trvá déle než jednu sekundu.
* Jestliže Aslan zjistí, že z nějakého důvodu zamrzl, začne automaticky propouštět všechny klávesy do systému, takže uživatel může problém snadněji vyřešit.
* Podpora přetahování objektů ARIA v produktech Mozilla myší. (#239)
* Při přepnutí do okna s dokumentem virtuálního prohlížeče Aslan nyní oznámí jeho název a aktuální řádek, případně označený text. Chování při přepnutí do okna virtuálního prohlížeče je tedy stejné jako v případě oken běžných dokumentů. (#210)
* Ve virtuálním prohlížeči je nyní možné pracovat s vnořenými objekty (jako je obsah Adobe Flash nebo Sun Java), jestliže na takovém objektu stisknete enter. Je-li pro Aslan přístupný, můžete se po něm poté pohybovat šipkami jako v běžných aplikacích. Stiskem Aslan+mezerník se lze přepnout zpět do vlastního dokumentu. (#431)
* Ve virtuálním prohlížeči byly přidány klávesy o a shift+o, jimiž se lze přesunout na další či předchozí vnořený objekt.
* Aslan má nyní plný přístup i k aplikacím spuštěným jako administrátor v systému Windows Vista nebo 7. Pozor! Toto se netýká přenosné verze Aslan ani snapshotů, ale jen oficiálních verzí Aslan. (#397)

== Změny ==
* Po spuštění Aslan byla odstraněna hláška "Aslan spuštěn". Start Aslan je nyní signalizován pouze úvodní znělkou.
* Znělky při spuštění a ukončení Aslan jsou nyní přehrávány prostřednictvím výstupního zvukového zařízení nastaveného pro Aslan, nikoli pomocí výchozího výstupního zařízení Windows. Toto nastavení se nachází v dialogu hlasové výstupy, položka výstupní zařízení. (#164)
* Bylo vylepšeno ohlašování indikátorů průběhu. Nejpatrnější změnou je to, že nyní můžete Aslan nastavit, aby je oznamoval pípáním i hlasem zároveň.
...
* Příkaz pro kopírování textu čtecím kurzorem (Aslan+F10) nyní zkopíruje text od nastavené počáteční značky až po momentální pozici kurzoru, kterou také do kopírovaného úseku textu zahrne, místo, aby ji ignoroval. Tak je nyní možné kopírovat i poslední znak daného řádku, což dříve možné nebylo. (#430)
* Příkaz pro zjištění aktuálně prohlíženého objektu (Ctrl+Aslan+numerická 5) byl odstraněn. Tato klávesová kombinace na některých klávesnicích nefungovala a příkaz ani nebyl shledán příliš užitečným.
* Příkaz pro zjištění rozměrů aktuálně prohlíženého objektu byl přemapován na Aslan+numerický delete neboli numerická desetinná čárka. Předchozí klávesová zkratka nefungovala na některých klávesnicích. Příkaz navíc také nyní oznamuje skutečnou výšku a šířku objektu namísto pouze pravé a dolní souřadnice.
...
* V RC a finálních verzích už Aslan nepřehrává chybový zvuk, nicméně chyby jsou stále zaznamenávány.
* Namísto "propouštění zapnuto" a "propouštění vypnuto" Aslan nyní ohlašuje "formulářový režim" (místo propouštění zapnuto) a "režim prohlížení" (místo propouštění vypnuto).
* Dialog nastavení čtení formátování textu byl přejmenován na ohlašování formátování dokumentu

### Opravy

* Pokud je Aslan spuštěn z dosovské cesty 8.3, ale přitom je nainstalován ve složce s odpovídající dlouhou cestou (např. progra~1 proti program files), dokáže správně určit, že se jedná o instalovanou kopii a načíst uživatelská nastavení.
* Ohlašování titulku aktuálního okna v popředí pomocí Aslan+T nyní funguje správně i v nabídkách.
...
* Vylepšená podpora pro dosovské konzole. Konkrétně: Aslan nyní dokáže přečíst obsah i takových konzolí, o kterých si vždycky myslel, že jsou prázdné. Stisk ctrl+break už neukončí Aslan.
* Pokud při instalaci Aslan ve Windows Vista nebo 7 zaškrtnete na závěrečné obrazovce, aby byl po dokončení instalace Aslan spuštěn, spustí se s oprávněními přihlášeného uživatele.
...
* Aslan nyní dokáže správně pracovat s návěstími objektů ARIA v produktech Mozilla, jestliže nejsou dostupné žádné jiné použitelné informace. (#156)
* Aslan se již automaticky nepřepíná do formulářového režimu u editačních polí, která mění svou hodnotu při změně fokusu; např. https://tigerdirect.com/. (#220)
* Aslan se nyní bude pokoušet zotavit z některých situací, které by dříve způsobily jeho úplné zamrznutí. Zjištění a vyřešení takové situace může trvat až deset sekund.
* Je-li jazyk Aslan nastaven na "user default", použije se nastavený jazyk zobrazení Windows namísto místních nastavení uživatele. (#353)
* Aslan nyní dokáže rozpoznat existenci prvků v AIM7.
* Příkaz propustit následující klávesu se již nezasekne, pokud některou klávesu podržíte. Dříve by Aslan v takovém případě přestal přijímat jakékoli další příkazy a musel by být restartován. (#413)
* Aslan již neignoruje úlohovou lištu, pokud se na ni přesune fokus, což se často děje např. při ukončování aplikací. Dříve se Aslan choval, jako by se fokus vůbec nezměnil.
* Při čtení textových polí v aplikacích, které používají Java Access Bridge (včetně OpenOffice.org), Aslan nyní pracuje správněi při zapnutém ohlašování čísel řádků.
* Příkaz pro kopírování textu čtecím kurzorem (Aslan+F10) se nyní dokáže korektně vypořádat se situací, kdy je použit na pozici před počáteční značkou. To by dříve vedlo k problémům, jako je např. pád programu Notepad++.
...
* Příkaz pro přečtení označeného textu (Aslan+shift+šipka nahoru) nyní správně ohlásí, že není vybrán žádný text, pokud daný objekt nepodporuje označování textu.
* Opravena chyba, kdy Aslan při stisku enteru na některých tlačítkách a odkazech v programu Miranda IM zamrzal. (#430)
...
* Aslan nyní obchází chybu Windows, která způsobovala, že po názvu odkazů v dialozích Průzkumníku Windows a Internet Exploreru byly ohlašovány irelevantní informace navíc. (#451)
* Opraven problém s příkazem pro ohlášení času a data (Aslan+F¨12). Na některých systémech dříve nebylo ohlašováno datum. (#471)
* Opravena chyba, která způsobovala, že po ukončení práce se zabezpečenou obrazovkou Windows Aslan někdy náležitě neaktualizoval systémové nastavení odečítače obrazovky. To mohlo vést k problémům v aplikacích, které toto nastavení sledují jako je Skype, Adobe Reader nebo Jart. (#462)
...
* Protože lišta se vzorci v Microsoft Excelu je pro Aslan nepřístupná, byl přidán vlastní Aslan editor buněk, který se spustí, pokud uživatel stiskne na kterékoli buňce F2.
* Podpora formátování v textových prvcích Iaccessible2, včetně aplikací Mozilla.
* Aslan nyní může oznamovat pravopisné chyby, kde je to možné.Toto nastavení se nachází v dialogu ohlašování formátování dokumentu.
* Aslan nyní lze nastavit tak, aby pípal buď pro všechny indikátory průběhu, nebo pouze pro ty v okně na popředí. Další možností je ohlašovat hodnotu indikátorů průběhu každých 10%.
...
* Aslan nyní umí zpracovat některé další stavy Java prvků.
* Je-li klávesová zkratka pro oznámení titulku okna (Aslan+t) stisknuta dvakrát, titulek bude vyhláskován. Pokud je stisknuta třikrát, bude titulek zkopírován do schránky.
...
* Přidán příkaz (Aslan+c) pro přečtení textu ve schránce. (#193)
* Pokud se Aslan ve virtuálním prohlížeči automaticky přepne do formulářového režimu, můžete se nyní vrátit do režimu prohlížení také stiskem klávesy escape. Stále je možné používat k tomuto účelu i kombinaci kláves Aslan+mezerník.
* Ve virtuálním prohlížeči se Aslan nyní může při změně fokusu nebo při pohybu textového kurzoru automaticky přepnout buď do formulářového režimu, nebo do režimu prohlížení, podle toho, jaký typ prvku se pod kurzorem nachází. Tuto volbu je možné nastavit v dialogu virtuální prohlížeč. (#157)
...
* Aplikace Aslan v sobě nyní obsahuje manifest, což znamená, že už se ve Windows Vista nespouští v režimu kompatibility.
* Instalační verze Aslan nyní ukládá konfigurační soubor a řečové slovníky do složky pro data aplikací aktuálního uživatele. To je nezbytné pro správnou funkčnost Aslan v systému Windows Vista a také to umožňuje více uživatelům najednou mít na jednom počítači uložena zvlášť svá individuální nastavení.
* Doplněna podpora pro informace o pozici u prvků Iaccessible2.
* Přidána možnost kopírovat text do schránky pomocí čtecího kurzoru. Nejprve nastavte zkratkou Aslan+F9 počáteční značku na aktuální pozici čtecího kurzoru. Poté stiskem Aslan+F10 zkopírujete blok textu mezi počáteční značkou a novou aktuální pozicí čtecího kurzoru, kde má kopírovaný úsek končit. Tuto funkci využijete zejména v situaci, kdy není možné text z daného prvku zkopírovat běžným způsobem pomocí zkratek ctrl+c a ctrl+v. (#240)
* Přidána podpora pro některé editovatelné prvky v softwaru pinacle tv.
* Pokud je označen dlouhý úsek textu (512 znaků nebo více), Aslan nyní pouze oznámí počet označených znaků, místo, aby četl celý text. (#249)

### Změny

* Pokud je Aslan nastaven, aby pro přehrávání zvuků a hlasový výstup používal výchozí zvukové zařízení Windows (např. Microsoft Sound Mapper), dokáže se přepnout na nově připojené výstupní zařízení, pokud se toto zařízení stane výchozím. Aslan se tedy dokáže automaticky přepnout třeba na USB zvukovou kartu, pokud ji systém po připojení registruje jako nové výchozí zařízení. Platí to pro zvuková návěstí Aslan a pro vestavěný hlasový výstup Espeak.
...
* Příkaz pro přečtení titulku okna (Aslan+t) nyní oznamuje pouze samotný název okna namísto celého objektu pro název. Pokud objekt v popředí nemá žádný název, bude oznámen název procesu aplikace, pod kterým je vidět např. ve Správci úloh.
...
* Poslední část právě čteného textu už nebude usekávána při použití Aslan s eSpeakem na vzdáleném počítači.
...
* Prohlížeč log souborů Aslan nyní zobrazuje text v richedit poli namísto pouhého editačního políčka. Lze se nyní s Aslan lépe pohybovat v textu po slovech.
* Odstraněny jisté problémy s vnořenými objekty v richedit prvcích.
* Aslan nyní čte čísla stránek v Microsoft Wordu. (#120)
* Opraven problém ve virtuálním prohlížeči pro produkty Mozilla, kdy Aslan při stisku mezerníku na zaškrtnutém zaškrtávacím políčku neoznámil, že políčko bylo právě odškrtnuto.
...
* Opraven problém, který způsoboval, že označený text ve standardních editačních prvcích Windows nebyl čten, dokud se poprvé po spuštění Aslan nezměnil fokus.
* Opraveno sledování myši v objektech Java. (#185)
* Aslan už neohlašuje položky stromových seznamů Java bez dalších větví jako sbalené.
* Pokud se do popředí dostane aplikace Java, bude oznámen zaměřený objekt. Dříve by Aslan oznámil pouze Java objekt nejvyšší úrovně.
...
* Aslan nyní ignoruje změny fokusu v oknech na pozadí. Dříve by změnu fokusu v pozadí mohl pokládat za změnu skutečného fokusu.
* Vylepšeno vyhledání fokusu při opouštění kontextových nabídek. Dříve Aslan po opuštění kontextové nabídky často vůbec nezareagoval.
* Aslan nyní oznámí kontextovou nabídku, i pokud je vyvolána z nabídky start.
...
* Aslan už nadále neoznamuje odznačení textu při opuštění některých editovatelných prvků (příklad: adresní řádek Internet Exploreru, políčka pro e-mailovou adresu v Thunderbirdu 3).
...
* Aslan nyní dokáže přečíst text delší než 65535 znaků i ve standardních editovatelných prvcích (např. velký soubor v Poznámkovém bloku).
...
* Aslan už nebude občas kompletně zamrzat při editování textu v Open Office. (#148,#180)

## 0.6p2

* Vylepšen výchozí hlas eSpeaku pro Aslan.
...
* Přidány zkratkové klávesy pro většinu konfiguračních dialogů Aslan.
* Podpora aplikací využívajících standardu Iaccessible2 jako je Mozilla Firefox i při spouštění Aslan z výměnných médií bez nutnosti registrovat jakékoli speciální dll knihovny.
* Odstraněn problém s padajícím seznamem odkazů ve virtuálním prohlížeči v produktech Mozilla. (#48)
* Aslan by už neměl způsobovat pád aplikací Mozilla jako jsou třeba Firefox a Thunderbird, pokud běží s vyššími právy, než samotná aplikace Mozilla např. je-li Aslan spuštěn jako administrátor.
...
* V dialogu virtuální prohlížeč je nyní možné nastavit, jestli se má Aslan u dokumentů virtuálního prohlížeče snažit zachovat stejný vzhled jako na obrazovce.
...
* Příkaz Aslan pro vyhledávání zadaného textu si nyní bude pamatovat poslední výraz, který jste hledali a to ve všech aplikacích. (#53)
* Opraven problém, kdy u některých zaškrtávacích políček a přepínačů ve virtuálním prohlížeči Aslan neoznamoval stav zaškrtnutí.
* Formulářový režim ve virtuálním prohlížeči se nyní nastavuje konkrétně pro každý dokument zvlášť, nikoli globálně pro Aslan. (#33)
* Odstraněna určitá prodleva odezvy při změnách fokusu a nesprávné přerušování řeči, pokud byl Aslan používán v systému, který je velice pomalý nebose právě probudil z úsporného režimu.
...
* Přidán nástroj pro interaktivní Python konzoli Aslan, aby vývojáři mohli sledovat a upravovat funkce Aslan i za chodu.
...
* Odstraněny skripty pro zvýšení a snížení tempa hlasu. K tomuto účelu je nyní třeba použít buď příkazy pro nastavení parametrů v kruhu nastavení hlasového výstupu (Ctrl+Aslan+šipky,) nebo přímo dialog nastavení hlasu.
...
* Do nového virtuálního prohlížeče byl přidán příkaz pro vyhledání předchozího výskytu naposledy hledaného textu (shift+Aslan+F3).
* Odstraněna pomalá odezva v dialozích Mozilla Firefoxu a Thunderbirdu. (#66)
* Přidána možnost zobrazit aktuální log soubor Aslan (zápis činnosti programu od posledního spuštění až do okamžiku ukončení). Tato volba se nachází v nabídce Aslan, menu nástroje.
* Příkazy jako např. ohlášení času a data nyní berou v potas nastavený jazyk; interpunkce a slovosled se přizpůsobuje jazyku.
* Pro snazší použití nyní rozbalovací seznam jazyků v Aslan dialogu obecné nastavení zobrazuje plné názvy jazyků.
...
* Opravena chyba, která při opuštění aplikačního menu v aplikacích jako např. Wordpad způsobovala, že Aslan přestával číst označený text.
...
* Odstraněna počáteční prodleva při otevírání dialogů Aslan na některých systémech. (#65)
...
* Příkaz pro přesun myši na právě prohlížený objekt (Aslan+numerické lomeno) nyní přesune myš doprostřed objektu místo do levého horního rohu.
...
* Pokud při pohybu v editačním poli držíte kurzorovou šipku a dostanete se tak na jeho konec, Aslan bude rychleji reagovat a již nebude číst nadbytečný text.
* Při čtení některých zpráv už není nutné čekat, než Aslan domluví. Tím bylo odstraněno padání a zamrzání některých hlasových výstupů. (#117)
...
* Odstraněno případné padání Aslan při pokusu uložit nastavení na neanglických verzích Windows. (#114)
* Přidán uvítací dialog. Tento dialog má poskytnout novým uživatelům základnní informace o používání Aslan a také umožňuje nastavit capslock jako Aslan klávesu. Bude se zobrazovat při každém spuštění Aslan, pokud to uživatel nepřenastaví.
* Opravena základní podpora Adobe readeru, takže je možné číst dokumenty i ve verzích 8 a 9.
* Odstraněny určité chyby, ke kterým mohlo docházet, pokud uživatel stiskl některou klávesu ještě předtím, než byl Aslan náležitě inicializován.
* Jestliže je zapnutá volba, aby se při ukončení Aslan ukládala nastavení, Aslan se ujistí, že nastavení byla skutečně správně uložena i při vypínání počítače nebo odhlašování uživatele z Windows.
* Při spuštění instalačního programu se nyní přehraje nové zvukové logo Aslan vytvořené Victerem Tsaranem.
* Všechny verze Aslan, instalační i přenosná, by nyní měly při ukončení spolehlivě vymazat ze systémové lišty svou ikonu.
* Názvy standardních prvků v dialozích Aslan (jako jsou tlačítka ok a storno) by se nyní měly zobrazovat v nastaveném jazyce Aslan, místo, aby zůstávaly v angličtině.
* Ikona Aslan nyní bude použita pro zástupce Aslan v nabídce start a na ploše, místo výchozí ikony pro aplikace.
...
* Zlepšeno sledování systémového kurzoru v aplikacích Iaccessible2 a Java; např. v Open Office nebo Lotus Symphony nyní Aslan skutečně počká, až se kurzor přestane pohybovat po dokumentu, místo, aby omylem přečetl jakékoli slovo nebo řádek na konci některého odstavce. (#119)
* Podpora pro prvky AkelEdit, které se nacházejí v programu Akelpad 4.0.
* Aslan se už nezasekne v Lotus Symphony při přesunu z dokumentu do aplikačního menu.
* Aslan už nezamrzne v ovládacím panelu přidat nebo odebrat programy systému Windows XP při spuštění odinstalace některého programu. (#30)
* Aslan už nezamrzá při spuštění Spybot Search and Destroy.
...
* Přidán dialog pro seznam odkazů (Aslan+F7).
* Vylepšen Aslan dialog pro vyhledávání (Ctrl+aslan+F), takže nerozlišuje velikost písmen a opraveny jisté problémy s fokusem v tomto dialogu.
* V novém virtuálním prohlížeči je nyní možné označovat a kopírovat text.
* Nový virtuální prohlížeč standardně zobrazuje dokumenty ve stejné podobě jako na obrazovce - odkazy a prvky nejsou na samostatných řádcích, pokud tak skutečně nejsou umístěny vizuálně. Tuto funkci můžete přepínat klávesami Aslan+v.
...
== Internacionalizace ==
* Nyní je možné psát národní znaky, které závisí na tzv. mrtvém znaku i za chodu Aslan.
* Aslan nyní oznamuje změny rozložení klávesnice při stisku Alt+Shift.
...
* Aslan je nyní dodáván spolu s eSpeakem 1.33, který obsahuje mnoho vylepšení. K nim patří např. upravené jazykové verze, pojmenované varianty hlasů, možnost mluvit rychleji.
...
* Aslan nyní může pípat při čtení velkých písmen. Toto nastavení lze ovlivnit zaškrtávacím políčkem v dialogu nastavení hlasu. Bylo také přidáno políčko "zvýšit hlas při čtení velkých písmen", kterým lze pro Aslan nastavit tuto dříve standardní interpretaci velkých písmen. Takže nyní mohou být velká písmena oznamována buď zvýšením hlasu, pípnutím, nebo jen slovem velké. Tyto možnosti mohou být libovolně kombinovány.
* Doplněna možnost pozastavit mluvení (jako je tomu u odečítače Voiceover pro operační systém Mac). Kdykoli Aslan něco říká, můžete stiskem klávesy Ctrl nebo shift přerušit řeč a pokud později opět stisknete shift, (jestliže jste do té doby nestiskli nic jiného), řeč bude pokračovat přesně od toho místa, kde byla přerušena.
* Přidán ovladač pro virtuální hlasový výstup, který vypisuje text do okna namísto předčítání hlasem. Tento způsob by měl být příjemnější pro vidící vývojáře, kteří nejsou zvyklí na hlasovou syntézu, ale chtějí vědět, co Aslan říká. Tato funkce má pravděpodobně ještě chyby, takže zpětná vazba je rozhodně velice vítána.
* Aslan už nadále nebude implicitně číst interpunkci. Čtení interpunkce můžete opět povolit zkratkou Aslan+p nebo přímo v dialogu nastavení hlasu.
* eSpeak nyní standardně mluví o dost pomaleji, takže noví uživatelé, kteří ho slyší poprvé při instalaci Aslan nebo celkově s Aslan teprve začínají, by mu nyní měli snáze rozumět.
* Do Aslan byly přidány uživatelské slovníky. S jejich pomocí můžete Aslan přimět, aby vyslovoval daný text určitým způsobem. Slovníky jsou celkem tři: výchozí, pro aktuální hlas a dočasný. Položky, které přidáte do výchozího slovníku, budou mít při běhu Aslan neustálou platnost. Slovník pro aktuální hlas je určen pouze pro hlasový výstup a konkrétní hlas, který je právě používán a dočasný slovník je navržen pro ty případy, kdy chcete rychle určit nějaké pravidlo při provádění konkrétního úkolu, ale nepřejete si, aby fungovalo stále (bude smazáno při ukončení Aslan). Prozatím musí být pravidla zadávána jako regulární výrazy, nikoli jako normální text.
...
* Aslan už nezabírá podstatnou část systémové paměti při editaci zpráv v editovatelných prvcích mshtml.
...
* Aslan už by neměl postupně zabírat stále více místa v systémové paměti bez jakéhokoli důvodu.
* Odstraněny chyby při pokusu umístit fokus do okna dosovské konzole zhruba třikrát za sebou. Aslan měl v takových případech tendenci úplně padat.

### Klávesové příkazy

* Nové klávesové zkratky Aslan+Shift+numerická 6 a Aslan+Shift+numerická 4 umožňují přesun na další, resp. předchozí nejbližší objekt v řadě (na stejné úrovni jako aktuální objekt). To znamená, že nyní můžete veškerou objektovou navigaci po aplikaci provádět pouze těmito dvěma klávesami, bez nutnosti strachovat se, že se omylem při procházení objektovou hierarchií přesunete na nadřazený objekt nebo na první z podřazených vzhledem k úrovni aktuálního objektu. Například ve webovém prohlížeči, jako je Firefox, se nyní můžete pohybovat po dokumentu pomocí objektové navigace s využitím pouze těchto dvou kláves. Pokud vás posun po objektech v řadě přemístí v objektové struktuře o úroveň výš nebo níž, Aslan to dá najevo odpovídajícím pípnutím znázorňujícím pohyb v příslušném směru.
* Nyní je možné měnit nastavení hlasu přímo bez nutnosti otevírat dialog nastavení hlasu, použitím tzv. kruhu nastavení hlasového výstupu. Jedná se o skupinu jednotlivých parametrů hlasu, mezi nimiž se můžete přepínat kombinacemi kláves Ctrl+Aslan+šipka doprava a Ctrl+Aslan+šipka doleva. Každou z hodnot jednotlivých parametrů pak můžete nastavovat kombinacemi kláves Ctrl+Aslan+šipka nahoru a Ctrl+Aslan+šipka dolů. Změny se projeví okamžitě.
* Přidán příkaz pro přečtení aktuálně označeného textu v editačním poli (Aslan+Shift+šipka nahoru).
* Celá řada příkazů Aslan pro přečtení určitého úseku textu např. aktuální řádek atd nyní umí text vyhláskovat, pokud jejich zkratkovou klávesu stisknete dvakrát rychle po sobě.
* Klávesy capslock, numerický insert a insert nyní mohou být všechny použity jako Aslan klávesa. Pokud je takto některá z těchto kláves použita a stisknete-li ji dvakrát rychle za sebou bez použití dalších kláves, Aslan ji propustí do systému, jako byste ji stiskli, kdyby Aslan nebyl vůbec spuštěn. Pokud chcete kteroukoli z těchto kláves nastavit jako Aslan klávesu, zaškrtněte její odpovídající zaškrtávací políčko v dialogu nastavení klávesnice (dříve byl tento dialog pojmenován odezva klávesnice).
...
* Aslan nyní dokáže číst e-mailové adresy v příslušných políčkách Outlook Expressu (komu, od, kopie atd).
* Aslan by nyní měl být přesnější při oznamování následující zprávy v Outlook Expressu po smazání některé zprávy ze seznamu zpráv.
...
* Doplněna podpora pro rozhraní Iaccessible2 vylepšující přístupnost některých aplikací. Nejen, že tak Aslan může číst více typů prvků, ale také díky Iaccessible2 dokáže v aplikacích jako Firefox 3 či Thunderbird 3 manipulovat s kurzorem, což vám umožňuje čtení a označování textu.
...
* Do dialogu nastavení myši byla přidána nová volba. Je-li zaškrtnuto "pohyb myši indikovat zvukovým signálem", pak se při každém pohybu myši přehraje 40 milisekund dlouhé pípnutí, jehož výška (mezi 220 a 1760 Hz) reprezentuje osu y (vertikální pozici na obrazovce) a hlasitost v levém a pravém kanále zase osu x (horizontální pozici na obrazovce). Tak může nevidomý člověk získat při pohybu myší přibližnou představu, kde na obrazovce se právě nachází. Tato funkce také závisí na tom, jestli je zapnuto ohlašování objektu pod kurzorem myši. Pokud budete potřebovat rychle vypnout jak ohlašování objektu pod myší, tak pípání, stačí stisknout Aslan+m. Pípání je také hlasitější nebo tišší podle toho, jaký jas má dané místo na obrazovce.

### Prezentace objektů a interakce s nimi

* Vylepšena podpora většiny standardních stromových seznamů. Aslan nyní při rozbalení kterékoli větve oznámí, kolik dalších položek obsahuje. Také při rozbalení či sbalení libovolné položky oznamuje úroveň vnoření, číslo aktuální položky a celkový počet všech položek v seznamu vzhledem k aktuální větvi, nikoli k celému seznamu.
* Vylepšeno oznamování změn fokusu při pohybu po aplikacích a v operačním systému. Nyní už namísto pouhého jména prvku, na němž je fokus nově umístěn, uslyšíte i informace o všech ostatních prvcích, uvnitř kterých se tento prvek nachází. Například, pokud umístíte fokus na tlačítko v rámci seskupení, Aslan nyní oznámí i toto seskupení.
* Aslan se nyní snaží číst při objevení dialogu i jeho obsah. Většinou se mu to daří poměrně přesně, i když ještě existuje mnoho dialogů, se kterými nepracuje tak dobře, jak by mohl.
* Do dialogu pro nastavení prezentace objektů byla přidána nová volba "hlásit popisky objektů: Pokročilí uživatelé si ji někdy mohou přát vypnout, aby zabránili Aslan ve čtení mnoha podrobných popisků u některých prvků jako např. v Java aplikacích.
* Pokud se fokus přemístí na editační políčko, Aslan nyní automaticky oznámí v tomto políčku označený text. Jestliže není označen žádný text, bude přečten aktuální řádek.
* Aslan nyní přehrává pípnutí pro oznámení hodnot indikátorů průběhu v různých aplikacích mnohem opatrněji. Nejvýrazněji to ovlivňuje práci v aplikacích od Eclipse např. Lotus Symphony nebo Accessibility Probe.

### Uživatelské rozhraní

* Okno aplikace Aslan bylo odstraněno a nahrazeno prostou kontextovou nabídkou Aslan.
* Dialog pro nastavení uživatelského rozhraní byl přejmenován na obecné nastavení. Obsahuje nyní také jednu novou volbu: rozbalovací seznam pro určení úrovně logování, tj. jaké zprávy se mají zapisovat do Aslan log souboru. Poznámka: Aslan log soubor se od této verze jmenuje aslan.log, nikoli debug.log.
...
* Aslan nyní obsahuje vestavěný hlasový syntezátor nazvaný eSpeak, vyvíjený Jonathanem Duddingtonem. Má velice rychlou odezvu, je velmi kompaktní a podporuje mnoho různých jazyků. Je stále možné používat SAPI syntezátory, ale eSpeak bude používán jako výchozí hlas.
* Espeak není závislý na žádném dalším speciálním softwaru, takže ho můžete použít spolu s Aslan na kterémkoli počítači, na USB klíčence nebo prostě na čemkoli. Pro více informací o eSpeaku nebo pro stažení aktuální verze navštivte https://espeak.sourceforge.net/.
* Odstraněna chyba při mazání znaku v editačních polích Internet Exploreru a Outlook Expressu, kdy Aslan četl nesprávný znak.
...
* Přidány parametry pro spouštění Aslan z příkazového řádku: -m, --minimal: Nepřehrávat při spuštění a ukončení znělky a nezobrazovat po spuštění dialog rozhraní, pokud je Aslan takto nastaven. -q, --quit: Ukončit všechny případné běžící instance Aslan (nejprve všechny ostatní a poté tu aktuální). -s, --stderr-file název_souboru: určit, kam má Aslan ukládat nezachycené chyby a výjimky. -d, --debug-file název souboru: Určit, kam má Aslan ukládat ladicí zprávy. -c, --config-file: Určit alternativní konfigurační soubor. -h, -help: Zobrazit nápovědnou zprávu s výpisem parametrů pro spuštění Aslan z příkazového řádku.
* Odstraněna chyba, kdy interpunkční znaménka nebyla překládána do správného jazyka, pokud byl jazyk Aslan nastaven na jiný, než angličtinu, a pokud byla zapnuta volba hlásit psané znaky.
...
* Více zpráv Aslan bylo nyní uzpůsobeno pro překlad do jiných jazyků. Patří k nim i popisy příkazů v klávesové nápovědě.
...
* Při zapnuté volbě hlásit psané znaky nyní může být oznamováno více znaků. Technicky řečeno nyní dokáže Aslan ohlásit při psaní ascii znaky s hodnotami v rozmezí 32 až 255.
...
* Pro rychlejší navigaci už Aslan nehlásí typ prvku při pohybu šipkami popoložkách běžného či stromového seznamu (položka seznamu, položka stromového seznamu atd).
...
* V případě, kdy některé jazyky používají Ctrl nebo Alt (případně altGR) k zadávání speciálních znaků, bude Aslan tyto znaky hlásit při zapnuté volbě hlásit psané znaky.
* Opraveny problémy při prohlížení statických textových prvků.
* Doplněn překlad pro tradiční čínštinu, zásluhou Coscella Kaa.
* Byla přepracována důležitá část kódu Aslan, čímž by měly být eliminovány problémy s uživatelským rozhraním (včetně konfiguračních dialogů).
* Do Aslan byla doplněna podpora pro Sapi4. Můžete použít dva ovladače, jeden je založený na kódu od společnosti Serotek Corporation a druhý využívá standardního ActiveVoice.ActiveVoice com rozhraní. Zkuste, který vám bude fungovat lépe.
* Jestliže se nyní pokusíte spustit novou kopii Aslan, pokud zároveň ještě běží starší, nově spuštěná kopie bude zkrátka ukončena. Tím odpadá dřívější problém při spouštění více kopií Aslan najednou, jehož důsledkem byl pak systém prakticky nepoužitelný.
* Dialog uživatelského rozhraní byl přejmenován na "Aslan" místo "rozhraní Aslan".
...
* Přidán hlasový ovladač nazvaný Silence (bez mluvení). Při jeho používání Aslan nic neříká, čímž je možné zajistit jeho stálé mlčení. Tento ovladač by mohl být v budoucnu využit v kombinaci s podporou braillských řádků, až bude do Aslan doplněna.
...
* Opravena chyba, kdy by Aslan buď přestal mluvit nebo by úplně spadl v okně dosovské konzole.
* Pokud existuje podpora pro konkrétní jazyk, Aslan nyní může automaticky zobrazovat své rozhraní a číst své zprávy v jazyce nastaveném ve Windows. Je však stále možné také zvolit určitý jazyk ručně v dialogu nastavení uživatelského rozhraní.
* Přidán nový příkaz "hlásit dynamicky změněný obsah" (Aslan+5); Přepíná, jestli má být automaticky oznamován nový text a další dynamické změny. Prozatím však funguje pouze v oknech dosovských konzolí.
* Přidán příkaz "textový kurzor pohybuje čtecím kurzorem" (Aslan+6). Přepíná, zda má být čtecí kurzor automaticky přizpůsobován poloze systémového kurzoru. Je užitečný při čtení textu v oknech dosovských konzolí, když se mění informace na obrazovce.
* Přidán příkaz "změna fokusu změní prohlížený objekt" (Aslan+7). Určuje, zda má být při změně fokusu právě prohlížený objekt nastaven na objekt pod fokusem.
* Doplněny překlady dokumentace do různých jazyků. Prozatím se to týká francouzštiny, španělštiny a finštiny.
* Odstraněna vývojářská dokumentace zbinární distribuce Aslan. Nyní je dostupná pouze v distribuci zdrojového kódu.
...
* Nové zprávy v okně konverzace MSN Messengeru budou nyní čteny automaticky. Toto zatím funguje pouze v anglické verzi Aslan.
* Okno historie v konverzaci Windows Live Messengeru je nyní čitelné. Toto zatím funguje pouze v anglické verzi Aslan.
* Přidán příkaz "propustit následující klávesu" (Aslan+F2). Stisknete-li tuto klávesu, tak jakákoli následující stisknutá klávesa bude odeslána přímo do Windows. To je užitečné, pokud potřebujete stisknout v nějaké aplikaci určitou klávesovou zkratku, ale Aslan ji používá k jinému účelu.
* Aslan už nebude zamrzat na déle než minutu při otevírání velmi rozsáhlých dokumentů v Microsoft Wordu.
* Odstraněna chyba, která při opuštění tabulky v MS Wordu a opětovném vstupu do ní způsobovala, že Aslan už nehlásil čísla aktuálního řádku a sloupce, pokud jste se vrátili přesně do téže buňky.
* Při spuštění Aslan s nastaveným hlasem, který neexistuje nebo nepracuje správně, se Aslan pokusí načíst výchozí hlas rozhraní Sapi5. Pokud ani to nefunguje, bude spuštěn bez mluvení.
...
* Aslan se nyní při změně jazyka v dialogu nastavení uživatelského rozhraní zeptá, zda má uložit nastavení a restartovat se. ABy se změna jazyka plně projevila, musí být Aslan restartován.
* Pokud není možné při volbě hlasového výstupu v dialogu hlasové výstupy zvolený syntezátor načíst, Aslan na tuto skutečnost upozorní.
* Při prvním načítání zvoleného syntezátoru nechá Aslan syntezátor, aby si sám zvolil nejvíce vyhovující hlas, tempo a výšku, místo, aby mu vnutil svá výchozí nastavení, která mu připadají v pořádku. Tím je odstraněn problém, kdy Sapi4 syntezátory eloquence a viavoice začínaly na to, že byly spouštěny poprvé, mluvit příliš rychle.
