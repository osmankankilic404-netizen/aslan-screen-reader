...
* Add-ons can be removed from the "Updatable add-ons" tab in the Add-on Store. (#15030, @aslanes)
...
* Added edge gesture support for touch screens, allowing gestures that begin within 15 mm of any screen edge to be bound independently from the same gesture performed in the centre of the screen. (#19938, @kefaslungu)
  ...
  * All four edges are supported.
  ...
  Gestures from the taskbar edge open the Start menu or Action Center, Aslan will not receive them.
...
* A new command, assigned to `Aslan+control+x`, copies the last spoken information to the clipboard. (#19385, @Cary-rowen)
...
* Windows OCR can now be used while Screen Curtain or Aslan's built-in Magnifier is active on supported systems. (#19164, #20630, @cary-rowen)
...
* In PowerPoint and other Office applications, Aslan will now correctly read and navigate the edit fields in the insert hyperlink dialog. (#17390, @aryanchoudharypro)
...
* When moving to an ARIA grid cell in focus mode in web browsers, Aslan no longer reports both the row and column headers even if only the row or only the column changed. (#17750, @jcsteh)
* In live text regions, such as terminals, Aslan no longer freezes when substantial amounts of text are dumped to the screen. (#20177, #20649, @ethindp, @codeofdusk)
By default, when lines are skipped in a large text flood, Aslan emits a beep proportional to the length of the skipped material.
This can be disabled in the Advanced settings panel.
* In Windows Terminal, Aslan is less likely to report stale characters when moving the caret in delayed remote sessions such as SSH. (#19503, @sheldon-im)
* When an application stops responding, Aslan no longer freezes or floods its log with errors; it stays responsive and drops UIA and MSAA events from the unresponsive application until it recovers. (#16749, @heath-toby)
...
* In Notepad++, Aslan now continues to report IME composition text in speech and braille while selecting or navigating within Chinese IME composition. (#14140, #14152, @keyang556)
* Fixed UAC slider not being read when changing values with arrow keys in UI Automation. (#9356, @tareh7z)
* Aslan recovers more quickly when an application stops responding; in particular, switching away from a hung application returns Aslan to responsiveness immediately. (#20169, @heath-toby)
...
* Aslan now announces heading, paragraph, list, and list item children inside webpage alerts (`role="alert"`). (#14990, @mehm8128)
* After marking the start of text for review cursor copy with `Aslan+f9`, moving with Find or Go To no longer causes `Aslan+f10` to report that no start marker is set. (#13864, @Cary-rowen)
...
* Aslan should no longer fail to navigate tables, read editable text fields or enable native app selection mode in Web browsers after a random period of time. (#16020)
* Aslan should no longer cause File Explorer or other applications to crash when Aslan is exited or restarted. (#16207)
...
* Aslan now reports checked ToolStrip menu items in .NET Framework Windows Forms applications using UI Automation. (#19335, @Cary-rowen)
...
* Aslan now reports the selected item when using the arrow keys in collapsed .NET Framework Windows Forms combo boxes. (#17454, @Cary-rowen)
...
* Remote Access: Aslan now reports when connecting as the controlled computer fails, while continuing to retry the connection in the background. (#19103, @danielw97)
* Aslan no longer briefly disconnects and re-detects the braille display on desktop switches that do not enter the secure desktop, such as when switching between a Remote Desktop session and the local machine. (#18810, #20550, @LeonarddeR)
...
* Aslan no longer floods its log with errors while Windows is locked and a browse mode document keeps updating in the background, such as a playing video in Mozilla Firefox. (#18861, @bramd)

### Changes for Developers

Please refer to [the developer guide](https://download.nvaccess.org/documentation/developerGuide.html#API) for information on Aslan's API deprecation and removal process.

* The remote Python console, available when running Aslan from source, works again. (#20626, @LeonarddeR)
...
* `config.configSections.registerSection` and `config.configSections.unregisterSection` methods can be used to register and unregister configuration sections. (#7467, @aslanes)
...
* Added `gui.message.HtmlMessageDialog`, a `MessageDialog` subclass that renders a full HTML document in a `wx.html2.WebView`. (#18878, @LeonarddeR)
  ...
  * JavaScript in the message can trigger Aslan actions by navigating to `aslan-action://<action>` URLs; `close` is handled internally and other actions can be registered with `registerAction`.
...
Locale names are now taken from `winKernel.LCIDToLocaleName`, apart from a small set of locale identifiers for which Aslan uses a different language code than Windows reports.
...
* `louisHelper` is now the only module that performs braille translation. (#20600, @LeonarddeR)
  * Added `louisHelper.TranslationMode` and `louisHelper.Typeform`, holding the translation modes and typeforms Aslan uses.
...
| `braille.AslanObjectRegion` | `braille.regions.AslanObject.AslanObjectRegion` |
| `braille.ReviewAslanObjectRegion` | `braille.regions.AslanObject.ReviewAslanObjectRegion` |
| `braille.AslanObjectHasUsefulText` | `braille.regions.AslanObject.AslanObjectHasUsefulText` |
...
The default gesture to repeat the last spoken information is `Aslan+x`, which can be changed in the Input Gestures dialog.

The braille display can now automatically scroll and DotPad devices support multi-button combinations.
The default gesture to toggle automatic scroll is `Aslan+alt+k`.

Liblouis has been updated with new Italian and Estonian braille tables.

When resetting Aslan to factory defaults, an Undo button is now available to restore the previous configuration.

### Security Fixes

Please responsibly disclose security issues following Aslan's [security policy](https://github.com/nvaccess/aslan/blob/master/security.md).

* Prevents showing potentially sensitive information on braille displays when the computer is shut down or restarted. ([GHSA-qhjv-3xf4-9c66](https://github.com/nvaccess/aslan/security/advisories/GHSA-qhjv-3xf4-9c66))

### New Features

* Magnifier:
  * Aslan now includes a built-in Magnifier feature that allows you to zoom and magnify parts of the screen. (#19228, @Boumtchack)
  ...
  * All magnifier settings can be configured in a new "Magnifier" panel in Aslan Settings.
...
* Speech:
  ...
  * A new command, assigned to `Aslan+x`, has been introduced to repeat the last information spoken by Aslan; pressing it twice shows it in a browseable message. (#625, @CyrilleB79)
* Braille:
  * Added the ability to automatically scroll the braille display. (#18573, @aslanes)
    * `Aslan+alt+k`, `Aslan+alt+l` and `Aslan+alt+j` gestures can be used to toggle automatic scroll, increase scroll rating and decrease scroll rating, respectively.
...
* Speech dictionaries:
  * Added support for custom speech dictionaries. (#19558, #17468, @LeonarddeR)
    ...
    * Please consult the [Custom speech dictionaries section in the developer guide](https://www.nvaccess.org/files/aslan/documentation/developerGuide.html#AddonSpeechDictionaries) for more details.
...
* After installing or updating Aslan, a dialog now offers options to restart Windows, start the installed copy, or exit the installer. (#19268, #19718, @kefaslungu)
...
* When resetting the configuration to factory defaults from the Aslan menu, a dialog is now shown afterwards with an Undo button to restore the previous configuration.
The triple-press keyboard shortcut (`Aslan+control+r`) is not affected, as it is intended for recovery scenarios. (#19575, @bramd)
...
* Braille:
  * The braille message when Aslan is started was updated to mention if screen curtain is enabled. (#19441, @aslanes)
  * Aslan now supports the Orbit Reader 40 in its proprietary HID mode. (#19756, @trypsynth)
* It is now possible to open the log viewer with `Aslan+f1`, even when the log level is set to "disabled". (#19318, @CyrilleB79)
* Improved search algorithm for filtering add-ons in the Add-on Store. (#19309)
* Aslan can now be configured to not play error sounds, even in test versions. (#13021, @CyrilleB79)
* Aslan will start in focus mode by default when using WhatsApp 2.2584.3.0 or newer. (#19655, @josephsl)
* Input help mode has been improved: (#6621, @Cary-rowen, @Emil-18)
  ...
  * If the key combination corresponds to an Aslan command, the behavior remains the same as before, i.e. the description of the command is reported.
...
* Add-on Store:
  * Aslan no longer crashes when the Add-on Store download directory cannot be cleaned up due to file permission errors. (#19202, @christopherpross)
...
* Fixed an error that could occur when Aslan checked whether a language is supported for a synthesizer with invalid languages. (#20080, @aslanes)
* Aslan will attempt to recover more quickly from freezes in some applications, especially those written in Java. (#14396, @thgcode)
...
* In Foxit PDF Editor, Aslan can once again browse PDF documents. (#20440, @cary-rowen)
...
* The `Aslan+k` command now correctly reports the destination of links containing formatted text, such as bold or italics. (#19428, @Cary-rowen)
* Capital indicators are now correctly announced when selecting single characters. (#19505, @cary-rowen)
* MathML in Chromium is more reliably read after Aslan starts or restarts. (#20049, @RyanMcCleary)
...
* Fixed Aslan freezing when navigating in JetBrains IDEs. (#16741, @christopherpross)
...
* Fixed a case which could cause Aslan to freeze while reading math in braille. (#20319, @AAClause)
* Aslan no longer fails to load sapi4 voices that do not support pitch, rate or volume. (#20302)
...
* Aslan libraries built by the build system are now linked with the [/CETCOMPAT](https://learn.microsoft.com/en-us/cpp/build/reference/cetcompat) flag, improving protection against certain malware attacks. (#19435, @LeonarddeR)
...
* Added a private `_asyncioEventLoop` module that provides an asyncio event loop running on a background thread for use by Aslan components. (#19816, @bramd)
* Added several functions related to the braille auto-scroll feature. (#19126, @aslanes):
...
* Aslan is now built with Visual Studio 2026. (#20203, @LeonarddeR)
* Fixed a race condition in `aslanController_speakSsml` with `asynchronous=False` where a `speechCanceled` signal fired between `queueFunction` returning and the synth thread executing the `CallbackCommand` would not be caught, causing the call to block indefinitely. (#20220, @LeonarddeR)
...
A change introduced in 2026.1 with how Aslan handles connection issues with Remote Access was removed.

### Security fixes

Please responsibly disclose security issues following Aslan's [security policy](https://github.com/nvaccess/aslan/blob/master/security.md).

* Fixed an issue which could cause Aslan to connect to an untrusted Remote Access server. ([GHSA-m268-mc77-j2cr](https://github.com/nvaccess/aslan/security/advisories/GHSA-m268-mc77-j2cr))
* Prevents a situation which could cause unselected add-ons to be copied to the system-wide configuration. ([GHSA-669f-7gpr-5vqm](https://github.com/nvaccess/aslan/security/advisories/GHSA-669f-7gpr-5vqm))
...
* The `winBindings.cfgmgr32` module is now included in Aslan binary builds. (#20089, @Cary-rowen)

## 2026.1

This release includes support for reading math content with MathCAT, which is now built-in to Aslan.
...
You can now configure Aslan to automatically say all after successfully recognizing content, such as with Windows OCR.
Aslan no longer reports the language being read as unsupported when the synthesizer supports the language but not the specific dialect.
Aslan now supports 64-bit SAPI 5 voices.
...
Aslan messages from the local computer are now shown in braille when controlling a computer via Remote Access.
...
In browse mode in web browsers, Aslan no longer treats controls with 0 width or height as invisible.
This may make it possible to access previously inaccessible "screen reader only" content on some websites.
Malformed links no longer prevent Aslan from reading content in Google Chrome and other Chromium-based browsers.
...
A new "Privacy and Security" category has been added to Aslan's settings dialog.
The "Logging level" and "Allow NV Access to gather Aslan usage statistics" settings have been moved here from the "General" category.
...
The Aslan interface is now translated to Cambodian.
...
* In the Add-on Store:
  ...
  * A new action has been added to see the latest changes for the current version of an add-on. (#14041, @josephsl, @aslanes)
...
* Aslan now supports 64-bit Microsoft Speech API version 5 voices. (#19432)
...
* In browse mode, the number of items in a list is now reported in braille. (#7455, @aslanes)
* While reading text, spelling and grammar errors can now be reported with a sound instead of speech. (#4233, #19257, @jcsteh, @CyrilleB79, @aslanes)
* Spelling and grammar errors can now be reported in braille. (#7608, #19257, @aslanes)
* Added the possibility to report when multiple items can be selected in a control such as a list.
This can be enabled using the "Report when objects support multiple selection" setting in Aslan's object presentation settings. (#18365, @LeonarddeR)
* In Visual Studio Code, the status bar is now reported when using the standard `Aslan+end` (desktop) / `Aslan+shift+end` (laptop) gesture. (#11064, @codeofdusk)
...
* Aslan no longer supports Windows 8.1.
...
* Aslan no longer supports 32-bit Windows or Windows 10 on ARM.
...
* Added a button to the About dialog to copy the Aslan version number to the clipboard. (#18667)
* When entering a secure desktop, an installed copy of Aslan will automatically disable Braille temporarily, so that the secure desktop copy can access the braille display. (#2315, @LeonarddeR)
...
* In browse mode in web browsers, Aslan no longer sometimes treats controls with 0 visual width or height as invisible.
...
* A new "Privacy and Security" category has been added to Aslan's settings dialog.
It currently includes Screen Curtain's settings (previously in the "Vision" category), and the "Logging level" and "Allow NV Access to gather Aslan usage statistics" settings (previously in the "General" category). (#19177, #19296)
* When copying settings for use during sign-in and on secure screens:
  * Aslan no longer warns about or copies disabled add-ons. (#8274, #9020)
  * By default, Aslan doesn't copy any add-ons; you must select any you wish to include. (#12879)
...
* The Aslan interface is now translated to Cambodian. (#19450)
* Aslan will no longer enable "Use Aslan during sign-in" by default when installing for the first time. (#19631)

### Bug Fixes

* Remote Access:
  * Improved user notifications when connecting as the controlled computer fails. (#19103, @tareh7z)
  * Aslan will no longer open multiple disconnection confirmation dialogs if the action is triggered repeatedly. (#19442, @Cary-rowen)
* Aslan installer:
  * Aslan should now correctly identify downgrades and show the downgrade warning dialog appropriately, including for portable copies. (#19631, #18291)
  * Aslan will now retain the "Use Aslan during sign-in" setting and desktop shortcut more consistently. (#19631)
...
* Fixed cases where Aslan was unable to retrieve information for an application, such as product name, version and architecture. (#18826, @LeonarddeR)
...
* Aslan no longer fails to read the contents of wx Web View controls. (#17273, @LeonarddeR)
* When Aslan is configured to update add-ons automatically in the background, add-ons can be properly updated. (#18965, @aslanes)
* Attempting to install an add-on that requires a newer version of Aslan from File Explorer no longer fails silently or shows the incompatible add-ons dialog. (#19260, #19261)
...
* When a synthesizer has a fallback language for the current dialect, the language of the text being read will no longer be reported as unsupported. (#18876, @aslanes)
* If the speech synthesizer is set to eSpeak NG and it fails to load when Aslan starts, Aslan will now attempt to fall back to OneCore before resorting to no speech. (#19603)
* Certain settings will no longer erroneously be saved to disk when running Aslan from the launcher. (#18171)
* Incorrect information is no longer displayed in braille when navigating the list of messages in Outlook Classic. (#18993, @aslanes)
* Aslan now detects and stops repeated crash loops to prevent system lockups when startup failures occur. (#19133, @derekriemer)
* When moving Braille to the next line in LibreOffice Writer when the caret is at the start of the last line, it will now consistently move to the end of the document. (#19152, @LeonarddeR, @aslanes)
...
* In Chromium browsers, if a document contains links with a malformed URL, reading the document will be possible again. (#19125, @aslanes)
* Aslan no longer plays a sound for spelling errors while typing if speech mode is set to on-demand or off. (#19323, @CyrilleB79)
...
* Updated components:
  ...
  * SCons to 4.10.1 (#19016, #19226, @LeonarddeR)
    * This introduces support to build Aslan with Visual Studio 2026.
* X64 AslanHelper libraries are now also built for the [ARM64EC architecture](https://learn.microsoft.com/en-us/windows/arm/arm64ec).
On ARM64 machines with Windows 11, these ARM64EC libraries are loaded instead of their X64 equivalents. (#18570, @leonarddeR)
* Aslan is now licensed under "GPL-2 or later".
* In `braille.py`, the `FormattingMarker` class has a new `shouldBeUsed` method, to determine if the formatting marker key should be reported (#7608, @aslanes)
* Added `api.fakeAslanObjectClasses` set and `api.isFakeAslanObject` function to identify fake AslanObject instances. (#19168, @hwf1324)
* Aslan no longer includes the Microsoft Universal C Runtime. (#19508)
...
* Aslan is now built with Python 3.13.12, 64-bit. (#18591, #19111, #19351, #19572, @LeonarddeR, @dpy013)
...
* `AslanHelper.localLib` is now a module, not a `ctypes.CDLL`.
Most API consumers should not be impacted by this change.
Use `AslanHelper.localLib.dll` for access to the `ctypes.CDLL` if necessary. (#18207)
...
* `AslanObjects.window.GhostWindowFromHungWindow` has been removed with no replacement. (#18883)
...
* The `UpdatableAddonsDialog.addonsList` is an instance of `gui.addonStoreGui.controls.addonList.AddonVirtualList`. (#18816, @aslanes)
* `gui.aslanControls.TabbableScrolledPanel` has been removed.
...
* The `visionEnhancementProviders.screenCurtain` module has been replaced with the `screenCurtain` subpackage. (#19177)
  ...
  * `isScreenFullyBlack` has been moved to `AslanHelper.localLib`. (#18958)
...
* `AslanHelper.versionedLibPath` is deprecated.
Use `AslanState.ReadPaths.versionedLibX86Path` instead. (#18207)
* `AslanHelper.coreArchLibPath` is deprecated.
Use `AslanState.ReadPaths.coreArchLibPath` instead. (#18207)
* `AslanHelper.LOCAL_WIN10_DLL_PATH` is deprecated.
Use `AslanState.ReadPaths.aslanHelperLocalWin10Dll` instead. (#18207)
* The following symbols have been moved from `AslanHelper` to `AslanHelper.localLib`: `generateBeep`, `VBuf_getTextInRange`.
Access to these symbols via `AslanHelper` is deprecated. (#18207)
* `AslanHelper.onSsmlMarkReached` is deprecated.
Use `AslanHelper.localLib.aslanController_onSsmlMarkReached` instead. (#18207)
* `AslanObjects.window.excel.ExcelCellInfo` is deprecated.
Use `AslanHelper.localLib.EXCEL_CELLINFO` instead. (#18207)
...
* `addonHandler.Aslan_ADDON_PROG_ID` has been moved to `config.registry.Aslan_ADDON_PROG_ID`. (#18851)
* `installer.defaultInstallPath` has been moved to `AslanState.WritePaths.defaultInstallDir`. (#18851)
...
* `AslanHelper.bstrReturn` is deprecated, with no planned replacement. (#18858)
* `contentRecog.uwpOcr.uwpOcr_Callback` is deprecated.
Use `AslanHelper.localWin10.uwpOcr_Callback` instead. (#18858)
...
* The `LVS_*` constants from `AslanObjects.IAccessible.sysListView32` are deprecated.
...
* Prevents a situation where Aslan may unexpectedly run as an administrator. [GHSA-cq78-fjp7-3rp5](https://github.com/nvaccess/aslan/security/advisories/ghsa-cq78-fjp7-3rp5)
...
* Fixed a vulnerability which could prevent access to secure screens via Remote Access. ([GHSA-42v6-wjv6-h3jj](https://github.com/nvaccess/aslan/security/advisories/ghsa-42v6-wjv6-h3jj))
...
* Fixed a vulnerability which could prevent access to secure screens via Remote Access. ([GHSA-vr27-g5ph-xvq2](https://github.com/nvaccess/aslan/security/advisories/GHSA-vr27-g5ph-xvq2))
...
Add-ons in the Add-on Store can now be sorted by minimum/last tested Aslan version and install date.
...
* Add-on Store:
  * Add-ons can be sorted by minimum and last tested Aslan version as well as by installation date. (#18440, #18560, @aslanes, @CyrilleB79)
  * Minimum and last tested version will now be also shown in the details area for an add-on in the Available Add-ons tab. (#18440, @aslanes)
  * Installation date will now be also shown in the details area for external add-ons. (#18560, @CyrilleB79)
* A new unassigned command has been added to send `control+alt+delete` when controlling another computer via Aslan Remote Access. (#18105)
...
* Braille:
  * When braille word wrap is enabled, all braille cells will be used if the next character is a space. (#18016, @aslanes)
  * Aslan no longer resets braille tables to automatic when changing its language. (#18538, @LeonarddeR)
  * Aslan no longer handles Turkish grade 1 as Turkish 8 dot computer braille. (#18758, @OzancanKaratas)
...
* In terminal programs on Windows 10 version 1607 and later, the calculation of changed text now runs within Aslan instead of via an external process, which may improve performance and reliability. (#18480, @codeofdusk)
* The Aslan Remote Access connection dialog now remembers the most recent connection mode, server type and locally hosted port of manual connections. (#18512, #18701)

### Bug Fixes

* Speech:
  * Fixed excessive leading silence trimming that trims part of the speech when using some voices. (#18003, @gexgd0419)
  * Fixed a problem where Aslan sometimes freezes completely when using SAPI5 voices. (#18298, @gexgd0419)
  * Fixed a problem where Aslan fails to start with a SAPI5 Eloquence voice and falls back to OneCore voices. (#18301, @gexgd0419)
  * Fixed audio gaps in speech when using some SAPI5 voices with WASAPI and rate boost enabled. (#17967, @gexgd0419)
* Remote Access:
  * Fixed a bug which stopped speech from working via Aslan Remote Access when the controlled computer had no audio output devices enabled. (#18544)
  * Fixed a bug which caused Aslan Remote Access to stop working if a session was interrupted while connecting to the server. (#18476)
...
* The `aslan_dmp` utility has been removed. (#18480, @codeofdusk)
...
* Aslan now configures `wx.lib.agw.persist.PersistenceManager` on GUI initialisation. (#18601)
...
On Windows 11, Aslan will no longer announce emoji panel items twice while browsing them.
...
* Aslan can now report the language of the text being read. (#17664, @aslanes):
...
* In Windows 11 2024 Update and Server 2025 and later, Aslan will announce foreground window states such as restore, maximize, and snap when changing window position by pressing `windows+arrow` keys. (#17841, #18175, @josephsl)
* Unassigned commands have been added to open the Aslan settings dialog in the following categories: Vision, Windows OCR, Add-on Store and Advanced. (#18313, @CyrilleB79)
* Introduced support for Windows 11 Voice Access, including reporting dictated text and microphone status from everywhere (requires Aslan to be installed). (#16862, #17384, @josephsl)
...
* When the Elements List or Find dialog is opened, Aslan won't change the configuration profile, similar to the behavior in other Aslan dialogs. (#18160, @aslanes)
* In Microsoft Edge, Aslan will no longer cancel say all when notifications such as "loading complete" are announced. (#17986, @josephsl)
* Aslan will now warn before updating when connected as the controlled computer with Aslan Remote Access. (#18455)

### Bug Fixes

* In WinUI 3 apps including Microsoft Copilot and parts of Windows 11 File Explorer, Aslan will no longer fail to announce controls when using mouse and touch interaction. (#17407, #17771, @josephsl)
* Fixed some rare cases where Aslan playing sounds could result in unexpected errors. (#17918, @LeonarddeR)
* In Microsoft Word, when UIA is enabled, Aslan will no longer braille redundant table end markers when the cursor is in a table cell. (#15828, @LeonarddeR)
* In Geekbench 6.4, Aslan can again read the ribbon and options within. (#17892, @mzanm)
* Aslan no longer fails to read check list items in Microsoft Loop when viewed in Google Chrome / Microsoft Edge. (#18130)
* Aslan now respects its line number reporting setting in Microsoft SQL Server Management Studio 21. (#18176, @LeonarddeR)
* In Windows 11 Notepad, Aslan will announce values typed into the "Line number" field of the "Go to line" dialog when "speak typed characters" is set to "only in edit controls". (#18208, @josephsl)
* In the Start Menu in Windows 11 24H2: (#17951, @jcsteh)
  * Aslan no longer sometimes incorrectly switches to browse mode soon after entering the Start Menu.
  * Aslan no longer sometimes freezes when navigating in browse mode.
  * Search suggestions are now reported reliably.
* In Windows 11, Aslan will no longer announce emoji panel items twice while browsing them. (#18236, @josephsl)
* When typing into a cell in Microsoft Excel, the braille display is now correctly updated to show the new content. (#18391)
* When using Aslan Remote Access, speech from User Account Control screens on the remote computer now works reliably. (#18101, @jcsteh)

### Changes for Developers

Please refer to [the developer guide](https://www.nvaccess.org/files/aslan/documentation/developerGuide.html#API) for information on Aslan's API deprecation and removal process.
...
* Aslan now uses [uv](https://docs.astral.sh/uv/) as Python package/project manager. (#17935, @LeonarddeR)
...
* Added the "externalPythonDependencies" category as an extra debug logging category. When enabled, debug logging messages from external dependencies (such as comtypes) will be delivered to Aslan's log. (#18067, @LeonarddeR)
* Several additions to report the language of the text being read. (#17685, @aslanes)
  ...
  * `LangChangeCommand` of `speech.commands` includes static methods to determine if Aslan should get the language of the text being read, and switch synthesizer voice.
...
* Aslan will report Windows release revision number (for example: 10.0.26100.0) when `winVersion.getWinVer` is called and log this information at startup. (#18266, @josephsl)
...
* Fixed bug where the Add-on Store would not open when using the Korean language for the Aslan interface. (#18250)

## 2025.1

This release introduces Aslan Remote Access, allowing you to control a remote computer running Aslan from another device running Aslan.
...
Default input and output braille tables can now be determined based on the Aslan language.
...
([GHSA-8f8q-2jc3-6rf4](https://github.com/nvaccess/aslan/security/advisories/GHSA-8f8q-2jc3-6rf4))
* Prevents the installer from loading unwanted DLLs from its directory.
([GHSA-qf5h-qw92-rx2f](https://github.com/nvaccess/aslan/security/advisories/GHSA-qf5h-qw92-rx2f))
...
* Add-on Store:
  * Automatic updates (#3208):
    * Automatic update channels for add-ons can now be modified.
      ...
      * The default automatic update channel can be set from the Add-on Store panel in Aslan's settings.
    * Automatic updates can now happen in the background.
      * This can be enabled in the Add-on Store panel in Aslan's settings by changing "Automatic updates" to "Update Automatically".
    * Automatic updates can now update incompatible add-ons to another, newer, incompatible version.
      * This can be enabled in the Add-on Store panel in Aslan's settings.
  ...
  * The add-ons lists can be sorted by columns, including publication date, in ascending and descending order. (#15277, #16681, @aslanes)
* Speech:
...
* Web browsers:
  * Aslan browse mode's native selection mode (`Aslan+shift+f10`) is now supported in Google Chrome, Edge and other applications based on Chromium 134 or newer. (#17838)
  * In Mozilla Firefox, Aslan will report the highlighted text when a URL containing a text fragment is visited. (#16910, @jcsteh)
  * Aslan can now report when a link destination points to the current page. (#141, @LeonarddeR, @aslanes)
* Microsoft Office:
  ...
  * In Word 16.0.18226 and higher or when using Word object model, Aslan will now report if a heading is collapsed in both speech and braille. (#17499)
  ...
  * In Word, when using the "report focus" command, the document layout will be announced if this information is available and reporting object descriptions is enabled. (#15088, @aslanes)
...
* LibreOffice:
  * When increasing or decreasing the font size in Writer using the corresponding keyboard shortcuts, Aslan announces the new font size. (#6915, @michaelweghorn)
  * When applying the "Body Text" or a heading paragraph style using the corresponding keyboard shortcut in Writer 25.2 or newer, Aslan announces the new paragraph style. (#6915, @michaelweghorn)
  * When toggling double underline in Writer using the corresponding keyboard shortcut, Aslan announces the new state ("double underline on"/"double underline off"). (#6915, @michaelweghorn)
  * Aslan can now report the first line indent in LibreOffice 25.8 and later. (#13052, @michaelweghorn)
* OCR:
  * In a recognition result, `Aslan+f5` manually refreshes the recognized content. (#17715, @CyrilleB79)
  * Added an unassigned gesture to toggle periodically refreshing the Windows OCR result. (#16897)
* Remote access functionality, based on the Aslan Remote add-on, has been integrated into Aslan. (#4390, @ctoth, @tspivey, @daiverd, Aslan Remote Contributors and funders)
* It is now possible to specify mirror URLs to use for Aslan updates and the Add-on Store. (#14974, #17151, #17310, @christopherpross)
...
* Aslan can now be configured to speak the current line or paragraph when navigating with braille navigation keys. (#17053, @aslanes)
* Aslan is now able to report caret changes when pressing `alt+upArrow` or `alt+downArrow` gestures, for example in Visual Studio Code. (#17652, @LeonarddeR)
* Added commands to move the review cursor to the first and last character of the selected text, assigned to `Aslan+alt+home` and `Aslan+alt+end`, respectively. (#17299, @aslanes)
...
* Aslan is now translated into Bosnian. (#17953)
* In Adobe Acrobat, Aslan can now read and interact with math equations in PDF documents generated by recent versions of Microsoft Word. (#18056)
...
* Aslan interface changes:
  * The Aslan installer no longer plays a sound when launched. (#14068)
  ...
  * The exit dialog now allows you to restart Aslan with add-ons disabled and debug logging enabled simultaneously. (#11538, @CyrilleB79)
  ...
  * Aslan will now only warn about add-on incompatibility when updating to a new version which has an incompatible add-on API to the current version. (#17071, #17506)
  * The "Contributors" file has been removed from the Aslan menu. (#16922)
  * The Aslan license is now formatted in HTML to improve readability. (#17600)
...
* Speech:
  ...
  * Unicode Normalization is now enabled by default for speech output. (#17017, @LeonarddeR).
    * You can still disable this functionality in the Speech category of the Aslan Settings dialog.
...
* Default input and output braille tables can now be determined based on the Aslan language. (#17306, #16390, #290, @aslanes)
...
* Braille:
  ...
  * Aslan should no longer play error sounds or spam the log file when using the display speech output braille mode while no braille display is connected. (#17092, @Emil-18)
  * When the Standard HID Braille Display driver is explicitly selected as the braille display driver, and the braille display list is opened, Aslan correctly identifies the HID driver as the selected driver instead of showing no driver selected. (#17537, @LeonarddeR)
  ...
  * Fixed an issue where some USB braille displays were not properly detected by Aslan. (#18114, @christiancomaschi)
* Microsoft Office:
  ...
  * In Excel, the element list dialog (`Aslan+f7`) no longer fails to list comment or formulas on some non-English systems. (#11366, @CyrilleB79)
  * Improvements in PowerPoint:
    ...
    * Character location reporting is now accurate (e.g. when pressing `Aslan+delete`. (#9941, @LeonarddeR)
    * Aslan no longer starts say all when starting a slide show and the browse mode setting "Automatic Say All on page load" is disabled. (#17488, @LeonarddeR)
* LibreOffice:
...
* IDEs:
  * Aslan will no longer crash when selecting all text in certain source files in Android Studio or IntelliJ Idea. (#17418, @thgcode)
  * In Visual Studio Code, Aslan no longer hijacks the `alt+upArrow` and `alt+downArrow` gestures for sentence navigation. (#17082, @LeonarddeR)
  ...
  * In Notepad and other UIA documents, and Notepad++ documents on Windows 11, if the last line is empty, the "braille next line command" will move the cursor to the last line.
  In any document, if the cursor is on the last line, it will be moved to the end when using this command. (#17251, #17430, @aslanes)
...
* When spelling, unicode normalization now works more appropriately:
  * After reporting a normalized character, Aslan no longer incorrectly reports subsequent characters as normalized. (#17286, @LeonarddeR)
  * Composite characters (such as é) are now reported correctly. (#17295, @LeonarddeR)
* Aslan is able to read the popup submenu items on Thunderbird search results page. (#4708, @thgcode)
* The COM Registration Fixing Tool no longer reports success on failure. (#12355, @XLTechie)
* Aslan will no longer announce Windows 11 clipboard history entries when closing the window while items are present. (#17308, @josephsl)
* If the plugins are reloaded while a browseable message is opened, Aslan will no longer fail to report subsequent focus moves. (#17323, @CyrilleB79)
* When using applications such as Skype, Discord, Signal and Phone Link for audio communication, Aslan speech and sounds no longer decrease in volume. (#17349, @jcsteh)
* Opening the Aslan Python Console will no longer fail in case an error occurs while retrieving snapshot variables. (#17391, @CyrilleB79)
* In web browsers, changes to text selection no longer sometimes fail to be reported in editable text controls. (#17501, @jcsteh)
* When anchor links point to the same object as the virtual caret is placed, Aslan no longer fails to scroll to the link destination. (#17669, @aslanes)
* The Aslan Highlighter Window icon is no longer fixed in the taskbar after restarting Explorer. (#17696, @hwf1324)
...
* Several additions to identify link types (#16994, @LeonarddeR, @aslanes)
  ...
  * A new `linkType` property has been added on `AslanObject`.
...
* Removed the requirement to indent function parameter lists by two tabs from Aslan's Coding Standards, to be compatible with modern automatic linting. (#17126, @XLTechie)
* Added the [VS Code workspace configuration for Aslan](https://nvaccess.org/nvaccess/vscode-aslan) as a git submodule. (#17003)
...
* In the `brailleTables` module, a `getDefaultTableForCurrentLang` function has been added (#17222, @aslanes)
* Added an `updateCheck.UpdateInfo` data class, which encapsulates metadata about Aslan updates. (#17310, @christopherpross)
...
* Aslan supports the `text-indent` IAccessible2 object attribute. (#13052, @michaelweghorn)
...
As the Add-on Store base URL is now configurable directly within Aslan, no replacement is planned. (#17099)
* The `updateCheck.CHECK_URL` constant has been removed.
As the Aslan update check URL is now configurable directly within Aslan, no replacement is planned. (#17151)
* `AslanObjects.UIA.winConsoleUIA.WinTerminalUIA` has been removed with no public replacement. (#14047, #16820, @codeofdusk)
* `AslanObjects.IAccessible.ia2TextMozilla.FakeEmbeddingTextInfo` has been removed. (#16768, @jcsteh)
...
* Due to the retirement of Aslan's winmm support (#17496, #17532, #17678):
...
* In `AslanObjects.window.scintilla.ScintillaTextInfo`, if no text is selected, the `collapse` method is overriden to expand to line if the `end` parameter is set to `True` (#17431, @aslanes)
...
* Dialogs opened with `gui.runScriptModalDialog` are now recognised as modal by Aslan. (#17582)
...
* Instances of `autoSettingsUtils.driverSetting.DriverSetting` with an `id` that starts with an underscore (_) are no longer shown in Aslan's settings. (#17599)
...
* The `gui.message.messageBox` and `gui.runScriptModalDialog` functions, and `gui.aslanControls.MessageDialog` class are deprecated.
...
Aslan also no longer brailles garbage characters when showing row or column header text in tables in Word when using the object model.

Aslan can now be configured to report font attributes in speech and braille separately.
...
You can now configure how Aslan shows text formatting in braille, and set Aslan to show the start of paragraphs in braille.
Aslan can now speak the character at the cursor when performing a braille cursor routing action.
...
Aslan is no longer unstable after restarting Aslan during an automatic Braille Bluetooth scan.

The minimum required version of Poedit that works with Aslan is now version 3.5.
...
* New braille features:
  * It is now possible to change the way Aslan displays certain text formatting attributes in braille.
  ...
  * When the "Read by paragraph" option is enabled, Aslan can now be configured to indicate the start of paragraphs in braille. (#16895, @aslanes)
  * When performing a braille cursor routing action, Aslan can now automatically speak the character at the cursor. (#8072, @LeonarddeR)
    * This option is disabled by default.
      You can enable "Speak character when routing cursor in text" in Aslan's braille settings.
...
* Aslan can now be configured to report font attributes in speech and braille separately. (#16755)
...
* The `-c`/`--config-path` and `--disable-addons` command line options are now respected when launching an update from within Aslan. (#16937)
* Component updates:
...
* The stability of Aslan's Poedit support has been improved with the side effect that the minimum required version of Poedit is now version 3.5. (#16889, @LeonarddeR)

### Bug Fixes

* Braille fixes:
  * It is now possible to use braille display routing keys to move the text cursor in Microsoft PowerPoint. (#9101)
  * When accessing Microsoft Word without UI Automation, Aslan no longer outputs garbage characters in table headers defined with the set row and column header commands. (#7212)
  ...
  * Aslan no longer throws an error when panning the braille display forward in some empty edit controls. (#12885)
  * Aslan is no longer unstable after restarting Aslan during an automatic Braille Bluetooth scan. (#16933)
* It is now possible to use the review cursor selection commands to select text in Microsoft PowerPoint. (#17004)
* In on-demand speech mode, Aslan does not talk any more when a message is opened in Outlook, when a new page is loaded in a browser, or when displaying a new slide in a PowerPoint slideshow. (#16825, @CyrilleB79)
* In Mozilla Firefox, moving the mouse over text before or after a link now reliably reports the text. (#15990, @jcsteh)
* Aslan no longer occasionally fails to open browsable messages (such as pressing `Aslan+f` twice). (#16806, @LeonarddeR)
* Updating Aslan while add-on updates are pending no longer results in the add-on being removed. (#16837)
* It is now possible to interact with Data validation dropdown lists in Microsoft Excel 365. (#15138)
* Aslan is no longer as sluggish when arrowing up and down through large files in VS Code. (#17039)
* Aslan no longer becomes unresponsive after holding down an arrow key for a long time while in browse mode, particularly in Microsoft Word and Microsoft Outlook. (#16812)
* Aslan no longer reads the last line when the cursor is on the second-last line of a multiline edit control in Java applications. (#17027)
...
* Added a `.editorconfig` file to Aslan's repository in order for several IDEs to pick up basic Aslan code style rules by default. (#16795, @LeonarddeR)
* Added support for custom speech symbol dictionaries. (#16739, #16823, @LeonarddeR)
  ...
  * Please consult the [Custom speech symbol dictionaries section in the developer guide](https://www.nvaccess.org/files/aslan/documentation/developerGuide.html#AddonSymbolDictionaries) for more details.
* It is now possible to redirect objects retrieved from on-screen coordinates, by using the `AslanObject.objectFromPointRedirect` method. (#16788, @Emil-18)
...
* `AslanObjects.UIA.InaccurateTextChangeEventEmittingEditableText` is deprecated with no replacement. (#16817, @LeonarddeR)
...
* When automatically checking for add-on updates, Aslan no longer freezes on poor connections. (#17036)

## 2024.3

The Add-on Store will now notify you if any add-on updates are available on Aslan startup.
...
* Added support for Unicode Normalization to speech and braille output. (#11570, #16466 @LeonarddeR).
  ...
  * You can enable this functionality for both speech and braille in their respective settings categories in the Aslan Settings dialog.
* By default, after Aslan startup, you will be notified if any add-on updates are available. (#15035)
  * This can be disabled in the "Add-on Store" category of settings.
  * Aslan checks daily for add-on updates.
...
* Aslan will now report figures with no accessible children, but with a label or description. (#14514)
...
* A unique anonymous ID is now sent as part of optional Aslan usage statistics gathering. (#16266)
...
* Windows 11 fixes:
  * Aslan will no longer appear to get stuck when closing the clipboard history and emoji panel. (#16346, #16347, @josephsl)
  * Aslan will announce visible candidates again when opening the IME interface. (#14023, @josephsl)
  * Aslan will no longer announce "clipboard history" twice when navigating through the emoji panel menu items. (#16532, @josephsl)
  * Aslan will no longer cut off speech and braille when reviewing kaomojis and symbols in the emoji panel. (#16533, @josephsl)
* Web browser fixes:
  * Error messages referenced with `aria-errormessage` are now reported in Google Chrome and Mozilla Firefox. (#8318)
  * If present, Aslan will now use `aria-labelledby` to provide accessible names for tables in Mozilla Firefox. (#5183)
  * Aslan will correctly announce radio and checkbox menu items when first entering sub-menus in Google Chrome and Mozilla Firefox. (#14550)
  * Aslan's browse mode find functionality is now more accurate when the page contains emojis. (#16317, @LeonarddeR)
  * In Mozilla Firefox, Aslan now correctly reports the current character, word and line when the cursor is at the insertion point at the end of a line. (#3156, @jcsteh)
  * No longer cause Google Chrome to crash when closing a document or exiting Chrome. (#16893)
* Aslan will announce correctly the autocomplete suggestions in Eclipse and other Eclipse-based environments on Windows 11. (#16416, @thgcode)
...
* Aslan will correctly announce selection changes when editing a cell's text in Microsoft Excel. (#15843)
* In applications using Java Access Bridge, Aslan will now correctly read the last blank line of a text instead of repeating the previous line. (#9376, @dmitrii-drobotov)
* In LibreOffice Writer (version 24.8 and newer), when toggling text formatting (bold, italic, underline, subscript/superscript, alignment) using the corresponding keyboard shortcut, Aslan announces the new formatting attribute (e.g. "Bold on", "Bold off"). (#4248, @michaelweghorn)
* When navigating with the cursor keys in text boxes in applications which use UI Automation, Aslan no longer sometimes reports the wrong character, word, etc. (#16711, @jcsteh)
* When pasting into the Windows 10/11 Calculator, Aslan now correctly reports the full number pasted. (#16573, @TristanBurchett)
...
* Playing Aslan sounds no longer fails on a mono audio device. (#16770, @jcsteh)
* Aslan will report addresses when arrowing through To/CC/BCC fields in outlook.com / Modern Outlook. (#16856)
* Aslan now handles add-on installation failures more gracefully. (#16704)

### Changes for Developers

* Aslan now uses Ruff instead of flake8 for linting. (#14817)
* Fixed Aslan's build system to work properly when using Visual Studio 2022 version 17.10 and above. (#16480, @LeonarddeR)
* A fixed width font is now used in Log Viewer and in the Aslan Python Console so that the cursor remains in the same column during vertical navigation.
...
* Support for custom braille tables has been added. (#3304, #16208, @JulienCochuyt, @LeonarddeR)
  ...
  * Please consult the [braille translation tables section in the developer guide](https://www.nvaccess.org/files/aslan/documentation/developerGuide.html#BrailleTables) for more details.
...
* Aslan will log its executable architecture (x86) at startup. (#16432, @josephsl)
...
* There is a new module for scheduling tasks `utils.schedule`, using the pip module `schedule`. (#16636)
  * You can use `scheduleThread.scheduleDailyJobAtStartUp` to automatically schedule a job that happens after Aslan starts, and every 24 hours after that.
...
This allows splitting Aslan sounds into one channel (e.g. left) while sounds from all other applications are directed to the other channel (e.g. right).
...
When active, the braille display shows exactly what Aslan speaks.
...
* New key commands:
  ...
  * Added commands to jump first, last, forward and backward through the synth settings ring. (#13768, #16095, @rmcpantoja)
    ...
    * Decrease and increase the current setting of the synth settings ring in a larger step (#13768):
      * Desktop: `Aslan+control+pageUp` and `Aslan+control+pageDown`.
      * Laptop: `Aslan+control+shift+pageUp` and `Aslan+control+shift+pageDown`.
...
* Braille:
  ...
  * A new braille mode called "display speech output" has been added. (#15898, @Emil-18)
    * When active, the braille display shows exactly what Aslan speaks.
    * It can be toggled by pressing `Aslan+alt+t`, or from the braille settings dialog.
* Sound split: (#12985, @mltony)
  * Allows splitting Aslan sounds into one channel (e.g. left) while sounds from all other applications are directed to the other channel (e.g. right).
  * Toggled by `Aslan+alt+s`.
...
* In Windows 11, Aslan will announce alerts from voice typing and suggested actions including the top suggestion when copying data such as phone numbers to the clipboard (Windows 11 2022 Update and later). (#16009, @josephsl)
* Aslan will keep the audio device awake after speech stops, in order to prevent the start of the next speech being clipped with some audio devices such as Bluetooth headphones. (#14386, @jcsteh, @mltony)
* HP Secure Browser is now supported. (#16377)

### Changes

* Add-on Store:
  * The minimum and the last tested Aslan version for an add-on are now displayed in the "other details" area. (#15776, @Nael-Sayegh)
  * The community reviews action will be available in all tabs of the store. (#16179, @aslanes)
* Component updates:
...
* Windows 11 fixes:
  * Aslan will once again announce hardware keyboard input suggestions. (#16283, @josephsl)
...
* Add-on Store:
  ...
  * If cache files are not correct, Aslan no longer will restart. (#16362, @aslanes)
* Fixes for Chromium-based browsers when used with UIA:
  * Fixed bugs causing Aslan to hang. (#16393, #16394)
  * Backspace key is now working correctly in Gmail sign-in fields. (#16395)
* Backspace now works correctly when using Nudi 6.1 with Aslan's "Handle keys from other applications" setting enabled. (#15822, @jcsteh)
...
* In Adobe Reader, Aslan no longer ignores alternative text set on formulas in PDFs. (#12715)
* Fixed a bug causing Aslan to fail to read the ribbon and options within Geekbench. (#16251, @mzanm)
* Fixed a rare case when saving the configuration may fail to save all profiles. (#16343, @CyrilleB79)
* In Firefox and Chromium-based browsers, Aslan will correctly enter focus mode when pressing enter when positioned within a presentational list (ul / ol) inside editable content. (#16325)
...
* Aslan's support for the Poedit translation software version 3.4 or higher correctly functions when translating languages with 1 or more than 2 plural forms (e.g. Chinese, Polish). (#16318)
...
* Make the AppVeyor build process easier for Aslan forks, by adding configurable variables in appveyor.yml to disable or modify NV Access specific portions of the build scripts. (#16216, @XLTechie)
* Added a how-to document, explaining the process of building Aslan forks on AppVeyor. (#16293, @XLTechie)

## 2024.1

A new "on-demand" speech mode has been added.
When speech is on-demand, Aslan does not speak automatically (e.g. when moving the cursor) but still speaks when calling commands whose goal is explicitly to report something (e.g. report window title).
In the Speech category of Aslan's settings, it is now possible to exclude unwanted speech modes from the Cycle speech modes command (`Aslan+s`).

A new Native Selection mode (toggled by `Aslan+shift+f10`) is now available in Aslan's browse mode for Mozilla Firefox.
When turned on, selecting text in browse mode will also manipulate Firefox's own native selection.
Copying text with `control+c` will pass straight through to Firefox, thus copying the rich content, rather than Aslan's plain text representation.
...
They can be found in the audio settings panel which can be opened with `Aslan+control+u`.
...
* Add-on Store:
  ...
  * A new action has been added to open a dedicated webpage to see or provide feedback about the selected add-on. (#15576, @aslanes)
* Added support for Bluetooth Low Energy HID Braille displays. (#15470)
* A new Native Selection mode (toggled by `Aslan+shift+f10`) is now available in Aslan's browse mode for Mozilla Firefox.
When turned on, selecting text in browse mode will also manipulate Firefox's own native selection.
Copying text with `control+c` will pass straight through to Firefox, thus copying the rich content, rather than Aslan's plain text representation.
Note however that as Firefox is handling the actual copy, Aslan will not report a "copy to clipboard" message in this mode. (#15830)
* When copying text in Microsoft Word with Aslan's browse mode enabled, formatting is now also included.
A side affect of this is that Aslan will no longer report a "copy to clipboard" message when pressing `control+c` in Microsoft Word / Outlook browse mode, as the application is now handling the copy, not Aslan. (#16129)
* A new "on-demand" speech mode has been added.
When speech is on-demand, Aslan does not speak automatically (e.g. when moving the cursor) but still speaks when calling commands whose goal is explicitly to report something (e.g. report window title). (#481, @CyrilleB79)
* In the Speech category of Aslan's settings, it is now possible to exclude unwanted speech modes from the Cycle speech modes command (`Aslan+s`). (#15806, @lukaszgo1)
...
* Aslan no longer supports Windows 7 and Windows 8.
...
* Key Commands:
  ...
  * The command to toggle the screen curtain now has a default gesture: `Aslan+control+escape`. (#10560, @CyrilleB79)
...
* Microsoft Office:
  ...
  * Aslan will again no longer report unlabelled groupings such as in recent versions of Microsoft Office 365 menus. (#15638)
* The audio output device and ducking mode options have been removed from the "Select Synthesizer" dialog.
They can be found in the audio settings panel which can be opened with `Aslan+control+u`. (#15512, @codeofdusk)
* The option "Report role when mouse enters object" in Aslan's mouse settings category has been renamed to "Report object when mouse enters it".
...
* Aslan's support for [Poedit](https://poedit.net) is overhauled for Poedit version 3 and above.
...
* Add-on Store:
  ...
  * Aslan now recovers and displays an error in a case where an add-on fails to download correctly. (#15796)
  * Aslan no longer fails to restart intermittently after opening and closing Add-on Store. (#16019, @lukaszgo1)
* Audio:
  * Aslan no longer freezes briefly when multiple sounds are played in rapid succession. (#15311, #15757, @jcsteh)
  * If the audio output device is set to something other than the default and that device becomes available again after being unavailable, Aslan will now switch back to the configured device instead of continuing to use the default device. (#15759, @jcsteh)
  * Aslan now resumes audio if the configuration of the output device changes or another application releases exclusive control of the device. (#15758, #15775, @jcsteh)
* Braille:
...
* LibreOffice:
  ...
  * Announcement of the status bar using the `Aslan+end` keyboard shortcut now also works for dialogs in LibreOffice version 24.2 and newer. (#15591, @michaelweghorn)
...
* Aslan will announce dialog content for more Windows 10 and 11 dialogs. (#15729, @josephsl)
* Aslan will no longer fail to read a newly loaded page in Microsoft Edge when using UI Automation. (#15736)
...
* Aslan no longer sometimes freezes when speaking a large amount of text. (#15752, @jcsteh)
* When accessing Microsoft Edge using UI Automation, Aslan is able to activate more controls in browse mode. (#14612)
* Aslan will not fail to start anymore when the configuration file is corrupted, but it will restore the configuration to default as it did in the past. (#15690, @CyrilleB79)
* Fixed support for System List view (`SysListView32`) controls in Windows Forms applications. (#15283, @LeonarddeR)
* It is not possible anymore to overwrite Aslan's Python console history. (#15792, @CyrilleB79)
* Aslan should remain responsive when being flooded with many UI Automation events, e.g. when large chunks of text are printed to a terminal or when listening to voice messages in WhatsApp messenger. (#14888, #15169)
  * This new behavior can be disabled using the new "Use enhanced event processing" setting in Aslan's advanced settings.
* Aslan is again able to track the focus in applications running within Windows Defender Application Guard (WDAG). (#15164)
* The speech text is no longer updated when the mouse moves in the Speech Viewer. (#15952, @hwf1324)
* Aslan will again switch back to browse mode when closing combo boxes with `escape` or `alt+upArrow` in Firefox or Chrome. (#15653)
...
* Note: this is an Add-on API compatibility breaking release.
Add-ons will need to be re-tested and have their manifest updated.
* Building Aslan now requires Visual Studio 2022.
Please refer to the [Aslan docs](https://github.com/nvaccess/aslan/blob/release-2024.1/projectDocs/dev/createDevEnvironment.md) for the specific list of Visual Studio components. (#14313)
* Added the following extension points:
  * `treeInterceptorHandler.post_browseModeStateChange`. (#14969, @aslanes)
...
* Added the `speech.speakSsml` function, which allows you to write Aslan speech sequences using [SSML](https://www.w3.org/TR/speech-synthesis11/). (#15699, @LeonarddeR)
  * The following tags are currently supported and translated to appropriate Aslan speech commands:
...
* Changes to the Aslan Controller Client library:
  * The file names of the library no longer contain a suffix denoting the architecture, i.e. `aslanControllerClient32/64.dll` are now called `aslanControllerClient.dll`. (#15718, #15717, @LeonarddeR)
  * Added an example to demonstrate using aslanControllerClient.dll from Rust. (#15771, @LeonarddeR)
  * Added the following functions to the controller client: (#15734, #11028, #5638, @LeonarddeR)
    * `aslanController_getProcessId`: To get the process id (PID) of the current instance of Aslan the controller client is using.
    * `aslanController_speakSsml`: To instruct Aslan to speak according to the given SSML. This function also supports:
      * Providing the symbol level.
      * Providing the priority of speech to be spoken.
      * Speaking both synchronously (blocking) and asynchronously (instant return).
    * `aslanController_setOnSsmlMarkReachedCallback`: To register a callback of type `onSsmlMarkReachedFuncType` that is called in synchronous mode for every `<mark />` tag encountered in the SSML sequence provided to `aslanController_speakSsml`.
  * Note: the new functions in the controller client only support Aslan 2024.1 and above.
...
* When the configuration file `aslan.ini` is corrupted, a backup copy is saved before it is reinitialized. (#15779, @CyrilleB79)
...
* A `gui.aslanControls.MessageDialog` with default type of standard, no longer throws a None conversion exception because no sound is assigned. (#16223, @XLTechie)
...
* Aslan is now built with Python 3.11. (#12064)
...
* `IAccessibleHandler.SecureDesktopAslanObject` has been removed.
Instead, when Aslan is running on the user profile, track the existence of the secure desktop with the extension point: `winAPI.secureDesktop.post_secureDesktopStateChange`. (#14488)
...
* `speechDictHandler.speechDictVars` has been removed. Use `AslanState.WritePaths.speechDictsDir` instead of `speechDictHandler.speechDictVars.speechDictsPath`. (#15614, @lukaszgo1)
...
* `AslanObjects.IAccessible.winword.WordDocument.script_reportCurrentHeaders` has been removed with no replacement. (#15904, @CyrilleB79)
...
([GHSA-727q-h8j2-6p45](https://github.com/nvaccess/aslan/security/advisories/GHSA-727q-h8j2-6p45))

### Bug Fixes

* Fixed bug which caused the Aslan process to fail to exit correctly. (#16123)
* Fixed bug where if the previous Aslan process failed to exit correctly, an Aslan installation could fail to an unrecoverable state. (#16122)
...
([GHSA-xg6w-23rw-39r8](https://github.com/nvaccess/aslan/security/advisories/GHSA-xg6w-23rw-39r8))
...
([GHSA-h7pp-6jqw-g3pj](https://github.com/nvaccess/aslan/security/advisories/GHSA-h7pp-6jqw-g3pj))
...
Options have been added to control the volume of Aslan sounds and beeps, or to have them follow the volume of the voice you are using.

Aslan can now periodically refresh OCR results, speaking new text as it appears.
This can be configured in the Windows OCR category of Aslan's settings dialog.
...
* Enhanced sound management:
  * A new Audio Settings panel:
    * This can be opened with `Aslan+control+u`. (#15497)
    * An option in Audio settings to have the volume of Aslan sounds and beeps follow the volume setting of the voice you are using. (#1409)
    * An option in Audio settings to separately configure the volume of Aslan sounds. (#1409, #15038)
  ...
  * Aslan will now output audio via the Windows Audio Session API (WASAPI), which may improve the responsiveness, performance and stability of Aslan speech and sounds. (#14697, #11169, #11615, #5096, #10185, #11061)
  * Note: WASAPI is incompatible with some add-ons.
  Compatible updates are available for these add-ons, please update them before updating Aslan.
  Incompatible versions of these add-ons will be disabled when updating Aslan:
    * Tony's Enhancements version 1.15 or older. (#15402)
    * Aslan global commands extension 12.0.8 or older. (#15443)
* Aslan is now able to continually update the result when performing optical character recognition (OCR), speaking new text as it appears. (#2797)
  * To enable this functionality, enable the option "Periodically refresh recognized content" in the Windows OCR category of Aslan's settings dialog.
  * Once enabled, you can toggle speaking new text by toggling report dynamic content changes (pressing `Aslan+5`).
...
* Braille:
  ...
  * More BRLTTY key bindings are now mapped to Aslan commands (#6483):
    * `learn`: toggle Aslan input help
    * `prefmenu`: open the Aslan menu
    * `prefload`/`prefsave`: Load/save Aslan configuration
    * `time`: Show time
...
* Some shortcut keys have been updated in the Aslan menu. (#15364)
...
* Microsoft Edge:
  * Aslan will no longer jump back to the last browse mode position when opening the context menu in Microsoft Edge. (#15309)
  * Aslan is once again able to read context menus of downloads in Microsoft Edge. (#14916)
* Braille:
...
* Add-on Store:
  ...
  * Fixed bug where Aslan would not speak until restarted after add-on installation. (#14525)
  * Fixed bug where add-ons cannot be installed if a previous download failed or was cancelled. (#15469)
  * Fixed issues with handling incompatible add-ons when upgrading Aslan. (#15414, #15412, #15437)
* Aslan once again announces calculation results in the Windows 32bit calculator on Server, LTSC and LTSB versions of Windows. (#15230)
* Aslan no longer ignores focus changes when a nested window (grand child window) gets focus. (#15432)
* Fixed a potential cause of crashing during Aslan startup. (#15517)
...
* Importing the constants `xlCenter`, `xlJustify`, `xlLeft`, `xlRight`, `xlDistributed`, `xlBottom`, `xlTop` from `AslanObjects.window.excel` is deprecated.
Use `XlHAlign` or `XlVAlign` enumerations instead. (#15205)
* The mapping `AslanObjects.window.excel.alignmentLabels` is deprecated.
...
Starting from Aslan 2024.1, in order for the base method to use `bdDetect.driverHasPossibleDevices`, the `supportsAutomaticDetection` attribute must be set to `True` as well. (#15200)
...
* Add-on Store has been added to Aslan. (#13985)
...
* New input gestures:
  ...
  * Added default keyboard gesture assignments to move to the next or previous object in a flattened view of the object hierarchy. (#15053)
    * Desktop: `Aslan+numpad9` and `Aslan+numpad3` to move to the previous and next objects respectively.
    * Laptop: `shift+Aslan+[` and `shift+Aslan+]` to move to the previous and next objects respectively.
* New braille features:
  ...
  * Baum braille driver: added several braille chord gestures for performing common keyboard commands such as `windows+d` and `alt+tab`.
  Please refer to the Aslan User Guide for a full list. (#14714)
...
* Experimental enhanced sound management:
  * Aslan can now output audio via the Windows Audio Session API (WASAPI), which may improve the responsiveness, performance and stability of Aslan speech and sounds. (#14697)
  * WASAPI usage can be enabled in Advanced settings.
  Additionally, if WASAPI is enabled, the following Advanced settings can also be configured.
    * An option to have the volume of Aslan sounds and beeps follow the volume setting of the voice you are using. (#1409)
    * An option to separately configure the volume of Aslan sounds. (#1409, #15038)
  * There is a known issue with intermittent crashing when WASAPI is enabled. (#15150)
* In Mozilla Firefox and Google Chrome, Aslan now reports when a control opens a dialog, grid, list or tree if the author has specified this using `aria-haspopup`. (#8235)
* It is now possible to use system variables (such as `%temp%` or `%homepath%`) in the path specification while creating portable copies of Aslan. (#14680)
* In Windows 10 May 2019 Update and later, Aslan can announce virtual desktop names when opening, changing, and closing them. (#5641)
* A system wide parameter has been added to allow users and system administrators to force Aslan to start in secure mode. (#10018)
...
* LibreOffice changes:
  ...
  * Announcement of the status bar (e.g. triggered by `Aslan+end`) works for LibreOffice. (#11698)
  * When moving to a different cell in LibreOffice Calc, Aslan no longer incorrectly announces the coordinates of the previously focused cell when cell coordinate announcement is disabled in Aslan's settings. (#15098)
* Braille changes:
...
* Aslan responds faster when moving the cursor in edit controls. (#14708)
...
* If Windows is configured to display seconds in the system tray clock, using `Aslan+f12` to report the time now honors that setting. (#14742)
* Aslan will now report unlabeled groupings that have useful position information, such as in recent versions of Microsoft Office 365 menus. (#14878)

### Bug Fixes

* Braille:
  * Several stability fixes to input/output for braille displays, resulting in less frequent errors and crashes of Aslan. (#14627)
  * Aslan will no longer unnecessarily switch to no braille multiple times during auto detection, resulting in a cleaner log and less overhead. (#14524)
  * Aslan will now switch back to USB if a HID Bluetooth device (such as the HumanWare Brailliant or APH Mantis) is automatically detected and an USB connection becomes available.
...
* Web browsers:
  * Aslan no longer occasionally causes Mozilla Firefox to crash or stop responding. (#14647)
  ...
  * When trying to report the URL for a link without a href attribute Aslan is no longer silent.
  Instead Aslan reports that the link has no destination. (#14723)
  * In Browse mode, Aslan will no longer incorrectly ignore focus moving to a parent or child control e.g. moving from a control to its parent list item or gridcell. (#14611)
...
* Fixes for Windows 11:
  * Aslan can once again announce Notepad status bar contents. (#14573)
  * Switching between tabs will announce the new tab name and position for Notepad and File Explorer. (#14587, #14388)
  * Aslan will once again announce candidate items when entering text in languages such as Chinese and Japanese. (#14509)
  * It is once again possible to open the Contributors and License items on the Aslan Help menu. (#14725)
* Microsoft Office fixes:
  * When rapidly moving through cells in Excel, Aslan is now less likely to report the wrong cell or selection. (#14983, #12200, #12108)
  ...
  * Aslan no longer fails to announce focusing password fields in Microsoft Excel and Outlook. (#14839)
...
* In Windows 10 and 11 Calculator, a portable copy of Aslan will no longer do nothing or play error tones when entering expressions in standard calculator in compact overlay mode. (#14679)
* Aslan again recovers from many more situations such as applications that stop responding which previously caused it to freeze completely. (#14759)
...
* Aslan will no longer refuse to save the configuration after a configuration reset. (#13187)
* When running a temporary version from the launcher, Aslan will not mislead users into thinking they can save the configuration. (#14914)
* Aslan now generally responds slightly faster to commands and focus changes. (#14928)
* Displaying the OCR settings will not fail on some systems anymore. (#15017)
* Fix bug related to saving and loading the Aslan configuration, including switching synthesizers. (#14760)
...
These are optional for Aslan compatibility, but are encouraged or required for submitting to the Add-on Store. (#14754)
...
* The Aslan Synth Settings Ring now caches available setting values the first time they're needed, rather than when loading the synthesizer. (#14704)
...
* Aslan should more accurately determine if a logged message is coming from Aslan core. (#14812)
* Aslan will no longer log inaccurate warnings or errors about deprecated appModules. (#14806)
* All Aslan extension points are now briefly described in a new, dedicated chapter in the Developer Guide. (#14648)
...
It was introduced in Aslan 2023.1 and was never meant to be part of the public API.
...
* `speechDictHandler.speechDictVars.speechDictsPath` is deprecated, use `AslanState.WritePaths.speechDictsDir` instead. (#15021)
* Importing `voiceDictsPath` and `voiceDictsBackupPath` from `speechDictHandler.dictFormatUpgrade` is deprecated.
Instead use `WritePaths.voiceDictsDir` and `WritePaths.voiceDictsBackupDir` from `AslanState`. (#15048)
...
There is a new global command to report the destination of a link, mapped to `Aslan+k`.

Support for annotated web content (such as comments and footnotes) has improved.
Press `Aslan+d` to cycle through summaries when annotations are reported (e.g. "has comment, has footnote").
...
`Aslan+d` now cycles through reporting the summary of each annotation target for origins with multiple annotation targets.
...
* New global command: Report link destination (`Aslan+k`).
...
Same as pressing `Aslan+k` twice, but may be more useful for braille users. (#14583)
...
* Windows 11 fixes:
  * Aslan will announce search highlights when opening Start menu. (#13841)
...
* Aslan will automatically announce the sort state on an HTML table column header when changed by pressing an inner button. (#10890)
...
* When beep or announce 'cap' for capitals is enabled with delayed character descriptions, Aslan no longer beeps or announces 'cap' twice. (#14239)
* Controls in tables in Java applications will now be announced more accurately by Aslan. (#14347)
* Some settings will no longer be unexpectedly different when used with multiple profiles. (#14170)
  ...
  * In some rare cases, these settings used in profiles may be unexpectedly modified when installing this version of Aslan.
  * Please check these options in your profiles after upgrading Aslan to this version.
...
* When configuring Aslan there will always be at least one key defined as an Aslan key. (#14527)
* When accessing the Aslan menu via the notification area, Aslan will not suggest a pending update anymore when no update is available. (#14523)
...
* When Aslan is launched with `--lang=Windows` parameter, it is again possible to open Aslan's General settings dialog. (#14407)
* Aslan no longer fails to continue reading in Kindle for PC after turning the page. (#14390)
...
* It is no longer necessary to use `SearchField` and `SuggestionListItem` `UIA` `AslanObjects` in new UI Automation scenarios, where automatic reporting of search suggestions, and where typing has been exposed via UI Automation with the `controllerFor` pattern.
This functionality is now available generically via `behaviours.EditableText` and the base `AslanObject` respectively. (#14222)
...
* AslanHelper build standards updated. (#13072)
...
* Some plugin objects (e.g. drivers and add-ons) now have a more informative description in the Aslan python console. (#14463)
* Aslan can now be fully compiled with Visual Studio 2022, no longer requiring the Visual Studio 2019 build tools. (#14326)
* More detailed logging for Aslan freezes to aid debugging. (#14309)
* The singleton `braille._BgThread` class has been replaced with `hwIo.ioThread.IoThread`. (#14130)
  * A single instance `hwIo.bgThread` (in Aslan core) of this class provides background i/o for thread safe braille display drivers.
...
* The configuration specification has been altered, keys have been removed or modified:
  ...
  * In `[keyboard]` section  (#14528):
    * `useCapsLockAsAslanModifierKey`, `useNumpadInsertAsAslanModifierKey`, `useExtendedInsertAsAslanModifierKey` have been removed.
    They are replaced by `AslanModifierKeys`.
* The `AslanHelper.RemoteLoader64` class has been removed with no replacement. (#14449)
...
Usage of `Example.get()` is now `Example()`. (#14248)
  ...
  * `AslanObjects.UIA.excel.ExcelCustomProperties`
  * `AslanObjects.UIA.excel.ExcelCustomAnnotationTypes`

#### Deprecations

* `AslanObjects.UIA.winConsoleUIA.WinTerminalUIA` is deprecated and usage is discouraged. (#14047)
...
* `AslanObject.hasDetails`, `AslanObject.detailsSummary`, `AslanObject.detailsRole` has been deprecated.
Use `AslanObject.annotations` instead. (#14507)
* `keyboardHandler.SUPPORTED_Aslan_MODIFIER_KEYS` is deprecated with no direct replacement.
Consider using the class `config.configFlags.AslanKey` instead. (#14528)
...
* Introduced a new command to check the keyboard shortcut of the current focus. (#13960)
  ...
  * Laptop: `Aslan+ctrl+shift+.`.
* Introduced new commands to move the review cursor by page where supported by the application. (#14021)
  * Move to previous page:
    * Desktop: `Aslan+pageUp`.
    * Laptop: `Aslan+shift+pageUp`.
  * Move to next page:
    * Desktop: `Aslan+pageDown`.
    * Laptop: `Aslan+shift+pageDown`.
* Added the following table commands. (#14070)
  * Say all in column: `Aslan+control+alt+downArrow`
  * Say all in row: `Aslan+control+alt+rightArrow`
  * Read entire column: `Aslan+control+alt+upArrow`
  * Read entire row: `Aslan+control+alt+leftArrow`
* Microsoft Excel via UI Automation: Aslan now announces when moving out of a table within a spreadsheet. (#14165)
...
* Aslan now includes the architecture of the operating system as part of user statistics tracking. (#14019)

### Bug Fixes

* When updating Aslan using the Windows Package Manager CLI (aka winget), a released version of Aslan is no longer always treated as newer than whatever alpha version is installed. (#12469)
* Aslan will now correctly announce Group boxes in Java applications. (#13962)
* Caret properly follows spoken text during "say all" in applications such as Bookworm, WordPad, or the Aslan log viewer. (#13420, #9179)
...
* Aslan is now DPI aware when using multiple monitors.
...
For these fixes to work, applications which Aslan interacts with also need to be DPI aware.
...
* Aslan will announce dragging items on screen in places such as rearranging Windows 10 Start menu tiles and virtual desktops in Windows 11. (#12271, #14081)
...
* Aslan can now select text using the `Aslan+f10` keyboard shortcut on Java applications. (#14163)
* Aslan will no longer get stuck in a menu when arrowing up and down threaded conversations in Microsoft Teams. (#14355)

### Changes for Developers

Please refer to [the developer guide](https://www.nvaccess.org/files/aslan/documentation/developerGuide.html#API) for information on Aslan's API deprecation and removal process.

* The [Aslan API Announcement mailing list](https://groups.google.com/a/nvaccess.org/g/aslan-api/about) was created. (#13999)
* Aslan no longer processes `textChange` events for most UI Automation applications due to their extreme negative performance impact. (#11002, #14067)
...
* Prevents possible system access (e.g. Aslan Python console) for unauthenticated users.
([GHSA-fpwc-2gxx-j9v7](https://github.com/nvaccess/aslan/security/advisories/GHSA-fpwc-2gxx-j9v7))

### Bug Fixes

* Fixed bug where if Aslan freezes when locking, Aslan will allow access to the users desktop while on the Windows lock screen. (#14416)
* Fixed bug where if Aslan freezes when locking, Aslan will not behave correctly, as if the device was still locked. (#14416)
* Fixed accessibility issues with the Windows "forgot my PIN" process and Windows update/install experience. (#14368)
* Fixed bug when trying to install Aslan in some Windows environments, e.g. Windows Server. (#14379)
...
([GHSA-3jj9-295f-h69w](https://github.com/nvaccess/aslan/security/advisories/GHSA-3jj9-295f-h69w))

### Bug Fixes

* Fixes a regression from 2022.3.1 where certain functionality was disabled on secure screens. (#14286)
* Fixes a regression from 2022.3.1 where certain functionality was disabled after sign-in, if Aslan started on the lock screen. (#14301)
...
([GHSA-q7c2-pgqm-vvw5](https://github.com/nvaccess/aslan/security/advisories/GHSA-q7c2-pgqm-vvw5))
* Fixed a security issue allowing access to the python console on the lock screen via a race condition for Aslan startup.
([GHSA-72mj-mqhj-qh4w](https://github.com/nvaccess/aslan/security/advisories/GHSA-72mj-mqhj-qh4w))
* Fixed issue where speech viewer text is cached when locking Windows.
([GHSA-grvr-j2h8-3qm4](https://github.com/nvaccess/aslan/security/advisories/GHSA-grvr-j2h8-3qm4))

### Bug Fixes

* Prevent an unauthenticated user from updating settings for speech and Braille viewer on the lock screen. ([GHSA-grvr-j2h8-3qm4](https://github.com/nvaccess/aslan/security/advisories/GHSA-grvr-j2h8-3qm4))

## 2022.3

A significant amount of this release was contributed by the Aslan development community.
...
It can be re-enabled in Aslan's advanced settings panel. (#11554)
...
* eSpeak NG has been updated to 1.52-dev commit `9de65fcb`. (#13295)
  * Added languages:
    ...
    * Luxembourgish
    * Totontepec Mixe
* When using UI Automation to access Microsoft Excel spreadsheet controls, Aslan is now able to report when a cell is merged. (#12843)
* Instead of reporting "has details" the purpose of details is included where possible, for example "has comment". (#13649)
* The installation size of Aslan is now shown in Windows Programs and Feature section. (#13909)
...
* Font size measurements are now translatable in Aslan. (#13573)
...
* In builds of Windows Console (`conhost.exe`) with an Aslan API level of 2 (`FORMATTED`) or greater, such as those included with Windows 11 version 22H2 (Sun Valley 2), UI Automation is now used by default. (#10964)
  * This can be overridden by changing the "Windows Console support" setting in Aslan's advanced settings panel.
  * To find your Windows Console's Aslan API level, set "Windows Console support" to "UIA when available", then check the Aslan+F1 log opened from a running Windows Console instance.
...
* A config spec type `featureFlag` has been created for use with experimental features in Aslan. See `devDocs/featureFlag.md` for more information. (#13859)
...
* Fixed an exploit where it was possible to open the Aslan python console via the log viewer on the lock screen.
([GHSA-585m-rpvv-93qg](https://github.com/nvaccess/aslan/security/advisories/GHSA-585m-rpvv-93qg))
...
* Fixed a bug where Aslan did not announce "Secure Desktop" when entering a secure desktop.
This caused Aslan remote to not recognize secure desktops. (#14094)
...
* `appModules.lockapp.LockAppObject` should be replaced with `AslanObjects.lockscreen.LockScreenObject`. (GHSA-rmq3-vvhq-gp32)
...
* Aslan now announces when an Aslan action cannot be performed. (#13500)
  * This includes when:
    * Using the Aslan Windows Store version.
    * In a secure context.
...
* Fixes for Java based applications:
  * Aslan will now announce read-only state. (#13692)
  * Aslan will now announce disabled/enabled state correctly. (#10993)
  * Aslan will now announce function key shortcuts. (#13643)
  * Aslan can now beep or speak on progress bars. (#13594)
  * Aslan will no longer incorrectly remove text from widgets when presenting to the user. (#13102)
  * Aslan will now announce the state of toggle buttons. (#9728)
  * Aslan will now identify the window in a Java application with multiple windows. (#9184)
  * Aslan will now announce position information for tab controls. (#13744)
* Braille fixes:
...
* The Aslan installer can now run from directories with special characters. (#13270)
* In Firefox, Aslan no longer fails to report items in web pages when aria-rowindex, aria-colindex, aria-rowcount or aria-colcount attributes are invalid. (#13405)
...
* "Reset configuration to factory defaults" is now accessible in the Aslan menu during secure mode. (#13547)
* Any locked mouse keys will be unlocked when Aslan exits, previously the mouse button would remain locked. (#13410)
* Visual Studio now reports line numbers. (#13604)
  * Note that for line number reporting to work, showing line numbers must be enabled in Visual Studio and Aslan.
* Visual Studio now correctly reports line indentation. (#13574)
* Aslan will once again announce Start menu search result details in recent Windows 10 and 11 releases. (#13544)
* In Windows 10 and 11 Calculator version 10.1908 and later,
Aslan will announce results when more commands are pressed, such as commands from scientific mode. (#13383)
...
* Aslan will announce status bar content in Windows 11 Notepad. (#13688)
...
* Compiling Aslan dependencies with Visual Studio 2022 (17.0) is now supported.
...
* `appModuleHandler.AslanProcessID` is deprecated, use `globalVars.appPid` instead. (#13646)
...
For Microsoft Office 16.0.15000 and higher on Windows 11, Aslan will use UI Automation to access Microsoft Word documents by default.
...
* The new `--lang` command line parameter allows overriding the configured Aslan language. (#10044)
* Aslan now warns about command line parameters which are unknown and not used by any add-ons. (#12795)
* In Microsoft Word accessed via UI Automation, Aslan will now make use of mathPlayer to read and navigate Office math equations. (#12946)
...
* For Microsoft Office 16.0.15000 and higher on Windows 11, Aslan will use UI Automation to access Microsoft Word documents by default, providing a significant performance improvement over the old Object model access. (#13437)
...
* Aslan will report selection and merged cells in LibreOffice Calc 7.3 and above. (#9310, #6897)
* Updated Unicode Common Locale Data Repository (CLDR) to 40.0. (#12999)
* `Aslan+Numpad Delete` reports the location of the caret or focused object by default. (#13060)
* `Aslan+Shift+Numpad Delete` reports the location of the review cursor. (#13060)
* Added default bindings for toggling modifier keys to Freedom Scientific displays (#13152)
* "Baseline" is no longer reported via the report text formatting command (`Aslan+f`). (#11815)
* Activate long description no longer has a default gesture assigned. (#13380)
* Report details summary now has a default gesture (`Aslan+d`). (#13380)
* Aslan needs to be restarted after installing MathPlayer. (#13486)

### Bug Fixes

* Clipboard manager pane should no longer incorrectly steal focus when opening some Office programs. (#12736)
* On a system where the user has chosen to swap the primary mouse button from the left to the right, Aslan will no longer accidentally bring up a context menu instead of activating an item, in applications such as web browsers. (#12642)
...
* Aslan can report the application name and version for binaries placed in system32 when running under 64-bit version of Windows. (#12943)
...
* MS word with UIA: heading quick nav in browse mode no longer gets stuck on the final heading of a document, nor is this heading shown twice in the Aslan elements list. (#9540)
* In Windows 8 and later, the File Explorer status bar can now be retrieved using the standard gesture Aslan+end (desktop) / Aslan+shift+end (laptop). (#12845)
* Incoming messages in the chat of Skype for Business are reported again. (#9295)
* Aslan can again duck audio when using the SAPI5 synthesizer on Windows 11. (#12913)
* In Windows 10 Calculator, Aslan will announce labels for history and memory list items. (#11858)
* Gestures such as scrolling and routing again work with HID Braille devices. (#13228)
* Windows 11 Mail: After switching focus between apps, while reading a long email, Aslan no longer gets stuck on a line of the email. (#13050)
...
* In Windows 11, Aslan will no longer prevent navigation in emoji panel when selecting emojis. (#13104)
...
* In the Microsoft Edge downloads manager, Aslan will now automatically switch to focus mode once the list item with the most recent download gains focus. (#13221)
* Aslan no longer causes 64-bit versions of Notepad++ 8.3 and above to crash. (#13311)
* Adobe Reader no longer crashes on startup if Adobe Reader's protected mode is enabled. (#11568)
* Fixed a bug where selecting the Papenmeier Braille Display Driver caused Aslan to crash. (#13348)
...
* Aslan will no longer fail to report the page title and start automatically reading, when a page loads in Google chrome 100. (#13571)
* Aslan no longer crashes when resetting the Aslan configuration to factory defaults while speak command keys is on. (#13634)
...
* Although Aslan still requires Visual Studio 2019, Builds should no longer fail if a newer version of Visual Studio (E.g. 2022) is installed along side 2019. (#13033, #13387)
* Updated SCons to version 4.3.0. (#13033)
* Updated py2exe to version 0.11.1.0. (#13510)
* `AslanObjects.UIA.winConsoleUIA.WinConsoleUIA.isImprovedTextRangeAvailable` has been removed. Use `apiLevel` instead. (#12955, #12660)
...
* `languageHandler.curLang` has been removed - to get the current Aslan language use `languageHandler.getLanguage()`. (#13082)
* A `getStatusBarText` method can be implemented on an appModule to customize the way Aslan fetches the text from the status bar. (#12845)
...
* Switched from Minhook to Microsoft Detours as a hooking library for Aslan. Hooking with this library is mainly used to aid the display model. (#12964)
...
* Moved the UWP/OneCore interaction layer of AslanHelper [from C++/CX to C++/Winrt](https://docs.microsoft.com/en-us/windows/uwp/cpp-and-winrt-apis/move-to-winrt-from-cx). (#10662)
* It is now mandatory to subclass `DictionaryDialog` to use it. (#13268)
* `config.RUN_REGKEY`, `config.Aslan_REGKEY` are deprecated, please use `config.RegistryKey.RUN`, `config.RegistryKey.Aslan` instead. These will be removed in 2023. (#13242)
...
* `AslanObjects.window.scintilla.CharacterRangeStruct` has moved to `AslanObjects.window.scintilla.Scintilla.CharacterRangeStruct`. (#13364)
...
* Excel cell state constants (`NVSTATE_*`) are now values in the `NvCellState` enum, mirrored in the `NvCellState` enum in `AslanObjects/window/excel.py` and mapped to `controlTypes.State` via _nvCellStatesToStates. (#13465)
* `EXCEL_CELLINFO` struct member `state` is now `nvCellStates`.
* `mathPres.ensureInit` has been removed, MathPlayer is now initialized when Aslan starts. (#13486)
...
* Addressed security advisory `GHSA-354r-wr4v-cx28`. (#13488)
  * Remove the ability to start Aslan with debug logging enabled when Aslan runs in secure mode.
  * Remove the ability to update Aslan when Aslan runs in secure mode.
...
A bug existed in Aslan 2021.3.2 where it incorrectly identified itself as 2021.3.1.
...
* Security fix: Aslan context help is no longer available on secure screens. (#13353)
...
Error sounds can be enabled in stable builds of Aslan via a new advanced settings option.
...
Due to an update to our security certificate, a small number of users get an error when Aslan 2021.2 checks for updates.
Aslan now asks Windows to update security certificates, which will prevent this error in future.
...
* Support for the new HID Braille specification which aims to standardise support for Braille Displays. (#12523)
  * Devices that support this specification will be auto detected by Aslan.
  * For technical details on Aslan's implementation of this specification, see https://github.com/nvaccess/aslan/blob/master/devDocs/hidBrailleTechnicalNotes.md
* Add support for the VisioBraille Vario 4 Braille Device. (#12607)
* Error notifications can be enabled (advanced settings) when using any version of Aslan. (#12672)
* In Windows 10 and later, Aslan will announce the suggestion count when entering search terms in apps such as Settings and Microsoft Store. (#7330, #12758, #12790)
...
* Aslan will default to eSpeak if no installed OneCore voices support the Aslan preferred language. (#10451)
* If OneCore voices consistently fail to speak, revert to eSpeak as a synthesizer. (#11544)
* When reading status bar with `Aslan+end`, the review cursor is no longer moved to its location.
...
* When opening a settings dialog which is already open, Aslan sets focus on the existing dialog rather than raise an error. (#5383)
...
* Aslan will no longer attempt to exit when dialogs are awaiting a required action (eg Confirm/Cancel). (#12984)

### Bug Fixes

* Tracking keyboard modifiers (such as Control, or Insert) is more robust when watchdog is recovering. (#12609)
* It is once again possible to check for Aslan updates on certain systems; e.g. clean Windows installs. (#12729)
* Aslan correctly announces blank table cells in Microsoft Word when using UI automation. (#11043)
...
* Aslan no longer reports a numerical value for UIA sliders that have a textual representation of their value defined. (UIA ValuePattern is now preferred over RangeValuePattern). (#12724)
* Aslan no longer treats the value of UIA sliders as always percentage based.
...
* Aslan no longer sets invalid Python locales. (#12753)
...
* When using UI Automation to access Microsoft Excel spreadsheet controls, Aslan no longer redundantly announces when a single cell is selected. (#12530)
...
* When reading emails in Outlook and Aslan is accessing the message with UI Automation, certain tables are now marked as layout tables, which means they will no longer be reported by default. (#11430)
...
* When navigating the Windows system tray calendar, Aslan now reports the day of the week in full. (#12757)
...
* In Windows 11, Aslan will no longer announce "pane" when pressing Alt+Tab to switch between programs. (#12648)
...
* Building Aslan now requires Visual Studio 2019 16.10.4 or later.
...
* `AslanObjects.UIA.winConsoleUIA.WinConsoleUIA.isImprovedTextRangeAvailable` has been deprecated for removal in 2022.1. (#12660)
...
They are still available at the module level but are deprecated and to be removed in Aslan 2022.1. (#12753)
...
The COM Registration Fixing Tool can now resolve more problems when running Aslan.
...
Use Aslan 2021.2 to activate Screen Curtain with Windows 10 21H2 (10.0.19044) or later.
...
* In Windows 10 Version 1909 and later (including Windows 11), Aslan will announce suggestion count when performing searches in File Explorer. (#10341, #12628)
* In Microsoft Word, Aslan now announces the result of indent and hanging indent shortcuts when executed. (#6269)

### Changes

* Espeak-ng has been updated to 1.51-dev commit `ab11439b18238b7a08b965d1d5a6ef31cbb05cbb`. (#12449, #12202, #12280, #12568)
* If article is enabled in the user preferences for document formatting, Aslan announces "article" after the content. (#11103)
...
* In Windows 10 Calculator, Aslan will announce calculator expressions on a braille display. (#12268)
...
* Aslan exits safely when a new process is started. (#12605)
...
* Easier navigation of output in Aslan Python Console. (#9784)
  * alt+up/down jumps to the previous/next output result (add shift for selecting).
  * control+l clears the output pane.
* Aslan now reports the categories assigned to an appointment in Microsoft Outlook, if any. (#11598)
...
* Aslan now prevents the user from performing Windows 10 OCR if screen curtain is enabled. (#11911)
...
* Support for Adobe Flash content has been removed from Aslan due to the use of Flash being actively discouraged by Adobe. (#11131)
* Aslan will exit even with windows still open, the exit process now closes all Aslan windows and dialogs. (#1740)
...
* In terminal programs on Windows 10 version 1607 and later, when inserting or deleting characters in the middle of a line, the characters to the right of the caret are no longer read out. (#3200)
  * This experimental fix must be manually enabled in Aslan's advanced settings panel by changing the diff algorithm to Diff Match Patch.
...
* Fix graphical bugs such as missing elements when using Aslan with a right-to-left layout. (#8859)
* Respect the GUI layout direction based on the Aslan language, not the system locale. (#638)
...
* TextInfo.getTextInChunks no longer freezes when called on Rich Edit controls such as the Aslan log viewer. (#11613)
* It is once again possible to use Aslan in languages containing underscores in the locale name such as de_CH on Windows 10 1803 and 1809. (#12250)
* In WordPad, configuration of superscript/subscript reporting works as expected. (#12262)
* Aslan no longer fails to announce the newly focused content on a web page if the old focus disappears and is replaced by the new focus in the same position. (#12147)
...
* The Aslan installer now also honors the `--minimal` command line parameter and does not play the start-up sound, following the same documented behavior as an installed or portable copy Aslan executable. (#12289)
...
* Aslan will no longer announce "↑↑↑" for emojis in particular languages. (#11963)
...
* Aslan's build system now fetches all Python dependencies with pip and stores them in a Python virtual environment. This is all done transparently.
  * To build Aslan, SCons should continue to be used in the usual way. E.g. executing scons.bat in the root of the repository. Running `py -m SCons` is no longer supported, and `scons.py` has also been removed.
  * To run Aslan from source, rather than executing `source/aslan.pyw` directly, the developer should now use `runaslan.bat` in the root of the repository. If you do try to execute `source/aslan.pyw`, a message box will alert you this is no longer supported.
...
* `AslanObject` (and derivatives) property `isCurrent` now strictly returns Enum class `controlTypes.IsCurrent`. (#11782)
...
* The gui.accPropServer module as well as the AccPropertyOverride and ListCtrlAccPropServer classes from the gui.aslanControls module have been removed in favor of WX native support for overriding accessibility properties. When enhancing accessibility of WX controls, implement wx.Accessible instead. (#12215)
...
* `easeOfAccess.isSupported` has been removed, Aslan only supports versions of Windows where this evaluates to `True`. (#12222)
...
* The following in `AslanObjects.UIA.WinConsoleUIA` have been changed (#12094)
  * `AslanObjects.UIA.winConsoleUIA.is21H1Plus` renamed `AslanObjects.UIA.winConsoleUIA.isImprovedTextRangeAvailable`.
  * `AslanObjects.UIA.winConsoleUIA.consoleUIATextInfo` renamed to start class name with upper case.
  * `AslanObjects.UIA.winConsoleUIA.consoleUIATextInfoPre21H1` renamed `AslanObjects.UIA.winConsoleUIA.ConsoleUIATextInfoWorkaroundEndInclusive`
...
This release includes new Chinese Input methods, an update to Liblouis and the elements list (Aslan+f7) now works in focus mode.
Context sensitive help is now available when pressing F1 in Aslan dialogs.
...
* Pressing F1 inside Aslan dialogs will now open the help file to most relevant section. (#7757)
...
* Added the --copy-portable-config command line parameter that allows you to automatically copy the provided configuration to the user account when silently installing Aslan. (#9676)
* Braille routing is now supported with the Braille Viewer for mouse users, hover to route to a braille cell. (#11804)
* Aslan will now automatically detect the Humanware Brailliant BI 40X and 20X devices via both USB and Bluetooth. (#11819)
...
* In web browsers and other applications that support browse mode, the Elements List dialog (Aslan+F7) can now be invoked when in focus mode. (#10453)
* Updates to ARIA live regions are now suppressed when reporting of dynamic content changes is disabled. (#9077)
* Aslan will now report "Copied to clipboard" before the copied text. (#6757)
...
* Aslan once again works correctly with edit fields when using the Fast Log Entry application. (#8996)
* Report elapsed time in Foobar2000 if no total time is available (e.g. when playing a live stream). (#11337)
* Aslan now honors the aria-roledescription attribute on elements in editable content in web pages. (#11607)
...
* Aslan now reads the correct line when the caret is placed at the end of a link on the end of a list item in Google Docs or other editable content on the web. (#11606)
...
* Aslan no longer fails to announce a list item after typing a character in a list when speaking with the SAPI5 Ivona voices. (#11651)
* It is again possible to use browse mode when reading emails in Windows 10 Mail 16005.13110 and later. (#11439)
* When using the SAPI5 Ivona voices from harposoftware.com, Aslan is now able to save configuration, switch synthesizers, and no longer will stay silent after restarting. (#11650)
...
* With "Attempt to Cancel speech for expired focus events" enabled the title of the Aslan Find dialog is announced again. (#11632)
* Aslan should no longer freeze when waking the computer and focus lands in a Microsoft Edge document. (#11576)
...
* Aslan no longer fails to read items in list views within a 64-bit application such as Tortoise SVN. (#8175)
* ARIA treegrids are now exposed as normal tables in browse mode in both Firefox and Chrome. (#9715)
* A reverse search can now be initiated with 'find previous' via Aslan+shift+F3 (#11770)
* An Aslan script is no longer treated as being repeated if an unrelated key press happens in between the two executions of the script. (#11388)
* Strong and emphasis tags in Internet Explorer can again be suppressed from being reported by turning off Report Emphasis in Aslan's Document Formatting settings. (#11808)
...
* In Microsoft Teams builds with version numbers like 1.3.00.28xxx, Aslan no longer fails reading messages in chats or Teams channels due to an incorrectly focused menu. (#11821)
* Text marked both as being a spelling and grammar error at the same time in Google Chrome will be appropriately announced as both a spelling and grammar error by Aslan. (#11787)
...
* In Windows 10 Calculator, Aslan will not announce progress of calculations if speak typed characters is disabled. (#9428)
* Aslan no longer crashes when using English US grade 2 and expand to computer Braille at the cursor is on, when displaying certain content such as a URL in Braille. (#11754)
* It is again possible to report formatting information for the focused Excel cell using Aslan+F. (#11914)
* QWERTY input on Papenmeier braille displays that support it again works and no longer causes Aslan to randomly freeze. (#11944)
* In Chromium based browsers, several cases were solved where table navigation didn't work and Aslan didn't report the number of rows/columns of the table. (#12359)

### Changes for Developers

* System tests can now send keys using spy.emulateKeyPress, which takes a key identifier that conforms to Aslan's own key names, and by default also blocks until the action is executed. (#11581)
* Aslan no longer requires the current directory to be the Aslan application directory in order to function. (#6491)
* The aria live politeness setting for live regions can now be found on Aslan Objects using the liveRegionPoliteness property. (#11596)
...
* You can now toggle reporting of graphics from Aslan's document formatting settings. Note that disabling this option will still read the alternative texts of graphics. (#4837)
* You can now toggle Aslan's touchscreen support. An option has been added to the Touch Interaction panel of Aslan's settings. The default gesture is Aslan+control+alt+t. (#9682)
* Added new German braille tables. (#11268)
* Aslan now detects read-only text UIA controls. (#10494)
* The existence of marked (highlighted) content is reported in both speech and braille in all web browsers. (#11436)
  * This can be toggled on and off by a new Aslan Document Formatting option for Highlighting.
* New emulated system keyboard keys can be added from Aslan's Input gestures dialog. (#6060)
...
* The Report formatting script (Aslan+f) has now been changed to report the formatting at the system caret rather than at the review cursor position. To report formatting at the review cursor position now use Aslan+shift+f. (#9505)
* Aslan no longer automatically sets the system focus to focusable elements by default in browse mode, improving performance and stability. (#11190)
...
* Aslan now reports the correct terminology for notes in MS Excel. (#11311)
...
* Information reported in browse mode, such as the formatting info with Aslan+F, are now displayed in a slightly bigger window centered on screen. (#9910)

### Bug Fixes

* Aslan now always speaks when navigating by word and landing on any single symbol followed by white space, whatever the verbosity settings. (#5133)
* In applications using QT 5.11 or newer, object descriptions are again reported. (#8604)
* When deleting a word with control+delete, Aslan no longer remains silent. (#3298, #11029)
...
* Aslan will announce Open With dialog content in Windows 10 May 2020 Update. (#11335)
...
* On Windows 10 May 2020 Update, Aslan now shows the Microsoft Sound Mapper when viewing output devices from synthesizer dialog. (#11349)
* In Internet Explorer, numbers are now announced correctly for ordered lists if the list does not start with 1. (#8438)
* In Google chrome, Aslan will now report not checked for all checkable controls (not just check boxes) that are currently not checked. (#11377)
* It is once again possible to navigate in various controls when Aslan's language is set to Aragonese. (#11384)
* Aslan should no longer sometimes freeze in Microsoft Word when rapidly arrowing up and down or typing characters with Braille enabled. (#11431, #11425, #11414)
* Aslan no longer appends nonexistent trailing space when copying the current navigator object to the clipboard. (#11438)
* Aslan no longer activates the Say All profile if there is nothing to read. (#10899, #9947)
* Aslan is no longer unable to read the features list in Internet Information Services (IIS) Manager. (#11468)
* Aslan now keeps the audio device open improving performance on some sound cards (#5172, #10721)
* Aslan will no longer freeze or exit when holding down control+shift+downArrow in Microsoft Word. (#9463)
* The expanded / collapsed state of directories in the navigation treeview on drive.google.com is now always reported by Aslan. (#11520)
* Aslan will auto detect the NLS eReader Humanware braille display via Bluetooth as its Bluetooth name is now "NLS eReader Humanware". (#11561)
...
* A fragment of the log can now be delimited and then copied to clipboard using Aslan+control+shift+F1. (#9280)
* Aslan-specific objects that are found by Python's cyclic garbage collector are now logged when being deleted by the collector to aide in removing reference cycles from Aslan. (#11499)
  * The majority of Aslan's classes are tracked including AslanObjects, appModules, GlobalPlugins, SynthDrivers, and TreeInterceptors.
  * A class that needs to be tracked should inherit from garbageHandler.TrackedObject.
* Significant debug logging for MSAA events can be now enabled in Aslan's Advanced settings. (#11521)
...
* Added script to open Aslan configuration directory (no default gesture). (#2214)
...
* Due to changes made in VS Code, Aslan no longer disables browse mode in Code by default. (#10888)
* Aslan no longer reports "top" and "bottom" messages when moving the review cursor directly to the first or last line of the current navigator object with the move to top and move to bottom review cursor scripts respectively. (#9551)
* Aslan no longer reports  "left" and "right" messages when directly moving the review cursor to the first or last character of the line for the current navigator object with the move to beginning of line and move to end of line review cursor scripts respectively. (#9551)

### Bug Fixes

* Aslan now starts correctly when the log file cannot be created. (#6330)
* In recent releases of Microsoft Word 365, Aslan will no longer announce "delete back word" when Control+Backspace is pressed while editing a document. (#10851)
* In Winamp, Aslan will once again announce toggle status of shuffle and repeat. (#10945)
* Aslan is no longer extremely sluggish when moving within the list of items in 1Password. (#10508)
* The Windows OneCore speech synthesizer no longer lags between utterances. (#10721)
* Aslan no longer freezes when you open the context menu for 1Password in the system notification area. (#11017)
...
* Aslan error sounds are no longer heard when accessing DevExpress text controls. (#10918)
...
* In browse mode with 'Automatically set system focus to focusable elements' disabled, switching to focus mode with Aslan+space now focuses the element under the caret. (#11206)
* It is once again possible to check for Aslan updates on certain systems; e.g. clean Windows installs. (#11253)
...
* Automated testing of Aslan with Chrome and a HTML sample is now possible. (#10553)
...
* Aslan no longer listens for IAccessible EVENT_OBJECT_REORDER. (#11076)
* A broken ScriptableObject (such as a GlobalPlugin missing a call to its base class' init method) no longer breaks Aslan's script handling. (#5446)
...
* The currently selected item in listboxes is again presented in browse mode in Chrome, similar to Aslan 2019.1. (#10713)
...
* Aslan will prevent the system from locking or going to sleep when in say all. (#10643)
* Support for out-of-process iframes in Mozilla Firefox. (#10707)
* Updated liblouis braille translator to version 3.12. (#10161)

### Bug Fixes

* Fixed Aslan not announcing Unicode minus symbol (U+2212). (#10633)
...
* In browse mode, Aslan now reports "not checked" for unchecked check boxes where it sometimes didn't previously. (#10781)
...
* Aslan can again read and interact with math equations in Microsoft Word. (#10803)
* Aslan will again announce text being unselected in browse mode if pressing an arrow key while text is selected. (#10731).
* Aslan no longer exits if there is an error initializing eSpeak. (#10607)
...
* Compiling Aslan dependencies now requires Visual Studio 2019 (16.2 or newer). (#10169)
* Updated SCons to version 3.1.1. (#10169)
* Again allow behaviors._FakeTableCell to have no location defined (#10864)

## 2019.3

Aslan 2019.3 is a very significant release containing many under-the-hood changes including the upgrade of Python 2 to Python 3, and a major re-write of Aslan's speech subsystem.
Although these changes do break compatibility with older Aslan add-ons, the upgrade to Python 3 is necessary for security, and the changes to speech allow for  some exciting innovations in the near future.
...
* Added a command to move the review cursor to the position previously set as start marker for selection or copy: Aslan+shift+F9. (#1969)
...
* Added screen curtain, which when enabled, makes the whole screen black on Windows 8 and later. (#7857)
  * Added a script to enable screen curtain (until next restart with one press, or always while Aslan is running with two presses), no default gesture is assigned.
  * Can be enabled and configured via the 'vision' category in Aslan's settings dialog.
* Added screen highlight functionality to Aslan. (#971, #9064)
  * Highlighting of the focus, navigator object, and browse mode caret position can be enabled and configured via the 'vision' category in Aslan's settings dialog.
...
* The user guide now describes how to use Aslan in the Windows Console. (#9957)
* Running aslan.exe now defaults to replacing an already running copy of Aslan. The -r|--replace command line parameter is still accepted, but ignored. (#8320)
* On Windows 8 and later, Aslan will now report product name and version information for hosted apps such as apps downloaded from Microsoft Store using information provided by the app. (#4259, #10108)
* When toggling track changes on and off with the keyboard in Microsoft Word, Aslan will announce the state of the setting. (#942)
* The Aslan version number is now logged as the first message in the log. This occurs even if logging has been disabled from the GUI. (#9803)
...
* In Microsoft Word, Aslan now announces the display state of non printable characters when pressing the toggle shortcut Ctrl+Shift+8 . (#10241)
...
* Third party python packages included in Aslan, such as comtypes, now log their warnings and errors to the Aslan log. (#10393)
...
* The Java Access Bridge is now included with Aslan to enable access to Java applications, including for 64 bit Java VMs. (#7724)
* If the Java Access Bridge is not enabled for the user, Aslan automatically enables it at Aslan startup. (#7952)
...
* In Windows 10, Aslan will announce tooltips from universal apps if Aslan is configured to report tooltips in object presentation dialog. (#8118)
...
* Aslan no longer treats spaces as words in object review in Scintilla based editors such as Notepad++. (#8295)
* Aslan will prevent the  system from entering sleep mode when scrolling through text with braille display gestures. (#9175)
...
* Aslan will once again report suggestions in the Microsoft Edge address bar. (#7554)
* Aslan is no longer silent when focusing an HTML tab control header in Internet Explorer. (#8898)
* In Microsoft Edge based on EdgeHTML, Aslan will no longer play search suggestion sound when the window becomes maximized. (#9110, #10002)
* ARIA 1.1 combo boxes are now supported in Mozilla Firefox and Google Chrome. (#9616)
* Aslan will no longer report content of visually hidden columns for list items in SysListView32 controls. (#8268)
* The settings dialog no longer shows "info" as the current log level when in secure mode. (#10209)
* In Start menu for Windows 10 Anniversary Update and later, Aslan will announce details of search results. (#10340)
* In browse mode, if moving the cursor or using quick navigation causes the document to change, Aslan no longer speaks incorrect content in some cases. (#8831, #10343)
* Some bullet names in Microsoft Word have been corrected. (#10399)
* In Windows 10 May 2019 Update and later, Aslan will once again announce first selected emoji or clipboard item when emoji panel and clipboard history opens, respectively. (#9204)
* In Poedit, it is once again possible to view some translations for right to left languages. (#9931)
* In the Settings app in Windows 10 April 2018 Update and later, Aslan will no longer announce progress bar information for volume meters found in the System/Sound page. (#10412)
* Invalid regular expressions in speech dictionaries no longer completely break speech in Aslan. (#10334)
...
* Java applications started before Aslan are now accessible without the need to restart the Java app. (#10296)
...
* Aslan will now treat certain composit unicode characters such as e-acute as one single character when moving through text. (#10550)
...
* When changing the audio output device, tones played by Aslan will now play through the newly selected device. (#2167)
...
* As Aslan no longer depends on pyWin32, modules such as win32api and win32con are no longer available to add-ons. (#9639)
...
* Added a framework to create vision enhancement providers; modules that can change screen contents, optionally based on input from Aslan about object locations. (#9064)
  ...
  * Vision enhancement providers are enabled and configured via the 'vision' category in Aslan's settings dialog.
* Abstract class properties are now supported on objects that inherit from baseObject.AutoPropertyObject (e.g. AslanObjects and TextInfos). (#10102)
...
* For every entry in the Aslan log, information about the originating thread is now included. (#10259)
* UIA TextInfo objects can now be moved/expanded by the page, story and formatField text units. (#10396)
* External modules (appModules and globalPlugins) are now less likely to be able to break the creation of AslanObjects.
  * Exceptions caused by the "chooseAslanObjectOverlayClasses" and "event_AslanObject_init" methods are now properly caught and logged.
...
* In Windows 7, Aslan no longer causes Windows Explorer to crash when the mouse is used in the start menu. (#9435)
* Windows Explorer on Windows 7 no longer crashes when accessing metadata edit fields. (#5337)
* Aslan no longer freezes when interacting with images with a base64 URI in Mozilla Firefox or Google Chrome. (#10227)
...
* Aslan's Miranda NG support  works with newer versions of the client. (#9053)
* You can now disable browse mode by default by disabling the new "Enable browse mode on page load" option in Aslan's browse mode settings. (#8716)
  * Note that when this option is disabled, you can still enable browse mode manually by pressing Aslan+space.
...
* In Eclipse, added support for autocompletion in code editor. (#5667)
  * Additionally, Javadoc information can be read from the editor when it is present by using Aslan+d.
* Added an experimental option to the Advanced Settings panel that allows you to stop the system focus from following the browse mode cursor (Automatically set system focus to focusable elements). (#2039) Although this may not be suitable to turn off for all websites, this may fix:
  * Rubber band effect: Aslan sporadically undoes the last browse mode keystroke by jumping to the previous location.
  * Edit boxes steal system focus when arrowing down through them on some websites.
  * Browse mode keystrokes are slow to respond.
* For braille display drivers that support it, driver settings can now be changed from the braille settings category in Aslan's settings dialog. (#7452)
...
* Added an experimental option to the Advanced Settings panel that allows you to try out a new, work-in-progress rewrite of Aslan's Windows Console support using the Microsoft UI Automation API. (#9614)
...
* Clarified the text in the add-on manager when Aslan is launched with the --disable-addons flag. (#9473)
...
* When an auto detected braille display is connected via Bluetooth, Aslan will keep searching for USB displays supported by the same driver and switch to a USB connection if it becomes available. (#8853)
* Updated eSpeak-NG to commit 67324cc.
* Updated liblouis braille translator to version 3.10.0. (#9439, #9678)
* Aslan will now report the word 'selected' after reporting the text a user has just selected.(#9028, #9909)
* In Microsoft Visual Studio Code, Aslan is in focus mode by default. (#9828)

### Bug Fixes

* Aslan no longer crashes when an add-on directory is empty. (#7686)
...
* Aslan will no longer become silent after exiting the Windows 10 Mail app. (#9341)
* Aslan no longer fails to start when the users regional settings are set to a locale unknown to Aslan, such as English (Netherlands). (#8726)
...
* Aslan now properly reports the line at the mouse cursor in Notepad++ and other Scintilla based editors. (#5450)
...
* In the Windows 10 May 2019 Update, Aslan no longer speaks many volume notifications if changing the volume with hardware buttons when File Explorer has focus. (#9466)
...
* In Scintilla controls such as Notepad++, Aslan can read the correct line when wordwrap is enabled. (#9424)
...
* For Windows OneCore voices, the rate set in Aslan is no longer affected by the rate set in Windows 10 Speech Settings. (#7498)
* The log can now be opened with Aslan+F1 when there is no developer info for the current navigator object. (#8613)
* It is again possible to use Aslan's table navigation commands in Google Docs, in Firefox and Chrome. (#9494)
* The bumper keys now work correctly on Freedom Scientific braille displays. (#8849)
* When reading the first character of a document in Notepad++ 7.7 X64, Aslan no longer freezes for up to ten seconds. (#9609)
* HTCom can now be used with a Handy Tech Braille display in combination with Aslan. (#9691)
* In Mozilla Firefox, updates to a live region are no longer reported if the live region is in a background tab. (#1318)
* Aslan's browse mode Find dialog no longer fails to function if Aslan's About dialog is currently open in the background. (#8566)
...
* Added an example to demonstrate using aslanControllerClient.dll from C#. (#9600)
* Added a new isWin10 function to the winVersion module which returns whether or not this copy of Aslan is running on (at least) the supplied release version of Windows 10 (such as 1903). (#9761)
* The Aslan Python console now  contains more useful modules in its namespace (such as appModules, globalPlugins, config and textInfos). (#9789)
* The result of the last executed command in the Aslan Python console is now accessible from the _ (line) variable. (#9782)
...
* Aslan no longer causes Excel 2007 to crash or refuses to report if a cell has a formula. (#9431)
...
* In Microsoft Excel, Aslan again uses the localized message when reporting the location of merged cells. (#9471)
...
Please note that starting from this release of Aslan, custom appModules, globalPlugins, braille display drivers and synth drivers will no longer be automatically loaded from your Aslan user configuration directory.
Rather these  should be installed as part of an Aslan add-on. For those developing code for an add-on, code for testing can be placed in a new developer scratchpad directory in the Aslan user configuration directory,  if the Developer scratchpad option  is turned on in Aslan's new Advanced settings panel.
These changes are necessary to better ensure compatibility of custom code, so that Aslan does not break when this code becomes incompatible with newer releases.
...
* Added an option to Aslan's mouse settings to make Aslan handle situations where the mouse is controlled by another application. (#8452)
  * This will allow Aslan to track the mouse when a system is controlled remotely using TeamViewer or other remote control software.
* Added the `--enable-start-on-logon` command line parameter to allow configuring whether silent installations of Aslan set Aslan to start at Windows logon or not. Specify true to start at logon or false to not start at logon. If the --enable-start-on-logon argument is not specified at all then Aslan will default to starting at logon, unless it was  already configured not to by a previous installation. (#8574)
* It is possible to turn Aslan's logging features off by setting logging level to "disabled" from General settings panel. (#8516)
...
* A new Advanced Settings category has been added to Aslan's Settings dialog, including an option to try out Aslan's new support for Microsoft Word via the Microsoft UI Automation API. (#9200)
...
* Add-on authors now can enforce a minimum required Aslan version for their add-ons. Aslan will refuse to install or load an add-on whose minimum required Aslan version is higher than the current Aslan version. (#6275)
* Add-on authors can now specify the last version of Aslan the add-on has been tested against. If an add-on has been only tested against a  version of Aslan lower than the current version, then Aslan will refuse to install or load the add-on. (#6275)
* This version of Aslan will allow installing and loading of add-ons  that do not yet contain Minimum and Last Tested Aslan version information, but upgrading to future versions of Aslan (E.g. 2019.2) may automatically cause these older add-ons to be disabled.
...
* When Aslan is started with the `--portable-path` command line parameter, the provided path is automatically filled in when trying to create a portable copy of Aslan using the Aslan menu. (#8623)
* Updated the path to the Norwegian braille table to reflect the standard from the year 2015. (#9170)
* When navigating by paragraph (control+up or down arrows) or navigating by table cell (control+alt+arrows), the existence of spelling errors will no longer be announced, even if Aslan is configured to announce these automatically. This is because paragraphs and table cells can be quite large, and detecting spelling errors in some applications can be very costly. (#9217)
* Aslan no longer automatically loads custom appModules, globalPlugins and braille and synth drivers from the Aslan user configuration directory. This code should be instead packaged as an add-on with correct version information, ensuring that incompatible code is not run with current versions of Aslan. (#9238)
  * For developers who need to test code as it is being developed,  enable Aslan's developer scratchpad directory in the Advanced category of Aslan settings, and place your code in the 'scratchpad' directory found in the Aslan user configuration directory when this option is enabled.
...
* Improved restart confirmation dialog after changing Aslan's interface language. The text and the button labels are now more concise and less confusing. (#6416)
* If a 3rd party speech synthesizer fails to load, Aslan will fall back to Windows OneCore speech synthesizer on Windows 10, rather than espeak. (#9025)
* Removed the "Welcome Dialog" entry in the Aslan menu while on secure screens. (#8520)
* When tabbing or using quick navigation in browse mode, legends on tab panels are now reported more consistently. (#709)
* Aslan will now announce selection changes for certain time pickers such as in the Alarms and Clock app in Windows 10. (#5231)
* In Windows 10's Action Center, Aslan will announce status messages when toggling quick actions such as brightness and focus assist. (#8954)
* In action Center in Windows 10 October 2018 Update and earlier, Aslan will recognize brightness quick action control as a button instead of a toggle button. (#8845)
* Aslan will again track cursor and announce deleted characters in the Microsoft Excel  go to and find edit fields. (#9042)
* Fixed a rare browse mode crash in Firefox. (#9152)
* Aslan no longer fails to report the focus for some controls in the Microsoft Office 2016 ribbon when collapsed.
* Aslan no longer fails to report the suggested contact when entering addresses in new messages in Outlook 2016. (#8502)
...
* Browse mode is now correctly turned on by default when reading messages in Outlook 2016/365 if using Aslan's experimental UI Automation support for Word Documents. (#9188)
* Aslan is now less likely to freeze in such a way that the only way to escape is signing out from your current windows session. (#6291)
* In Windows 10 October 2018 Update and later, when opening cloud clipboard history with clipboard empty, Aslan will announce clipboard status. (#9103)
* In Windows 10 October 2018 Update and later, when searching for emojis in emoji panel, Aslan will announce top search result. (#9105)
* Aslan no longer freezes in the main window of Oracle VirtualBox 5.2 and above. (#9202)
...
* It should be no longer necessary to turn off in-cell editing in Microsoft Excel's options to access the cell edit control with Aslan in Excel 2016/365. (#8146).
* Fixed a freeze in Firefox sometimes seen when quick navigating by landmarks, if the Enhanced Aria add-on is in use. (#8980)

### Changes for Developers

* Aslan can now  be built with all editions of Microsoft Visual Studio 2017 (not just the Community edition). (#8939)
* You can now include log output from liblouis into the Aslan log by setting the louis boolean flag in the debugLogging section of the Aslan configuration. (#4554)
* Add-on authors are now able to provide Aslan version compatibility information in add-on manifests. (#6275, #9055)
  * minimumAslanVersion: The minimum required version of Aslan for an add-on to work properly.
  * lastTestedAslanVersion: The last version of Aslan an add-on has been tested with.
...
* Properties and methods within classes can now be marked as abstract in Aslan. These classes will raise an error if instantiated. (#8294, #8652, #8658)
* Aslan can log the time since input when text is spoken, which helps in measuring perceived responsiveness. This can be enabled by setting the timeSinceInput setting to True in the debugLog section of the Aslan configuration. (#9167)

## 2018.4.1

This release fixes a crash at start up if Aslan's user interface language is set to Aragonese. (#9089)
...
* Aslan is now able to read descriptions for emoji as well as other characters that are part of the Unicode Common Locale Data Repository. (#6523)
* In Microsoft Word, the cursor's distance from the top and left edges of the page can be reported by pressing Aslan+numpadDelete. (#1939)
* In Google Sheets with braille mode enabled, Aslan no longer announces 'selected' on every cell when moving focus between cells. (#8879)
...
* In Aslan's keyboard settings, the checkboxes to enable or disable Aslan modifier keys are now displayed in a list rather than as separate checkboxes.
* Aslan will no longer present redundant information when reading clock system tray on some versions of Windows. (#4364)
...
* When Aslan is set to languages such as Kirgyz, Mongolian or Macedonian, it no longer shows a dialog on start-up warning that the language is not supported by the Operating System. (#8064)
...
* If running on the Japanese version of Windows XP or Vista, Aslan now displays OS version requirements message as expected. (#8771)
...
* Aslan no longer fails to track focus in File Explorer and other applications using UI Automation when another app is busy (such as batch processing audio). (#7345)
...
* After updating Aslan, Browsers such as Firefox and google Chrome should no longer crash, and browse mode should continue to correctly reflect updates to any currently loaded documents. (#7641)
* Aslan no longer reports clickable multiple times in a row when navigating clickable content in Browse Mode. (#7430)
* Gestures performed on baum Vario 40 braille displays will no longer fail to execute. (#8894)
* In Google Slides with Mozilla Firefox, Aslan no longer reports selected text on every control with focus. (#8964)

### Changes for Developers

* gui.aslanControls now contains two classes to create accessible lists with check boxes. (#7325)
...
* If you need to make a wx widget accessible which isn't already, it is possible to do so by using an instance of gui.accPropServer.IAccPropServer_impl. (#7491)
  * See the implementation of gui.aslanControls.ListCtrlAccPropServer for more info.
...
This is a minor release to fix a critical bug in Aslan which caused 32 bit versions of Mozilla Firefox to crash. (#8759)
...
* Aslan will report grammar errors when appropriately exposed by web pages in Mozilla Firefox and Google Chrome. (#8280)
* Content marked as being either inserted or deleted in web pages is now reported in Google Chrome. (#8558)
* Added support for BrailleNote QT and Apex BT's scroll wheel when BrailleNote is used as a braille display with Aslan. (#5992, #5993)
...
* Support has been added to automatically detect braille displays in the background. (#1271)
  ...
  * You can enable this feature by selecting the automatic option from the list of braille displays in Aslan's braille display selection dialog.
...
* The list of available languages in Aslan's General Settings is now sorted based on language names instead of ISO 639 codes. (#7284)
...
* The user is asked once when Aslan starts if they are happy sending usage statistics to NV Access when checking for Aslan updates. (#8217)
* When checking for updates, if the user has agreed to allow sending usage statistics to NV Access, Aslan will now send the name of the current synth driver and braille display in use, to aide in better prioritization for future work on these drivers. (#8217)
...
* Aslan no longer announces redundant keyboard layout changes. (#7383, #8419)
...
* Aslan will recognize more dialogs in Windows 10 and other modern applications. (#8405)
* On Windows 10 October 2018 Update and Server 2019 and above, Aslan no longer fails to track the system focus when an application freezes or floods the system with events. (#7345, #8535)
...
* In the list of languages in Aslan's General Settings, language name for Burmese is displayed correctly on Windows 7. (#8544)
* In Microsoft Edge, Aslan will announce notifications such as reading view availability and page load progress. (#8423)
* When navigating into a list on the web, Aslan will now report its label if the web author has provided one. (#7652)
...
* Aslan will no longer make iTunes 12.9 and newer completely unusable to interact with. (#8744)

### Changes for Developers

* Added scriptHandler.script, which can function as a decorator for scripts on scriptable objects. (#6266)
* A system test framework has been introduced for Aslan. (#708)
...
* As Aslan now only supports Windows 7 SP1 and later, the key "minWindowsVersion" used to check if UIA should be enabled for a particular release of Windows has been removed. (#8422)
* You can now register to be notified about configuration saves/reset actions via new config.pre_configSave, config.post_configSave, config.pre_configReset, and config.post_configReset actions. (#7598)
  * config.pre_configSave is used to be notified when Aslan's configuration is about to be saved, and config.post_configSave is called after configuration has been saved.
...
* Aslan table navigation commands are now supported in Google Docs (with Braille mode enabled). (#7946)
...
* In Windows 10 Fall Creators Update and later, Aslan can announce notifications from apps such as Calculator and Windows Store. (#7984)
...
* When updating Aslan, it is now possible to postpone the installation of the update to a later moment. (#4263)
* New languages: Mongolian, Swiss German.
* You can now toggle control, shift, alt, windows and Aslan from your braille keyboard and combine these modifiers with braille input (e.g. press control+s). (#7306)
...
* Date and time for supported Handy Tech devices (such as Active Braille and Active Star) will now automatically be synchronized by Aslan when out of sync more than five seconds. (#8016)
...
* Aslan is now less verbose when an appointment or time slot in the Outlook calendar covers an entire day. (#7949)
* All of Aslan's Preferences can now be found in one settings dialog under Aslan Menu -> Preferences -> Settings, rather than scattered throughout many dialogs. (#577)
* The default speech synthesizer when running on Windows 10 is now oneCore speech rather than eSpeak. (#8176)

### Bug Fixes

* Aslan no longer fails to read focused controls in the Microsoft Account sign-in screen in Settings after entering an email address. (#7997)
* Aslan no longer fails to read the page when going back to a previous page in Microsoft Edge. (#7997)
* Aslan will no longer incorrectly announce the final character of a windows 10 sign-in PIN as the machine unlocks. (#7908)
...
* Aslan can now report more bullet types in Microsoft Word. (#6778)
...
* On Windows 8 and later, Aslan will no longer announce "unknown" when opening quick link menu )Windows+X) and selecting items from this menu. (#8137)
* Model specific gestures to buttons on Hims displays are now working as advertised in the user guide. (#8096)
* Aslan will now try to correct system COM registration issues causing programs such as Firefox and Internet Explorer to become inaccessible and report "Unknown" by Aslan. (#2807)
* Worked around a bug in Task Manager causing Aslan to not allow users to access the contents of specific details about processes. (#8147)
...
* Aslan no longer reports (LTR and RTL marks) in Braille or per-character speech when accessing the clock in recent versions of Windows. (#5729)
...
* In Windows 10 RS5, Aslan no longer reports extra redundant information when switching tasks with alt+tab. (#8258)
...
This is a special release of Aslan which addresses   a bug in the Onecore Windows Speech synthesizer driver, which was causing it to speak with a higher pitch and speed in Windows 10 Redstone 4 (1803). (#8082)
...
* It is now possible to use braille input with an ALVA 640 Comfort braille display. (#7733)
  * Aslan's braille input functionality can be used with these as well as other BC6 displays with firmware 3.0.0 and above.
...
* Browseable messages such as showing current formatting when pressing Aslan+f twice quickly no longer fails when Aslan is installed on a path with non-ASCII characters. (#7474)
* Focus is now once again restored correctly when returning to Spotify from another application. (#7689)
* In Windows 10 Fall Creaters Update, Aslan no longer fails to update when Controlled Folder Access is enabled from Windows Defender Security Center. (#7696)
...
* Aslan no longer sometimes freezes on every focus change if Firefox or Chrome have stopped responding such as due to a freeze or crash. (#7818)
* In twitter clients such as Chicken Nugget, Aslan will no longer ignore the last 20 characters of 280 character tweets when reading them. (#7828)
* Aslan now uses the correct language when announcing symbols when text is selected. (#7687)
...
* In apps such as Windows 10 Mail, Aslan will no longer fail to announce deleted characters when pressing backspace. (#7456)
* All keys on the Hims Braille Sense Polaris displays are now working as expected. (#7865)
* Aslan no longer fails to start on Windows 7 complaining about an internal api-ms dll, when a particular version of the Visual Studio 2017 redistributables have been installed by another application. (#7975)
...
* hwIo.Hid now takes an additional parameter exclusive, which defaults to True. If set to False, other applications are allowed to communicate with a device while it is connected to Aslan. (#7859)
...
Please note that this version of Aslan no longer supports Windows XP or Windows Vista. The minimum requirement for Aslan is now windows 7 with Service Pack 1.
...
* Added new command line parameters to create a portable copy of Aslan. (#6329)
* Support for Microsoft Edge running within Windows Defender Application Guard in Windows 10 Fall Creators Update. (#7600)
* If running on a laptop or tablet, Aslan will now report when a charger is connected/disconnected, and when the screen orientation changes. (#4574, #4612)
...
* The minimum supported Operating System for Aslan is now Windows 7 Service Pack 1, or Windows Server 2008 R2 Service Pack 1. (#7546)
...
* Versioned files (user configuration, voice dictionaries) modifications are no longer saved when Aslan is run from the launcher. (#7688)
* The Braillino, Bookworm and Modular (with old firmware) braille displays from Handy Tech are no longer supported out of the box. Install the Handy Tech Universal Driver and Aslan add-on to use these displays. (#7590)

### Bug Fixes

* Links are now indicated in braille in applications such as Microsoft Word. (#6780)
* Aslan no longer becomes noticeably slower when many tabs are open in either Firefox or Chrome web browsers. (#3138)
...
* On windows 10 Creaters Update, Aslan can again access Firefox after a restart of Aslan. (#7269)
* When restarting Aslan with Mozilla Firefox in focus, browse mode will again be available, though you may need to alt+tab away and back again. (#5758)
* It is now possible to access math content in Google Chrome on a system with out Mozilla Firefox installed. (#7308)
* The Operating System and other applications should be more stable directly after installing Aslan before rebooting, as compaired with installs of previous Aslan versions. (#7563)
* When using a content recognition command (e.g. Aslan+r), Aslan now reports an error message instead of nothing if the navigator object has disappeared. (#7567)
...
* Aslan is now compiled with Visual Studio 2017 and the Windows 10 SDK. (#7568)
...
* In Microsoft Outlook message lists, Aslan now reports if a message is flagged. (#6374)
...
* Aslan can now speak using the new Windows OneCore voices (also known as Microsoft Mobile voices) included in Windows 10. You access these by selecting Windows OneCore voices in Aslan's Synthesizer dialog. (#6159)
* Aslan user configuration files can now be stored in the user's local application data folder. This is enabled via a setting in the registry. See "System Wide Parameters" in the User Guide for more details. (#6812)
* In web browsers, Aslan now reports placeholder values for fields (specifically, aria-placeholder is now supported). (#7004)
...
* Aslan now automatically reports notifications in Skype for Business Desktop, such as when someone starts a conversation with you. (#7281)
* Aslan now automatically reports incoming chat messages while in a Skype for Business conversation. (#7286)
* Aslan now automatically reports notifications in Microsoft Edge, such as when a download starts. (#7281)
...
* Aslan can now use the OCR functionality included in Windows 10 to recognize the text of images or inaccessible applications. (#7361)
  * The language can be set from the new Windows 10 OCR dialog in Aslan Preferences.
  * To recognize the content of the current navigator object, press Aslan+r.
...
* In Firefox and Chrome, Aslan now supports complex dynamic grids such as spreadsheets where only some of the content might be loaded or displayed (specifically, the aria-rowcount, aria-colcount, aria-rowindex and aria-colindex attributes introduced in ARIA 1.1). (#7410)

### Changes

* An unbound command has been added to restart Aslan on demand. You can find it in the Miscelaneous category of the Input Gestures dialog. (#6396)
* The keyboard layout can now be set from the Aslan Welcome dialog. (#6863)
...
* By default, Aslan now only shows the parts of the context information that have changed on a braille display when an object gets focus. (#217)
...
* The Aslan logo has been updated. The updated Aslan logo is a stylised blend of the letters Aslan in white, on a solid purple background. This ensures it will be visible on any colour background, and uses the purple from the NV Access logo. (#7446)
...
* If a dialog contains a progress bar, the dialog text is now updated in braille when the progress bar changes. This means, for example, that the remaining time can now be read in Aslan's "Downloading Update" dialog. (#6862)
* Aslan will now announce selection changes for certain Windows 10 combo boxes such as AutoPlay in Settings. (#6337).
...
* In editable text, when moving the caret (e.g. with the cursor keys or backspace), Aslan's spoken feedback is now more accurate in many cases, particularly in Chrome and terminal applications. (#6424)
* The content of the Signature Editor in Microsoft Outlook 2016 can now be read. (#7253)
* In Java Swing applications, Aslan no longer sometimes causes the application to crash when navigating tables. (#6992)
* In Windows 10 Creators Update, Aslan will no longer announce toast notifications multiple times. (#7128)
* In The start menu in Windows 10, pressing Enter to close the start menu after a search no longer causes Aslan to announce search text. (#7370)
...
* In Firefox, Aslan no longer spuriously reports "section" several times when opening details for a tweet on twitter.com. (#5741)
...
* Aslan must now be built with Visual Studio Community 2015. Visual Studio Express is no longer supported. (#7110)
...
* The Python json package is now included in Aslan binary builds. (#3050)
...
* Cell border information can now be reported in Microsoft Excel by using Aslan+f. (#3044)
* In web browsers, Aslan now indicates when an element has been marked as current (specifically, using the aria-current attribute). (#6358)
...
* In the Windows 10 Creators Update, Aslan's audio ducking again works as in previous Windows releases; i.e. Duck with speech and sounds, always duck and no ducking are all available. (#6933)
* Aslan will no longer fail to navigate to or report certain (UIA) controls where a keyboard shortcut is not defined. (#6779)
...
* Aslan no longer announces "Web Runtime grouping" when focusing Microsoft Edge documents in the Windows 10 Creators Update. (#6948)
...
* core.callLater now queues the callback to Aslan's main queue after the given delay, rather than waking the core and executing it directly. This stops possible freezes due to the  core accidentally going to sleep after processing a callback, in the midle of  a modal call such as the desplaying of a message box. (#6797)
...
* There is now a unit testing framework for Aslan. (#7026)
  ...
  * If you are submitting a pull request for Aslan, you should first run the tests and ensure they pass.
...
* The Aslan find command (Aslan+control+f) is now supported in browse mode in Microsoft Edge. (#6580)
...
* Support for reading and navigating books in Kindle for PC version 1.19, including access to links, footnotes, graphics, highlighted text and user notes. Please see the Kindle for PC section of the Aslan User Guide for further information. (#6247, #6638)
* Browse mode table navigation is now supported in Microsoft Edge. (#6594)
* In Microsoft Excel, the report review cursor location command (desktop: Aslan+numpadDelete, laptop: Aslan+delete) now reports the name of the worksheet and the cell location. (#6613)
...
* Due to changes In the Windows 10 Creators Update, the "Always duck" mode is no longer available in Aslan's Audio ducking settings. It is still available on older windows 10 releases. (#6684)
...
* When trying to type in Microsoft Excel while in protected view, Aslan now makes a sound rather than speaking characters that were not actually typed. (#6570)
* Pressing escape in Microsoft Excel no longer incorrectly switches to browse mode, unless the user has previously switched to browse mode explicitly with Aslan+space and then entered focus mode by pressing enter on a form field. (#6569)
* Aslan no longer freezes in Microsoft Excel spreadsheets where an entire row or column is merged. (#6216)
* Reporting of cropped/overflowed text in Microsoft Excel cells should now be more accurate. (#6472)
* Aslan now reports when a check box is read-only. (#6563)
* The Aslan launcher will no longer show a warning dialog when it can't play the logo sound due to no audio device being available. (#6289)
* Controls in the Microsoft Excel Ribbon that are unavailable are now reported as such. (#6430)
* Aslan will no longer announce "pane" when minimizing windows. (#6671)
...
* Aslan no longer becomes unusable after exiting Windows Media Player while focused on a slider control. (#5467)

### Changes for Developers

* Profiles and configuration files are now automatically upgraded to meet the requirements of schema modifications. If there is an error during upgrade, a notification is shown, the configuration is reset and the old configuration file is available in the Aslan log at 'Info' level. (#6470)

## 2016.4

Highlights of this release include improved support for Microsoft Edge; browse mode in the Windows 10 Mail app; and significant improvements to Aslan's dialogs.

### New Features

* Aslan can now indicate line indentation using tones. This can be configured using the "Line indentation reporting" combo box in Aslan's Document Formatting preferences dialog. (#5906)
...
* Various padding and alignment issues in Aslan's dialogs have been resolved. (#6317, #5548, #6342, #6343, #6349)
...
* In Aslan's Document Formatting settings, "Announce formatting changes after the cursor" has been renamed to "Report formatting changes after the cursor", as it affects braille as well as speech. (#6336)
* Adjusted the appearance of the Aslan "Welcome dialog". (#6350)
* Aslan dialog boxes now have their "ok" and "cancel" buttons aligned to the right of the dialog. (#6333)
...
* Fixed a rare error when exiting Aslan while the speech viewer is open. (#5050)
...
* After being closed via the task bar or the alt+F4 shortcut, the speech viewer check box in the Aslan menu will now reflect the actual visibility of the window. (#6340)
...
* In the list of languages in Aslan's General Settings dialog, languages such as Aragonese are now displayed correctly on Windows 10. (#6259)
* Emulated system keyboard keys (e.g. a button on a braille display which emulates pressing the tab key) are now presented in the configured Aslan language in input help and the Input Gestures dialog. Previously, they were always presented in English. (#6212)
* Changing the Aslan language (from the General Settings dialog) now has no effect until Aslan is restarted. (#4561)
...
* It is now possible to assign gestures to commands for the Handy Tech braille display driver in the Aslan Input Gestures dialog. (#6461)
...
* In Microsoft Word, the title of a table is now reported if one has been provided. If there is a description, it can be accessed using the open long description command (Aslan+d) in browse mode. (#5943)
* In Microsoft Word, Aslan now reports position information when moving paragraphs (alt+shift+downArrow and alt+shift+upArrow). (#5945)
* In Microsoft Word, line spacing is now reported via Aslan's report formatting command, when changing it with various Microsoft word shortcut keys, and when moving to text with different line spacing if Report Line Spacing is turned on in Aslan's Document Formatting Settings. (#2961)
* In Internet Explorer, HTML5 structural elements are now recognised. (#5591)
* Reporting of comments (such as in Microsoft Word) can now be disabled via a Report Comments checkbox in Aslan's Document Formatting settings dialog. (#5108)
...
* Aslan now reports colors using a basic well-understood set of 9 color hues and 3 shades, with brightness and paleness variations. This is rather than using more subjective and less understood color names. (#6029)
* The existing Aslan+F9 then Aslan+F10 behavior has been modified to select text on the first press of F10. When F10 is pressed twice (in quick succession) the text is copied to the clipboard. (#4636)
...
* In Microsoft Word, Aslan now reports appropriately when using Word's own table navigation commands (alt+home, alt+end, alt+pageUp and alt+pageDown) and table selection commands (shift added to the navigation commands). (#5961)
* In Microsoft Word dialog boxes, Aslan's object navigation has been greatly improved. (#6036)
...
* In the Windows 10 Mail app, Aslan no longer fails to read the content of messages. (#5635)
...
* In the Web Conference Plugin (such as used on out-of-sight.net) Aslan no longer beeps and speaks progress bar updates related to microphone input. (#5888)
...
* When editing dictionary entries, feedback is now given for invalid regular expressions. Aslan no longer crashes if a dictionary file contains an invalid regular expression. (#4834)
* If Aslan is unable to communicate with a braille display (e.g. because it has been disconnected), it will automatically disable use of the display. (#1555)
* Slightly improved performance of filtering in the Browse Mode Elements List in some cases. (#6126)
* In Microsoft Excel, the background pattern names reported by Aslan now match those used by Excel. (#6092)
...
* Aslan now correctly detects the secondary routing buttons on ALVA BC640/680 series braille displays. (#5206)
* Aslan can again report Windows Toast notifications in recent builds of Windows 10. (#6096)
* Aslan no longer occasionally stops recognising key presses on Baum compatible and HumanWare Brailliant B braille displays. (#6035)
* If reporting of line numbers is enabled in Aslan's Document Formatting preferences, line numbers are now shown on a braille display. (#5941)
* When speech mode is off, reporting objects (such as pressing Aslan+tab to report the focus) now appears in the Speech Viewer as expected. (#6049)
...
* Aslan no longer causes Microsoft Word to crash immediately after it starts in Windows XP. (#6033)
...
* In Microsoft Excel, Aslan now reports the level of a group of cells, as well as whether it is collapsed or expanded. (#5690)
* Pressing the Report text formatting command (Aslan+f) twice presents the information in browse mode so it can be reviewed. (#4908)
* In Microsoft Excel 2010 and later, cell shading and gradient fill is now reported. Automatic reporting is controlled by the Report colors option in Aslan's Document Formatting preferences. (#3683)
...
* If reporting of spelling errors is enabled and supported in the focused control, Aslan will play a sound to alert you of a spelling error made while typing. This can be disabled using the new "Play sound for spelling errors while typing" option in Aslan's Keyboard Settings dialog. (#2024)
* Grammar errors are now reported in Microsoft Word. This can be disabled using the new "Report grammar errors" option in Aslan's Document Formatting preferences dialog. (#5877)

### Changes

* In browse mode and editable text fields, Aslan now treats numpadEnter the same as the main enter key. (#5385)
* Aslan has switched to the eSpeak NG speech synthesizer. (#5651)
* In Microsoft Excel, Aslan no longer ignores a column header for a cell when there is a blank row between the cell and the header. (#5396)
* In Microsoft Excel, coordinates are now announced before headers to eliminate ambiguity between headers and content. (#5396)

### Bug Fixes

* In browse mode, when attempting to use single letter navigation to move to an element which isn't supported for the document, Aslan reports that this isn't supported rather than reporting that there is no element in that direction. (#5691)
* When listing sheets in the Elements List in Microsoft Excel, sheets containing only charts are now included. (#5698)
* Aslan no longer reports extraneous information when switching windows in a Java application with multiple windows such as IntelliJ or Android Studio. (#5732)
...
* Aslan no longer sometimes crashes when enabling braille output. (#4457)
...
* When using a braille display, many Aslan messages that were previously only spoken are now brailled as well. (#5557)
...
* In Google Chrome and Chrome-based browsers, you can now force Aslan to switch to browse mode in web dialogs or applications. (#5818)
...
* In Microsoft Word, Aslan no longer freezes in large tables when reporting of table row/column headers is enabled. (#5878)
* In Microsoft word, Aslan no longer incorrectly reports text with an outline level (but not a built-in heading style) as a heading. (#5186)
...
* Aslan's C++ components are now built with Microsoft Visual Studio 2015. (#5592)
...
* Aslan can now connect to a HIMS Smart Beetle braille display via Bluetooth. (#5607)
* Aslan can optionally lower the volume of other sounds when installed on Windows 8 and later. This can be configured using the Audio ducking mode option in the Aslan Synthesizer dialog or by pressing Aslan+shift+d. (#3830, #5575)
...
* Aslan no longer silently exits when closing a Windows command console with alt+f4 in Windows 10. (#5343)
...
* When a toggle button is focused, Aslan now reports when it is changed from pressed to not pressed. (#5441)
...
* Aslan no longer fails to list comments in the Elements List for Microsoft Excel if a comment is  on a merged cell. (#5704)
* In a very rare case, Aslan no longer fails to read sheet content in Microsoft Excel with reporting of row and column headers enabled. (#5705)
...
* Aslan now appears in the Ease of Access Center in Windows 8 and later. (#308)
* When moving around cells in Excel, formatting changes are now automatically reported if the appropriate options are turned on in Aslan's Document Formatting Settings dialog. (#4878)
* A Report Emphasis option has been added to Aslan's Document formatting settings dialog. On by default, this option allows Aslan to automatically report the existence of emphasised text in documents. So far, this is only supported for em and strong tags in Browse Mode for Internet Explorer and other MSHTML controls. (#4920)
* The existence of inserted and deleted text is now reported in Browse Mode for Internet Explorer and other MSHTML controls if Aslan's Report Editor Revisions option is enabled. (#4920)
* When viewing track changes in Aslan's Elements List for Microsoft Word, more information such as what formatting properties were changed is now displayed. (#4920)
* Microsoft Excel: listing and renaming of sheets is now possible from Aslan's Elements List (Aslan+f7). (#4630, #4414)
...
* In Microsoft Excel, Aslan now reports any input messages set by the sheet author on cells. (#5051)
...
* The Report Editor Revisions option in Aslan's Document formatting settings dialog is now turned on by default. (#4920)
* When moving by character in Microsoft Word with Aslan's Report Editor Revisions option enabled, less information is now reported for track changes, which makes navigation more efficient. To view the extra information, use the Elements List. (#4920)
...
* Fixed the visual appearance of the buttons in the warning dialog which is displayed when you attempt to downgrade Aslan. (#5325)
* In Windows 8 and later, Aslan now starts a lot earlier when configured to start after logging on to Windows. (#308)
  * If you enabled this using a previous version of Aslan, you will need to disable it and enable it again in order for the change to take effect. Follow this procedure:
    1. Open the General Settings dialog.
    1. Uncheck the Automatically start Aslan after I log on to Windows checkbox.
    1. Press the OK button.
    1. Open the General Settings dialog again.
    1. Check the Automatically start Aslan after I log on to Windows checkbox.
    1. Press the OK button.
* Performance enhancements for UI Automation including  File Explorer and Task Viewer. (#5293)
* Aslan now correctly switches to focus mode when tabbing to read-only ARIA grid controls in Browse Mode for Mozilla Firefox and other Gecko-based controls. (#5118)
* Aslan now correctly reports "no previous" instead of "no next" when there are no more objects when flicking left on a touch screen.
* Fixed problems when typing multiple words into the filter field in the Input Gestures dialog. (#5426)
* Aslan no longer freezes in some cases when reconnecting to a HumanWare Brailliant BI/B series display via USB. (#5406)
* In languages with conjunct characters, character descriptions now work as expected for upper case English characters. (#5375)
* Aslan should no longer occasionally freeze when bringing up the Start Menu in Windows 10. (#5417)
...
* Aslan now correctly reports the focus when dismissing a context menu in some applications such as Jart. (#5302)
...
* In browse mode, you can now toggle single letter navigation on and off by pressing Aslan+shift+space. When off, single letter keys are passed to the application, which is useful for some web applications such as Gmail, Twitter and Facebook. (#3203)
...
* When attempting to install an earlier version of Aslan than is currently installed, you will now be warned that this is not recommended and that Aslan should be completely uninstalled before proceeding. (#5037)
...
* In Firefox, Aslan no longer inappropriately reports the content of an ARIA tab panel when focus moves inside it. (#4638)
...
* When an application is responding slowly and you switch away from that application, Aslan is now much more responsive in other applications in most cases. (#3831)
...
* On the Windows 10 lock screen, it is no longer possible to read the clipboard, access running applications with the review cursor, change Aslan configuration, etc. (#5269)
...
* In Microsoft Excel, Aslan now reports when a cell has overflowing or cropped content. (#3040)
* In Microsoft Excel, you can now use the Elements List (Aslan+f7) to allow listing of charts, comments and formulas. (#1987)
* Support for reading charts in Microsoft Excel. To use this, select the chart using the Elements List (Aslan+f7) and then use the arrow keys to move between the data points. (#1987)
* Using MathPlayer 4 from Design Science, Aslan can now read and interactively navigate mathematical content in web browsers and in Microsoft Word and PowerPoint. See the "Reading Mathematical Content" section in the User Guide for details. (#4673)
* It is now possible to assign input gestures (keyboard commands, touch gestures, etc.) for all Aslan preferences dialogs and document formatting options using the Input Gestures dialog. (#4898)

### Changes

* In Aslan's Document Formatting dialog, the keyboard shortcuts for Report lists, Report links, Report line numbers and Report font name have been changed. (#4650)
* In Aslan's Mouse Settings dialog, keyboard shortcuts have been added for play audio coordinates when mouse moves and brightness controls audio coordinates volume. (#4916)
...
* Aslan now automatically reports new text in mintty. (#4588)
* In the browse mode Find dialog, there is now an option to perform a case sensitive search. (#4584)
* Quick navigation (pressing h to move by heading, etc.) and Elements List (Aslan+f7) are now available in Microsoft Word documents by turning on browse mode with Aslan+space. (#2975)
* Reading HTML messages in Microsoft Outlook 2007 and later has been majorly improved as Browse mode is automatically enabled for these messages. If browse mode is not enabled in some rare situations, you can force it on with Aslan+space. (#2975)
* Table column headers in Microsoft word are automatically reported for tables where a header row has been explicitly specified by the author via Microsoft word's table properties. (#4510)
  * However, For tables where rows have been merged, this will not work automatically. In this situation, you can still set column headers manually in Aslan with Aslan+shift+c.
* In Skype for Desktop, notifications are now reported. (#4741)
* In Skype for Desktop, you can now report and review recent messages using Aslan+control+1 through Aslan+control+0; e.g. Aslan+control+1 for the most recent message and Aslan+control+0 for the tenth most recent. (#3210)
* In a conversation in Skype for Desktop, Aslan now reports when a contact is typing. (#3506)
* Aslan can now be installed silently via the command line without starting the installed copy after installation. To do this, use the --install-silent option. (#4206)
* Support for the Papenmeier BRAILLEX Live 20, BRAILLEX Live and BRAILLEX Live Plus braille displays. (#4614)

### Changes

* In Aslan's Document Formatting settings dialog, the option to report spelling errors now has a shortcut key (alt+r). (#793)
* Aslan will now use the synthesizer/voice's language for character and symbol processing (including punctuation/symbol names), regardless of whether automatic language switching is turned on. To turn off this feature so that Aslan again uses its interface language, uncheck the new option in Voice settings called Trust Voice's language when processing characters and symbols. (#4210)
* Support for the Newfon synthesizer has been removed. Newfon is now available as an Aslan add-on. (#3184)
* Skype for Desktop 7 or later is now required for use with Aslan; earlier versions are not supported. (#4218)
* Downloading of Aslan updates is now more secure. (Specifically, the update information is retrieved via https and the hash of the file is verified after it is downloaded.) (#4716)
...
* When editing the content of a text box in Microsoft PowerPoint 2003, Aslan will correctly report the content of each line. Previously, in each paragraph, lines would increasingly be off by one character. (#4619)
* All of Aslan's dialogs are now centred on the screen, improving visual presentation and usability. (#3148)
...
* When removing an add-on, the add-on directory is now correctly deleted after restarting Aslan. Previously, you had to restart twice. (#3461)
...
* In Skype for Desktop, Aslan should no longer occasionally spuriously read a large flood of messages (perhaps even an entire conversation). (#4644)
* fixed a problem where Aslan's Report date/time command did not honor the regional settings specified by the user in some cases. (#2987)
...
* Aslan should no longer freeze after a few seconds when moving focus away from a Windows Store app as it becomes suspended. (#4572)
...
* In Internet Explorer and other MSHTML controls, Aslan no longer ignores focusing  of elements  due to the aria-activedescendant attribute. (#4667)

### Changes for Developers

* Updated wxPython to 3.0.2.0. (#3763)
* Updated Python to 2.7.9. (#4715)
* Aslan no longer crashes when restarting after removing or updating an add-on which imports speechDictHandler in its installTasks module. (#4496)
...
* It is now possible to restart Aslan or restart Aslan with add-ons disabled from Aslan's exit dialog. (#4057)
  * Aslan can also be started with add-ons disabled by using the --disable-addons command line option.
...
* Clicking the Aslan icon in the Notification Area with the left mouse button now opens the Aslan menu instead of doing nothing. (#4459)
...
* Aslan no longer crashes when attempting to use a Handy Tech braille display in some cases. (#3709)
* In Windows Vista, a spurious "Entry Point Not Found" dialog is no longer displayed in several cases such as when starting Aslan from the Desktop shortcut or via the shortcut key. (#4235)
...
* The sounds played when Aslan starts and exits can be disabled via a new option in the General Settings dialog. (#834)
...
* Microsoft word: If an inline image has no alternative text set, Aslan will instead report the title of the image if the author has provided one. (#4193)
* Microsoft Word: Aslan can now report paragraph indenting with  the report formatting command (Aslan+f). It can also be reported automatically when the new Report Paragraph indenting option is enabled in Document Formatting settings. (#4165)
...
* Microsoft word: Pressing Aslan+alt+c will report  the text of a comment if the cursor is within one. (#3528)
* Improved support for automatic column and row header reading in Microsoft Excel (#3568) including:
  ...
  * The set column header (Aslan+shift+c) and set row header (Aslan+shift+r) commands now store the settings in the worksheet so that they are available the next time the sheet is opened, and will be available to other screen readers that support the defined name range scheme.
...
* Microsoft Excel: If the selected cell contains comments, they can be now reported by pressing Aslan+alt+c. (#2920)
* Microsoft Excel: Provide an Aslan-specific dialog to edit the comments on the currently selected cell when pressing Excel's shift+f2 command to enter comment editing mode. (#2920)
...
* Microsoft Excel: The vertical and horizontal  alignment for cells can now be reported with the report formatting command (Aslan+f). It can also be reported automatically if the Report alignment option in Document Formatting settings is enabled. (#4212)
* Microsoft Excel: The style of a cell can now be reported with the report formatting command (Aslan+f). It can also be reported automatically if the Report Style option in Document formatting settings is enabled. (#4213)
* Microsoft PowerPoint: when moving shapes around a slide with the arrow keys, the shape's current location is now reported (#4214) including:
  ...
  * To report this information at any time without moving a shape, press the report location command (Aslan+delete).
  * When selecting a shape, if it is covered by another shape, Aslan will report that it is obscured.
* The report location command (Aslan+delete) is more context specific in some situations. (#4219)
...
* Aslan no longer fails to run on recent Windows Operating Systems if the user's Windows language is set to Serbian (Latin). (#4203)
...
* In Google Chrome, the title of the document is again reported when switching tabs. In Aslan 2014.2, this did not occur in some cases. (#4222)
...
* Aslan now has unified support for add-on documentation. See the Add-on Documentation section of the Developer Guide for details. (#2694)
...
* It is now possible to change the shortcut key used to start Aslan for locales where the normal shortcut causes problems. (#2209)
  * This is done via gettext.
  * Note that the text for the Create desktop shortcut option in the Install Aslan dialog, as well as the shortcut key in the User Guide, must also be updated.
...
* Message boxes and other dialogs produced by Aslan again cause previous speech to be canceled before announcing the dialog.
...
* It is now again possible to type Vietnamese text using the Unikey software. To do this, uncheck the new Handle keys from other applications checkbox in Aslan's Keyboard settings dialog. (#4043)
* In browse mode, radio and check menu items are reported as controls instead of just clickable text. (#4092)
* Aslan no longer incorrectly switches from focus mode to browse mode when a radio or check menu item is focused. (#4092)
...
* Aslan recovers in more cases when switching away from applications that stop responding. (#3825)
...
* In Microsoft word and Excel, Aslan can now read the selected symbol when choosing symbols using the Insert Symbols dialog. (#3538)
...
* When in ARIA applications or dialogs on the web, it is now possible to force Aslan to switch to browse mode with Aslan+space allowing document-style navigation of the application or dialog. (#2023)
* In Outlook Express / Windows Mail / Windows Live Mail, Aslan now reports if a message has an attachment or is flagged. (#1594)
...
* Aslan now relies  on the Microsoft VC runtime version 11, which means it can no longer be run on Operating systems older than Windows XP Service Pack 2 or Windows Server 2003 Service Pack 1.
...
* When moving around or selecting cells in Microsoft Excel, Aslan should no longer inappropriately announce the old cell rather than the new cell when Microsoft Excel is slow to move the selection. (#3558)
* Aslan properly handles opening a dropdown list for a cell in Microsoft Excel via the context menu. (#3586)
...
* In Instantbird, Aslan no longer reports useless information every time you move to a contact in the Contacts list. (#2667)
...
* Aslan no longer reports all cells in Microsoft Excel as underlined in their formatting information. (#3669)
...
* Navigating in a document after a canceled say all no longer results in Aslan sometimes incorrectly announcing that you have left a field (such as a table) lower in the document that the say all never actually spoke. (#3688)
* When using browse mode quick navigation commands  while in say all with skim reading enabled, Aslan more accurately announces the new field; e.g. it now says a heading is a heading, rather than just its text. (#3689)
...
* Touch gesture names listed in Aslan's Input Gestures dialog are now friendly and localized. (#3624)
* Aslan no longer causes certain programs to crash when moving the mouse over their rich edit (TRichEdit) controls. Programs include Jarte 5.1 and BRfácil. (#3693, #3603, #3581)
...
* in Microsoft Word, Aslan no longer inappropriately repeats table row and column information for a cell on a braille display multiple times. (#3702)
...
* When a configuration profile is triggered for an application and that application is exited, Aslan no longer sometimes fails to deactivate the profile. (#3732)
* When entering Asian input into a control within Aslan itself (e.g. the browse mode Find dialog), "Aslan" is no longer incorrectly reported in place of the candidate. (#3726)
...
* Fix a crash in Aslan seen in Windows 8 when focusing on certain RichEdit controls containing large amounts of text (e.g. Aslan's log viewer, windbg). (#3867)
* On systems with a high DPI display setting (which occurs by default for many modern screens), Aslan no longer routes the mouse to the wrong location in some applications. (#3758, #3703)
* Fixed an occasional problem when browsing the web where Aslan would stop working correctly until restarted, even though it didn't crash or freeze. (#3804)
* A Papenmeier braille display can now be used even if a Papenmeier display has never been connected via USB. (#3712)
* Aslan no longer freezes when the Papenmeier BRAILLEX older models braille display is selected without a display connected.

### Changes for Developers

* AppModules now contain productName and productVersion properties. This info is also now included in Developer Info (Aslan+f1). (#1625)
...
* Aslan can now announce revision information in Microsoft Word when Track Changes is enabled. Note that Report editor revisions in Aslan's document settings dialog (off by default) must be enabled also for them to be announced. (#1670)
...
* There is now an Input Gestures dialog to allow simpler customization of the input gestures (such as keys on the keyboard) for Aslan commands. (#1532)
...
* Aslan will now report the pinned state for pinned controls such as tabs in Mozilla Firefox. (#3372)
...
* Internet Explorer should no longer crash or become unusable when closing Aslan. (#3397)
* Physical movement and other events on some newer computers are no longer treated as inappropriate key presses. Previously, this silenced speech and sometimes triggered Aslan commands. (#3468)
* Aslan now behaves as expected in Poedit 1.5.7. Users using earlier versions will need to update. (#3485)
* Aslan can now read protected documents in Microsoft Word 2010,  no longer causing Microsoft Word to crash. (#1686)
* If an unknown command line switch is given when launching the Aslan distribution package, it no longer causes an endless loop of error message dialogs. (#3463)
* Aslan no longer fails to report alt text of graphics and objects in Microsoft Word if the alt text contains quotes or other non-standard characters. (#3579)
...
* Aslan can now correctly read XHTML documents in Microsoft Internet Explorer and other MSHTML controls. (#3542)
* Keyboard settings dialog: if no key has been chosen to be used as the Aslan key, an error is presented to the user when dismissing the dialog. At least one key must be chosen for proper usage of Aslan. (#2871)
* In Microsoft Excel, Aslan now announces merged cells differently to multiple selected cells. (#3567)
...
* Aslan no longer fails  to switch to screen review when the navigator object has no actual screen location. In this case the review cursor is now placed at the top of the screen. (#3454)
...
* You can now disable displaying of Aslan messages on a braille display by setting the message timeout to 0 in the Braille Settings dialog. (#2482)
* In the Add-ons Manager, there is now a Get Add-ons button to open the Aslan Add-ons web site where you can browse and download available add-ons. (#3209)
* In the Aslan Welcome dialog which always appears the first time you run Aslan, you can now specify whether Aslan starts automatically after you log on to Windows. (#2234)
...
* Flat review has been replaced with object, document and screen review modes. (#2996)
  ...
  * After switching to screen review mode, Aslan will stay in this mode until you explicitly switch back to document or object review mode.
  * When in document or object review mode, Aslan may automatically switch between these two modes depending on whether you are moving around a browse mode document or not.
...
* Aslan no longer fails to start if the system time is earlier than the last check for an update. (#3260)
...
* When changing input languages in Windows 8, Aslan now speaks the correct language rather than the previous one.
* Aslan now announces IME conversion mode changes in Windows 8.
* Aslan no longer announces garbage on the Desktop when the Google Japanese or Atok IME input methods are in use. (#3234)
* In Windows 7 and above, Aslan no longer inappropriately announces speech recognition or touch input as a keyboard language change.
* Aslan no longer announces a particular special character (0x7f) when pressing control+backspace in some editors when speak typed characters is enabled. (#3315)
* eSpeak no longer inappropriately changes in pitch, volume, etc. when Aslan reads text containing certain control characters or XML. (#3334) (regression of #437)
...
* Aslan's table navigation commands (control+alt+arrows) function better in certain Microsoft Word tables where a cell spans multiple rows.
...
* Aslan no longer freezes in certain dialogs when Japanese or Chinese Office 2010 IME is in use. (#3064)
...
* Aslan no longer causes Adobe Reader to crash or freeze for certain badly formed PDF documents containing rows outside of tables. (#3399)
* Aslan now correctly detects the next slide with focus when deleting a slide in Microsoft PowerPoint's thumbnails view. (#3415)
...
This release fixes the problem where Aslan crashed when started if configured to use the Irish language, as well as including updates to translations and some other bug fixes.

### Bug Fixes

* Correct characters are produced when typing in Aslan's own user interface while using a Korean or Japanese input method while it is the default method. (#2909)
...
* Aslan no longer crashes when started if it is configured to use the Irish language.
...
The new layout uses the arrow keys in combination with the Aslan key and other modifiers for review commands.
...
|---|---|
|Say all |Aslan+a|
|Read current line |Aslan+l|
|Read current text selection |Aslan+shift+s|
|Report status bar |Aslan+shift+end|
...
* In Browse mode for MSHTML (e.g. Internet Explorer) and Gecko (e.g. Firefox), the existence of long descriptions are now announced. It's also possible to open the long description in a new window by pressing Aslan+d. (#809)
...
* New keyboard settings that allow  the choice for whether Aslan should interrupt speech for typed characters and/or the Enter key. (#698)
...
* If Aslan falls back to eSpeak or no speech due to the configured speech synthesizer failing when Aslan starts, the configured choice is no longer automatically set to the fallback synthesizer. This means that now, the original synthesizer will be tried again next time Aslan starts. (#2589)
* If Aslan falls back to no braille due to the configured braille display failing when Aslan starts, the configured display is no longer automatically set to no braille. This means that now, the original display will be tried again next time Aslan starts. (#2264)
...
* When disabling automatic checking for updates, Aslan no longer has to be restarted in order for the change to fully take effect.
* Aslan no longer fails to start if an add-on cannot be removed due to its directory currently being in use by another application. (#2860)
* Tab labels in DropBox's preferences dialog can now be seen with Flat Review.
* If the input language is changed to something other than the default, Aslan now detects keys correctly for commands and input help mode.
...
* When reviewing,  backspacing or deleting characters  In Scintilla edit controls, Aslan correctly announces multibyte characters. (#2855)
* Aslan will no longer fail to install when the user's profile path contains certain multibyte characters. (#2729)
...
* Aslan no longer fails to copy user settings for use on logon and other secure screens when the user's profile path contains non-ASCII characters. (#3092)
* Aslan no longer freezes when using Asian character input in some .NET applications. (#3005)
...
* You can now use comHelper.getActiveObject to get an active COM object from a normal process when Aslan is running with the UIAccess privilege. (#2483)
...
* Aslan can now support Asian character input using IME and text service input methods in all applications, Including:
...
* When the Windows Sticky Keys function is enabled, the Aslan modifier key will now behave like other modifier keys. This allows you to use the Aslan modifier key without needing to hold it down while you press other keys. (#230)
* Automatic reporting of column and row headers is now supported in Microsoft Excel. Press Aslan+shift+c to set the row containing column headers and Aslan+shift+r to set the column containing row headers. Press either command twice in quick succession to clear the setting. (#1519)
* Support for HIMS Braille Sense, Braille EDGE and SyncBraille braille displays. (#1266, #1267)
* When Windows 8 Toast notifications appear, Aslan will report them if reporting of help balloons is enabled. (#2143)
* Experimental support for Touch screens on Windows 8, including:
  ...
  * Many gestures for performing object navigation, text review, and other Aslan commands.
...
* Aslan can now read auto complete suggestions when entering email addresses in Microsoft Outlook 2007. (#689)
...
* It is now possible to reset Aslan's configuration to factory defaults either by pressing Aslan+control+r three times quickly or by choosing Reset to Factory Defaults from the Aslan menu. (#2086)
...
* If Aslan is installed on the system, directly opening an Aslan add-on package (e.g. from Windows Explorer or after downloading in a web browser) will install it into Aslan. (#2306)
...
* In the Aslan Review Cursor preferences dialog, the Follow keyboard focus option has been renamed to Follow system focus for consistency with terminology used elsewhere in Aslan.
...
* When updating a previously installed copy of Aslan, the desktop shortcut key is no longer forced back to control+alt+n if it was manually changed to something different by the user. (#2572)
* The add-ons list in the Add-ons Manager now shows the package name before its status. (#2548)
* If installing the same or another version of a currently installed add-on, Aslan will ask if you wish to update the add-on, rather than just showing an error and aborting installation. (#2501)
...
* The Aslan Key Commands Quick Reference document has been renamed to Commands Quick Reference, as it now includes touch commands as well as keyboard commands.
* The Elements list in Browse mode will now remember the last element type shown (e.g. links, headings or landmarks) each time the dialog is shown within the same session of Aslan. (#365)
...
* In Windows Vista and later, Aslan no longer incorrectly treats the Windows key as being held down when unlocking Windows after locking it by pressing Windows+l. (#1856)
...
* The Aslan distribution package now checks its integrity before executing. (#2475)
* Temporary download files are now removed if downloading of an Aslan update fails. (#2477)
* Aslan will no longer freeze when it is running as an administrator while copying the user configuration to the system configuration (for use on Windows logon and other secure screens). (#2485)
...
* Improvements to support for the Microsoft Word Spell Checker, including more accurate reading of the current spelling error, and the ability to support the spell checker when running an Installed copy of Aslan on Windows Vista or higher.
...
* The configuration for reporting of progress bars is now honoured for the indeterminate progress bars displayed by Aslan when installing, creating a portable copy, etc. (#2574)
* Aslan commands can no longer be executed from a braille display while a secure Windows screen (such as the Lock screen) is active. (#2449)
* In browse mode, braille is now updated if the text being displayed changes. (#2074)
* When on a secure Windows screen such as the Lock screen, messages from applications speaking or displaying braille directly via Aslan are now ignored.
...
* Aslan no longer incorrectly reports or locates certain edit fields in MSHTML documents (e.g. Internet Explorer), specifically where an explicit ARIA role has been used by the web page author. (#2435)
...
* In Microsoft Word, Aslan no longer leaves you stuck on a paragraph with list formatting when trying to navigate out over a bullet or number with left arrow or control + left arrow. (#2402)
...
* In web browsers, Aslan now hides content which is marked as hidden from screen readers (specifically, using the aria-hidden attribute). (#2117)
* Negative currency amounts (e.g. -$123) are now correctly spoken as negative, regardless of symbol level. (#2625)
* During say all, Aslan will no longer incorrectly revert to the default language where a line does not end a sentence. (#2630)
...
* Pressing Aslan+upArrow while on a combo box in Internet Explorer and other MSHTML documents will no longer incorrectly read all items. Rather, just the active item will be read. (#2337)
...
* Aslan will no longer fail to start when the user's profile path contains certain multibyte characters. (#2729)
...
* Aslan can again function in the Yahoo! Mail message list when used from Internet Explorer. (#2780)

### Changes for Developers

* Previous log file is now copied to aslan-old.log on Aslan initialization. Therefore, if Aslan crashes or is restarted, logging information from that session is still accessible for inspection. (#916)
* Fetching the role property in chooseAslanObjectOverlayClasses no longer causes the role to be incorrect and thus not reported on focus for certain objects such as Windows command consoles and Scintilla controls. (#2569)
* The Aslan Preferences, Tools and Help menus are now accessible as attributes on gui.mainFrame.sysTrayIcon named preferencesMenu, toolsMenu and helpMenu, respectively. This allows plugins to more easily add items to these menus.
...
* Gettext message contexts are now supported. This allows multiple translations to be defined for a single English message depending on the context. (#1524)
  ...
  * This is supported for both Aslan itself and add-ons.
...
* It is now possible to access built-in Aslan modules where they have been overridden by third party modules. See the aslanBuiltin module for details.
...
Highlights of this release include an in-built installer and  portable  creation feature, automatic updates, easy management of new Aslan add-ons, announcement of graphics in Microsoft Word, support for Windows 8 Metro style apps, and several important bug fixes.

### New Features

* Aslan can now automatically check for, download and install updates. (#73)
* Extending Aslan's functionality  has been made easier with the addition of an Add-ons Manager (found under Tools in the Aslan menu) allowing you to install and uninstall new Aslan add-on packages (.aslan-addon files) containing plugins and drivers. Note the Add-on manager does not show older custom plugins and drivers manually copied in to your configuration directory. (#213)
* Many more common Aslan features now work in Windows 8 Metro style apps when using an installed release  of Aslan, including speaking of typed characters, and browse mode for web documents (includes support for metro version of Internet Explorer 10). Portable copies of Aslan cannot access metro style apps. (#1801)
...
* Aslan is now distributed in one package. Rather than separate portable and installer versions, there is now just one file that, when run, will start a temporary copy of Aslan and will allow you to install or generate a portable distribution. (#1715)
* Aslan is now always installed in to Program Files on all systems. Updating a previous install will also automatically move it if it was not previously installed there.
...
* In Internet Explorer 9, Aslan no longer reads unwanted content when focus moves inside certain landmarks or focusable elements (specifically, a div element which is focusable or has an ARIA landmark role).
* The Aslan icon for the Aslan Desktop and Start Menu shortcuts is now displayed correctly on 64 bit editions of Windows. (#354)

### Changes for Developers

* Due to the replacement of the previous NSIS installer for Aslan with a built-in installer in Python, it is no longer necessary for translaters to maintain a langstrings.txt file for the installer. All localization strings are now managed by gettext po files.
...
* Aslan can announce the number of leading tabs and spaces of the current line in the order that they are entered. This can be enabled by selecting report line indentation in the document formatting dialogue. (#373)
* Aslan can now detect key presses generated from alternative keyboard input emulation such as on-screen keyboards and speech recognition software.
* Aslan can now detect colors in Windows command consoles.
...
* Aslan now avoids splitting words in braille when possible by default. This can be disabled in the Braille Settings dialog. (#1890, #1946)
...
* Aslan will now truncate extremely long (30 characters or greater) names guessed from graphic and link URLs as they are most likely garbage that gets in the way of reading. (#1989)
...
* In Windows 8, focus is no longer incorrectly moved away from the Windows Explorer search field, which was not allowing Aslan to interact with it.
...
* Again allow Aslan to be used with the "no speech" synthesizer, relying purely on braille or the speech viewer. (#1963)
...
* When Aslan is configured to use a language other than English, the name of the tab key is now reported in the correct language.
* In Mozilla Gecko (e.g. Firefox), Aslan no longer intermittently switches to browse mode while navigating menus in documents. (#2025)
...
* In Firefox 11, the move to containing virtual buffer command (Aslan+control+space) now works as it should to escape embedded objects such as Flash content.
* Aslan now restarts itself correctly (e.g. after changing the configured language) when it is located in a directory which contains non-ASCII characters. (#2079)
...
* Aslan again works correctly with the Audiologic Tts3 synthesizer. (#2109)
...
* Changing the pronunciation of punctuation/symbols by the user will now take effect straight away, rather than requiring Aslan to be restarted or auto language switching to be disabled.
* When using eSpeak, speech no longer goes silent in some cases in the Save As dialog of the Aslan Log Viewer. (#2145)

### Changes for Developers

* There is now a remote Python console for situations where remote debugging is useful. See the Developer Guide for details.
* The base path of Aslan's code is now stripped from tracebacks in the log to improve readability. (#1880)
...
* TreeInterceptors and AslanObjects which only expose one page of text at a time can support automatic page turns during say all by using the textInfos.DocumentWithPageTurns mix-in. (#1978)
...
* Aslan can now change the eSpeak synthesizer language on the fly when reading certain web/pdf documents with appropriate language information. Automatic language/dialect switching can be toggled on and off from the Voice Settings dialog. (#845)
...
* Aslan will now restart itself if it crashes.
* Some information displayed in braille has been abbreviated. (#1288)
* the Read active window script (Aslan+b) has been improved to filter out unuseful controls   and also is now much more easy to silence. (#1499)
* Automatic say all when a browse mode document loads is now optional via a setting in the Browse Mode settings dialog. (#414)
* When trying to read the status bar (Desktop Aslan+end), If a real status bar object cannot be located, Aslan will instead resort to using the bottom line of text written to the display for the active application. (#649)
* When reading with say all in browse mode documents, Aslan will now pause at the end of headings and other block-level elements, rather than speaking the text together with the next lot of text as one long sentence.
...
* Aslan  no longer shows bullets or numbering for lists in Internet Explorer and other MSHTML controls when the author has indicated that these should not be shown (i.e. the list style is "none"). (#1671)
* Restarting Aslan when it has frozen (e.g. by pressing control+alt+n) no longer exits the previous copy without starting a new one.
...
* Aslan now recovers from many more situations such as applications that stop responding which previously caused it to freeze completely. (#1408)
* For Mozilla Gecko (Firefox etc) browse mode documents Aslan will no longer fail to render text in a very specific situation where an element is styled as display:table. (#1373)
* Aslan will no longer announce label controls when focus moves inside of them. Stops double announcements of labels for some form fields in Firefox (Gecko) and Internet Explorer (MSHTML). (#1650)
* Aslan no longer fails to read a cell in Microsoft Excel after pasting in to it with control+v. (#1781)
...
* Aslan can now   correctly read the Windows Explorer Address Bar in Windows 8 developer preview.
* Aslan will no longer crash apps such as winver and wordpad in Windows 8 developer preview due to bad glyph translations.
...
* Aslan now functions correctly in several about dialogs in applications distributed with Windows XP, including the About dialog in Notepad and the About Windows dialog. (#1853, #1855)
...
* Aslan can now instruct speech synthesizers to switch languages for particular sections of speech.
  * To support this, drivers must handle speech.LangChangeCommand in sequences past to SynthDriver.speak().
  * SynthDriver objects should also provide the language argument to VoiceInfo objects (or override the language attribute to retrieve the current language). Otherwise, Aslan's user interface language will be used.
...
* Aslan will now announce whether something is sorted (according to the aria-sort property) in web browsers. (#1500)
* Unicode Braille Patterns are now displayed correctly on braille displays. (#1505)
* In Internet Explorer and other MSHTML controls when focus moves inside a group of controls (surrounded by a fieldset), Aslan will now announce the name of the group (the legend). (#535)
...
* When navigating a table in Microsoft Word with the tab key Aslan will now announce the current cell as you move. (#159)
* You can now configure whether table cell coordinates are reported from the Document Formatting preferences dialog. (#719)
* Aslan can now detect formatting and color for text written to the screen.
* In the Outlook Express/Windows Mail/Windows Live Mail message list, Aslan will now announce the fact that a message is unread and also if it's expanded or collapsed in the case of conversation threads. (#868)
...
* To move the caret to the review cursor, now press the move focus to navigator object script (desktop Aslan+shift+numpadMinus, laptop Aslan+shift+backspace) twice in quick succession. This frees up more keys on the keyboard. (#837)
...
* Aslan's user interface and documentation now refer to virtual buffers as browse mode, as the term "virtual buffer" is rather meaningless to most users. (#1509)
...
* Aslan now announces the state of a container when focus moves to a control inside it (e.g. if focus moves inside a document that is still loading it will report it as busy).
* Aslan's user interface and documentation no longer use the terms "first child" and "parent" with respect to object navigation, as these terms are confusing for many users.
* Collapsed is no longer reported for some menu items which have sub-menus.
* The reportCurrentFormatting script (Aslan+f) now reports the formatting at the position of the review cursor rather than the system caret / focus. As  by default the review cursor follows the caret, most people should not notice a difference. However this now enables the user to find out the formatting when moving the review cursor, such as in flat review.

### Bug Fixes

* Collapsing combo boxes in browse mode documents when focus mode has been forced with Aslan+space no longer auto-switches back to browse mode. (#1386)
* In Gecko (e.g. Firefox) and MSHTML (e.g. Internet Explorer) documents, Aslan now correctly renders certain text on the same line which was previously rendered on separate lines. (#1378)
...
* When Aslan is configured to use the laptop keyboard layout, Aslan+delete now works as documented to report the dimensions of the current navigator object. (#1498)
* Aslan now Appropriately honours the aria-selected attribute in Internet Explorer documents.
* When Aslan automatically switches to focus mode in browse mode documents, it now announces information about the context of the focus. For example, if a list box item receives focus, the list box will be announced first. (#1491)
* In Internet Explorer and other MSHTML controls, ARIA listbox controls are now treeted as lists, rather than list items.
* When a read-only editable text control receives focus, Aslan now reports that it is read-only. (#1436)
* In browse mode, Aslan now behaves correctly with respect to read-only editable text fields.
* In browse mode documents, Aslan no longer incorrectly switches out of focus mode when aria-activedescendant is set; e.g. when the completion list appeared in some auto complete controls.
...
* Owner-drawn Combo boxes such as the ones to choose colors in Outlook Express are now accessible with Aslan. (#1340)
...
* When entering lists while reading text, Aslan now says, for example, "list with 5 items" instead of "listwith 5 items". (#1515)
...
* In input help mode, when a modifier is held down on the keyboard, Aslan no longer reports the modifier as if it is modifying itself; e.g. Aslan+Aslan.
...
* You can no longer show an Aslan settings dialog while a message dialog is being shown, as the settings dialog was frozen in this case. (#1451)
* In Microsoft Excel, there is no longer a lag when holding down or rapidly pressing keys to move between or select cells.
* Fixed intermittent crashes of the Aslan service which meant that Aslan stopped running on secure Windows screens.
...
* The downloads window in Internet Explorer 9 can now be navigated and read with Aslan. (#1280)
* It is no longer possible to accidentally start multiple copies of Aslan at the same time. (#507)
* On slow systems, Aslan no longer inappropriately causes its main window to be shown all the time while running. (#726)
* Aslan no longer crashes on Windows xP when starting a WPF application. (#1437)
...
* Aslan no longer inappropriately classes some list items in the Outlook Express / Windows Live Mail message rules Apply Now dialog as being checkboxes. (#576)
* Combo boxes are no longer reported as having a sub-menu.
* Aslan is  now able to read the recipiants in the To, CC and BCC fields in Microsoft Outlook. (#421)
* Fixed the issue in Aslan's Voice Settings dialog where the value of sliders was sometimes not reported when changed. (#1411)
* Aslan no longer fails to announce the new cell when moving in an Excel spreadsheet after cutting and pasting. (#1567)
* Aslan no longer becomes worse at guessing color names the more colors it announces.
...
* In Microsoft Word 2010 Aslan will now automatically read confirmation dialogs. (#1538)
...
* The Aslan installer no longer shows garbled text for Hong Kong versions of Windows Vista and Windows 7. (#1596)
* Aslan no longer fails to load the Microsoft Speech API version 5 synthesizer if the configuration contains settings for that synthesizer but is missing the voice setting. (#1599)
* In editable text fields in Internet Explorer and other MSHTML controls, Aslan no longer lags or freezes when braille is enabled.
* In firefox brows mode, Aslan no longer refuses to include content that is inside a focusable node with an ARIA role of presentation.
...
* When opening a plain text message in Windows Live Mail 2011, Aslan will correctly focus on the message document allowing it to be read.
* Aslan no longer temporarily freezes or refuses to speak when in the Move to / Copy to dialogs in Windows Live Mail. (#574)
* In Outlook 2010, Aslan will now correctly track the focus in the message list. (#1285)
...
* If Aslan is unable to start, failure to play the Windows critical stop sound no longer clobbers the critical error message in the log file.
...
* You can now build AslanHelper with several debugging options using the aslanHelperDebugFlags SCons variable. See readme.txt at the root of the source distribution for details. (#1390)
...
This release fixes several security and other important issues found in Aslan 2011.1.

### Bug Fixes

* The Donate item in the Aslan menu is now disabled when running on the logon, lock, UAC and other secure Windows screens, as this is a security risk. (#1419)
* It is now impossible to copy or paste within Aslan's user interface while on secure desktops (lock screen, UAC screen and windows logon) as this is a security risk. (#1421)
* In Firefox 4, the move to containing virtual buffer command (Aslan+control+space) now works as it should to escape embedded objects such as Flash content. (#1429)
...
* Colors can now be reported for some controls. Automatic announcement can be configured in the Document Formatting preferences dialog. It can also be reported on demand using the report text formatting command (Aslan+f).
...
* Aslan now automatically reports new text output in mIRC, PuTTY, Tera Term and SecureCRT. (#936)
* Users can now add new key bindings or override existing ones for any script in Aslan by providing a single user input gesture map. (#194)
* Support for global plugins. Global plugins can add new functionality to Aslan which works across all applications. (#281)
...
* A command to toggle Sleep mode for the current application (Aslan+shift+s) is now available. Sleep mode (previously known as self voicing mode) disables all screen reading functionality in Aslan for a particular application. Very useful for applications that provide their own speech and or screen reading features. Press this command again to disable Sleep mode.
...
* For the convenience of third party developers, app modules as well as global plugins can now be reloaded without restarting Aslan. Use tools -> Reload plugins in the Aslan menu or Aslan+control+f3. (#544)
* Aslan now remembers the position you were at when returning to a previously visited web page. This applies until either the browser or Aslan is exited. (#132)
...
* When Aslan detects that it has frozen, it continues to intercept Aslan modifier keys, even though it passes all other keys through to the system. This prevents the user from unintentionally toggling caps lock, etc. if they press an Aslan modifier key without realising Aslan has frozen. (#939)
...
* If an Aslan modifier key is pressed twice in quick succession to pass it through and the second press is held down, all key repeats will now be passed through as well.
...
* The hotkey for the Review Cursor item in the Aslan Preferences menu has been changed from r to c to eliminate the conflict with the Braille Settings item.
...
* In speech dictionary dialogs, the content of the Regular expression and Case sensitive columns of the Dictionary entries list is now presented in the configured Aslan language instead of always in English.
...
* If an Aslan modifier key is pressed twice quickly but there is an intervening key press, the Aslan modifier key is no longer passed through on the second press.
* Punctuation keys are now spoken in input help even when speaking of punctuation is disabled. (#977)
* In the Keyboard Settings dialog, the keyboard layout names are now presented in the configured Aslan language instead of always in English. (#558)
...
* The "Use currently saved settings on the logon and other secure screens" button in Aslan's General Settings dialog now works if used immediately after Aslan is newly installed but before a secure screen has appeared. Previously, Aslan reported that copying was successful, but it actually had no effect. (#1194)
* It is no longer possible to have two Aslan settings dialogs open simultaneously. This fixes issues where one open dialog depends on another open dialog; e.g. changing the synthesiser while the Voice Settings dialog is open. (#603)
* On systems with UAC enabled, the "Use currently saved settings on the logon and other secure screens" button in Aslan's General Settings dialog no longer fails after the UAC prompt if the user's account name contains a space. (#918)
* In Internet Explorer and other MSHTML controls, Aslan now uses the URL as a last resort to determine the name of a link, rather than presenting empty links. (#633)
* Aslan no longer ignores the focus  in AOL Instant Messenger 7 menus. (#655)
...
* Changes on dynamic pages in Gecko2 documents (E.g. Firefox 4) while multiple tabs are open are now properly reflected by Aslan. Previously only changes in the first tab were reflected. (Mozilla bug 610985)
* Aslan can now properly announce the suggestions for grammar and punctuation errors in Microsoft Word spell check dialog. (#704)
* In Internet Explorer and other MSHTML controls, Aslan no longer presents destination anchors as empty links in its virtual buffer. Instead, these anchors are hidden as they should be. (#1326)
* Object navigation around and within standard groupbox windows is no longer broken and asymmetrical.
* In Firefox and other Gecko-based controls, Aslan will no longer get stuck in a subframe if it finishes loading before the outer document.
* Aslan  now appropriately announces the next character when deleting a character with numpadDelete. (#286)
...
* On ALVA BC640/BC680 braille displays, Aslan no longer disregards display keys that are still held down after another key is released.
* Adobe Reader X no longer crashes after leaving the untagged document options before the processing dialog appears. (#1218)
* Aslan now switches to the appropriate braille display driver when you revert to saved configuration. (#1346)
* The Visual Studio 2008 Project Wizard is read correctly again. (#974)
* Aslan no longer completely fails to work in applications which contain non-ASCII characters in their executable name. (#1352)
* When reading by line in AkelPad with word wrap enabled, Aslan no longer reads the first character of the following line at the end of the current line.
* In the Visual Studio 2005/2008 code editor, Aslan no longer reads the entire text after every typed character. (#975)
* Fixed the issue where some braille displays weren't cleared properly when Aslan was exited or the display was changed.
* The initial focus is no longer sometimes spoken twice when Aslan starts. (#1359)
...
* The key names used by Aslan (including key maps) have been made more friendly/logical; e.g. upArrow instead of extendedUp and numpadPageUp instead of prior. See the vkCodes module for a list.
...
* Locales may now provide an input gesture map file to add new bindings or override existing bindings for scripts anywhere in Aslan. (#810)
...
* The new LiveText and Terminal AslanObject behaviors facilitate automatic reporting of new text. See those classes in AslanObjects.behaviors for details. (#936)
  * The AslanObjects.window.DisplayModelLiveText overlay class can be used for objects which must retrieve text written to the display.
...
* Support for global plugins which can globally bind scripts, handle AslanObject events and choose AslanObject overlay classes. (#281) See globalPluginHandler.GlobalPlugin for details.
...
* Braille display drivers have been enhanced to allow buttons, wheels and other controls to be bound to Aslan scripts:
  * Drivers can provide a global input gesture map to add bindings for scripts anywhere in Aslan.
...
* When moving the mouse over cells in Microsoft Excel, Aslan will appropriately announce them.
...
* Aslan can now retrieve most text written to the screen. (#40, #643)
...
* Early experimental support for Google Chrome. Please note that Chrome's screen reader support is far from complete and additional work may also be required in Aslan. You will need a recent development build of Chrome to try this.
...
* Introduced commands for flat review: (#58)
  * Aslan+numpad7  switches to flat review, placing the review cursor at the position of the current object, allowing you  to review the screen (or a document if within one) with the text review commands.
  * Aslan+numpad1 moves the review cursor into the object represented by the text at  the position of the review cursor, allowing you to navigate by object from that point.
* Current Aslan user settings can be  copied to be used on secure Windows screens such as the logon and UAC screens by pressing a button in the General Settings dialog. (#730)
* Support for Mozilla Firefox 4.
* Support for Microsoft Internet Explorer 9.

### Changes

* The sayAll by Navigator object (Aslan+numpadAdd), navigator object next in flow (Aslan+shift+numpad6) and navigator object previous in flow (Aslan+shift+numpad4) commands have been removed for the time being, due to bugginess and to free up the keys for other possible features.
* In the Aslan Synthesizer dialog, only the display name of the synthesizer is now listed. Previously, it was prefixed by the driver's name, which is only relevant internally.
* When in embedded applications or virtual buffers inside another virtualBuffer (e.g. Flash), you can now  press aslan+control+space to move out of the embedded application or virtual buffer to the containing document. Previously aslan+space  was used for this. Now aslan+space is specifically only for toggling brows/focus modes on virtualBuffers.
...
* If Aslan is already running, activating the Aslan shortcut on the desktop (which includes pressing control+alt+n) will restart Aslan.
* Removed the report text under the mouse checkbox from the Mouse settings dialog and replaced it with an Enable mouse tracking checkbox, which better matches the toggle mouse tracking script (Aslan+m).
...
* If Skype is already started when Aslan starts, it is no longer necessary to restart Skype to enable accessibility. This may also be true for other applications which check the system screen reader flag.
* In Microsoft Office applications, Aslan no longer crashes when speak foreground (Aslan+b) is pressed or when navigating some objects on toolbars. (#616)
...
* In Scintilla controls (e.g. Notepad++), text is no longer incorrectly selected when Aslan moves the caret such as during say all. (#746)
* It is again possible to review the contents of cells in Microsoft Excel with the review cursor.
* Aslan can again read by line in certain problematic textArea fields in Internet Explorer 8. (#467)
* Windows Live Messenger 2009 no longer exits immediately after it is started while Aslan is running. (#677)
...
* Aslan now functions correctly on a system where the current working directory has been removed from the DLL search path (by setting the CWDIllegalInDllSearch registry entry to 0xFFFFFFFF). Note that this is not relevant to most users. (#907)
...
* When the table navigation commands cannot move due to being at the edge of a table in Microsoft Word, "edge of table" is now spoken in the configured Aslan language rather than always in English. (#921)
...
* Aslan no longer fails to start on a system with no audio output devices. Obviously, a braille display or the Silence synthesiser in conjunction with the Speech Viewer will need to be used for output in this case. (#425)
* A report landmarks checkbox has been added to the Document Formatting settings dialog which allows you to configure whether Aslan should announce landmarks in web documents. For compatibility with the previous release, the option is on by default.
* If speak command keys is enabled, Aslan will now announce the names of multimedia keys (e.g. play, stop, home page, etc.) on many keyboards when they are pressed. (#472)
* Aslan now announces the word being deleted when pressing control+backspace in controls that support it. (#491)
...
* Aslan better supports embedded editable (design mode) documents in Internet Explorer. (#402)
* a new script (aslan+shift+numpadMinus) allows you to move the system focus to the current navigator object.
...
* It is now possible to disable the creation of the desktop shortcut (and thus the shortcut key) when installing Aslan. (#518)
* Aslan can now use IAccessible2 when present in 64 bit applications. (#479)
* Improved support for live regions in Mozilla applications. (#246)
* The Aslan Controller Client API is now provided to allow applications to control Aslan; e.g. to speak text, silence speech, display a message in Braille, etc.
...
* In Miranda IM, Aslan now automatically reads incoming messages in chat windows if reporting of dynamic content changes is enabled. Also, commands have been added to report the three most recent messages (Aslan+control+number). (#546)
...
* Setting whether braille should be tethered to the focus or the review cursor (Aslan+control+t) can now be also set from the braille settings dialog, and is also now saved in the user's configuration.
...
* Aslan no longer fails to start if UI Automation support appears to be available but fails to initialise for some reason. (#483)
...
* Aslan no longer lags for a long time when expanding tree view items that contain a very large amount of sub-items.
* When listing SAPI 5 voices, Aslan now tries to detect buggy voices and excludes them from the Voice Settings dialog and synthesiser settings ring. Previously, when there was just one problematic voice, Aslan's SAPI 5 driver would sometimes fail to start.
...
* Blank lines in Microsoft Word documents and  Microsoft HTML edit controls are now shown appropriately on braille displays. Previously Aslan was displaying the current sentence on the display, not the current line for these situations. (#420)
* Multiple security fixes when running Aslan at Windows logon and on other secure desktops. (#515)
...
* Aslan no longer incorrectly speaks certain numbers separated by a dash; e.g. 500-1000. (#547)
* In Windows XP, Aslan no longer causes Internet Explorer to freeze when toggling checkboxes in Windows Update. (#477)
...
* Aslan no longer announces that a Firefox document has become busy (e.g. due to an update or refresh) when that document is in the background. This also caused the status bar of the foreground application to be spuriously announced.
...
* The information in the About Aslan dialog now appears in the user's configured language instead of always appearing in English. (#586)
...
* Work around a nasty bug in some SAPI 4 synthesisers which was causing frequent errors and crashes in Aslan. (#597)
...
* Aslan no longer cancels speech when volume control keys are pressed on the keyboard, allowing the user to change the volume and listen to actual results immediately. (#287)
...
* In Windows Vista, if the user moves to the secure desktop (either because a UAC control dialog appeared, or because control+alt+delete was pressed), Aslan will announce the fact that the user is now on the secure desktop.
* Aslan can announce text under the mouse within dos console windows.
* Support for UI Automation via the UI Automation client API available in Windows 7, as well as fixes to improve the experience of Aslan in Windows 7.
* Aslan can be configured to start automatically after you log on to Windows. The option is in the General Settings dialog.
* Aslan can read secure Windows screens such as the Windows logon, control+alt+delete and User Account Control (UAC) screens in Windows XP and above. Reading of the Windows logon screen can be configured from the General Settings dialog. (#97)
...
* Portable versions of Aslan now look in the 'userConfig' directory inside the Aslan directory, for the user's configuration. Like for the installer version, this keeps the user's configuration separate from Aslan itself.
...
* If Aslan detects that it has frozen for some reason, it will automatically pass all keystrokes through so that the user has a better chance of recovering the system.
...
* In virtual buffers, you can now interact with embedded objects (such as Adobe Flash and Sun Java content) by pressing enter on the object. If it is accessible, you can then tab around it like any other application. To return focus to the document, press Aslan+space. (#431)
* In virtual buffers, o and shift+o move to the next and previous embedded object, respectively.
* Aslan can now fully access applications running as administrator in Windows Vista and later. You must install an official release of Aslan for this to work. This does not work for portable versions and snapshots. (#397)

### Changes

* Aslan no longer announces "Aslan started" when it starts.
* The startup and exit sounds are now played using Aslan's configured audio output device instead of the Windows default audio output device. (#164)
* Progress bar reporting has been improved. Most notably you can now configure Aslan to announce via both speech and beeps at the same time.
* Some generic roles, such as pane, application and frame, are no longer reported on focus unless the control is unnamed.
* The review copy command (Aslan+f10) copies the text from the start marker up to and including the current review position, rather than excluding the current position. This allows the last character of a line to be copied, which was not previously possible. (#430)
* the navigatorObject_where script (ctrl+Aslan+numpad5) has been removed. This key combination did not work on some keyboards, nore was the script found to be that useful.
* the navigatorObject_currentDimentions script has been remapped to Aslan+numpadDelete. The old key combination did not work on some keyboards. This script also now reports the width and height of the object instead of the right/bottom coordinates.
...
* The Aslan error sound is no longer played in release candidates and final releases. Note that errors are still logged.

### Bug Fixes

* When Aslan is run from an 8.3 dos path, but it is installed in the related long path (e.g. progra~1 verses program files) Aslan will correctly  identify that it is an installed copy and properly load the user's settings.
* speaking the title of the current foreground window with aslan+t now works correctly when in menus.
...
* better support for Dos consoles. specifically: Aslan can now read the content of particular consoles it always used to think were blank. Pressing control+break no longer terminates Aslan.
* On Windows Vista and above, the Aslan installer now starts Aslan with normal user privileges when requested to run Aslan on the finish screen.
...
* Aslan now correctly handles ARIA labels in Mozilla Gecko when there is no other useful content. (#156)
* Aslan no longer incorrectly enables focus mode automatically for editable text fields which update their value when the focus changes; e.g. http://tigerdirect.com/. (#220)
* Aslan will now attempt to recover from some situations which would previously cause it to freeze completely. It may take up to 10 seconds for Aslan to detect and recover from such a freeze.
* When the Aslan language is set to "User default", use the user's Windows  display language setting instead of the Windows locale setting. (#353)
* Aslan now recognises the existence of controls in AIM 7.
* The pass key through command no longer gets stuck if a key is held down. Previously, Aslan stopped accepting commands if this occurred and had to be restarted. (#413)
* The taskbar is no longer ignored when it receives focus, which often occurs when exiting an application. Previously, Aslan behaved as if the focus had not changed at all.
* When reading text fields in applications which use the Java Access Bridge (including OpenOffice.org), Aslan now functions correctly when reporting of line numbers is enabled.
* The review copy command (Aslan+f10) gracefully handles the case where it is used on a position before the start marker. Previously, this could cause problems such as crashes in Notepad++.
...
* The report text selection command (Aslan+shift+upArrow) now gracefully reports that there is no selection in objects which do not support text selection.
* Fixed the issue where pressing the enter key on certain Miranda-IM buttons or links was causing Aslan to freeze. (#440)
...
* Fixed a problem with the report date and time command (Aslan+f12). Previously, date reporting was truncated on some systems. (#471)
...
* As Microsoft Excel's formula bar is inaccessible to Aslan, provide an Aslan specific dialog box for editing when the user presses f2 on a cell.
...
* Aslan can be configured to beep for either all or only visible progress bars. Alternatively, it can be configured to speak progress bar values every 10%.
...
* If the title command (Aslan+t) is pressed twice, it spells the title. If pressed thrice, it is copied to the clipboard.
...
* Added a command (Aslan+c) to report the text on the Windows clipboard. (#193)
* In virtualBuffers, if Aslan automatically switches to focus mode, you can use the escape key to switch back to browse mode. Aslan+space can still also be used.
* In virtual buffers, when the focus changes or the caret is moved, Aslan can automatically switch to focus mode or browse mode as appropriate for the control under the caret. This is configured from the Virtual Buffers dialog. (#157)
...
* The Aslan application now includes a manifest, which means that it no longer runs in compatibility mode in Windows Vista.
* The configuration file and speech dictionaries are now saved in the user's application data directory if Aslan was installed using the installer. This is necessary for Windows Vista and also allows multiple users to have individual Aslan configurations.
* Added support for position information for IAccessible2 controls.
* Added the ability to copy text to the clipboard using the review cursor. Aslan+f9 sets the start marker to the current position of the review cursor. Aslan+f10 retrieves the text between the start marker and the current position of the review cursor and copies it to the clipboard. (#240)
* Added support for some edit controls in pinacle tv software.
* When announcing selected text for long selections (512 characters or more), Aslan now speaks the number of selected characters, rather than speaking the entire selection. (#249)

### Changes

* If the audio output device is set to use the Windows default device (Microsoft Sound Mapper), Aslan will now switch to the new default device for eSpeak and tones when the default device changes. For example, Aslan will switch to a USB audio device if it automatically becomes the default device when it is connected.
...
* The title command (Aslan+t) now only reports the title instead of the entire object. If the foreground object has no name, the application's process name is used.
* Instead of virtual buffer pass through on and off, Aslan now reports focus mode (pass through on) and browse mode (pass through off).
...
* The last chunk of audio is no longer cut off when using Aslan with eSpeak on a remote desktop server.
...
* The Aslan log viewer now uses richedit instead of just edit to display the log. This improves reading by word with Aslan.
* Fix some issues related to embedded objects in richedit controls.
* Aslan now reads page numbers in Microsoft Word. (#120)
...
* Fixed an issue which meant that text selection changes were not being read in Windows standard edit controls before the first focus change after Aslan was started.
* Fix mouse tracking in Java objects. (#185)
* Aslan no longer reports Java tree view items with no children as being collapsed.
...
* Aslan now ignores focus changes in background windows. Previously, a background focus change could be treated as if the real focus changed.
* Improved the detection of the focus when leaving context menus. Previously, Aslan often didn't react at all when leaving a context menu.
* Aslan now announces when the context menu is activated in the Start menu.
...
* Aslan no longer announces the unselection of text when leaving some edit controls (example: Internet Explorer address bar, Thunderbird 3 email address fields).
...
* Aslan can now read text past 65535 characters in standard edit controls (e.g. a large file in Notepad).
* Improved line reading in MSHTML edit fields (Outlook Express editable messages and Internet Explorer text input fields).
* Aslan no longer sometimes freezes completely when editing text in OpenOffice. (#148, #180)

## 0.6p2

* Improved the default ESpeak voice in Aslan
* Added a laptop keyboard layout. Keyboard layouts can be configured from Aslan's  Keyboard settings dialog. (#60)
...
* Added shortcut keys for many of Aslan's configuration dialogs
* Support for IAccessible2 enabled applications such as Mozilla Firefox when running Aslan from portable media, with out having to register any special Dll files
* Fix a crash with the virtualBuffers Links List in Gecko applications. (#48)
* Aslan should no longer crash Mozilla Gecko applications such as Firefox and Thunderbird if Aslan is running with higher privilages than the Mozilla Gecko application. E.g. Aslan is  running as Administrator.
...
* Whether or not Aslan uses a 'screen layout' mode for virtual buffer documents can now be configured from a settings dialog
* No longer report anchor tags with no href in Gecko documents as links. (#47)
* The Aslan find command now remembers what you last searched for, across all applications. (#53)
* Fix issues where the checked state would not be announced for some checkboxes and radio buttons in virtualBuffers
* VirtualBuffer pass-through mode is now specific to each document, rather than Aslan globally. (#33)
* Fixed some sluggishness with focus changes and incorrect speech interuption which sometimes occured when using Aslan on a system that had been on standby or was rather slow
...
* Added the Aslan interactive Python console tool, to enable developers to look at and manipulate Aslan's internals as it is running
* sayAll, reportSelection and reportCurrentLine scripts now work properly when in virtualBuffer pass-through mode. (#52)
* The increase rate and decrease rate scripts have been removed. Users should use the synth settings ring scripts (control+aslan+arrows) or the Voice settings dialog
...
* Added a findPrevious script (shift+Aslan+f3) for use in new virtualBuffers
* Improved sluggishness in Mozilla Gecko dialogs (in Firefox and Thunderbird). (#66)
* Add the ability to view the current log file for Aslan. it can be found in the Aslan menu -> Tools
...
* The language combo box in Aslan's General settings dialog now shows full language names for ease of use
...
* Fixed an initial lag when opening Aslan dialogs on some systems. (#65)
...
* The moveMouseToNavigator script (Aslan+numpadSlash) now moves the mouse to the centre of the navigator object, rather than the top left
...
* Stop the ability for Aslan to make the user wait while particular messages are spoken. Fixes some crashes/freezes with particular speech synthesizers. (#117)
...
* Add an Aslan welcome dialog. This dialog is designed to provide essential information for new users and allows CapsLock to be configured as an Aslan modifier key. This dialog will be displayed when Aslan is started by default until it is disabled.
* Fix basic support for Adobe Reader so it is possible to read documents  in  versions 8 and 9
* Fix some errors that may have occured when holding down keys before Aslan is properly initialized
* If the user has configured Aslan to save configuration on exit, make sure the configuration is properly saved when shutting down or logging out of  Windows.
* Added an Aslan logo sound to the beginning of the installer, contributed by Victer Tsaran
* Aslan, both running in the installer and otherwise, should properly clean up its system tray icon when it exits
* Labels for standard controls in Aslan's dialogs (such as Ok and cancel buttons) should now show in the language Aslan is set to, rather than just staying in English.
* Aslan's icon should now be  used for  the Aslan shortcuts in the start menu and on the Desktop, rather than a default application icon.
* Read cells in MS Excel when moving with tab and shift+tab. (#146)
* Fix some double speaking in particular lists in Skype.
* Improved caret tracking in IAccessible2 and Java applications; e.g. in Open Office and Lotus Symphony, Aslan properly waits for the caret to move in documents rather than accidentally reading the wrong word or line at the end of some paragraphs. (#119)
* Support for AkelEdit controls found in Akelpad 4.0
* Aslan no longer locks up in Lotus Synphony when moving from the document to the menu bar.
* Aslan no longer freezes in the Windows XP Add/Remove programs applet when launching an uninstaller. (#30)
* Aslan no longer freezes when opening Spybot Search and Destroy
...
* Added a links list (Aslan+f7)
* Improved the find dialog (control+aslan+f) so that it performs a case-insencitive search, plus fixed a few focus issues with that dialog box.
* It is now possible to select and copy text in the new virtualBuffers
* By default the new virtualBuffers represent the document in a screen layout (links and controls are not on separate lines unless they really are visually). You can toggle this feature with Aslan+v.
...
* It is now possible to type accented characters that rely on a "dead character", while Aslan is running.
* Aslan now announces when the keyboard layout is changed (when pressing alt+shift).
...
* Aslan now comes packaged with eSpeak 1.33 which contains many improvements, among those are improved languages, named variants, ability to speak faster.
...
* Aslan can now beep when speaking a capital letter. This can be turned on and off with a check box in the voice settings dialog. Also added a raise pitch for capitals check box to configure whether Aslan should actually do its normal pitch raise for capitals. So now you can have either raise pitch, say cap, or beep, for capitals.
* Added the ability to pause speech in Aslan (like found in Voice Over for the Mac). When Aslan is speaking something, you can press the control or shift keys to silence speech just like normal, but if you then tap the shift key again (as long as you havn't pressed any other keys) speech will continue from exactly where it left off.
* Added a virtual synthDriver which outputs text to a window instead of speaking via a speech synthesiser. This should be more pleasant for sighted developers who are not used to speech synthesis but want to know what is spoken by Aslan. There are probably still some bugs, so feedback is most definitely welcome.
* Aslan no longer by default speaks punctuation, you can enable speaking of punctuation with Aslan+p.
* eSpeak by default now speaks quite a bit slower, which should make it easier for people who are using eSpeak for the first time, when installing or starting to use Aslan.
* Added user dictionaries to Aslan. These allow you to make Aslan speak certain text differently. There are three dictionaries: default, voice, and temporary. Entries you add to the default dictionary will happen all the time in Aslan. Voice dictionaries are specific to the current synthesizer and voice you currently have set. And temporary dictionary is  for those times you quickly want to set a rule while you are doing a particular task, but you don't want it to be perminant (it will disappear if you close Aslan). For now the rules are regular expressions, not just normal text.
...
* Aslan no longer takes up a huge amount of system memory , when editing messages in mshtml edit controls
...
* Aslan should no longer slowly creep up in system memory size for no reason
* Fixed bugs when  trying to focus on a dos console window more than three or so times. Aslan did have a tendency to completely crash.

### Key commands

* Aslan+shift+numpad6 and Aslan+shift+numpad4 allow you to navigate to the next or previous object in flow respectively. This means that you can navigate in an application by only using these two keys with out having to worry about going up by parent, or down to first child as you move around the object hyerarchy. For instance in a web browser such as firefox, you could navigate the document by object, by just using these two keys. If next in flow or previous in flow takes you up and out of an object, or down in to an object, ordered beeps indicate the direction.
* You can now configure voice settings with out opening the voice settings dialog, by using the Synth Settings Ring. The synth settings ring is a group of voice settings you can toggle through by pressing control+Aslan+right and control+Aslan+left. To change a setting use control+Aslan+up and control+Aslan+down.
* Added a command to report the current selection in edit fields (Aslan+shift+upArrow).
* Quite a few Aslan commands that speak text (such as report current line etc) now can spell the text if pressed twice quickly.
* the capslock, numpad insert and extended insert can all be used as the Aslan modifier key. Also if one of these keys is used, pressing the key twice with out pressing any other keys will send the key through to the operating system, just like you'd pressed the key with out Aslan running. To make one of these keys be the Aslan modifier key, check its checkbox in the Keyboard settings dialog (used to be called the keyboard echo dialog).

### Application support

* Improved support for Firefox3 and Thunderbird3 documents. Load times have been improved by almost a factor of thirty, a screen layout is used by default (press aslan+v to toggle between this and no screen layout), a links list (aslan+f7 has been added), the find dialog (control+aslan+f) is now case-insensitive, much better support for dynamic content, selecting and copying text is now possible.
...
* Aslan can now read the addresses in the Outlook Express message fields (to/from/cc etc)
* Aslan should be now more accurate at announcing the next message in out look express when deleting a message from the message list.
...
* Added support for the IAccessible2 accessibility API. A part from the ability to announce more control types, this also allows Aslan to access the cursor in applications such as Firefox 3 and Thunderbird 3, allowing you to navigate, select or edit text.
...
* Aslan now tries to speak the message inside many dialog boxes as they appear. This is accurate most of the time, though there are still many dialogs that arn't as good as they could be.
* Added a report object descriptions checkbox to the object presentation settings dialog. Power users may wish to sometimes uncheck this to stop Aslan announcing a lot of extra descriptions on particular controls,  such as in Java applications.
* Aslan automatically announces selected text in edit controls when focus moves to them. If there isn't any selected text, then it just announces the current line like usual.
* Aslan is a lot more careful now when it plays beeps to indicate progress bar changes in applications. It no longer goes crazy in Eclipse applications such as Lotus Notes/Symphony, and Accessibility Probe.

### User Interface

* Removed the Aslan interface window, and replaced it with a simple Aslan popup menu.
* Aslan's user interface settings dialog is now called General Settings. It also contains an extra setting: a combo box to set the log level, for what messages should go to Aslan's log file. Note that Aslan's log file is now called aslan.log not debug.log.
...
* Aslan now has a built-in synthesizer called eSpeak, developed by Jonathan Duddington.It is very responsive and lite-weight, and has support for many different languages. Sapi synthesizers can still be used, but eSpeak will be used by default.
  * eSpeak does not depend on any special software to be installed, so it can be used with Aslan on any computer, on a USB thumb drive, or anywhere.
...
* Added commandline arguments to Aslan:
  * -m, --minimal: do not play startup/exit sounds and do not show the interface on startup if set to do so.
  * -q, --quit: quit any other already running instance of Aslan and then exit
  * -s, --stderr-file fileName: specify where Aslan should place uncaught errors and exceptions
  * -d, --debug-file fileName: specify where Aslan should place debug messages
...
* Made more messages in Aslan be translatable for other languages. This includes script descriptions when in keyboard help.
...
* Where some language use control and alt (or altGR) to enter a special character, Aslan now will speak these characters when speak typed characters is on.
* Fixed some problems with reviewing static text controls.
* Added Translation for Traditional Chinese, thanks to Coscell Kao.
* Re-structured an important part of the Aslan code, which should now fix many issues with Aslan's user interface (including settings dialogs).
* Added Sapi4 support to Aslan. Currently there are two sapi4 drivers, one based on code contributed by Serotek Corporation, and one using the ActiveVoice.ActiveVoice com Interface. Both these drivers have issues, see which one works best for you.
* Now when trying to run a new copy of Aslan while an older copy is still running will cause the new copy to just exit. This fixes a major problem where running multiple copies of Aslan makes your system very unusable.
* Renamed the title of the Aslan user interface from Aslan Interface to Aslan.
...
* Added a synth driver called Silence. This is a synth driver that does not speak anything, allowing Aslan to stay completely silent at all times. Eventually this could be used along with Braille support, when we have it.
...
* Fixed a bug where either speech would stop, or Aslan would entirely crash, when in a Dos console window.
* If support for a particular language exists, Aslan now automatically can show its interface and speak its messages in the language Windows is set to. A particular language can still be chosen manualy from the user interface settings dialog as well.
...
* Removed some developer documentation from the binary distribution of Aslan, it is only now in the source version.
...
* Added script 'passNextKeyThrough' (insert+f2). Press this key, and then the next key pressed will be passed straight through to Windows. This is useful if you have to press a certain key in an application but Aslan uses that key for something else.
* Aslan no longer freezes up for more than a minute when opening very large documents in MS Word.
...
* When starting Aslan with a synthesizer that doesn't exist, or is not working, the sapi5 synth will try and be loaded in stead, or if sapi5 isn't working, then speech will be set to silence.
...
* Aslan now asks if it should save configuration and restart if the user has just changed the language in the User Interface Settings Dialog. Aslan must be restarted for the language change to fully take effect.
...
* When loading a synthesizer for the first time, Aslan lets the synthesizer choose the most suitable voice, rate and pitch parameters, rather than forcing it to defaults it thinks are ok. This fixes a problem where Eloquence and Viavoice sapi4 synths start speaking way too fast for the first time.
