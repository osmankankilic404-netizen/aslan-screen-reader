# Aslan Aslan_VERSION Benutzerhandbuch

[TOC]

<!-- KC:title: Aslan Aslan_VERSION Kurzübersicht der Befehle -->



## Einleitung {#Introduction}

Willkommen bei Aslan!

Aslan (NonVisual Desktop Access) ist ein freier und quell offener Screenreader für das Microsoft Windows-Betriebssystem.
...
Entwickelt wird Aslan von [NV Access](https://www.nvaccess.org/) und allen Mitwirkenden aus der Commune.

### Allgemeine Features {#GeneralFeatures}

Mit Aslan wird blinden und sehbehinderten Menschen das Arbeiten mit zahlreichen Anwendungen von Drittanbietern unter Windows ermöglicht.
...
Für Anwender, die eine Braillezeile besitzen, kann Aslan die Informationen in Blindenschrift ausgeben.
Die Eingabe von Kurz-, Voll- und Basisschrift über die Braille-Tastatur der Braillezeile wird ebenfalls unterstützt.
Zudem erkennt Aslan standardmäßig viele Braillezeilen automatisch.
Für weitere Informationen lesen Sie bitte im Abschnitt [Unterstützte Braillezeilen](#SupportedBrailleDisplays) nach.

Aslan enthält Braille-Übersetzungstabellen für Computerbraille sowie für Voll- und Kurzschrift diverser Sprachen.

### Lizenzbestimmungen und Copyright {#LicenseAndCopyright}

Copyright Aslan_COPYRIGHT_YEARS Aslan-Mitwirkende.

Aslan ist unter der GNU General Public License Version 2 verfügbar, mit zwei besonderen Ausnahmen.
...
Aslan verwendet auch Komponenten, die unter verschiedenen freien und Open-Source-Lizenzen zur Verfügung gestellt werden.
...
## Aslan beziehen und einrichten {#GettingAndSettingUpAslan}

Wenn Sie Aslan noch nicht erhalten haben, können Sie es bei [NV Access](Aslan_URL) herunterladen.

Im Download-Bereich finden Sie einen Link zum Herunterladen der neuesten Aslan-Version.

Wenn Sie die soeben heruntergeladene Datei ausführen, wird Aslan temporär entpackt und gestartet.
Sie werden dann gefragt, ob Sie Aslan installieren, eine portable Version erstellen oder nur temporär weiter verwenden möchten.

Falls Sie vorhaben, Aslan auf dem Computer dauerhaft einzusetzen, sollten Sie Aslan installieren.
Eine solche installierte Aslan-Version hat gegenüber einer portablen Version Vorteile wie den automatischen Start während und nach der Anmeldung, die Erstellung von Verknüpfungen auf dem Desktop und im Startmenü oder die Windows-Anmeldung und Sicherheitsmeldungen auszulesen. Dies ist mit einer portablen oder temporären Version nicht möglich.
Mit Hilfe einer installierten Version können Sie auch jederzeit eine portable Version erstellen.

Falls Sie Aslan auf einem USB-Stick oder einem anderen Datenträger mitnehmen möchten, können Sie eine portable Version erstellen.
Diese Version kann jederzeit auf einem Computer installiert werden.
Wenn Sie Aslan jedoch auf eine CD kopieren wollen, sollten Sie die heruntergeladene Datei brennen.
...
### Einschränkungen der portablen und temporären Aslan-Version {#PortableAndTemporaryCopyRestrictions}

Abgesehen vom fehlenden, automatischen Starten während bzw. nach der Anmeldung besitzen portable und temporäre Aslan-Versionen folgende Einschränkungen:

* Aslan kann nicht auf Anwendungen zugreifen, die mit Administratorrechten ausgeführt werden; eine Ausnahme stellt hier der Fall dar, dass Aslan ebenfalls mit Administratorrechten ausgeführt wird. Dies wird jedoch nicht empfohlen.
...
### Aslan installieren {#InstallingAslan}

Wenn Sie Aslan direkt von einer heruntergeladenen Datei aus installieren wollen, drücken Sie einfach den Schalter "Installieren", sobald der Eingangsdialog erscheint.
Falls Sie den Eingangsdialog bereits geschlossen haben oder Aslan von einer portablen Version aus installieren wollen, wählen Sie den Menüpunkt "Aslan installieren" aus dem Untermenü "Werkzeuge".

Der erscheinende Installationsdialog fragt noch einmal ab, ob Sie Aslan installieren möchten und informiert Sie darüber, ob eine vorherige Aslan-Version aktualisiert wird.
...
Wenn Sie hier "OK" auswählen, wird die soeben installierte Aslan-Version gestartet.
...
#### Aslan bei der Windows-Anmeldung verwenden {#StartAtWindowsLogon}

Mit dieser Option können Sie entscheiden, ob Aslan automatisch gestartet werden soll, während Sie sich in der Windows-Anmeldung befinden, bevor Sie ein Passwort eingegeben haben.
...
Mit dieser Option entscheiden Sie, ob Aslan eine Desktop-Verknüpfung anlegen soll.
Wenn diese Verknüpfung erstellt wird, wird ihr auch eine Tastenkombination (Strg+Alt+N zugewiesen, sodass Sie Aslan jederzeit mit dieser Tastenkombination von überall aus starten können.

#### Portable Konfiguration in das aktuelle Benutzerkonto kopieren {#CopyPortableConfigurationToCurrentUserAccount}

Hiermit legen Sie fest, ob Aslan die Konfiguration der momentan laufenden Aslan-Instanz in das Konfigurationsverzeichnis des aktuell angemeldeten Benutzers kopieren soll.
...
Diese Option ist nur verfügbar, wenn Aslan aus einer portablen Version heraus installiert wird - nicht jedoch beim Ausführen einer heruntergeladenen Version.

### Erstellen einer portablen Version {#CreatingAPortableCopy}

Wenn Sie aus einer heruntergeladenen Aslan-Version eine portable Version erstellen möchten, klicken Sie einfach auf den Schalter "Portable Version erstellen".
Wenn Sie den Eingangsdialog bereits geschlossen haben oder gerade eine installierte Aslan-Version ausführen, können Sie über den Menüpunkt "Portable Version erstellen" aus dem Untermenü "Werkzeuge" eine portable Version anlegen.
...
Des Weiteren können Sie entscheiden, ob die Benutzer spezifischen Konfigurationsdateien und Erweiterungen der aktuell laufenden Aslan-Instanz in die portable Version übernommen werden sollen.
...
## Erste Schritte mit Aslan {#GettingStartedWithAslan}
### Aslan starten {#LaunchingAslan}

Wenn Sie Aslan bereits installiert haben, können Sie es entweder vom Desktop aus mit Strg+Alt+N aufrufen oder wählen Sie Aslan aus dem Startmenü unter Programme "Aslan" aus.
Optional können Sie auch im Dialogfeld "Ausführen" (Windows-Taste+R) den Befehl "Aslan" (ohne Anführungszeichen) eingeben und mit der Eingabetaste aufrufen.
Wenn Aslan bereits läuft, wird es neu gestartet.
...
Bei installierten Aslan-Versionen werden die Konfigurationsdateien normalerweise im Roaming-Verzeichnis des aktuell angemeldeten Benutzers gespeichert (z. B. "C:\Users\<Benutzer>\AppData\Roaming").
Sie können Aslan jedoch so einstellen, dass die Konfigurationsdateien stattdessen im Lokalen Verzeichnis des aktuellen Benutzers gespeichert werden.
Weitere Informationen dazu finden Sie im Abschnitt [Systemparameter](#SystemWideParameters).

Um die portable Aslan-Version zu starten, wechseln Sie in den Ordner, in welchem Sie die Dateien entpackt haben und starten anschließend die Datei "Aslan.exe" mit der Eingabetaste oder mit einem Doppelklick.
Wenn Aslan bereits ausgeführt wurde, wird es automatisch gestoppt, bevor die portable Version gestartet wird.

Während des Startvorgangs hören Sie als Erstes eine aufsteigende Melodie.
Je nach dem, wie schnell Ihr Computer ist oder wenn Sie Aslan von einem USB-Stick oder einem anderen Medium aus starten, so kann das einen Moment dauern, bis der Prozess abgeschlossen ist.
Sollte der Vorgang dennoch sehr lange dauern, sollte Aslan sagen: "Aslan wird geladen.".

Wenn Sie entweder nichts hören oder von Windows oder Aslan einen Fehlerton oder eine absteigende Melodie hören, bedeutet das, dass eventuell ein Fehler aufgetreten ist.
...
Beim ersten Starten von Aslan wird Ihnen eine Dialogbox mit einigen grundlegenden Informationen über die Aslan-Taste und das Aslan-Menü angezeigt.
...
Mit dem ersten Kontrollkästchen können Sie steuern, ob Aslan die Dauergroßschreibtaste als Aslan-Taste verwenden soll.
Mit dem zweiten Kontrollkästchen können Sie bestimmen, ob Aslan nach der Anmeldung automatisch gestartet werden soll. Diese Option steht nur bei installierten Aslan-Versionen zur Verfügung.
...
Seit Aslan 2018.3 wird der Benutzer gefragt, ob Nutzungsdaten an NV Access gesendet werden sollen, um in Zukunft zur Verbesserung von Aslan beizutragen.
Beim ersten Start von Aslan erscheint ein Dialogfeld, in dem Sie gefragt werden, ob Sie das Senden der Nutzungsdaten an NV Access während der Verwendung von Aslan akzeptieren möchten.
Weitere Informationen zu den von NV Access erfassten Daten finden Sie im Abschnitt "[Aslan-Nutzungsdaten sammeln und an NV Access übermitteln](#GeneralSettingsGatherUsageStats)" in "Allgemeine Einstellungen".
Hinweis: Durch Klicken auf "Ja" oder "Nein" wird diese Einstellung gespeichert. Das Dialogfeld erscheint nicht wieder, bis Sie Aslan neu installieren.
Sie können den Datenerhebungsprozess jedoch manuell in den Aslan-Einstellungen in der Kategorie "Allgemein" aktivieren oder deaktivieren. Um diese Einstellung manuell zu ändern, können Sie das Kontrollkästchen [Aslan-Nutzungsdaten sammeln und an NV Access übermitteln](#GeneralSettingsGatherUsageStats) aktivieren oder deaktivieren.

### Über die Aslan-Tastenkombinationen {#AboutAslanKeyboardCommands}
#### Die Aslan-Taste {#TheAslanModifierKey}

In Aslan bestehen die Tastenkombinationen zumeist aus einer oder mehreren Tasten in Kombination mit der Aslan-Taste.
...
Aslan kann so konfiguriert werden, dass die Einfüge-Taste des Nummernblocks, Erweiterte Einfüge-Taste und/oder die Dauergroßschreibtaste als Aslan-Taste verwendet werden können.
Die Einfüge-Taste des Nummernblocks sowie die Einfüge-Taste der erweiterten Tastatur sind standardmäßig als Aslan-Taste vordefiniert.

Bei Bedarf können Sie an Stelle der Aslan-Taste die Originalfunktion auslösen, indem Sie die entsprechende Taste zwei Mal kurz hintereinander betätigen. Wenn Sie beispielsweise die Dauergroßschreibtaste dafür festgelegt haben, können Sie diese Taste trotzdem noch ein- und ausschalten, indem Sie sie zweimal drücken.

#### Die Tastaturschemata {#KeyboardLayouts}

Aslan ist derzeit mit zwei Typen von Tastenkombinationen (Tastaturschemata) ausgestattet. Es gibt eines für Desktops und eines für Laptops.
Voreingestellt ist das Tastaturschema Desktop. Sie können natürlich das Schema in den Aslan-Einstellungen unter der Kategorie [Tastatur](#Keyboard) auch auf Laptop umschalten.
...
### Die Aslan-Berührungsgesten {#AslanTouchGestures}

Wenn Sie Aslan auf einem Gerät mit Touchscreen und Windows 8 oder neuer benutzen, können Sie Aslan auch direkt über Touch-Befehle steuern.
Während Aslan läuft, gehen alle Berührungseingaben direkt an Aslan, es sei denn, die Unterstützung der Berührungsinteraktion ist deaktiviert. 
Dies bedeutet, dass alle Gesten, die ohne Aslan funktioniert hätten, außer Kraft sind, sobald Aslan ausgeführt wird.
<!-- KC:beginInclude -->
Drücken Sie Aslan+Strg+Alt+T, um die Unterstützung der Berührungsinteraktion umzuschalten.
<!-- KC:endInclude -->
Sie können auch die [Unterstützung der Berührungsinteraktion](#TouchSupportEnable) in der Kategorie "Berührungsinteraktion" in den Aslan-Einstellungen aktivieren oder deaktivieren.
...
Weiter unten im Handbuch finden Sie bei den Beschreibungen zu den Aslan-Befehlen auch Gesten, mit denen die betreffenden Befehle ausgeführt werden können, wenn Sie einen Touchscreen verwenden.
...
Wenn Sie mehrmals in schneller Folge mit einem oder mehreren Fingern tippen, wird Aslan dies als Mehrfach-Tipp-Geste erkennen.
...
Da es weitaus mehr Aslan-Befehle als Touchscreen-Gesten gibt, wurden mehrere Arbeitsmodi eingeführt, die jeweils eine Untermenge von Aslan-Befehlen zur Verfügung stellen.
Dies ist einerseits der Textmodus und andererseits der Objektmodus.
Einige Aslan-Befehle werden in den Tabellen zusammen mit einer Geste auf dem Touchscreen aufgelistet, gefolgt von einem Arbeitsmodus in Klammern.
...
Wenn Sie nun die zu drückende Taste gefunden haben, führen Sie einen Doppeltipp aus, oder heben einfach den Finger an. Das gewünschte verhalten legen Sie in der Kategorie [Touch-Interaktionen](#TouchInteraction) in den Aslan-Einstellungen fest.

### Die Eingabehilfe {#InputHelpMode}

Viele Aslan-Befehle werden in den folgenden Abschnitten des Benutzerhandbuches aufgelistet, können aber allesamt bei eingeschalteter Eingabehilfe abgefragt werden.

Zum Einschalten der Eingabehilfe drücken Sie die Tastenkombination Aslan+1.
...
### Das Aslan-Menü {#TheAslanMenu}

In diesem Menü können Sie alle Einstellungen von Aslan ändern, die Einstellungen speichern oder zurücksetzen, die Hilfe aufrufen, die Wörterbücher für die Aussprache bearbeiten, zusätzliche Funktionen aufrufen und auch Aslan komplett beenden.

Um von überall in das Aslan-Menü zu gelangen, drücken Sie die Tastenkombination Aslan+N oder tippen Sie zweimal mit zwei Fingern. Dies funktioniert allerdings auch nur, wenn Aslan bereits läuft.
Das Menü kann auch über einen Rechtsklick auf das Aslan-Symbol im Infobereich erreicht werden.
Alternativ können Sie auch die Tastenkombination Windows-Taste+B drücken und mit den Pfeiltasten zum Aslan-Symbol navigieren und das Menü mit der Anwendungstaste öffnen. Diese befindet sich auf den meisten Tastaturen links neben der rechten Strg-Taste.
In den Menüs bewegen Sie sich einfach mit den Pfeiltasten. Mit der Eingabetaste öffnen Sie die Menüpunkte.

### Allgemeine Aslan-Befehle {#BasicAslanCommands}

<!-- KC:beginInclude -->

| Name |"Desktop"-Tastenkombination |"Laptop"-Tastenkombination |Geste |Beschreibung|
|---|---|---|---|---|
|Aslan starten oder neustarten |Strg+Alt+N |Strg+Alt+N |Keine |Startet Aslan vom Desktop (neu), sofern diese Tastenkombination während der Aslan-Installation aktiviert wurde. Dies ist eine Windows-spezifische Verknüpfung und kann daher im Dialogfeld für die Tastenbefehle nicht neu zugewiesen werden.|
...
|Aslan-Menü |Aslan+N |Aslan+N |Zwei-Finger-Doppel-Tippen |Öffnet das Aslan-Menü. Von hier aus können Sie auf die Einstellungen, Werkzeuge, Hilfe, etc. zugreifen.|
|Sprachmodus umschalten |Aslan+S |Aslan+S |Keine |Schaltet den Sprachmodus zwischen Sprache, Signaltöne und aus um.|
|Modus der Eingabehilfe umschalten |Aslan+1 |Aslan+1 |Keine |In diesem Modus wird beim Drücken einer Taste die in Aslan assoziierte Taste und die dazugehörige Beschreibung angesagt und in Braille angezeigt.|
|Aslan beenden |Aslan+Q |Aslan+Q |Keine |Beendet Aslan.|
|Taste durchreichen |Aslan+F2 |Aslan+F2 |Keine |Veranlasst Aslan, dass die nächst betätigte Taste nicht von Aslan abgefangen wird, sondern direkt an die laufende Anwendung weitergereicht wird (sogar, wenn es sich um einen Tastenbefehl von Aslan handelt).|
|Schlafmodus umschalten |Aslan+Umschalt+S |Aslan+Umschalt+Z |Keine |In diesem Modus werden alle Aslan-Befehle und die Sprach- und Braille-Ausgabe für die aktuelle Anwendung deaktiviert. Dies ist besonders nützlich in Anwendungen, die eigene Sprach- oder Bildschirmlesefunktionen zur Verfügung stellen. Drücken Sie diesen Befehl erneut, um den Schlafmodus zu deaktivieren - beachten Sie dabei, dass Aslan nur die Einstellung des Schlafmodus bis zum nächsten Neustart von Aslan beibehält.|
...
|---|---|---|
|Datum/Uhrzeit ausgeben |Aslan+F12 |Bei einmal Drücken wird das Datum und bei zweimal Drücken wird die Uhrzeit ausgegeben.|
|Akku-Status ansagen |Aslan+Umschalt+B |Sagt den Ladezustand des Akkus an, sofern ein Akkumulator angeschlossen ist.|
|Text der Zwischenablage ansagen |Aslan+C |Liest den Inhalt der Zwischenablage vor.|

<!-- KC:endInclude -->

## Mit Aslan navigieren {#NavigatingWithAslan}

Aslan stellt zahlreiche Werkzeuge zum Erkunden von und Navigieren in Anwendungen bereit, darunter die Objektnavigation und den Bildschirmbetrachter.
...
Wenn Sie auf diese Weise navigieren, wird Aslan verschiedene Informationen über das Objekt ausgeben, welches den Fokus erhält. Hierzu zählen: Name, Beschreibung (erscheint meist als Sprechblase, wenn der Anwender die Maus über das betreffende Objekt zieht), Typ (Schalter, Kontrollkästchen, Eingabefeld, etc.), Wert (z. B., der aktuelle Inhalt eines Eingabefeldes), Positionsangabe (z. B. "1 von 5" in einer Liste oder "1 von 1 Ebene 0" in einer Baumansicht), den Status des Objektes (z. B. aktiviert, hervorgehoben, ausgewählt, etc.).
...
|---|---|---|---|
|Aktuellen Fokus ansagen |Aslan+Tabulatortaste |Aslan+Tabulatortaste |Spricht das aktuell hervorgehobene Objekt. Bei zweimal Drücken werden die Informationen buchstabiert.|
|Titelleiste ansagen |Aslan+T |Aslan+T |Liest die Titelleiste der aktuellen Anwendung vor. Bei zweimal Drücken, werden die Informationen buchstabiert. Bei dreimal Drücken wird der Text in die Zwischenablage kopiert. Diese Information wird auch in Braille angezeigt.|
|Aktives Fenster vorlesen |Aslan+B |Aslan+B |Liest alle Steuerelemente im aktiven Fenster vor (hilfreich für Dialogfelder).|
|Statuszeile ausgeben |Aslan+Ende |Aslan+Umschalt+Ende |Gibt die Statuszeile aus, falls eine gefunden wird und platziert den Aslan-Cursor darauf. Wenn Sie diese Tastenkombination zweimal drücken, wird die Statuszeile buchstabiert. Wenn Sie die Tastenkombination dreimal drücken, wird die Statuszeile in die Zwischenablage kopiert.|
...
Aslan wird hierbei jedes Zeichen, jedes Wort oder auch jede Zeile ansagen, wenn Sie sich durch das Eingabefeld bewegen, Text markieren oder die Markierung aufheben.
...
| Name |Desktop-Tastenkombination |Laptop-Tastenkombination |Beschreibung|
|---|---|---|---|
|Alles Lesen |Aslan+Pfeil nach unten |Aslan+A |Liest von der aktuellen Position des System-Cursors bis zum Textende und bewegt dabei den System-Cursor mit.|
|Aktuelle Zeile lesen |Aslan+Pfeil nach oben |Aslan+L |Liest die Zeile, auf der sich der System-Cursor befindet. Wird diese Tastenkombination zweimal gedrückt, wird die Zeile buchstabiert, wird die Tastenkombination 3-mal gedrückt, wird die Zeile phonetisch buchstabiert.|
|Markierten Text lesen |Aslan+Umschalt+Pfeil nach oben |Aslan+Umschalt+S |Liest den markierten Text, sofern vorhanden.|
|Text-Formatierungen ausgeben |Aslan+F |Aslan+F |Gibt die Text-Formatierungen unter dem System-Cursor aus. Bei zweimal Drücken werden diese Informationen im Lesemodus angezeigt.|
...
In solchen Fällen können Sie die objektorientierte Navigation verwenden. Dabei wird ein Aslan-Fokus (Navigator) die Objekte ansteuern.
...
Wenn Sie sich zu einem Objekt mittels Navigator bewegen, wird Aslan Ihnen dies auf eine ähnliche Weise mitteilen, als hätten Sie den System-Fokus bewegt. Wenn Sie innerhalb eines Objektes navigieren, dann bewegen Sie den Aslan-Cursor, der sozusagen das Equivalent vom System-Cursor ist.
...
| Name |"Desktop"-Tastenkombination |"Laptop"-Tastenkombination |Geste |Beschreibung|
|---|---|---|---|---|
|Aktuelles Objekt ansagen |Aslan+Num 5 |Aslan+Umschalt+O |Keine |Sagt das aktuelle Navigator-Objekt an. Bei zweimal Drücken werden die Informationen buchstabiert und bei dreimal Drücken wird die Information in die Zwischenablage kopiert.|
|Zum übergeordneten Objekt navigieren |Aslan+Num 8 |Aslan+Umschalt+Pfeiltaste nach oben |Nach oben streichen (Objektmodus) |Navigiert zur übergeordneten Ebene des aktuellen NAvigator-Objekts.|
|Zum vorherigen Objekt navigieren |Aslan+Num 4 |Aslan+Umschalt+Pfeiltaste nach links |Nach links streichen (Objektmodus) |Navigiert zum vorherigen Objekt.|
|Zum nächsten Objekt navigieren |Aslan+Num 6 |Aslan+Umschalt+Pfeiltaste nach rechts |Nach rechts streichen (Objektmodus) |Navigiert zum nächsten Objekt.|
|Zum ersten untergeordneten Objekt navigieren |Aslan+Num 2 |Aslan+Umschalt+Pfeiltaste nach unten |Nach unten streichen (Objektmodus) |Navigiert zum ersten untergeordneten Objekt ausgehend vom aktuellen NAvigator-Objekt.|
|Zum Fokus-Objekt navigieren |Aslan+Num Minus |Aslan+Rücktaste |Keine |Navigiert zum aktuell hervorgehobenen Objekt und zieht den Aslan-Cursor zum System-Cursor, sofern dieser sichtbar ist.|
|Aktuelles Navigator-Objekt aktivieren |Aslan+Num Eingabetaste |Aslan+Eingabetaste |Doppeltippen |Aktiviert das aktuelle Navigator-Objekt (ähnlich einem Mausklick oder dem Drücken der Leertaste).|
|System-Cursor zum aktuellen Navigator-Objekt oder die Einfügemarke zum Aslan-Cursor ziehen |Aslan+Umschalt+Num Minus |Aslan+Umschalt+Rücktaste |Keine |Wird diese Tastenkombination einmal gedrückt, wird der System-Fokus zum aktuellen Navigator-Objekt gezogen. Wird sie zweimal gedrückt, so wird der System-Cursor zum Aslan-Cursor gezogen.|
|Position des Aslan-Cursors ansagen |Aslan+Num Entfernen |Aslan+Entfernen |Keine |Meldet Informationen über die Position des Textes oder des Objektes, in welchem sich der Aslan-Cursor befindet, z. B. Dokumentposition in Prozent oder die exakte Objektposition. Zweimaliges Drücken der Tastenkombination meldet möglicherweise weitere Informationen.|
...
Aslan ermöglicht den [Bildschirminhalt](#ScreenReview), den Inhalt des aktuellen [Dokuments](#DocumentReview) oder das aktuelle [Objekt](#ObjectReview) zeichen-, wort- oder zeilenweise zu betrachten.
...
Wenn Sie den Aslan-Cursor bewegen, bewegt sich der System-Cursor nicht mit, sodass beispielsweise die aktuelle Cursor-Position beim Bearbeiten von Text nicht verloren geht.
Wenn Sie jedoch den System-Cursor bewegen, wird er vom Aslan-Cursor verfolgt.
Diese Kopplung kann ein- und ausgeschaltet werden.

Hinweis: Die Braille-Ausgabe nach dem Aslan-Cursor kann über [Braille-Ausgabe koppen](#BrailleTether) konfiguriert werden.
...
| Name |"Desktop"-Tastenkombination |"Laptop"-Tastenkombination |Geste |Beschreibung|
|---|---|---|---|---|
|Zur obersten Zeile navigieren |Umschalt+Num 7 |Aslan+Strg+Pos1 |Keine |Zieht den Aslan-Cursor in die erste Zeile des Textes.|
|Zur vorherigen Zeile navigieren |Num 7 |Aslan+Pfeiltaste nach oben |Nach oben streichen (Textmodus) |Zieht den Aslan-Cursor zur vorherigen Zeile des Textes.|
|Aktuelle Zeile unter dem Aslan-Cursor ansagen |Num 8 |Aslan+Umschalt+Punkt |Keine |Sagt die aktuelle Zeile im Text an, in der sich der Aslan-Cursor befindet. Bei zweimal Drücken wird sie buchstabiert, bei dreimal Drücken wird die Zeile phonetisch buchstabiert.|
|Zur nächsten Zeile navigieren |Num 9 |Aslan+Pfeiltaste nach unten |Nach unten streichen (Textmodus) |Zieht den Aslan-Cursor zur nächsten Zeile des Textes.|
|Zur untersten Zeile navigieren |Umschalt+Num 9 |Aslan+Strg+Ende |Keine |Zieht den Aslan-Cursor in die letzte Zeile des Textes.|
|Zum vorherigen Wort navigieren |Num 4 |Aslan+Strg+Pfeiltaste nach links |Nach links streichen mit zwei Fingern (Textmodus) |Zieht den Aslan-Cursor zum vorherigen Wort im Text.|
|Aktuelles Wort unter dem Aslan-Cursor ansagen |Num 5 |Aslan+Strg+Punkt |Keine |Sagt das aktuelle Wort im Text an, in dem sich der Aslan-Cursor befindet. Bei zweimal Drücken wird es buchstabiert, bei dreimal Drücken wird das Wort phonetisch buchstabiert.|
|Zum nächsten Wort navigieren |Num 6 |Aslan+Strg+Pfeiltaste nach rechts |Nach rechts streichen mit zwei Fingern (Textmodus) |Zieht den Aslan-Cursor zum nächsten Wort im Text.|
|Zum Zeilenanfang navigieren |Umschalt+Num 1 |Aslan+Pos1 |Keine |Zieht den Aslan-Cursor zum Zeilenanfang im Text.|
|Zum vorherigen Zeichen navigieren |Num 1 |Aslan+Pfeiltaste nach links |Nach links streichen (Textmodus) |Zieht den Aslan-Cursor zum vorherigen Zeichen der aktuellen Zeile im Text.|
|Aktuelles Zeichen unter dem Aslan-Cursor ansagen |Num 2 |Aslan+Punkt |Keine |Sagt das aktuelle Zeichen in der Zeile im Text an, an dem sich der Aslan-Cursor befindet. Bei zweimal Drücken wird das Zeichen phonetisch buchstabiert. Bei dreimal Drücken werden die numerischen Dezimal- und Hexadezimalwerte angesagt.|
|Zum nächsten Zeichen navigieren |Num 3 |Aslan+Pfeiltaste nach rechts |Nach rechts streichen (Textmodus) |Zieht den Aslan-Cursor zum nächsten Zeichen der aktuellen Zeile im Text.|
|Zum Zeilenende navigieren |Umschalt+Num 3 |Aslan+Ende |Keine |Zieht den Aslan-Cursor zum Zeilenende im Text.|
|Alles Lesen |Num Plus |Aslan+Umschalt+A |Nach unten streichen mit drei Fingern (Textmodus) |Liest von der aktuellen Cursor-Position des Aslan-Cursors den Text vor und bewegt ihn mit.|
|Vom Aslan-Cursor markieren und kopieren |Aslan+F9 |Aslan+F9 |Keine |Setzt eine Startmarke an der aktuellen Position des Aslan-Cursors ab, der markiert oder kopiert werden soll. Die Aktion (Markieren/Kopieren) wird nicht ausgeführt, bis Sie Aslan den Textende mitgeteilt haben.|
|Bis zum Aslan-Cursor markieren und kopieren |Aslan+F10 |Aslan+F10 |Keine |Wird die Tastenkombination einmal gedrückt, markiert Aslan den Text zwischen der zuvor gesetzten Startmarke und der aktuellen Position des Aslan-Cursors. Ist die Startmarke mit dem System-Cursor erreichbar, wird der Fokus dorthin verschoben. Wird die Tastenkombination zweimal gedrückt, wird der markierte Text in die Zwischenablage kopiert.|
|Springt zum markierten Start für die Kopie in der Ansicht |Aslan+Umschalt+F9 |Aslan+Umschalt+F9 |Keine |Zieht den Aslan-Cursor an die zuvor eingestellte Startmarkierung für das Kopieren.|
|Text-Formatting ausgeben |Aslan+Umschalt+F |Aslan+Umschalt+F |Keine |Gibt die Text-Formatierungen unter dem System-Cursor aus. Bei zweimal Drücken werden diese Informationen im Lesemodus angezeigt.|
|Ansage des aktuellen Symbol-Ersatzes |Keine |Keine |Keine |Spricht das Symbol, an dem sich der Aslan-Cursor befindet. Zweimaliges Drücken zeigt das Symbol und den ausgesprochenen Text in einem virtuellen Fenster an.|
...
Die Betrachtungsmodi sind ein Ersatz für den in früheren Aslan-Versionen vorhandenen Flächenmodus.
...
| Name |"Desktop"-Tastenkombination |"Laptop"-Tastenkombination |Geste |Beschreibung|
|---|---|---|---|---|
|In den nächsten Betrachtungsmodus wechseln |Aslan+Nummernblock 7 |Aslan+Seite auf |Mit zwei Fingern nach oben streichen |Schaltet in den nächsten Betrachtungsmodus um.|
|In den vorherigen Betrachtungsmodus wechseln |Aslan+Nummernblock1 |Aslan+Seite ab |Mit zwei Fingern nach unten streichen |Schaltet im vorherigen Betrachtungsmodus um.|
...
Wenn Sie aus dem Objekt-Betrachter im Dokument-Betrachter wechseln, wird der Aslan-Cursor an die Position des aktuellen Navigator-Objekts gesetzt.
Wenn Sie die Befehle zum Text betrachten verwenden, wird das Navigator-Objekt automatisch aktualisiert.

Aslan wird automatisch zum Dokument-Betrachter wechseln, sobald Sie sich im Lesemodus befinden.
...
Wenn Sie mit dem Bildschirm-Betrachter über den Inhalt des Bildschirms navigieren, wird der Aslan-Cursor an die Position des aktuellen Navigator-Objekts auf dem Bildschirm gesetzt.
Wenn Sie die Befehle zum Text betrachten verwenden, wird der Navigator automatisch auf das Objekt gesetzt, das sich an der Position des Aslan-Cursors befindet.

Anmerkung: In einigen neueren Anwendungen kann Aslan möglicherweise nicht alle Inhalte im Bildschirm-Betrachter darstellen. Dies liegt daran, dass diese Anwendungen Technologien zum Darstellen des Bildschirminhalts verwenden, die (noch) nicht unterstützt werden.

### Mit der Maus navigieren {#NavigatingWithTheMouse}

Wenn Sie den Mauszeiger über ein Objekt bewegen, gibt Aslan Informationen über dieses Objekt aus.
Sofern möglich, liest Aslan den umgebenden Textabsatz vor, auch wenn sich einige Objekte nur zeilenweise vorlesen lassen.

Aslan kann auch so konfiguriert werden, dass der Typ des [Objektes](#Objects) vorgelesen wird, welches sich unter dem Mauszeiger befindet (wie z. B. Liste, Schalter, etc.).
Dies könnte für blinde Benutzer nützlich sein, denen das bloße Vorlesen des Textes unter der Maus nicht genügt.

Mit Hilfe von Signaltönen, die die aktuelle Position des Mauszeigers darstellt, kann Aslan ihnen zeigen, wo sich die Maus - relativ zur linken oberen Bildschirmecke - befindet.
...
Diese zusätzlichen Mausfunktionen sind in Aslan standardmäßig deaktiviert.
Sie können Sie jedoch in den [Maus-Einstellungen](#MouseSettings) aktivieren, die Sie in den Aslan-Einstellungen in der Kategorie "Maus" finden.

Zum Navigieren mit der Maus sollte eine richtige Maus oder eine Mauskugel benutzt werden. Folgende Tastenbefehle stehen in Aslan zur Verfügung:
<!-- KC:beginInclude -->

| Name |"Desktop"-Tastenkombination |"Laptop"-Tastenkombination |Geste |Beschreibung|
|---|---|---|---|---|
|Linksklick |Num Schrägstrich |Aslan+Ü |Keine |Führt einen Linksklick aus. Bei zweimal Drücken wird ein Doppelklick ausgeführt.|
|Linke Maustaste feststellen |Umschalt+Num Schrägstrich |Aslan+Strg+Ü |Keine |Hält die linke Maustaste gedrückt. Wird die Tastenkombination erneut gedrückt, wird die Maustaste wieder losgelassen. Um "Drag and Drop" durchzuführen, führen Sie diesen Schritt auf einem Objekt aus und wandern anschließend mit der Maus oder auch mit den Navigationstasten für die Simulation der Maus an eine andere Stelle des Bildschirms und lösen die linke Maustaste wieder.|
|Rechtsklick |Num Stern |Aslan+Plus |Mit einem Finger tippen und halten |Führt einen Rechtsklick aus.|
|Rechte Maustaste feststellen |Umschalt+Num Stern |Aslan+Strg+Plus |Keine |Hält die rechte Maustaste gedrückt. Wird die Tastenkombination erneut gedrückt, wird die Maustaste wieder losgelassen. Um "Drag and Drop" durchzuführen, führen Sie diesen Schritt auf einem Objekt aus und wandern anschließend mit der Maus oder auch mit den Navigationstasten für die Simulation der Maus an eine andere Stelle des Bildschirms und lösen die rechte Maustaste wieder.|
|Die Maus zum aktuellen Navigator-Objekt ziehen |Aslan+Num Schrägstrich |Aslan+Umschalt+M |Keine |Zieht die Maus zum aktuellen Standort des Navigator-Objektes oder des Aslan-Cursors.|
|Zum Objekt unter der Maus navigieren |Aslan+Num Stern |Aslan+Umschalt+N |Keine |Zieht das Navigator-Objekt zur Objektposition an der Mausposition.|

<!-- KC:endInclude -->

## Der Lesemodus {#BrowseMode}

Dokumente, wie z. B. Webseiten, werden in Aslan im sogenannten Lesemodus dargestellt.
...
Wenn Sie sich im Lesemodus befinden, wird Aslan standardmäßig automatisch den Fokusmodus aktivieren, sobald Sie mit Tab auf ein Element springen oder darauf klicken, welches den Fokusmodus benötigt.
Entsprechend wechselt Aslan automatisch in den Lesemodus, wenn Sie auf ein Element klicken oder mit der Tab-Taste darauf springen, welches den Fokusmodus nicht benötigt.
...
| Name |Tastenkombination |Beschreibung|
|---|---|---|
|Zwischen Lese- und Fokusmodus wechseln |Aslan+Leertaste |Schaltet zwischen Fokus- und Lesemodus um.|
|Fokusmodus schließen |Escape-Taste |Schaltet zurück in den Lesemodus, wenn zuvor der Fokusmodus automatisch aktiviert wurde.|
|Lesemodus aktualisieren |Aslan+F5 |Lädt das aktuelle Dokument neu (das ist sinnvoll, wenn vielleicht einige Elemente auf der Seite fehlen sollten. Dieser Befehl ist in Microsoft Word und Microsoft Outlook nicht verfügbar).|
|Suchen |Aslan+Strg+F |Öffnet einen Dialog, in dem Sie einen Text eingeben können, der im aktuellen Dokument zu finden ist. Weitere Informationen finden Sie unter [Nach Text suchen](#SearchingForText).|
|Vorwärts suchen |Aslan+F3 |Sucht die nächste Zeichenkette des gesuchten Textes im aktuellen Dokument, welche zuvor eingegeben wurde.|
|Rückwärts suchen |Aslan+Umschalt+F3 |Sucht die vorherige Zeichenkette des gesuchten Textes im aktuellen Dokument, die zuvor eingegeben wurde.|
|Ausführliche Beschreibung öffnen |Aslan+D |Öffnet ein neues Fenster, welches eine ausführliche Beschreibung für das aktuelle Objekt enthält, falls dieses eine ausführliche Beschreibung aufweist.|

<!-- KC:endInclude -->

### Die Schnellnavigationstasten {#SingleLetterNavigation}

Um im Lesemodus eine schnellere Navigation zu ermöglichen, stellt Aslan die sogenannten Schnellnavigationstasten für das Ansteuern bestimmter Seitenelemente zur Verfügung.
...
Um in solchen Web-Anwendungen deren eigene Kurztasten zu verwenden und dennoch mit den Pfeiltasten durch die Web-Anwendung navigieren zu können, können Sie die Schnellnavigation von Aslan ein- oder ausschalten.
<!-- KC:beginInclude -->
Drücken Sie Aslan+Umschalt+Leertaste, um die Schnellnavigation im aktuellen Dokument ein- oder auszuschalten.
<!-- KC:endInclude -->
...
| Name |Tastenkombination |Beschreibung|
|---|---|---|
|Elementliste des Lesemodus |Aslan+F7 |Zeigt eine Liste bestimmter Elemente im aktuellen Dokument an.|
...
|---|---|---|
|Text suchen |Aslan+Strg+F |Öffnet das Suchfeld.|
|Weitersuchen |Aslan+F3 |Sucht das nächste Vorkommen des aktuellen Suchbegriffs.|
|Rückwärtssuchen |Aslan+Umschalt+F3 |Sucht das vorherige Vorkommen des aktuellen Suchbegriffs.|
...
Wenn Sie im Lesemodus auf einen dieser Inhalte stoßen, meldet Aslan dem entsprechend "Eingebettetes Objekt", "Anwendung" oder "Dialog".
...
| Name |Tastenkombination |Beschreibung|
|---|---|---|
|Zum Inhalt des Lesemodus zurückkehren |Aslan+Strg+Leertaste |Verlässt den Fokus des eingebetteten Objekts und kehrt in den Lesemodus zurück.|

<!-- KC:endInclude -->

## Mathematische Inhalte auslesen {#ReadingMath}

Sie können MathPlayer 4 von Design Science verwenden, um mit Aslan in mathematischen Inhalten zu navigieren.
...
Aslan unterstützt die folgenden mathematischen Inhalte:
...
Wenn Sie ein Dokument lesen, das mathematischen Inhalt enthält, liest Aslan diesen vor, sofern dieser von Aslan unterstützt wird.
...
Wenn Sie Aslan hauptsächlich mit einer Sprachausgabe verwenden, können Sie einen mathematischen Ausdruck in Segmente unterteilt abfragen, anstatt ihn in einem Stück angesagt zu bekommen.
...
1. Bewegen Sie den Aslan-Cursor auf den mathematischen Ausdruck
Standardmäßig ist der Aslan-Cursor an den System-Cursor gekoppelt. Sie können also die Standard-Navigationstasten verwenden, um den Aslan-Cursor auf den mathematischen Inhalt zu bewegen.
...
| Name |Tastenkombination |Beschreibung|
|---|---|---|
|In mathematischen Inhalten navigieren |Aslan+Alt+M |Beginnt die Navigation in mathematischen Inhalten|
...
Wenn Sie eine Braillezeile besitzen, kann Aslan diese verwenden, um Informationen in Blindenschrift darzustellen.
...
Dieser Abschnitt enthält auch Informationen darüber, welche Braillezeilen die automatische Braillezeilenerkennung von Aslan unterstützen.
Verwenden Sie die Kategorie "[Braille](#BrailleSettings)" in den Aslan-Einstellungen, um Aslan an Ihre Braillezeile anzupassen.
...
Aslan unterstützt die Eingabe von Kurz-, Voll- und Basisschrift über eine Braille-Tastatur.
...
Während sich Aslan in erster Linie an blinde oder sehbehinderte Menschen richtet, die in erster Linie Sprache und/oder Blindenschrift zur Bedienung eines Computers verwenden, bietet es auch integrierte Möglichkeiten, den Inhalt des Bildschirms zu ändern.
Mit Aslan wird einer solchen Sehhilfe als Quelle zur Bildverbesserung bezeichnet.

Aslan bietet mehrere integrierte Quellen zur Bildverbesserung an, die im Folgenden beschrieben werden.
Zusätzliche Quellen für die Bildverbesserung können über [Aslan-Erweiterungen](#AddonsManager) bereitgestellt werden.

Diese Einstellungen können Sie in den [Aslan-Einstellungen](#AslanSettings), in der Kategorie [Visuelle Darstellungen](#VisionSettings) vornehmen.
...
Wenn die Fokus-Hervorhebung in der Kategorie "[Visuelle Darstellungen](#VisionSettings)" der [Aslan-Einstellungen](#AslanSettings) aktiviert ist, können Sie [ändern, ob Sie den Fokus, das Navigator-Objekt oder den Cursor im Lesemodus](#VisionSettingsFocusHighlight) hervorheben möchten.
...
In Aslan gibt es dafür den Bildschirmvorhang, der aktiviert werden kann, um den Bildschirm zu verdunkeln.

Sie können den Bildschirmvorhang in den [Aslan-Einstellungen](#AslanSettings), in der Kategorie [Visuelle Darstellungen](#VisionSettings) aktivieren.
...
Aslan unterstützt die in Windows 10 enthaltene Texterkennung, um Text aus Bildern auszulesen.
Über Aslan-Erweiterungen können zusätzliche Werkzeuge zur Inhaltserkennung bereitgestellt werden.
...
Sie können die Erkennungssprache einstellen, indem Sie die Kategorie [Windows 10-Texterkennung](#Win10OcrSettings) in den Aslan-Einstellungen unter dem Menü "Optionen" aufrufen.
...
<!-- KC:beginInclude -->
Drücken Sie Aslan+R, um den Text im aktuellen NAvigator-Objekt mit Hilfe der Windows 10-Texterkennung zu erkennen.
<!-- KC:endInclude -->

## Anwendungsspezifische Funktionen {#ApplicationSpecificFeatures}

Aslan stellt für bestimmte Anwendungen zusätzliche Funktionen bereit, um bestimmte Aufgaben zu erleichtern oder um Zugriff auf Funktionalität bereitzustellen, die sonst nicht zugänglich wäre.

### Microsoft Word {#MicrosoftWord}
#### Automatisches Lesen von Zeilen- und Spaltenüberschriften {#WordAutomaticColumnAndRowHeaderReading}

Wenn Sie in Microsoft Word in einer Tabelle navigieren, ist Aslan in der Lage, automatisch die Zeilen- und Spaltenüberschriften zu lesen.
Hierfür muss die Option "Reihen- und Spaltenüberschriften von Tabellen ansagen" unter der Kategorie "Dokumentformatierungen" in den [Aslan-Einstellungen](#AslanSettings) aktiviert sein.
Aslan muss die Reihen und Spalten mit den Überschriften kennen.
...
| Name |Tastenkombination |Beschreibung|
|---|---|---|
|Spaltenüberschriften festlegen |Aslan+Umschalt+C |Wenn Sie diese Tastenkombination einmal drücken, legen sie die aktuelle Tabellenzelle als erste Zelle mit Spaltenüberschriften fest. Diese Spaltenüberschriften werden dann automatisch gelesen, sobald Sie unterhalb dieser Zeile navigieren. Zweimaliges Drücken dieser Tastenkombination löscht diese Zuweisung wieder.|
|Zeilenüberschriften festlegen |Aslan+Umschalt+R |Wenn Sie diese Tastenkombination einmal drücken, wird die aktuelle Tabellenzelle als erste Zelle mit Zeilenbeschriftung festgelegt. Diese Spaltenüberschriften werden dann automatisch gelesen, sobald Sie unterhalb dieser Zeile navigieren. Zweimaliges Drücken dieser Tastenkombination löscht diese Zuweisung wieder.|
...
<!-- KC:beginInclude -->
Um den Lesemodus in Microsoft Word ein- oder auszuschalten, drücken Sie Aslan+Leertaste.
<!-- KC:endInclude -->
...
<!-- KC:beginInclude -->
Wenn Sie sich in Microsoft Word im Lesemodus befinden, können Sie mit der Tastenkombination Aslan+F7 auf die Elementliste zugreifen.
<!-- KC:endInclude -->
...
<!-- KC:beginInclude -->
Verwenden Sie die Tastenkombination Aslan+Alt+C, um sich einen Kommentar an der aktuellen Cursorposition anzeigen zu lassen, sofern vorhanden.
<!-- KC:endInclude -->
...
Wenn Sie in Microsoft Excel in einer Tabelle navigieren, ist Aslan in der Lage, automatisch die Zeilen- und Spaltenüberschriften zu lesen.
Hierfür muss die Option "Reihen- und Spaltenüberschriften von Tabellen ansagen" unter der Kategorie "Dokumentformatierungen" in den [Aslan-Einstellungen](#AslanSettings) aktiviert sein.
Aslan muss die Reihen oder Spalten mit den Überschriften kennen.
Verwenden Sie einen der folgenden Befehle, nachdem Sie sich in die erste Zelle mit der Reihen- und Spaltenüberschrift bewegt haben:
<!-- KC:beginInclude -->
...
<!-- KC:beginInclude -->
Verwenden Sie die Tastenkombination Aslan+F7, um auf die Elementliste zuzugreifen.
<!-- KC:endInclude -->
...
Aslan zeigt zu jedem Formularfeld dessen Alternativtext sowie die Adresse der Zelle an, die das Formularfeld enthält.
...
<!-- KC:beginInclude -->
Verwenden Sie die Tastenkombination Aslan+Alt+C, um sich die Anmerkungen der aktuellen Zelle anzeigen zu lassen, sofern vorhanden.
In Microsoft 2016 / 365 und neuer wurden die klassischen Kommentare in Microsoft Excel in "Anmerkungen" umbenannt.
<!-- KC:endInclude -->
Alle Anmerkungen im Arbeitsblatt können nach Drücken von Aslan+F7 auch in der Aslan-Elementliste aufgelistet werden.

Aslan kann auch ein spezielles Dialogfeld zum Hinzufügen oder Bearbeiten bestimmter Anmerkung anzeigen.
Aslan überschreibt auf Grund von Einschränkungen der Zugänglichkeit den nativen Bearbeitungsbereich für die Anmerkungen in Microsoft Excel, aber die Tastenkombination für das Dialogfeld wird von Microsoft Excel übernommen und funktioniert daher auch ohne Aslan.
<!-- KC:beginInclude -->
...
Dadurch wird jedoch der unzugängliche Bereich zur Bearbeitung von Anmerkungen geöffnet und nicht das Aslan-spezifische Dialogfeld zur Bearbeitung der Anmerkungen.
...
<!-- KC:beginInclude -->
Um auf gesperrte Zellen zuzugreifen, wechseln Sie zunächst mit Aslan+Leertaste in den Lesemodus und navigieren Sie anschließend mit den Standard-Excel-Tastenkombinationen wie z. B. den Pfeiltasten zu den gewünschten Zellen.
<!-- KC:endInclude -->
...
| Name |Tastenkombination |Beschreibung|
|---|---|---|
|Letzte Nachricht ausgeben |Aslan+Strg+1 bis 4 |Gibt die zur Nummer passende Nachricht aus; Aslan+Strg+2 gibt beispielsweise die vorletzte Nachricht aus.|
...
Aslan unterstützt das Lesen von und das Navigieren in Büchern in Kindle für PC.
...
Aslan bietet frühe Unterstützung beim Lesen und interaktiver Navigation von mathematischen Inhalten für Bücher mit barrierefreier Mathematik.
...
Aslan bietet Unterstützung für die Windows-Befehlskonsole, die von der Eingabeaufforderung, PowerShell und dem Windows-Subsystem für Linux verwendet wird.
...
Text, der im Fenster nicht sichtbar angezeigt wird, ist mit den Textanzeigebefehlen von Aslan nicht zugänglich.
Daher ist es notwendig, im Konsolenfenster zu navigieren, um frühere Texte zu lesen.
<!-- KC:beginInclude -->
Die folgenden integrierten Tastenkombinationen der Windows-Konsole können nützlich sein, wenn Sie [Text](#ReviewingText) mit Aslan lesen möchten:
...
## Aslan konfigurieren {#ConfiguringAslan}

Die meisten Aslan-Einstellungen können über Dialogfelder vorgenommen werden, die im Untermenü des Aslan-Menüs zu finden sind.
Viele Einstellungen können in den mehrseitigen [Aslan-Einstellungen](#AslanSettings) vorgenommen werden.
...
Für manche Einstellungen können Sie auch auf den Schalter "Übernehmen" klicken, um weiterhin in den Aslan-Einstellungen zu bleiben.
...
### Die Aslan-Einstellungen {#settings}

<!-- KC:settingsSection: || Name | "Desktop"-Tastenkombination | "Laptop"-Tastenkombination | Beschreibung | -->
Mit Hilfe der Aslan-Einstellungen können viele Parameter in Aslan verändert werden.
...
Wenn Sie eine solche Kurztaste drücken, wird das Dialogfeld der Aslan-Einstellungen mit der passenden Kategorie aufgerufen.
...
Falls Sie Kategorien oder Dialogfelder öffnen wollen, die nicht über zugewiesene Tastenkombinationen verfügen, können Sie über "[Tastenbefehle](#InputGestures)" im Aslan-Menü unter "Optionen" verwenden, um eigene Tastenkombinationen zuzuweisen.

Weiter unten finden Sie Erläuterungen zu jeder Kategorie aus den Aslan-Einstellungen.

#### Allgemein (Aslan+Strg+G) {#GeneralSettings}

Die Kategorie "Allgemein" in den Aslan-Einstellungen legt das allgemeine Verhalten von Aslan fest, wie z. B. die Sprache der Benutzeroberfläche und ob nach Updates gesucht werden soll oder nicht.
Diese Kategorie enthält die folgenden Optionen:

##### Sprache {#GeneralSettingsLanguage}

In dieser Liste können Sie die Sprache festlegen, in welcher die Benutzeroberfläche und die Meldungen von Aslan ausgegeben werden sollen.
Standardmäßig ist die Option "Benutzerstandard, Windows" ausgewählt. Aslan enthält eine Vielzahl integrierter Sprachen.
Dies besagt, dass Aslan die Sprache verwendet, welche in Windows eingestellt ist.

Bitte beachten Sie: Um die Sprache zu ändern, muss Aslan neu gestartet werden.
...
Diese Option ist ein Kontrollfeld, wenn aktiviert, speichert Aslan jedes Mal beim Beenden automatisch die aktuelle Konfiguration.

##### Optionen zum Beenden von Aslan anzeigen {#GeneralSettingsShowExitOptions}

Mit diesem Kontrollkästchen legen Sie fest, ob beim Beenden von Aslan ein Dialogfeld angezeigt wird. Hier können Sie verschiedene Aktionen auslösen.
Wenn das Kontrollkästchen aktiviert ist, können Sie in diesem Dialogfeld auswählen, ob Sie Aslan beenden, neu starten, mit deaktivierten Erweiterungen neu starten oder ein ausstehendes Update installieren möchten.
Wenn das Kontrollkästchen nicht aktiviert ist, wird Aslan sofort beendet.

##### Sounds beim Starten und Beenden von Aslan {#GeneralSettingsPlaySounds}

Dieses Kontrollkästchen legt fest, ob Aslan beim Starten oder Beenden einen Sound wiedergeben soll.

##### Protokollierungsstufe {#GeneralSettingsLogLevel}

Mit diesem Kombinationsfeld können Sie einstellen, wie viel Aslan während der Ausführung protokollieren soll.
...
* Ausgeschaltet: Neben einer kurzen Startmeldung protokolliert Aslan während der Ausführung keine weiteren Meldungen.
* Informationen: Aslan protokolliert grundlegende Informationen wie Startmeldungen und Informationen, die für Entwickler nützlich sind.
...
##### Aslan automatisch nach der Windows-Anmeldung starten {#GeneralSettingsStartAfterLogOn}

Wenn diese Option aktiviert ist, wird Aslan automatisch gestartet, sobald Sie sich bei Windows anmelden.
Diese Option ist nur in der installierten Version verfügbar!

##### Aslan bei der Windows-Anmeldung verwenden (Administratorrechte sind erforderlich) {#GeneralSettingsStartOnLogOnScreen}

Wenn Sie sich bei Windows mit Ihrem Benutzernamen und Ihrem Passwort anmelden, bewirkt dieses Kontrollfeld, dass Aslan automatisch bereits bei der Anmeldung gestartet wird.
Diese Option ist nur in der installierten Version verfügbar!
...
Mit diesem Schalter wird Ihre aktuell gespeicherte Aslan-Benutzerkonfiguration in das Systemkonfigurationsverzeichnis von Aslan kopiert, so dass Aslan sie während der Anmeldung und beim Ausführen der Benutzerkontensteuerung und anderer Sicherheitsmeldungen unterstützt.
Speichern Sie zuvor Ihre Konfiguration, um sicherzustellen, dass auch tatsächlich all Ihre Einstellungen übernommen wurden. Sie können dies im Aslan-Menü oder mit der Tastenkombination Aslan+Strg+C vornehmen.
Diese Option ist nur in der installierten Version verfügbar!

##### Automatisch nach Updates suchen {#GeneralSettingsCheckForUpdates}

Wenn diese Option aktiviert ist, sucht Aslan automatisch nach Updates und informiert Sie, sobald eine neue Version verfügbar ist.
Sie können auch manuell nach neuen Versionen suchen, indem Sie im Hilfe-Menü von Aslan den Eintrag "Nach Updates suchen" auswählen.
...
* Aktuelle Aslan-Version
* Betriebssystem-Version
* Systemarchitektur (32- oder 64-Bit)

##### Aslan-Nutzungsdaten sammeln und an NV Access übermitteln {#GeneralSettingsGatherUsageStats}

Ist diese Option aktiviert, sammelt NV Access zusätzliche Informationen wie die Landessprache, das Land oder die geografische Region. Diese Informationen werden benutzt, um Nutzungsstatistiken über die Anzahl der Aslan-Nutzer zu erstellen.
...
* Eingestellte Landessprache in Aslan.
* Art der Aslan-Instanz (portabel oder installiert).
...
Diese Informationen helfen NV Access bei der zukünftigen Entwicklung von Aslan.

##### Beim Aslan-Start auf ausstehende Updates hinweisen {#GeneralSettingsNotifyPendingUpdates}

Wenn diese Option aktiviert ist, informiert Aslan nach dem Start darüber, falls ein Update noch aussteht, um diese zu installieren.
Sie können ausstehende Updates auch manuell über das Aslan-Menü, das Dialogfeld zum Beenden von Aslan oder durch eine erneute Suche nach Updates aus dem Hilfe-Menü installieren.

#### Stimme- und Sprachausgabe (Aslan+Strg+V) {#SpeechSettings}

Die Kategorie "Sprache" in den Aslan-Einstellungen enthält Optionen, mit denen Sie die Sprachausgabe sowie die Sprachmerkmale der ausgewählten Sprachausgabe ändern können.
...
Wenn das Dialogfeld zum Ändern der Sprachausgabe mit "OK" oder "Abbrechen" geschlossen wird, erscheinen wieder die Aslan-Einstellungen mit den dazugehörigen Kategorien.
...
Wenn Sie die Sprachausgabe eSpeak NG verwenden, die bei Aslan mitgeliefert wird, handelt es sich um ein Kombinationsfeld, in dem Sie die Variante der Sprachausgabe auswählen können.
...
Dieses Kontrollfeld erlaubt Ihnen einzustellen, ob Aslan die Sprache der Sprachausgabe automatisch passend zur angegebenen Dokumentsprache einstellen soll.
Diese Option ist standardmäßig aktiviert.

##### Dialekte automatisch wechseln (falls unterstützt) {#SpeechSettingsDialectSwitching}

Wenn der automatische Sprachenwechsel aktiviert ist, können Sie Aslan mit dieser Option dazu bringen, nicht nur die eigentliche Sprache, sondern auch den Dialekt passend zur Dokumentsprache während des Lesens zu ändern.
Wenn Sie beispielsweise ein amerikanisch-englisches Dokument lesen, in dem Textpassagen als britisch-englisch ausgewiesen sind, wird Aslan automatisch den Dialekt der Sprachausgabe anpassen, sofern diese Option aktiviert ist.
...
Tastenkombination: Aslan+P

Hier kann eingestellt werden, ob und wie viele Satz- und Sonderzeichen Aslan als Wort aussprechen soll.
Wenn hier "Alle" ausgewählt ist, spricht Aslan alle Symbole, als wären diese ausgeschrieben.
...
Sollte in Aslan die Aussprache von Symbolen und Sonderzeichen nicht korrekt funktionieren, können Sie diese Option deaktivieren.
...
Wenn dieses Kontrollkästchen aktiviert ist, wird Aslan bei der Aussprache von Zeichen und Symbolen zusätzliche Wörterbücher für die Aussprache der Symbole einbeziehen.
...
Wenn Sie möchten, dass Aslan auf Basis dieser Daten Beschreibungen von Emoji-Zeichen vorliest, sollten Sie diese Option aktivieren.
...
Standardmäßig erhöht Aslan die Stimme für jeden Großbuchstaben leicht, aber einige Synthesizer unterstützen dies möglicherweise nur unzureichend.
...
#### Sprachausgabe auswählen (Aslan+Strg+S) {#SelectSynthesizer}

Das Dialogfeld, welches durch das Betätigen des Schalters "Ändern" geöffnet wird, ermöglicht die Sprachausgabe auszuwählen. Diese wird dann von Aslan dazu genutzt Informationen mittels Sprache zugänglich zu machen.
Aslan wird die Sprachausgabe verwenden, sobald Sie einen Eintrag ausgewählt und den Schalter "OK" betätigt haben.
Wenn ein Fehler auftritt, wird Aslan einen Fehlerton abspielen und die vorherige Sprachausgabe weiterhin verwenden.

##### Sprachausgabe {#SelectSynthesizerSynthesizer}

Diese Option legt fest, welche Sprachausgabe beim Verwenden von Aslan wiedergegeben werden soll.

Eine Liste aller von Aslan unterstützten Sprachausgaben finden Sie im Abschnitt [Unterstützte Sprachausgaben](#SupportedSpeechSynths).

In der Liste der verfügbaren Sprachausgaben gibt es auch den Eintrag "Keine Sprachausgabe". Damit wird Aslan stumm geschaltet.
...
Tastenkombination: Aslan+Umschalt+D

In Windows 8 und neuer kann diese Option die Systemlautstärke automatisch immer dann reduzieren, wenn Aslan spricht oder die ganze Zeit, wenn Aslan ausgeführt wird.

* Niemals: Die Lautstärke anderer Audioquellen wird nicht reduziert.
* Während des Sprechens und bei Wiedergabe der Signaltöne: Aslan wird die Lautstärke anderer Audioquellen nur dann reduzieren, wenn es spricht oder Töne ausgibt. Diese Option funktioniert nicht bei allen Sprachausgaben.
* Immer: Die Lautstärke anderer Audioquellen wird immer reduziert, solange Aslan ausgeführt wird.

Diese Funktion läuft nur bei installierten Aslan-Versionen.
Bei portable oder temporäre Aslan-Versionen wird diese Funktion nicht unterstützt.

#### Einstellungsring der Sprachausgabe {#SynthSettingsRing}

Wenn Sie rasch durch die verschiedenen Sprachausgabeneinstellungen schalten möchten, ohne zuvor extra in die Kategorie der Aslan-Einstellungen gehen zu müssen, so gibt es die folgenden Tastenkombinationen in Aslan, die Sie von überall aus benutzen können:
<!-- KC:beginInclude -->

| Name |"Desktop"-Tastenkombination |"Laptop"-Tastenkombination |Beschreibung|
|---|---|---|---|
|Zur nächsten Sprachausgabeneinstellung springen |Aslan+Strg+Pfeiltaste nach rechts |Aslan+Strg+Umschalt+Pfeiltaste nach rechts |Springt zur nächsten verfügbaren Sprachausgabeneinstellung ausgehend von der aktuellen Einstellung; beginnt von vorne, nach dem letzten Eintrag.|
|Zur vorherigen Sprachausgabeneinstellung springen |Aslan+Strg+Pfeiltaste nach links |Aslan+Strg+Umschalt+Pfeiltaste nach links |Springt zur vorherigen verfügbaren Sprachausgabeneinstellung ausgehend von der aktuellen Einstellung; beginnt von hinten, nach dem ersten Eintrag.|
|Aktuelle Sprachausgabeneinstellung erhöhen |Aslan+Strg+Pfeiltaste nach oben |Aslan+Strg+Umschalt+Pfeiltaste nach oben |Erhöht die aktuelle Einstellung (z. B. die Sprechgeschwindigkeit erhöhen, die nächste Stimme auswählen, die Lautstärke erhöhen).|
|Aktuelle Sprachausgabeneinstellung verringern |Aslan+Strg+Pfeiltaste nach unten |Aslan+Strg+Umschalt+Pfeiltaste nach unten |Verringert die aktuelle Einstellung (z. B. die Sprechgeschwindigkeit verringern, wählt die vorherige Stimme aus, verringert die Lautstärke).|

<!-- KC:endInclude -->

#### Braille {#BrailleSettings}

Die Kategorie "Braille" in den Aslan-Einstellungen bietet verschiedene Optionen an, um Aspekte der Ein- und -Ausgabe in Braille einzustellen.
Folgende Optionen sind enthalten:

##### Braillezeile ändern {#BrailleSettingsChange}

Mit dem Schalter "Ändern" in den Aslan-Einstellungen in der Kategorie "Braille" wird das Dialogfeld zum [Ändern der Braillezeile](#SelectBrailleDisplay) geöffnet. Dort können Sie die Braillezeile auswählen.
Dieses Dialogfeld wird zusätzlich zu den Aslan-Einstellungen geöffnet.
Wenn das Dialogfeld zum Ändern der Sprachausgabe mit "OK" oder "Abbrechen" geschlossen wird, erscheinen wieder die Aslan-Einstellungen mit den dazugehörigen Kategorien.
...
Dies betrifft den System- und den Aslan-Cursor, nicht jedoch die Kennzeichnung von markiertem Text.
...
##### Cursor-Form für Aslan-Cursor {#BrailleSettingsCursorShapeForReview}

Hiermit legen Sie das Punktmuster des Cursors fest, welches verwendet werden soll, wenn die Braillezeile an den Aslan-Cursor gekoppelt wird.
Die Anzeige für Markierungen wird dadurch nicht beeinflusst, Markierungen werden immer durch statische Punkte 7 und 8 angezeigt.

##### Anzeigedauer für Meldungen {#BrailleSettingsMessageTimeout}

Dieses numerische Eingabefeld legt fest, wie viele Sekunden die Aslan-Meldungen auf der Braillezeile angezeigt werden sollen. Wenn eine Meldung erscheint, kann diese durch Drücken einer Routing-Taste geschlossen werden. Durch Drücken einer Navigationstaste kann die Anzeigedauer einer Meldung verlängert werden.
Um die Anzeige der Meldungen abzuschalten, geben Sie hier 0 an.
Die Aslan-Meldung wird beim Drücken einer Routing-Taste auf der Braillezeile sofort verworfen, erscheint aber wieder, wenn eine entsprechende Taste gedrückt wird, die die Nachricht auslöst.

##### Meldungen dauerhaft anzeigen {#BrailleSettingsNoMessageTimeout}

Wird diese Option aktiviert, zeigt Aslan alle Meldungen dauerhaft auf der Braillezeile an.

<!-- KC:setting -->

##### Braillezeile koppeln {#BrailleTether}

Tastenkombination: Aslan+Strg+T

Mit dieser Option können Sie bestimmen, ob die Braillezeile dem System-Fokus / -Cursor, dem Navigator-Objekt / Aslan-Cursor oder beiden folgen soll.
...
Sobald Sie den Navigator oder die Befehle zum Text betrachten verwenden, wird die Braillezeile automatisch an den Aslan-Cursor gekoppelt, bis sich der Fokus bewegt.
...
In diesem Fall folgt die Braillezeile nicht dem Aslan-Navigator während der Objekt-Navigation oder dem Aslan-Cursor während der Anzeige.
...
Wenn diese Option aktiviert ist, trennt Aslan ein Wort nicht, das für die Darstellung auf der Braillezeile zu lang ist.
...
Wenn diese Option ausgeschaltet ist, zeigt Aslan so viel wie möglich vom Wort an, aber ein Teil kann abgeschnitten sein.
Aslan zeigt dann den Rest des Wortes im nächsten Bereich an.
...
Für das obige Beispiel bedeutet das, dass Aslan zunächst so viel Information wie möglich auf der Braillezeile anzeigt, sobald ein Listeneintrag den Fokus erhalten hat.
...
Bei der Einstellung "immer Zeile auffüllen" wird Aslan immer so viel Kontextinformation wie möglich auf der Braillezeile anzeigen, selbst wenn Sie diese Information schon vorher bekommen haben.
...
Dies ist das Standardverhalten von Aslan 2017.2 und älter.

Bei der Einstellung "Nur beim Rückwärtsnavigieren" wird Aslan niemals automatisch Kontextinformationen anzeigen.
Für das obige Beispiel bedeutet dies, dass Aslan immer nur den aktuellen Listeneintrag anzeigt.
...
#### Braillezeile auswählen (Aslan+Strg+A) {#SelectBrailleDisplay}

Dies ist das Dialogfeld zum Ändern der Braillezeile, welches über den Schalter "Ändern" in der Kategorie "Braille" geöffnet wird. Es ermöglicht die Braillezeile auszuwählen, die Aslan dazu verwendet, Informationen in Punktschrift darzustellen.
Aslan wird die Braillezeile verwenden, sobald Sie einen Eintrag ausgewählt und den Schalter "OK" betätigt haben.
Falls ein Fehler auftritt, wird Aslan einen Fehlerton wiedergeben und die vorherige Braillezeile weiterhin verwenden, sofern vorhanden.
...
Die automatische Option ermöglicht Aslan die Suche nach vielen unterstützten Braillezeilen im Hintergrund.
Wenn diese Funktion aktiviert ist und Sie eine unterstützte Braillezeile über USB oder Bluetooth anschließen, verbindet sich Aslan automatisch mit dieser Braillezeile.
...
Standardmäßig wird Aslan automatisch sämtliche USB-Anschlüsse und Bluetooth-Geräte nach Ihrer Braillezeile absuchen.
...
Mit der Kategorie Visuelle Darstellungen im Dialogfeld Aslan-Einstellungen können Sie[visuelle Verbesserungen](#Vision) aktivieren, deaktivieren und konfigurieren.

Beachten Sie, dass die verfügbaren Optionen in dieser Kategorie durch [Aslan-Erweiterungen](#AddonsManager) erweitert werden können.
Standardmäßig enthält diese Einstellungskathegorie die folgenden Optionen:

##### Fokus hervorheben {#VisionSettingsFocusHighlight}

Die Kontrollkästchen in der Gruppierung Fokus hervorheben steuern das Verhalten der integrierten Aslan-Funktion[Fokus Hervorheben](#VisionFocusHighlight).
...
Zusätzliche Quellen für die Verbesserung der visuellen Darstellungen können mittels [Aslan-Erweiterungen](#AddonsManager) von Drittanbietern bereitgestellt werden.
...
#### Tastatur (Aslan+Strg+K) {#KeyboardSettings}

Die Kategorie "Tastatur" im Dialogfeld Aslan-Einstellungen enthält Optionen, die das Verhalten von Aslan bei der Verwendung und Eingabe auf Ihrer Tastatur festlegen.
...
##### Aslan-Tasten auswählen {#KeyboardSettingsModifiers}

Die Kontrollkästchen in dieser Liste legen fest, welche Tasten als [Aslan-Tasten](#TheAslanModifierKey) verwendet werden können. Die folgenden Tasten stehen zur Auswahl:
...
Wenn Sie keine dieser Tasten als Aslan-Tasten festlegen, können Sie nicht mehr auf bestimmte Funktionen von Aslan zugreifen.
Aus diesem Grund wird eine Fehlermeldung angezeigt, wenn Sie keine Taste als Aslan-Taste verwenden.
Legen Sie daher immer mindestens eine Taste als Aslan-Taste fest, nachdem Sie die Fehlermeldung mit "OK" geschlossen haben.

<!-- KC:setting -->

##### Eingegebene Zeichen ansagen {#KeyboardSettingsSpeakTypedCharacters}

Tastenkombination: Aslan+2

Wenn dieses Kontrollfeld aktiviert ist, gibt Aslan jedes Zeichen wieder, dass Sie auf der Tastatur eingegeben haben.

<!-- KC:setting -->

##### Eingegebene Wörter ansagen {#KeyboardSettingsSpeakTypedWords}

Tastenkombination: Aslan+3

Wenn dieses Kontrollfeld aktiviert ist, gibt Aslan jedes Wort wieder, dass Sie auf der Tastatur eingegeben haben.
...
Wenn diese Option aktiviert ist, Unterbrechen Navigationsbefehle nun nicht mehr den Lesevorgang beim Lesen eines Dokuments mit Aslan+Pfeil Ab. Hierzu gehören sowohl die Schnellnavigationstasten als auch die Befehle zum Navigieren zwischen Zeilen und Absätzen. Diese Option ist standardmäßig deaktiviert.
...
Tastenkombination: Aslan+4

Wenn dieses Kontrollfeld aktiviert ist, gibt Aslan alle Tasten wieder, die nicht mit einem Zeichen belegt sind. Das können Tastenkombinationen wie z. B. "Strg+Buchstabe" sein.
...
Diese Option funktioniert nur, wenn in den Aslan-Einstellungen in der [Kategorie "Dokumentformatierungen"](#DocumentFormattingSettings) die Anzeige bei Rechtschreibfehlern aktiviert ist.

##### Tastendrücke anderer Anwendungen verarbeiten {#KeyboardSettingsHandleKeys}

Mit dieser Option legen Sie fest, ob per Software emulierte Tastendrücke von Aslan verarbeitet werden sollen. Hierzu zählen beispielsweise Tastendrücke auf einer Bildschirmtastatur oder per Spracherkennung erkannte Zeichen.
...
#### Maus (Aslan+Strg+M) {#MouseSettings}

Die Kategorie "Maus" in den Aslan-Einstellungen bietet Optionen zur Mausverfolgung, zu akustischer Koordinatenanzeige, etc.
Es sind folgende Optionen enthalten:

##### Änderungen der Mauszeigerform ansagen {#MouseSettingsShape}

Dieses Kontrollfeld legt fest, ob Aslan bei Änderung des Mauszeigers die Form mit ansagen soll.
...
Tastenkombination: Aslan+M

Dieses Kontrollfeld weist Aslan an, den Text unter dem Mauszeiger vorzulesen. Das erleichtert Ihnen das Auffinden von Informationen am Bildschirm, wenn Sie mittels der Maus den Mauszeiger über den Bildschirm bewegen.
...
Wenn diese Option aktiviert ist, sagt Aslan den Typ an, sobald sich der Mauszeiger auf dem Objekt befindet.

##### Audio-Koordinaten bei Maus-Bewegungen {#MouseSettingsAudio}

Das Aktivieren dieser Option bewirkt, dass Aslan die Position des Mauszeigers durch Signaltöne an Ort und Stelle wiedergibt, sodass der Anwender die Mausposition auf dem Bildschirm akustisch mitverfolgen und selbst ermitteln kann.
Je näher sich der Mauszeiger am oberen Bildschirmrand befindet, desto höher ist der Signalton.
Richtig ausgerichtete Stereolautsprecher oder Kopfhörer vorausgesetzt, wird der Signalton umso mehr links oder rechts abgespielt, je weiter links oder rechts sich der Mauszeiger bewegt.

##### Lautstärke der Audio-Koordinaten durch Helligkeit kontrollieren {#MouseSettingsBrightness}

Wenn die Option "Audio-Koordinaten bei Maus-Bewegungen" und diese Option aktiviert sind, passt Aslan die Lautstärke der Signaltöne an die Helligkeit unter dem Mauszeiger an.
Diese Option ist standardmäßig deaktiviert.
...
Diese Option ist standardmäßig deaktiviert.
Wenn Sie diese Option aktivieren und die Option "Mausverfolgung aktivieren" aktiviert haben, wird Aslan nicht bekannt geben, was sich unter der Maus befindet, wenn die Maus von einer anderen Anwendung bewegt wird.

#### Touch-Interaktion {#TouchInteraction}

Diese Kategorie ist nur auf Computern mit Windows 8 oder neuer verfügbar, sofern dieser ein berührungsempfindlicher Bildschirm (Touchscreen) besitzt. Hier können Sie einstellen, wie Aslan mit Touchscreens interagiert.
Folgende Optionen sind enthalten:

##### Unterstützung für die Berührungsinteraktion aktivieren {#TouchSupportEnable}

Dieses Kontrollkästchen aktiviert die Aslan-Unterstützung für die Berührungsinteraktion.
...
Diese Einstellung kann auch mit Aslan+Strg+Alt+T umgeschaltet werden. 
...
#### Aslan-Cursor {#ReviewCursorSettings}

Diese Kategorie beeinflusst das Verhalten des Aslan-Cursors.
Folgende Optionen sind enthalten:
...
Tastenkombination: Aslan+7

Wenn eingeschaltet, platziert sich der Aslan-Cursor immer auf dem Objekt, das den Fokus besitzt, sobald sich der Fokus ändert.

<!-- KC:setting -->

##### System-Cursor verfolgen {#ReviewCursorFollowCaret}

Tastenkombination: Aslan+6

Bei eingeschalteter Option wird automatisch bei Bewegung der Aslan-Cursor zur Position des System-Cursors gezogen.

##### Mauszeiger verfolgen {#ReviewCursorFollowMouse}

Wenn eingeschaltet, verfolgt der Aslan-Cursor bei Bewegungen die Maus.

##### Einfacher Darstellungsmodus {#ReviewCursorSimple}

Bei eingeschaltetem Modus filtert Aslan nicht wichtige Objekte aus der Hierarchie der navigierbaren Objekte heraus. Dabei werden z. B. unsichtbare Objekte und Objekte, welche nur zu Gestaltungszwecken eingebaut wurden, übersprungen.
...
#### Objekt-Darstellung (Aslan+Strg+O) {#ObjectPresentationSettings}

Hier kann eingestellt werden, inwieweit Aslan über Position, Beschreibungen von Objekten, etc. informieren soll.
Diese Kategorie enthält die folgenden Optionen:

##### QuickInfos vorlesen {#ObjectPresentationReportToolTips}

Wenn dieses Kontrollkästchen aktiviert ist, meldet Aslan die QuickInfos, sobald sie erscheinen.
...
Wenn dieses Kontrollfeld aktiviert ist, teilt Aslan die Hilfesprechblasen und Toast-Benachrichtigungen mit, sobald diese erscheinen.
...
Aslan nennt zu den Einträgen eines Menüs oder Steuerelements zusätzlich die Kurztaste, sofern die Ansage dieser Option auch aktiviert ist.
...
Wenn die Ausgabe der Informationen zur Objektposition aktiviert ist, wird Aslan die Informationen zur Objektposition für bestimmte Steuerelemente ermitteln, falls diese nicht anderweitig verfügbar ist.

Wenn diese aktiviert ist, meldet Aslan Positionsinformationen für weitere Steuerelemente, wie Menüs und Symbolleisten, jedoch können diese Informationen etwas ungenau sein.
...
Tastenkombination: Aslan+U

Diese Option kontrolliert, wie Aslan die Aktualisierungen der Fortschrittsbalken ansagen soll.

Die folgenden Optionen stehen zur Auswahl:

* Aus: Bei Änderungen werden keine Fortschrittsmeldungen angesagt.
* Nur Ansage: Diese Option sagt den Fortschritt in Prozent an. Sobald sich der Fortschrittsbalken ändert, sagt Aslan den neuen Wert an.
...
* Ansage und Signaltöne: Bei dieser Option ertönt in Aslan beides; der Fortschritt wird mit einem Signalton und gleichzeitig mit der Sprache quittiert.

##### Im Hintergrund befindliche Fortschrittsbalken ausgeben {#ObjectPresentationReportBackgroundProgressBars}

Wenn dieses Kontrollfeld aktiviert ist, hält Aslan Sie über den Fortgang der Aktionen der minimierten Anwendungen trotzdem auf dem Laufenden.
Wenn Sie ein Fenster, welches einen Fortschrittsbalken enthält, minimieren oder zu einer anderen Anwendung wechseln, wird Aslan den Fortschrittsbalken weiterhin verfolgen. Dies erlaubt Ihnen anderes zu tun, während Aslan den Fortschrittsbalken weiterhin ausgibt.

<!-- KC:setting -->

##### Änderungen dynamischer Inhalte ansagen {#ObjectPresentationReportDynamicContent}

Tastenkombination: Aslan+5
...
Wenn diese Option aktiviert ist, spielt Aslan einen Klang ab, wenn eine Vorschlagsliste erscheint bzw. ausgeblendet wird.
...
Die Vorschlagsliste wird geschlossen, sobald Sie mit dem Fokus das Eingabefeld verlassen. Aslan kann Sie manchmal darüber informieren, sobald dies geschieht.

#### Eingabemethoden {#InputCompositionSettings}

Mit den Optionen aus dieser Kategorie können Sie steuern, wie Aslan die Eingabe von Zeichen asiatischer sprachen wiedergibt. dies betrifft sowohl IME als auch die Textdienste.
...
Diese Option weist Aslan an, den ausgewählten Schriftsatz auszugeben, sobald die Liste der Zeichensätze erscheint oder wenn Sie einen Schriftsatz auswählen.
...
Auch wenn Sie diese Option deaktiviert haben, wird der Aslan-Cursor auf den ausgewählten Schriftsatz gesetzt, sodass Sie den Navigator oder die Befehle zum Betrachten von Text benutzen können, um diesen oder andere Schriftsätze zu lesen.
...
mit dieser Option, die standardmäßig aktiviert ist, können Sie festlegen, ob Aslan eine kurze Beschreibung für einen zeichensatz ausgeben soll - entweder wenn Sie ihn auswählen oder wenn Aslan die Liste der Schriftsätze ausgibt, sobald diese erscheint.
...
Mit dieser Option teilen Sie Aslan mit, ob neue Zeichen, die in diese Zeichenketten eingegeben werden, zurückgemeldet werden sollen.
Diese Option ist standardmäßig aktiviert.
...
Mit dieser Option legen Sie fest, ob Aslan Änderungen an den Symbolen im composition string wiedergeben soll.
Diese Option ist standardmäßig aktiviert.

#### Lesemodus (Aslan+Strg+B) {#BrowseModeSettings}

Hier kann das Verhalten von Aslan in komplexen Dokumenten wie Webseiten beeinflusst werden.
Diese Kategorie enthält die folgenden Optionen:
...
Wenn die Option aktiviert ist, sagt Aslan die Elemente so an, wie sie visuell dargestellt werden. Wenn die Option deaktiviert ist, werden die Steuerelemente jeweils auf einer eigenen Zeile dargestellt.
...
Mit dieser Option können Sie bestimmen, wie Aslan mit Tabellen umgeht, die nur zu Gestaltungszwecken verwendet werden.
Wenn die Option aktiviert ist, wird Aslan solche Tabellen auf der Grundlage der [Einstellungen zur Dokumentformatierung](#DocumentFormattingSettings) anzeigen und über die Schnellnavigationstasten zugänglich machen.
...
Zum Beispiel: Sie befinden sich auf einer Webseite und landen beim Drücken der Tabulatortaste auf einem Formularfeld, schaltet Aslan in den Fokusmodus um, sofern diese Option eingeschaltet ist.

##### Bei Bewegungen des System-Cursors automatisch den Fokusmodus einschalten {#BrowseModeSettingsAutoPassThroughOnCaretMove}

Wenn diese Option eingeschaltet ist, schaltet Aslan den Fokusmodus beim Navigieren mit den Pfeiltasten automatisch ein oder aus.
Wenn Sie beispielsweise auf eine Webseite nach unten navigieren und auf einem Eingabefeld treffen, schaltet Aslan automatisch in den Fokusmodus um.
Wenn Sie das Eingabefeld verlassen, schaltet Aslan wieder zurück in den Lesemodus um.

##### Akustische Ausgabe von Fokus- und Lesemodus {#BrowseModeSettingsPassThroughAudioIndication}

Bei eingeschalteter Option ertönt in Aslan vor der Ansage über die Änderung ein besonderer Hinweiston, wenn Sie von einem in den anderen Modus wechseln.
...
In diesem Fall weist Aslan Windows an, einen Standardsound abzuspielen, wenn eine Taste gedrückt wird, die festgestellt wird.
...
#### Dokumentformatierungen (Aslan+Strg+D) {#DocumentFormattingSettings}
...
Wenn Sie z. B. das Kontrollkästchen Schriftartennamen ansagen aktivieren und anschließend in einem Text navigieren, wird Aslan jedes Mal den Namen der Schriftart ansagen, sobald sich die Schriftart ändert.
...
Diese Einstellung veranlasst Aslan dazu, ALLE Änderungen in den Formatierungen innerhalb einer Zeile anzusagen, wenn sie eingeschaltet ist. Das kann sehr leistungsintensiv sein.

Standardmäßig erkennt Aslan die Formatierung an der Position des System-Cursors. In einigen Situationen erkennt auch Aslan die Formatierungen der gesamten Zeile. Dies könnte allerdings die Systemleistung beeinträchtigen.
...
Mit dieser Option legen Sie fest, wie Zeileneinrückungen durch Aslan wiedergegeben werden sollen.
Das Kombinationsfeld enthält vier Einträge:

* Aus: Aslan wird Zeileneinrückungen in keiner Weise gesondert ausgeben
* Sprache: Diese Einstellung bringt Aslan dazu die Anzahl von Zeichen anzusagen, mit denen eine Zeile eingerückt wurde (z. B. 12 Leerzeichen oder 4 Tabulatoren)
* Signaltöne: Diese Einstellung bringt Aslan dazu, die Einrückung mit Hilfe von Signaltönen anzugeben. Wenn sich die Einrückung ändert, wird der Signalton höher, je stärker eine Zeile eingerückt ist.
...
Achtung! Die Einstellungen in dieser Kategorie sind für fortgeschrittene Benutzer und können dazu führen, dass Aslan bei falscher Konfiguration nicht richtig funktioniert.
Nehmen Sie Änderungen an diesen Einstellungen nur dann vor, wenn Sie sicher sind, dass Sie wissen, was Sie tun oder wenn Sie von einem Aslan-Entwickler ausdrücklich dazu angewiesen wurden.
...
Bei der Entwicklung von Aslan-Erweiterungen ist es sinnvoll, Code bereits beim Schreiben testen zu können.
Diese Option ermöglicht es Aslan, benutzerdefinierte AppModule, globalPlugins, BrailleDisplayDrivers und synthDrivers aus einem speziellen Developer Scratchpad-Verzeichnis in Ihrem Aslan-Benutzerkonfigurationsverzeichnis zu laden.
Bisher hat Aslan benutzerdefinierten Code direkt aus dem Konfigurationsverzeichnis des Benutzers geladen, ohne dass dies deaktiviert werden konnte.
Diese Option ist standardmäßig deaktiviert und stellt sicher, dass kein ungetesteter Code ohne explizites Wissen des Benutzers in Aslan ausgeführt wird.
Wenn Sie benutzerdefinierten Code an andere weitergeben möchten, sollten Sie ihn als Aslan-Erweiterungspaket verpacken.
...
Diese Schaltfläche ist nur aktiviert, wenn Aslan so konfiguriert ist, dass benutzerdefinierter Code aus dem Developer Scratchpad-Verzeichnis geladen werden kann.
...
Diese Option ändert die Art und Weise, wie Aslan sich für Ereignisse registriert, die von der Microsoft UI Automation Accessibility API ausgelöst werden.
Wenn diese Option deaktiviert ist, registriert Aslan für viele UIA-Ereignisse, die innerhalb von Aslan selbst verarbeitet und verworfen werden.
Dies hat erhebliche negative Auswirkungen auf die Leistung, insbesondere bei Anwendungen wie Microsoft Visual Studio.
Wenn diese Option aktiviert ist, beschränkt Aslan daher die Ereignisregistrierung bei den meisten Ereignissen auf den System-Fokus.
...
Wenn diese Option aktiviert ist, versucht Aslan, die Microsoft-API für die Zugänglichkeit der UIA zu verwenden, um Informationen von Steuerelementen in Microsoft Word-Dokumenten abzurufen.
...
Wenn diese Option aktiviert ist, verwendet Aslan eine neue, experimentelle Version der Unterstützung für die Windows-Konsole, die die Vorteile von [Verbesserungen der Zugänglichkeit durch Microsoft](https://devblogs.microsoft.com/commandline/whats-new-in-windows-console-in-windows-10-fall-creators-update/) nutzt. Diese Funktion ist sehr experimentell und noch unvollständig, daher wird ihre Verwendung noch nicht empfohlen. Es wird jedoch erwartet, dass diese neue Unterstützung nach ihrer Fertigstellung zur Standardeinstellung wird und die Leistung und Stabilität von Aslan in Windows-Kommandokonsolen verbessert.

##### Passwörter vorlesen in der Windows-Konsole mit UIA-Unterstützung {#AdvancedSettingsWinConsoleSpeakPasswords}

Diese Einstellung steuert, ob Zeichen während der Eingabe #KeyboardSettingsSpeakTypedCharacters] oder [Wörter während der Eingabe](#KeyboardSettingsSpeakTypedWords) in Situationen vorgelesen werden sollen, in denen der Bildschirm in der Windows-Konsole bei aktivierter Unterstützung von UIA nicht aktualisiert wird (z. B. bei der Kennworteingabe). Aus Sicherheitsgründen sollte diese Einstellung deaktiviert bleiben. Sie sollten sie jedoch aktivieren, wenn Sie bei der Verwendung der neuen experimentellen Konsolenunterstützung von Aslan Leistungsprobleme oder Instabilität bei der Meldung von Zeichen und/oder Wörtern während der Eingabe feststellen.
...
Insbesondere das schnelle Durchlaufen von Nachrichten in Gmail mit Google Chrome kann dazu führen, dass Aslan alte Informationen noch vorliest.
Diese Funktionalität ist ab Aslan 2020.2 experimentell.

=== Zeitüberschreitung beim Bewegen des System-Cursors ====[AdvancedSettingsCaretMoveTimeout]
Mit dieser Option können Sie die Anzahl der Millisekunden konfigurieren, die Aslan wartet, bis sich der Cursor (Einfügepunkt) in den editierbaren Textsteuerelementen bewegt.
Wenn Sie feststellen, dass Aslan die Einfügemarke falsch zu verfolgen scheint, z. B., wenn Aslan immer ein Zeichen hinterher liegt oder beim Navigieren manche Zeilen wiederholt, dann sollten Sie versuchen, diesen Wert zu erhöhen.

##### Kategorien der Protokollierungsstufe {#AdvancedSettingsDebugLoggingCategories}

Die Kontrollkästchen in dieser Liste ermöglichen es Ihnen, bestimmte Kategorien von Debug-Meldungen im Protokoll von Aslan zu aktivieren.
Das Protokollieren dieser Meldungen kann zu Leistungseinbußen und großen Protokolldateien führen.
Schalten Sie nur eines davon ein, wenn dies von einem Aslan-Entwickler ausdrücklich angeordnet wurde, z. B. beim Debuggen, warum ein Braille-Treiber nicht ordnungsgemäß funktioniert.

### Verschiedene Optionen {#MiscSettings}

Neben den [Aslan-Einstellungen](#AslanSettings), befinden sich weitere Einträge im Aslan-Menü unter "Optionen". Diese werden weiter unten erläutert.

#### Die Aussprache-Wörterbücher {#SpeechDictionaries}

Im Menü Optionen finden Sie den Eintrag Aussprache-Wörterbücher. Dieses Menü enthält Dialogfelder mit deren Hilfe Sie festlegen können, wie Aslan einzelne Wörter oder Satzteile ausspricht.
Derzeit gibt es drei unterschiedliche Formen der Aussprache-Wörterbücher:
Diese sind:

* Standard: Regeln in diesem Wörterbuch haben Einfluss auf alle Aussprachen in Aslan.
* Stimme: Regeln in diesem Wörterbuch haben Einfluss auf die momentan verwendete Stimme der Sprachausgabe in Aslan.
* Temporär: Regeln in diesem Wörterbuch haben nur vorläufigen Einfluss auf alle Stimmen in Aslan (gilt nur für diese Sitzung). Beim nächsten Neustart von Aslan sind diese nicht mehr vorhanden.
...
Mit den Regeln für die Aussprache-Wörterbücher von Aslan können Sie eine Zeichenkette in eine andere ändern.
Ein einfaches Beispiel wäre, wenn Sie möchten, dass Aslan bei dem Wort "Frosch" jedes Mal "Vogel" sagt.
...
Standardmäßig ignoriert Aslan die Groß- und Kleinschreibung.
...
In diesem Dialogfeld können Sie Tastenbefehle (Tastenkombinationen, Tasten an einer Braillezeile, Aktionen auf einem Touchscreen) an Aslan-Funktionen zuweisen.
...
In diesem Dialogfeld werden alle verfügbaren Aslan-Funktionen in einer Baumansicht angezeigt, die nach Kategorien sortiert sind.
...
Oft kann eine Aslan-Funktion durch mehr als eine Tastenkombination ausgeführt werden.
...
Die Kategorie "Tasten der emulierten Systemtastatur" enthält Aslan-Befehle, die Tasten auf der Systemtastatur emulieren.
 ...
 * Ein Tastenbefehl mit Aslan-Tasten kann möglicherweise nicht auf eine emulierten Tastenbefehl ohne Aslan-Tasten abgebildet werden. Zum Beispiel kann das definieren der emulierten Eingabe "a" und das Konfigurieren des Tastenbefehls von "Strg+M" dazu führen, dass die Anwendung "Strg+A" interpretiert werden könnte.
...
Standardmäßig speichert Aslan die Einstellungen automatisch beim Beenden.
Sie können dies jedoch im Einstellungsmenü in den Optionen "Allgemein" ändern.
Über "Konfiguration speichern" können Sie im Aslan-Menü die Konfiguration manuell speichern.

Im Aslan-Menü können Sie über den Eintrag "Auf gespeicherte Einstellungen zurücksetzen" diese wiederherstellen.
Wenn Sie die Konfiguration auf die Standard-Einstellungen zurücksetzen möchten, können Sie dies über den Menüpunkt "Auf Standard-Einstellungen zurücksetzen" im Aslan-Menü erreichen.
...
| Name |"Desktop"-Tastenkombination |"Laptop"-Tastenkombination |Beschreibung|
|---|---|---|---|
|Konfiguration speichern |Aslan+Strg+C |Aslan+Strg+C |Speichert die Konfiguration ab, sodass sie nicht beim Beenden von Aslan verloren geht.|
|Konfiguration zurücksetzen |Aslan+Strg+R |Aslan+Strg+R |Setzt bei einmal Drücken die Aslan-Einstellungen auf den letzten gespeicherten Stand zurück. Bei dreimal Drücken wird auf die Standard-Einstellung zurückgesetzt.|
...
In Aslan können Sie dafür die Konfigurationsprofile verwenden.

Ein Konfigurationsprofil enthält nur die geänderten Einstellungen für das aktuelle Profil.
In Konfigurationsprofilen können die meisten Einstellungen geändert werden. Eine Ausnahme sind die Einstellungen aus der Kategorie "Allgemein" in den [Aslan-Einstellungen](#AslanSettings), da diese Aslan generell betreffen.
...
Konfigurationsprofile sind über den Menüpunkt "Konfigurationsprofile verwalten" im Aslan-Menü zu erreichen.
Dieses Dialogfeld ist auch über eine Tastenkombination erreichbar:
<!-- KC:beginInclude -->

* Aslan+Strg+P: Zeigt das Dialogfeld für die Konfigurationsprofile an.
...
In der portablen Aslan-Version werden sämtliche Einstellungen und Erweiterungen im Ordner "userConfig" unterhalb des Stammordners von Aslan abgelegt.

Bei den installierten Aslan-Versionen werden sämtliche Einstellungen und Erweiterungen in einem speziellen Ordner in dem Ordner des angemeldeten Benutzers in dessen Windows-Profil abgelegt.
...
Zusätzlich können Sie bei einer installierten Version von Aslan im Startmenü zu auf "Programme -> Aslan -> Benutzerkonfigurationsverzeichnis öffnen" gehen.

Die Einstellungen von Aslan für das Anmeldefenster und die Sicherheitsmeldungen der Benutzerkontensteuerung werden im Ordner "systemConfig" im Installationsordner von Aslan abgelegt.
Für gewöhnlich sollte diese Konfiguration unberührt bleiben!
Um die Konfiguration von Aslan während der Anmeldung oder bei Sicherheitsmeldungen zu ändern, konfigurieren Sie Aslan wie gewünscht, während Sie bei Windows angemeldet sind, speichern Sie die Konfiguration und betätigen Sie dann den Schalter "Aktuell gespeicherte Einstellungen während der Anmeldung und bei Sicherheitsmeldungen verwenden" in der Kategorie "Allgemein" der [Aslan-Einstellungen](#AslanSettings).

## Werkzeuge {#ExtraTools}
### Der Protokollbetrachter {#LogViewer}

Den Protokollbetrachter finden Sie im Aslan-Menü unter "Werkzeuge". Dieses Werkzeug gibt Ihnen die Möglichkeit, alles was seit dem Start von Aslan intern abläuft, einzusehen.
Mit Aslan+F1 wird der Protokollbetrachter geöffnet und Entwickler-Informationen über das aktuelle Navigator-Objekt angezeigt.
...
Für sehende Softwareentwickler oder für diejenigen, die Aslan anderen Sehenden vorführen möchten, ist ein freischwebendes Fenster verfügbar, dass Ihnen anzeigt, was alles von Aslan gegenwärtig gesprochen wird.

Um den Sprachausgaben-Betrachter einzuschalten, aktivieren Sie den Eintrag "Sprachausgaben-Betrachter", welchen Sie im Untermenü "Werkzeuge" im Aslan-Menü finden.
...
Wenn dieses Kontrollkästchen aktiviert ist, wird der Sprachausgaben-Betrachter geöffnet, sobald Aslan gestartet wird.
...
Wenn Sie in das Fenster des Betrachters klicken, unterbricht Aslan augenblicklich die weitere Aktualisierung des vorgelesenen Textes, sodass Sie nun die entsprechenden Einträge markieren und in die Zwischenablage kopieren können.
...
Für sehende Software-Entwickler oder Personen, die Aslan für ein sehendes Publikum demonstrieren, steht ein schwebendes Fenster zur Verfügung, mit dem Sie die Braille-Ausgabe und das Textäquivalent für jede Braillezeile anzeigen können.
...
Um den Braille-Betrachter zu aktivieren, wählen Sie im Aslan-Menü unter "Werkzeuge" den Menüpunkt "Braille-Betrachter" aus.
...
Wenn dies aktiviert ist, öffnet sich der Braille-Betrachter beim Starten von Aslan.
Sofern möglich, wird dessen Fenster dieselbe Position sowie die gleiche Größe wie bei der letzten Verwendung einnehmen.

### Die Python-Konsole {#PythonConsole}

Die Python-Konsole in Aslan, zu finden im Menü "Werkzeuge" im Aslan-Menü, ist ein Entwicklungswerkzeug, das für das Debugging, die allgemeine Inspektion von Aslan-Interna oder die Inspektion der Zugänglichkeitshierarchie einer Anwendung nützlich ist.
Weitere Informationen finden Sie im [Aslan-Entwicklerhandbuch](https://www.nvaccess.org/files/aslan/documentation/developerGuide.html).

### Erweiterungen verwalten {#AddonsManager}

Im Dialogfeld "Erweiterungen verwalten", den Sie über das Untermenü "Werkzeuge" im Aslan-Menü erreichen, können Sie Erweiterungspakete für Aslan installieren, deinstallieren, aktivieren oder deaktivieren.
Diese Pakete werden von der Aslan-Community bereitgestellt und können zusätzlichen Code enthalten, um Aslan-Funktionen zu verändern oder zu erweitern. Es ist auf diese Weise sogar möglich, neue Braillezeilen- oder Sprachausgabentreiber hinzuzufügen.
...
Hiermit wird die Webseite für Aslan-Erweiterungen https://addons.aslan-project.org geöffnet.
Falls Aslan installiert ist und gerade verwendet wird, können Sie Erweiterungen direkt vom Browser aus installieren.
...
Jetzt können Sie auf Ihrem Rechner oder im Netzwerk nach der gewünschten Erweiterung (einer Datei mit der Endung ".aslan-addon") suchen.
...
Da Erweiterungen uneingeschränkt auf Aslan zugreifen können, bedeutet dies theoretisch, dass die Erweiterungen auf Ihre persönlichen Daten (oder im Falle einer fest installierten Version) auf das gesamte System zugreifen können. Installieren Sie deshalb nur Erweiterungen aus vertrauenswürdigen Quellen.
Ist die Erweiterung dann installiert, muss Aslan neu gestartet werden, damit die Erweiterung ausgeführt wird.
Solange Sie den Neustart von Aslan nicht durchführen, wird Ihnen im Menü "Erweiterung verwalten" in der Spalte Status der Zustand "installieren" angezeigt.
...
Genau wie beim Installieren von Erweiterungen muss Aslan auch hier neu gestartet werden, damit die Erweiterung tatsächlich entfernt wird.
Solange Sie den Neustart von Aslan nicht durchführen, wird im Erweiterungs-Manager in der Spalte "Status" der Zustand "Entfernen" angezeigt.
...
Nach jedem Tastendruck des Aktivieren-/Deaktivieren-Schalters zeigt der Status der Erweiterung an, was nach dem Neustart von Aslan passiert.
...
Ähnlich wie beim Installieren oder Entfernen von Erweiterungen müssen Sie Aslan neu starten, damit die Änderungen wirksam werden.

Die Erweiterungsverwaltung enthält außerdem noch den Schalter "Schließen", um das Dialogfeld zu schließen.
Wenn Sie diesen Schalter zu einem Zeitpunkt drücken, zu dem Erweiterungen installiert oder entfernt werden müssen oder deren Status geändert werden muss, werden Sie gefragt, ob Aslan neu gestartet werden soll, damit die Änderungen wirksam werden.

Einige ältere Erweiterungen sind möglicherweise nicht mehr mit der Aslan-Version kompatibel, die Sie besitzen.
Wenn Sie eine ältere Aslan-Version verwenden, sind einige neue Erweiterungen möglicherweise ebenfalls nicht kompatibel.
...
Erweiterungen gelten als inkompatibel, wenn sie nicht laufend aktualisiert wurden, um mit den aktuellen wesentlichen Änderungen im Kern des Aslan zu entsprechen. Ein Grund, warum ein Entwickler auf aktualisierungen verzichten könnte, wäre gegeben, wenn man auf eine Funktion angewiesen ist, die in der aktuellen verwendeten Aslan-Version nicht verfügbar ist.
...
Wird die portable Version ausgeführt, erscheint an der gleichen Stelle im Menü "Extras" stattdessen der Eintrag "Aslan auf diesem Computer installieren".
...
* Kopieren der aktuellen Benutzerkonfiguration (inklusive der Dateien in "%appdata%\roaming\Aslan" oder in der Benutzerkonfiguration Ihrer portablen Version und Erweiterungen/Module).
* Starten Sie die neue portable Version nach der Erstellung oder starten Sie Aslan nach der Installation (Aslan startet automatisch nach der Erstellung der portablen Version oder der Installation).
...
* Aslan meldet beim Navigieren in Firefox, Thunderbird, etc. anstelle "Unbekannt".
* In Aslan funktioniert das Umschalten zwischen dem Fokus- und dem Lesemodus nicht mehr richtig.
* Aslan reagiert sehr langsam beim Navigieren im Lesemodus in Web-Browsern.
* Und möglicherweise andere Probleme.

### Plugins erneut laden {#ReloadPlugins}

Dieser Eintrag lädt alle Anwendungsmodule sowie globale Plugins neu, ohne dass Aslan extra neu gestartet wird.

## Unterstützte Sprachausgaben {#SupportedSpeechSynths}

Dieser Bereich beinhaltet Informationen über die in Aslan unterstützten Sprachausgaben.
Eine ausführliche Liste freier und kommerzieller Sprachausgaben, die zusammen mit Aslan verwendet werden können, finden Sie unter [https://github.com/nvaccess/aslan/wiki/ExtraVoices].

### eSpeak NG {#eSpeakNG}

Die in Aslan integrierte Sprachausgabe [eSpeak NG](https://github.com/espeak-ng/espeak-ng) benötigt keine speziellen Treiber oder andere extra installierten Komponenten.
Die Sprachausgabe eSpeak wird unter Windows 7, 8 und 8.1 von Aslan standardmäßig verwendet, während unter Windows 10 [Windows OneCore](#OneCore) standardmäßig verwendet wird.
Das hat den enormen Vorteil, dass Aslan auf einen Betriebssystemen mit einem USB-Stick ohne weiteres betrieben werden kann.
...
Aslan unterstützt noch die alte Schnittstelle, für all jene, die bereits SAPI 4 Sprachausgaben installiert haben.
Die Treiber hierfür werden von Microsoft schon lange nicht mehr zum Download angeboten.

Wenn diese Sprachausgabe mit Aslan verwendet wird, finden Sie die Stimmen aller SAPI 4 Engines in der Kategorie [Stimme und Sprachausgabe](#SpeechSettings) oder im [Sprachausgaben-Einstellungsring](#SynthSettingsRing).
...
Wenn Sie diese Sprachausgaben mit Aslan benutzen möchten, werden in der Kategorie [Stimme und Sprachausgabe](#SpeechSettings) oder dem [Einstellungsring der Sprachausgaben](#SynthSettingsRing) alle Stimmen von der SAPI5-Engine aufgelistet, die auf Ihrem System gefunden werden können.
...
Diese Stimmen können auch in Aslan eingebunden werden.
...
Unter Windows 10 verwendet Aslan standardmäßig Windows OneCore-Stimmen ([eSpeak-NG](#eSpeakNG) wird in anderen Versionen verwendet).
...
Dieser Bereich beinhaltet Informationen über Braillezeilen, die mit Aslan betrieben werden können.

### Braillezeilen mit automatischer Erkennung im Hintergrund {#AutomaticDetection}

Aslan ermöglicht, viele Braillezeilen im Hintergrund automatisch zu erkennen, entweder über USB oder Bluetooth.
...
Für gewöhnlich kann Aslan den verwendeten Anschluss automatisch erkennen, gleichgültig ob die Braillezeile via USB oder Bluetooth verbunden ist.
...
Dies kann sinnvoll sein, wenn Sie z. B. eine Focus-Braillezeile per Bluetooth mit Aslan verbinden wollen, aber dennoch den USB-Anschluss Ihres Rechners zum Laden der Zeile verwenden.
Die automatische Braillezeilenerkennung von Aslan erkennt auch die Anzeige auf USB oder Bluetooth.

Folgende Tastenkombinationen bei dieser Braillezeile sind für Aslan zugeordnet.
...
|Aslan-Menü anzeigen |Leertaste+Punkt1+Punkt3+Punkt4+Punkt5|
...
Sie können alternativ auch ältere Optelec-Braillezeilen wie beispielsweise das Braille Voyager mit Aslan verwenden. Dazu benötigen Sie den Protokoll-Converter von Optelec.
Sie benötigen keine installierten Treiber, um diese Zeile verwenden zu können.
Sie müssen die Zeile lediglich anschließen und in Aslan konfigurieren.

Hinweis! Die Bluetooth-Verbindung einer ALVA BC6-Braillezeile mit Hilfe des ALVA Bluetooth-Werkzeugs könnte unter Aslan fehlschlagen.
...
Folgende Tastenkombinationen für diese Braillezeilen sind in Aslan zugeordnet.
<!-- KC:beginInclude -->
...
|Aslan-Cursor zur ersten Zeile bewegen |T1+T2|
|Aslan-Cursor zur letzten Zeile bewegen |T4+T5|
...
Die meisten Braillezeilen der Firma [HandyTech GmbH](https://www.handytech.de/) werden von Aslan via seriellem Port, USB und Bluetooth unterstützt.
bei älteren Usb-Braillezeilen werden Sie den Universaltreiber von handy Tech installieren müssen.

Die folgenden Braillezeilenmodelle werden nicht ohne weiteres unterstützt; sie können Sie jedoch unter Zuhilfenahme des [universaltreibers](https://handytech.de/de/service/kundenservice/service-software/universeller-braillezeilentreiber) und der Aslan-Erweiterung verwenden:
...
Folgende Tastenkombinationen bei diesen Braillezeilen sind für Aslan zugeordnet.
...
Sie benötigen keine installierten Treiber, um diese Zeile verwenden zu können.
Sie müssen lediglich die Braillezeile anschließen und in Aslan konfigurieren.

Diese Braillezeile unterstützt nicht die automatische Braillezeilenerkennung im Hintergrund von Aslan.

Folgende Tastenzuweisungen sind in Aslan bei dieser Zeile verfügbar:
...
Folgende Tastenzuweisungen sind in Aslan bei diesen Zeilen verfügbar:
Bitte lesen Sie in der Dokumentation zu Ihrer Braillezeile nach, wo die entsprechenden Tasten zu finden sind.
<!-- KC:beginInclude -->
...
Die folgenden Tastenkombinationen für diese Braillezeile sind in Aslan verfügbar:
...
|Aslan-Menü |Lertaste+Punkt1+Punkt3+Punkt4+Punkt5 (Leerttaste+N)|
...
| Name |Taste|
|---|---|
|Aslan-Menü |C1+C3+C4+C5 (Befehl n)|
...
Aslan unterstützt die Braillezeilenmodelle Braille Sense, Braille EDGE, SyncBraille und Smart Beetle von [HIMS](https://www.hims-inc.com/), wahlweise mit USB- oder Bluetooth-Anbindung.
...
Folgende Tastenkombinationen sind in Aslan verfügbar:
Bitte sehen Sie in der Dokumentation Ihrer Braillezeile nach, wo die entsprechenden Tasten zu finden sind.
<!-- KC:beginInclude -->
...
|Aslan-Cursor zur vorherigen Zeile bewegen |Pfeil nach oben auf der rechten Seite|
|Aslan-Cursor zur nächsten Zeile bewegen |Pfeil nach unten auf der rechten Seite|
|Aslan-Cursor zum vorherigen Zeichen bewegen |Pfeil nach links auf der rechten Seite|
|Aslan-Cursor zum nächsten Zeichen bewegen |Pfeil nach rechts auf der rechten Seite|
...
Diese Braillezeilen unterstützen noch nicht die automatische Braillezeilenerkennung im Hintergrund von Aslan.
...
Diese Braillezeilen unterstützt nicht die automatische Braillezeilenerkennung im Hintergrund von Aslan.
...
Die Braillezeilen der C-Serie und einige weitere verfügen über eine doppelte Routingreihe, wobei die obere in Aslan verwendet wird, um Formatierungsinformationen über das aktuell gewählte Zeichen anzuzeigen.
...
Folgende Tastenkombinationen sind bei Papenmeier-Braillezeilen in Kombination mit Aslan verfügbar:
<!-- KC:beginInclude -->
...
Aus diesem Grund unterstützen diese Displays nicht die automatische Braillezeilenerkennung im Hintergrund von Aslan.
...
Die folgenden Befehle sind in Aslan verfügbar:
...
|Aktuelles Zeichen am Aslan-Cursor ausgeben |L1|
...
Aslan unterstützt die BrailleNote-Modelle von [Humanware](https://www.humanware.com) sofern diese als Braillezeilen betrieben werden.
...
Zudem müssen Sie den verwendeten Anschluss in den Braille-Einstellungen von Aslan festlegen.
...
Bei der Braillenote Apex BT können Sie das Scrollrad für die Ausführung von Aslan-Befehlen verwenden. Es befindet sich zwischen den Punkten 1 und 4.
...
Folgende BrailleNote-Befehle können Sie in Aslan verwenden:
...
|Cursor zum Braille-Modul bewegen |Routing-Taste|
|Aslan-Menü |Leertaste+Punkt1+Punkt3+Punkt4+Punkt5 (Leertaste+N)|
...
Aslan unterstützt die Ecobraille von [ONCE](https://www.once.es/).
Die folgenden Modelle werden unterstützt:
...
In dem [Dialogfeld zum Auswählen der Braillezeile](#SelectBrailleDisplay) in den Aslan-Einstellungen können Sie den seriellen Anschluss festlegen, mit dem die Braillezeile verbunden ist.
Diese Braillezeilen unterstützen nicht die automatische Erkennung der Braillezeilen im Hintergrund von Aslan.
...
|Position des Aslan-Cursors angeben |F0|
...
Aslan unterstützt die Esys-, Esytime- und Iris-Braillezeilen von [Eurobraille](https://www.eurobraille.fr/).
Esys und Esytime-Evo-Geräte können mit Aslan via USB oder Bluetooth verbunden werden.
...
Deshalb müssen Sie für diese Braillezeile gesondert die Schnittstelle auswählen, nachdem der Treiber in den Braille-Einstellungen von Aslan aktiviert wurde.
...
Folgende Tastenkombinationen wurden für diese Braillezeilen in Aslan zugewiesen.
...
|Aslan-Cursor zur vorherigen Zeile ziehen |Joystick1oben|
|Aslan-Cursor zur nächsten Zeile ziehen |Joystick1unten|
|Aslan-Cursor zum vorherigen Zeichen ziehen |Joystick1links|
|Aslan-Cursor zum nächsten Zeichen ziehen |Joystick1rechts|
...
Die Einstellung der Braille-Eingabetabelle innerhalb von Aslan ist daher wirkungslos.

BRLTTY ist nicht an der automatischen Erkennung von Braillezeilen im Hintergrund beteiligt.

Folgende BRLTTY-Befehle wurden in Aslan zugewiesen.
...
Mit Hilfe von Befehlszeilenoptionen können Sie das Verhalten von Aslan beeinflussen.
...
Kommandozeilenoptionen müssen vom Namen der ausführbaren Datei von Aslan sovie voneinander durch Leerzeichen getrennt werden.
Eine nützliche Option ist beispielsweise "-disable-addons", die Aslan anweist, alle laufenden Erweiterungen auszusetzen.
So können Sie beispielsweise feststellen, ob ein Problem durch eine bestimmte Erweiterung verursacht wird.

Mit dem folgenden Befehl können Sie über das Dialogfeld "Ausführen" eine laufende Aslan-Instanz beenden:
...
|aslan -mc <Konfigurationspfad> |Dadurch wird Aslan mit Starttönen und deaktivierter Meldung sowie der angegebenen Konfiguration gestartet.|
|aslan -mc <Konfigurationspfad> --disable-addons |Wie zuvor, jedoch mit deaktivierten Erweiterungen.|
...
|aslan -l 10 |Startet Aslan mit der Protokollierungsstufe "Fehlerkorrektur".|
|aslan --log-file=C:\aslan.log |Speichert das Protokoll in der Datei "C:\aslan.log".|
|aslan --log-level=20 -f C:\aslan.log |Legt die Protokollierungsstufe "Informationen" fest und speichert das Protokoll in der Datei C:\aslan.log.|

Folgende Kommandozeilenoptionen gibt es:

| Kurzform |Langform |Beschreibung|
|---|---|---|
|-h |--help |Zeigt eine Hilfe zur Befehlszeile an und beendet Aslan.|
|-q |--quit |Beendet eine laufende Instanz von Aslan.|
|-k |--check-running |Prüft, ob Aslan bereits ausgeführt wird. Falls ja, wird ein Fehlercode 0 zurückgegeben, anderenfalls wird 1 zurückgegeben.|
...
|-c Konfigurationspfad |--config-path=KonfigurationsPfad |Gibt den Ordner an, in dem alle Aslan-Einstellungen gespeichert werden sollen.|
|-m |--minimal |Startet Aslan minimalistisch (keine Klänge, keine Benutzeroberfläche).|
|-s |--secure |Sicherer Modus: Deaktiviert die Python-Konsole, Profil-Funktionen wie Erstellen, Löschen, Umbenennen von Profilen, etc., Update-Check, einige Kontrollkästchen im Willkommensdialog und in der Kategorie "Allgemein" (z. B. Aslan nach der Anmeldung starten, Konfiguration nach dem Beenden speichern, etc.) sowie im Protokoll-Betrachter- und Funktionen zur Protokollierung (werden oft in Sicherheitsmeldungen verwendet). Beachten Sie auch, dass dieser Befehl die Möglichkeit zum Speichern von Einstellungen in der Systemkonfiguration deaktiviert und die Tastenbefehle nicht gespeichert werden.|
...
|Keine |~~no-logging |Deaktivieren Sie die Protokollierung vollständig, während Sie Aslan verwenden. Diese Einstellung kann überschrieben werden, wenn eine Protokollebene ("~~loglevel"", -l) von der Kommandozeile aus angegeben wird oder wenn die Debug-Protokollierung eingeschaltet ist.|
|Keine |--no-sr-flag |Keine Mitteilung in Windows über einen installierten Screenreader.|
|Keine |--install |Installiert Aslan ohne Ausgabe von Rückmeldungen und startet die neu installierte Version.|
|Keine |--install-silent |Installiert Aslan ohne Ausgabe von Rückmeldungen, startet die Version aber nicht.|
|Keine |--enable-start-on-logon=True/False |Aktivieren Sie bei der Installation [Aslan bei der Windows-Anmeldung starten](#StartAtWindowsLogon).|
|Keine |--create-portable |Erstellt und startet eine neue portable Aslan-Version. Hierfür müssen Sie außerdem den Parameter --portable-path angeben.|
|Keine |--create-portable-silent |Erstellt eine neue portable Aslan-Version, ohne diese zu starten. Hierfür müssen Sie außerdem den Parameter --portable-path angeben.|
...
Sie können einige Werte in der Registrierungsdatenbank von Windows verwenden, um das Verhalten von Aslan zu beeinflussen.
Diese Werte werden unter einem der folgenden Schlüssel gespeichert:

* Für 32-Bit-Systeme: "HKEY_LOCAL_MACHINE\SOFTWARE\Aslan"
* Für 64-Bit-Systeme: "HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Aslan"
...
Wenn Sie weitere Informationen oder Hilfe bezüglich Aslan benötigen, besuchen Sie bitte die Aslan-Homepage unter Aslan_URL.
Neben Ressourcen der Community und dem technischen Support finden Sie auch zusätzliche Dokumentationen.
Auf diesen Seiten werden Informationen und Ressourcen zur Aslan-Entwicklung ebenfalls bereitgestellt.
