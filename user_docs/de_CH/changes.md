...
* Sie können nun die Ansage von Grafiken aus den Dokumentformatierungseinstellungen von Aslan ausschalten. Beachten Sie, dass die Deaktivierung dieser Option weiterhin die alternativen Texte von Grafiken vorliest. (#4837)
* Sie können jetzt die Touchscreen-Unterstützung von Aslan umschalten. In den Einstellungen von Aslan wurde eine Option für Berührungsinteraktion auf Touchscreens hinzugefügt. Die Standard-Tastenkombination ist Aslan+Strg+Alt+T. (#9682)
* Neue deutsche Braille-Tabellen hinzugefügt. (#11268)
* Aslan erkennt jetzt schreibgeschützte Text-UIA-Steuerelemente. (#10494)
 ...
 * Dies kann durch die Option in den Dokument-Formatierung von Aslan für die Hervorhebung ein- und ausgeschaltet werden.
* Tasten für die emulierte Systemtastatur können über das Dialogfeld für die Tastenbefehle von Aslan hinzugefügt werden. (#6060)
...
* Das Skript für die Berichtsformatierung (Aslan+F) wurde jetzt so geändert, dass die Formatierung unter dem System-Cursor und nicht an der Cursor-Position des Aslan-Cursors ausgegeben wird. Um die Formatierung an der Cursor-Position zu erhalten, betätigen Sie Aslan+Umschalt+F. (#9505)
* Aslan setzt den System-Fokus im Lesemodus nicht mehr automatisch standardmäßig auf fokussierbare Elemente, wodurch Leistung und Stabilität verbessert werden. (#11190)
...
* Wenn Sie eine Erweiterung in Aslan entfernen und Sie aufgefordert werden, dies zu bestätigen, ist der Schalter "Nein" jetzt standardmäßig ausgewählt. (#10015)
...
* Aslan teilt nun die korrekte Terminologie für Anmerkungen in Microsoft Excel mit. (#11311)
* Wenn Sie den Befehl "Aslan-Cursor zum Fokus ziehen" im Lesemodus verwenden, wird der Aslan-Cursor jetzt auf die Position des virtuellen Cursors gesetzt. (#9622)
* Im Lesemodus angezeigte Informationen, wie z. B. die Informationen zur Textformatierung mit Aslan+F, werden nun in einem etwas größeren Fenster zentriert auf dem Bildschirm angezeigt. (#9910)

### Fehlerkorrekturen

* Aslan spricht nun immer, wenn man nach Wörtern navigiert und auf einem einzelnen Symbol, gefolgt von einem Leerzeichen, landet, unabhängig von den Ausführlichkeitseinstellungen. (#5133)
* Bei Anwendungen, die QT 5.11 oder neuer verwenden, werden wieder Objekt-Beschreibungen mitgeteilt. (#8604)
* Wenn ein Wort mit Strg+Entf gelöscht wird, schweigt Aslan nicht mehr. (#3298, #11029)
...
* Aslan sagt das Dialogfeld "Öffnen mit" in Windows 10 Mai 2020 Update an. (#11335)
...
* Mit dem Windows 10 Mai 2020 Update zeigt Aslan jetzt den Microsoft Sound Mapper an, wenn die Ausgabegeräte im Dialogfeld für die Sprachausgabe eingestellt wird. (#11349)
...
* In Google Chrom meldet Aslan jetzt für alle aktivierbaren Kontrollen (nicht nur für Kontrollkästchen), die derzeit nicht aktiviert sind, dass sie nicht aktiviert sind. (#11377)
* Es ist wieder möglich, in verschiedenen Bedienelementen zu navigieren, wenn  Aragonisch als Sprache in Aslan verwendet wird. (#11384)
* Aslan sollte teilweise nicht mehr verstummen in Microsoft Word, wenn man schnell die Tasten Pfeil nach oben und/oder Pfeil nach unten drückt oder Zeichen über die Braille-Eingabe eintippt. (#11431, #11425, #11414)
* Aslan hängt keine Leerzeichen beim Kopieren des aktuellen Navigator-Objekts in die Zwischenablage mehr am Ende an. (#11438)
* Wenn kein Text vorhanden ist, führt Aslan auch die "Alles Lesen"-Funktion nicht mehr aus. (#10899, #9947)
* Aslan liest nun die Features-Liste im Internet-Informations-Service (IIS) vor. (#11468)
* Aslan hält das Audiogerät nun offen und verbessert die Leistung einiger Soundkarten (#5172, #10721)
* Aslan friert nicht mehr ein oder wird nicht mehr beendet, wenn in Microsoft Word die Tastenkombination Strg+Umschalttaste+Pfeil nach unten gedrückt wird. (#9463)
* In der Navigationsbaumansicht auf drive.google.com wird der Zustand, ob ein Verzeichnis eingeklappt oder ausgeklappt ist, immer von Aslan angesagt. (#11520)
* Aslan erkennt nun die Braillezeile NLS eReader von Humanware über Bluetooth automatisch, da der Bluethooth-Name nun "NLS eReader Humanware" lautet. (#11561)
...
* Ein Protokoll-Abschnitt kann nun markiert werden und dann mit Aslan+Strg+Umschalt+F1 in die Zwischenablage kopiert werden. (#9280)
* Spezifische Objekte in Aslan, die vom zyklischen Garbage-Collector von Python gefunden werden, werden jetzt beim Löschen durch den Collector protokolliert, um das Entfernen von Referenzzyklen aus Aslan zu unterstützen. (#11499)
 * Die Mehrzahl der Aslan-Klassen wird verfolgt, darunter AslanObjects, appModules, GlobalPlugins, SynthDrivers und TreeInterceptors.
 * Eine Klasse, die verfolgt werden muss, sollte von "garbageHandler.TrackedObject" erben.
* Signifikante Debug-Protokollierung für MSAA-Ereignisse kann nun in den erweiterten Einstellungen von Aslan aktiviert werden. (#11521)
...
* Ein Skript zum Öffnen des Aslan-Benutzerverzeichnis wurde hinzugefügt (ist keinem Tastenbefehl zugeordnet). (#2214)
...
* Auf Grund von Änderungen im VS-Code deaktiviert Aslan den Lesemodus im Code standardmäßig nicht mehr. (#10888)
* Aslan sagt nicht mehr "Nach oben" und "Nach unten" an, wenn der Aslan-Cursor direkt auf das erste oder letzte Zeichen der Zeile für das aktuelle Navigator-Objekt mit der Bewegung zum Anfang der Zeile bzw. zum Ende der Zeile bewegt wird. (#9551)
* Aslan sagt nicht mehr "Nach links" und "Nach rechts" an, wenn der Aslan-Cursor direkt auf das erste oder letzte Zeichen der Zeile für das aktuelle Navigator-Objekt mit der Bewegung zum Anfang der Zeile bzw. zum Ende der Zeile bewegt wird. (#9551)

### Fehlerbehebungen

* Aslan startet nun korrekt, wenn keine Protokolldatei angelegt werden konnte. (#6330)
* In neueren Versionen von Microsoft Word 365 meldet Aslan nicht mehr, dass "Rückwort löschen" beim Drücken von Strg+Rücktaste während der Bearbeitung eines Dokuments gelöscht wird. (#10851)
* In Winamp meldet Aslan wieder die Umschaltung von der Zufallswiedergabe und der Wiederholen. (#10945)
* Korrektur der Mausverfolgung für einige MSHTML-Elemente in Internet Explorer. (#10736)
* Aslan reagiert nicht mehr extrem langsam beim navigieren in den Listeneinträgen in 1Password. (#10508)
* Die Windows OneCore-Sprachausgabe verzögert nicht mehr bei den Ansagen. (#10721)
* Aslan hängt sich nicht mehr auf, sobald Sie das Kontextmenü für 1Passwort aus dem Infobereich heraus öffnen. (#11017)
...
* Es sind keine Aslan-Fehlertöne mehr zu hören, wenn auf Textsteuerelemente von DevExpress zugegriffen wird. (#10918)
...
* Im Lesemodus mit deaktiviertem "Automatisch den System-Fokus auf fokussierbare Elemente setzen" fokussiert der Wechsel in den Fokusmodus mit Aslan+Leertaste nun das Element unter dem Cursor. (#11206)
* Auf bestimmten Systemen kann wieder nach Aslan-Updates gesucht werden, z. B. nach einer sauberen Windows-Installationen. (#11253)
...
* Automatisiertes Testen von Aslan mit Google Chrome und einem HTML-Beispiel ist nun möglich. (#10553)
...
* Aslan reagiert nicht mehr auf "EVENT_OBJECT_REORDER" von "IAccessible". (#11076)
* Ein defektes ScriptableObject (z. B. ein GlobalPlugin, bei dem ein Aufruf der "init"-Methode dessen Basisklasse fehlt) unterbricht nicht mehr die Skriptbehandlung von Aslan. (#5446)
...
* Das aktuell ausgewählte Element in Ausklapplisten wird im Lesemodus in Google Chrome wieder angezeigt, ähnlich wie in Aslan 2019.1. (#10713)
...
* Aslan verhindert nun, dass das System sich abschaltet oder in den Schlafmodus wechselt, während alles lesen aktiviert ist. (#10643)
...
* Aslan kann wieder mathematische Gleichungen in Microsoft Word lesen und mit ihnen interagieren. (#10803)
* Aslan sagt wieder an, dass der Text im Lesemodus nicht ausgewählt ist, wenn nach dem Auswählen des Textes eine Pfeiltaste gedrückt wird. (#10731).
* Aslan beendet sich nicht mehr selbst, wenn ein Fehler bei der Initialisierung von eSpeak auftritt. (#10607)
...
* Visual Studio 2019 (16.2 oder neuer) wird zum Kompilieren von Aslan-Abhängigkeiten benötigt. (#10169)
...
Aslan 2019.3 ist ein umfangreiches Release, das viele Änderungen unter der Haube mit sich bringt. Darunter das Upgrade von Python 2 auf Python 3 und ein komplett neu überarbeitetes Subsystem für die Sprachausgaben in Aslan.
Obwohl diese Änderungen die Kompatibilität mit älteren Aslan-Erweiterungen nicht mehr besteht, ist das Upgrade auf Python 3 aus Sicherheitsgründen notwendig. Weitere Änderungen an der Sprachausgabe folgen in naher Zukunft.
...
* Es wurde ein Befehl hinzugefügt, um den Aslan-Cursor an die zuvor als Startmarke für die Auswahl oder das Kopieren markierte Position zu bewegen: Aslan+Umschalt+F9. (#1969)
 ...
 * Ein Skript zum Aktivieren des Bildschirmvorhangs wurde hinzugefügt, jedoch wurde kein Tastenbefehl zugewiesen. Bei einmal Drücken wird der Bildschirmvorhang bis zum nächsten Neustart aktiviert. Bei zweimal Drücken wird er immer ausgeführt, sofern Aslan verwendet wird.
 * Dies kann über die Kategorie "Visuelle Darstellungen" im Einstellungsdialog von Aslan aktiviert und konfiguriert werden.
* Aslan wurde um eine Funktion zur visuellen Hervorhebung des Fokus erweitert. (#971, #9064)
 * Die Hervorhebung von System Fokus, Navigator-Objekt und virtuellen Cursor im Lesemodus kann über die Kategorie "visuelle Darstellungen" im Einstellungsdialog von Aslan aktiviert und konfiguriert werden.
...
* Im Benutzerhandbuch wird nun beschrieben, wie Sie Aslan in der Windows-Konsole verwenden können. (#9957)
* Wenn Sie die Datei "aslan.exe" ausführen, wird nun standardmäßig ein bereits laufender Prozess von Aslan ersetzt. Der Befehlszeilenparameter "-r" oder "---replace" wird weiterhin akzeptiert, jedoch ignoriert. (#8320)
* Unter Windows 8 und neuer wird Aslan nun Produktnamen und Versionsinformationen für gehostete Anwendungen, wie beispielsweise Anwendungen, die aus dem Microsoft Store heruntergeladen wurden, unter Verwendung der von der Anwendung bereitgestellten Informationen mitteilen. (#4259, #10108)
* Beim Ein- und Ausschalten der Änderungsverfolgung mit der Tastatur in Microsoft Word meldet Aslan den Status der Einstellung. (#942) 
* Die Aslan-Versionsnummer wird nun als erste Meldung im Protokoll angezeigt. Dies geschieht auch dann, wenn die Protokollierung über die Einstellungen deaktiviert wurde. (#9803)
...
* In Microsoft Word meldet Aslan nun den Anzeigestatus von nicht druckbaren Zeichen, wenn Sie die Umschalt-Taste Strg+Umschalt+8 drücken. (#10241)
...
* Python-Pakete von Drittanbietern, die in Aslan enthalten sind, wie beispielsweise comtypes, protokollieren nun ihre Warnungen und Fehler im Aslan-Protokoll. (#10393)
...
* Die Java Access Bridge ist nun im Aslan enthalten, um den Zugriff auf Java-Anwendungen zu ermöglichen, auch für 64-Bit-Java-VMs. (#7724)
* Wenn die Java Access Bridge für den Benutzer nicht aktiviert ist, aktiviert Aslan sie automatisch beim Start von Aslan. (#7952)
...
* Unter Windows 10 meldet Aslan Sprechblasen aus universellen Anwendungen, wenn im Dialogfeld "Objektdarstellung" Aslan konfiguriert ist, um Benachrichtigungen mitzuteilen. (#8118)
...
* Aslan behandelt Leerzeichen nicht mehr als Wörter in der Objektprüfung in Scintilla-basierten Editoren wie Notepad++. (#8295)
* Aslan verhindert, dass das System in den Ruhezustand wechselt, wenn mit den Gesten auf der Braillezeile durch Text navigiert wird. (#9175)
...
* Aslan meldet wieder Vorschläge in der Adressleiste von Microsoft Edge. (#7554)
* Aslan ist nicht mehr stumm, wenn eine HTML-Registerkartenüberschrift im Internet Explorer fokussiert wird. (#8898)
* Im EdgeHTML basierten Microsoft Edge gibt Aslan keinen Sound für Suchvorschläge mehr aus, wenn das Fenster maximiert ist. (#9110, #10002)
* Kombinationsfelder mit ARIA 1.1 werden nun in Mozilla Firefox und Google Chrome unterstützt. (#9616)
* Aslan meldet nicht mehr den Inhalt von visuell ausgeblendeten Spalten für Listenelemente in SysListView32-Steuerelementen. (#8268)
* Der Einstellungsdialog zeigt im geschützten Modus nicht mehr "Information" als aktuelle Protokollierungsstufe an. (#10209)
* Im Startmenü für Windows 10 Anniversary Update und neuer gibt nun Aslan Details der Suchergebnisse bekannt. (#10340)
* Im Lesemodus, wenn sich das Dokument durch Bewegen des Cursors oder durch Schnellnavigation ändert, spricht Aslan in einigen Fällen nicht mehr den falschen Inhalt. (#8831, #10343)
* Die Aussprache einiger Aufzählungszeichen in Microsoft Word wurde korrigiert. (#10399)
* In Windows 10 Mai 2019 Update und neuer meldet Aslan wieder das erste ausgewählte Emoji oder das Element der Zwischenablage mitteilen, wenn das Dialogfeld für die Emojis oder der Verlauf der Zwischenablage geöffnet ist. (#9204)
* In Poedit ist es wieder möglich, einige Übersetzungen für Sprachen von rechts nach links anzuzeigen. (#9931)
* In den Einstellungen in Windows 10 April 2018 Update und neuer gibt nun Aslan keine Informationen zum Fortschrittsbalken mehr für die Lautstärke auf der Seite für System/Sound bekannt. (#10412)
* Ungültige reguläre Ausdrücke in Sprachwörterbüchern unterbrechen die Sprache in Aslan nicht mehr vollständig. (#10334)
...
* Java-Anwendungen, die vor Aslan gestartet wurden, sind nun zugänglich, ohne dass die Java-App neu gestartet werden muss. (#10296)
...
* Aslan behandelt nun bestimmte zusammengesetzte Unicode-Zeichen, wie z. B. das Zeichen é, als ein einziges Zeichen, wenn Sie sich durch Text bewegen. (#10550)
...
* Wenn Sie das Audio-Ausgabegerät wechseln, werden die von Aslan ausgegebenen Töne nun durch das aktuell ausgewählte Gerät wiedergegeben. (#2167)
...
* Da Aslan nicht mehr von pyWin32 abhängig ist, sind Module wie win32api und win32con für Erweiterungen nicht mehr verfügbar. (#9639)
...
* Ein Framework zur Erstellung von Quellen für Verbesserungen visueller Darstellungen wurde hinzugefügt; Module, die Bildschirminhalte ändern können, optional basierend auf Eingaben von Aslan über Objektpositionen. (#9064)
 ...
 * Die Quelle für Verbesserungen visueller Darstellungen ist über die Kategorie "Visuelle Darstellungen" im Aslan-Einstellungsdialog aktivierbar.
* Abstrakte Klassen-Eigenschaften werden nun auch für Objekte unterstützt, die von "baseObject.AutoPropertyObject" erben (z. B. "AslanObjects" und "TextInfos"). (#10102)
...
* Für jeden Eintrag im Aslan-Protokoll werden nun Informationen über den Ursprungs-Thread angezeigt. (#10259)
* TextInfo-Objekte der UIA können nun verschoben / erweitert werden von den Text-Einheiten "page", "story" und "formatField". (#10396)
* Externe Module (appModules und globalPlugins) sind nun weniger wahrscheinlich in der Lage, die Erstellung von Aslan-Objekten zu unterbrechen. 
 * Ausnahmen, die durch die Methoden "chooseAslanObjectOverlayClasses" und "event_AslanObject_init" verursacht werden, werden nun korrekt abgefangen und protokolliert.
...
* In Windows 7 führt Aslan nicht mehr zum Absturz von Windows Explorer, wenn die Maus im Startmenü verwendet wird. (#9435)
* Unter Windows 7 stürzt Windows Explorer beim Zugriff auf Metadaten-Eingabefelder nicht mehr ab. (#5337)
* Aslan friert bei der Interaktion mit Bildern mit einem base64-URI in Mozilla Firefox oder Google Chrome nicht mehr ein. (#10227)
...
* Aslan unterstützt nun auch neuere Client-Versionen des Miranda NG. (#9053)
* Sie können den Lesemodus nun standardmäßig deaktivieren, wenn eine Webseite geladen wird. Deaktivieren Sie dazu die neue Option "Lesemodus beim Laden von Webseiten verwenden" in den Lesemodus-Einstellungen im Einstellunfsfenster von Aslan. (#8716)
 * Beachten Sie, dass Sie den Lesemodus mit Aslan+Leertaste manuell aktivieren können, auch wenn diese Option deaktiviert ist.
 ...
 * Zusätzlich können vorhandene JavaDoc-Informationen aus dem Editor über den Tastenbefehl Aslan+d gelesen werden.
...
* Einstellungen für Braillezeilen-Treiber können nun direkt in der Benutzeroberfläche von Aslan in der Kategorie Braille-Einstellungen im Einstellungsdialog angepasst werden. Voraussetzung ist, dass der Treiber der Braillezeile dies unterstützt. (#7452)
...
* Dem Bereich Erweiterte Einstellungen wurde eine experimentelle Option hinzugefügt, mit der Sie ein neues, noch nicht abgeschlossenes Refactoring der Unterstützung der Windows-Konsolen von Aslan über die API der UIA von Microsoft ausprobieren können. (#9614)
...
* Der Text im Dialogfeld "Erweiterungen verwalten..." wurde angepasst, wenn Aslan mit deaktivierten Erweiterungen neu gestartet wird. (#9473)
...
* Wenn eine automatisch erkannte Braillezeile über Bluetooth angeschlossen ist, sucht Aslan weiterhin nach Braillezeilen via USB, die vom gleichen Treiber unterstützt werden und wechselt zu einer USB-Verbindung, sobald diese verfügbar ist. (#8853)
* eSpeak-NG auf Commit 67324cc aktualisiert.
* Liblouis Braille-Übersetzer auf Version 3.10.0 aktualisiert. (#9439, #9678)
* Aslan spricht nun das Wort "ausgewählt", nachdem es den Text gemeldet hat, den ein Benutzer gerade ausgewählt hat. (#9028, #9909)
* In Microsoft Visual Studio Code ist der Lesemodus nun standardmäßig deaktiviert. (#9828)

### Fehlerbehebungen

* Aslan stürzt nicht mehr ab, wenn ein Erweiterungsverzeichnis leer ist. (#7686)
...
* Aslan stürzt nicht mehr ab, wenn die Windows 10 Mail-App beendet wird. (#9341)
* Aslan startet nun auch wenn die regionalen Einstellungen des Benutzers auf ein für Aslan unbekanntes Gebietsschema wie Englisch (Niederlande) eingestellt sind. (#8726)
...
* Aslan meldet nun korrekt die Zeile am Mauszeiger in Notepad++ und anderen Scintilla-basierten Editoren. (#5450)
...
* Im Windows 10 Mai 2019 Update, wenn das Explorer oder Desktop-Fenster fokusiert ist, sagt Aslan nicht mehr viele Lautstärkebenachrichtigungen an, wenn Sie die Lautstärke mit Lautstärketasten ändern. (#9466)
...
* Aslan liest nun die richtige Zeile, wenn der Wortumbruch in Scintilla-Steuerelementen wie Notepad++ aktiviert ist. (#9424)
...
* Die in Aslan eingestellte Rate für Geschwindigkeit oder Stimmhöhe bei Windows OneCore-Stimmen wird nicht mehr von der in Windows 10 Sprach-Einstellungen eingestellten Rate beeinflusst. (#7498)
* Der Protokollbetrachter kann nun mit Aslan+F1 geöffnet werden, auch wenn es keine Entwicklerinformationen für das aktuelle Navigatorobjekt gibt. (#8613)
* Es ist wieder möglich, die Tabellen-Navigationsbefehle von Aslan in Google Docs unter Mozilla Firefox und Google Chrome zu verwenden. (#9494)
* Die Kipptasten funktionieren nun korrekt bei den Braillezeilen von Freedom Scientific. (#8849)
* Beim Lesen des ersten Zeichens eines Dokuments in Notepad++ 7.7 x64 friert Aslan nicht mehr ein. (#9609)
...
* Der Aslan-spezifische Suchmodus funktioniert nun wieder, auch wenn das Dialogfeld für die Aslan-Informationen im Hintergrund geöffnet ist. (#8566)
* HTCom kann nun mit einer Braillezeile von HandyTech in Kombination mit Aslan benutzt werden. (#9691)
...
* Ein Beispiel zur Demonstration der Verwendung der Datei "aslanControllerClient.dll" aus C# hinzugefügt. (#9600)
* Dem Modul "winVersion" wurde eine neue Funktion "isWin10" hinzugefügt, die zurückgibt, ob Aslan auf (mindestens) der mitgelieferten Release-Version von Windows 10 (wie 1903) läuft oder nicht. (#9761)
* Die Python-Konsole in Aslan enthält nun weitere nützliche Module im Namensraum (z. B. "appModules", "globalPlugins", "config" und "textInfos"). (#9789)
* Das Ergebnis des zuletzt ausgeführten Befehls in der Python-Konsole in Aslan ist nun über die Variable "_" zugänglich. (#9782)
...
* Es kommt in Microsoft Excel 2007 nicht mehr zu Abstürzen, wenn Aslan ansagen soll, ob eine Zelle eine Formel enthält. (#9431)
...
* Bei der Angabe der Zellkoordinaten für verbundene Zellen in Excel verwendet Aslan nun wieder die übersetzten Meldungen. (#9471)
...
Bitte beachten Sie, dass ab dieser Aslan-Version benutzerdefinierte Anwendungsmodule, GlobalPlugins, Treiber für Braillezeilen und Sprachausgaben nicht mehr automatisch aus Ihrem Aslan-Benutzerkonfigurationsverzeichnis geladen werden.
Vielmehr sollten diese als Teil einer Aslan-Erweiterung installiert werden. Für diejenigen, die Code für eine Erweiterung entwickeln, kann Code zum Testen in ein neues Developer Scratchpad-Verzeichnis im Aslan-Benutzerkonfigurationsverzeichnis verschoben werden. Dies funktioniert nur, wenn die Option Developer-Scratchpad in der neuen Aslan-Einstellungskathegorie "erweitert" aktiviert ist.
Diese Änderungen sind notwendig, um die Kompatibilität von benutzerdefiniertem Code besser zu gewährleisten, sodass Aslan stabil bleibt, wenn dieser Code mit neueren Versionen inkompatibel wird.
...
* Den Maus-Einstellungen von Aslan wurde eine Option hinzugefügt, womit Aslan nun auch die Maus erkennen kann, selbst wenn diese von einer anderen Anwendung gesteuert wird. (#8452)
 * Dies ermöglicht die Mausverfolgung in Aslan, wenn ein System durch Teamviewer oder ähnliche Software ferngesteuert wird.
* Der Befehlszeilenparameter "--enable-start-on-logon" wurde hinzugefügt, um stille Installationen von Aslan zu ermöglichen, bei welchen Aslan danach standardmäßig nicht auf dem Anmeldebildschirm ausgeführt wird. (#8574)
* Es ist nun möglich die Protokollierungsfunktionen von Aslan komplett auszuschalten, indem Sie im Bereich Allgemeine Einstellungen die Protokollierungsstufe auf "deaktiviert" setzen. (#8516)
...
* Eine neue Kategorie "Erweiterte Einstellungen" wurde dem Aslan-Einstellungsdialog hinzugefügt, welche u. a. eine Option enthält, um die neue Unterstützung für Microsoft Word durch Aslan über die API-Schnittstelle der UIA von Microsoft testen zu können. (#9200)
...
* Entwickler können nun eine Aslan-Version angeben, die für eine Erweiterung mindestens erforderlich ist. Aslan wird eine Erweiterung nicht laden, wenn die angegebene Minimalversion neuer ist als die aktuell verwendete Aslan-Version. (#6275)
* Autoren von Erweiterungen können nun die letzte Aslan-Version angeben, mit der die Erweiterung getestet wurde. Wenn eine Erweiterung nur mit einer Aslan-Version getestet wurde, die niedriger ist als die aktuelle Version, wird Aslan die Installation oder das Laden der entsprechenden Erweiterungen ablehnen. (#6275)
* Diese Aslan-Version ermöglicht die Installation und das Laden von Erweiterungen, die noch keine Informationen zur minimalen und letzten getesteten Aslan-Version enthalten, aber ein Upgrade auf zukünftige Aslan-Versionen (z. B. 2019.2) kann automatisch dazu führen, dass diese älteren Erweiterungen deaktiviert werden.
* Der Befehl zum Bewegen der Maus zum Navigator-Objekt (Aslan-Cursor) funktioniert nun sowohl in Microsoft Word als auch für UIA-Steuerungen, insbesondere in Microsoft Edge. (#7916, #8371)
* Das Melden des Textes unter dem Maus-Cursor wurde in Microsoft Edge und anderen UIA-Anwendungen verbessert. (#8370)
* Wenn Aslan mit dem Befehlszeilenparameter "--portable-path" gestartet wird, wird der angegebene Pfad beim Erstellen einer portablen Version aus dem Menü automatisch ausgefüllt. (#8623)
...
* Aslan lädt nicht mehr automatisch benutzerdefinierte Anwendungsmodulle, globalPlugins sowie Braille- und Sprachausgabentreiber aus dem Aslan-Benutzer-Konfigurationsverzeichnis. Dieser Code sollte stattdessen als Erweiterung mit korrekten Versionsinformationen verpackt werden, um sicherzustellen, dass inkompatibler Code nicht mit aktuellen Aslan-Versionen ausgeführt wird. (#9238)
 * Für Entwickler, die Code während der Entwicklung testen müssen, aktivieren Sie das Developer Scratchpad-Verzeichnis von Aslan in der Kategorie Erweiterte Einstellungen in den Aslan-Einstellungen und legen Sie Ihren Code in das Verzeichnis 'scratchpad' im Aslan-Benutzerkonfigurationsverzeichnis ab, nachdem diese Option aktiviert wurde.
...
* Der Bestätigungsdialog für den Neustart nach dem Ändern der Oberflächensprache von Aslan wurde verbessert. Der Text und die Beschriftungen der Schaltflächen sind nun prägnanter und weniger verwirrend. (#6416)
* Wenn ein Sprachsynthesizer von Drittanbietern nicht geladen werden kann, greift Aslan unter Windows 10 auf den Windows OneCore-Sprachsynthesizer zurück, anstatt eSpeak. (#9025)
* Der Eintrag für den Willkommensdialog im Aslan-Menü wird in geschützten Bildschirmen nicht mehr angezeigt. (#8520)
...
* Aslan wird nun Auswahländerungen für bestimmte Zeitpicker bekannt geben, z. B. in der App Wecker- und Uhren unter Windows 10. (#5231)
* Im Aktionscenter von Windows 10 sagt Aslan Statusmeldungen an, wenn Sie zwischen schnellen Aktionen wie Helligkeit und Fokusunterstützung umschalten. (#8954)
* Im Aktionscenter in Windows 10 Oktober 2018 Update und früher erkennt Aslan das Steuerelement für die Helligkeit als Schaltfläche anstelle einer Umschalttaste. (#8845)
* Aslan verfolgt nun den Cursor wieder und meldet wieder gelöschte Zeichen in den Dialogfeldern "Suchen, "Gehe zu" und anderen Dialogfeldern in Microsoft Excel. (#9042)
* Ein seltener Absturz im Lesemodus in Firefox wurde behoben. (#9152)
* Aslan meldet nun wieder den Fokus für einige Steuerelemente im Menüband von Microsoft Office 2016, wenn das Menüband reduziert wird.
* Aslan meldet den vorgeschlagenen Kontakt wieder automatisch, wenn Adressen in neuen Nachrichten in Outlook 2016 eingegeben werden. (#8502)
...
* Der Lesemodus ist nun standardmäßig richtigerweise eingeschaltet, wenn Nachrichten in Outlook 2016 / 365 gelesen werden, sofern die experimentelle UI-Automation in Aslan für Word-Dokumente aktiviert ist. (#9188)
* Ein Problem wurde behoben, bei welchem Aslan abstürzte und wo der einzige Ausweg darin bestandt, die komplette Windows-Sitzung ab- und wieder anzumelden. (#6291)
* In Windows 10 Oktober 2018 Update und später, beim Öffnen des Cloud-Clipboard-Verlaufs mit leerer Zwischenablage, wird Aslan den Status der Zwischenablage melden. (#9103)
* In Windows 10 Oktober 2018 Update und später, bei der Suche nach Emojis im Emoji-Panel, wird Aslan die besten Suchergebnisse melden. (#9105)
* Aslan friert im Hauptfenster von Oracle VirtualBox 5.2 und neuer nicht mehr ein. (#9202)
...
* Die Bearbeitung in Zellen in MS Excel 2016/365 funktioniert nun wieder. Das "Editieren in Zelle" in den Optionen von Microsoft Excel kann wieder aktiviert werden, um in Excel 2016 / 365 auf die Steuerung der Zellenbearbeitung mit Aslan zuzugreifen. (#8146).
...
* Aslan kann nun mit allen Editionen von Microsoft Visual Studio 2017 (nicht nur mit der Community-Version) kompiliert werden. (#8939)
* Sie können nun Protokollausgaben von Liblouis in das Aslan-Protokoll aufnehmen, indem Sie das boolesche Flag "louis" im Abschnitt "debugprotokollierung" in den erweiterten Einstellungen von Aslan aktivieren. (#4554)
* Autoren von Erweiterungen können nun Informationen zur Kompatibilität mit Aslan-Versionen im Manifest der Erweiterungen bereitstellen. (#6275, #9055)
 * minimumAslanVersion: Die minimale erforderliche Aslan-Version, damit eine Erweiterung ordnungsgemäß funktioniert.
 * lastTestedAslanVersion: Die zuletzt getestete Aslan-Version, mit der die Erweiterung getestet wurde.
...
* Eigenschaften und Methoden innerhalb von Klassen können nun in Aslan als abstrakt gekennzeichnet werden. Diese Klassen lösen einen Fehler aus, wenn sie instanziiert werden. (#8294, #8652, #8658)
* Aslan kann die Zeit seit der Eingabe protokollieren, wenn Text gesprochen wird, was bei der Messung der wahrgenommenen Reaktionsfähigkeit hilft. Dies kann durch Aktivieren der Einstellung "timeSinceInput" im Abschnitt "Debugprotokollierung" in den erweiterten Einstellungen von Aslan aktiviert werden. (#9167)

## 2018.4.1

Diese Version behebt einen Absturz beim Start, wenn die Sprache der Benutzeroberfläche von Aslan auf Aragonesisch eingestellt ist. (#9089)
...
* In der Nachrichtenansicht in Microsoft Outlook meldet Aslan nun den Status "Beantwortet" und "Weitergeleitet" für jede Nachricht. (#6911)
* Aslan kann jetzt Beschreibungen für Emoji und andere Zeichen vorlesen, welche Teil der gemeinsamen Unicode-Gebietsschemata sind. (#6523)
* In Microsoft Word kann Aslan jetzt den Abstand des Cursors vom oberen und vom linken Rand der aktuellen Seite melden. Drücken Sie dafür Aslan+Entfernen. (#1939)
* Aslan sagt nicht mehr "Ausgewählt" bei jeder Zelle während der Navigation mit den Pfeiltasten zwischen Zellen in Google Sheets, wenn Braille-Modus aktiviert wurde. (#8879)
...
* In den Tastatur-Einstellungen von Aslan werden die Kontrollkästchen zum Aktivieren oder Deaktivieren der Aslan-Taste nun in einer Liste und nicht mehr als separate Kontrollkästchen angezeigt.
* Aslan zeigt unter einigen Windows-Versionen keine doppelten Informationen mehr an, wenn die Uhr in der Taskleiste vorgelesen wird. (#4364)
* Liblouis Braille-Übersetzer auf Version 3.7.0 aktualisiert. (#8697)
* eSpeak-NG wurde auf Commit 919f3240cbb aktualisiert
* Aslan meldet nicht mehr mehrfach hintereinander "Anklickbar", wenn Sie im Lesemodus durch klickbare Inhalte navigieren. (#7430)
* Gesten, die auf Baum Vario 40 Braillezeilen ausgeführt werden, funktionieren nun korrekt. (#8894)
* Aslan meldet nicht mehr "Text ausgewählt" bei jedem fokussierten Objekt in Google-Präsentationen mit Mozilla Firefox. (#8964)

### Fehlerbehebungen

* In Microsoft Outlook 2016/365 werden der Kategorie- und Kennzeichenstatus für Nachrichten gemeldet. (#8603)
* Wenn Aslan auf Sprachen wie Kirgyz, Mongolisch oder Mazedonisch eingestellt ist, wird beim Start kein Dialogfeld mehr angezeigt, das davor warnt, dass die Sprache vom Betriebssystem nicht unterstützt wird. (#8064)
...
* Im Windows-Explorer und anderen Anwendungen bei Verwendung von UI-Automation wird der Fokus zuverlässig von Aslan verfolgt, wenn eine andere Anwendung lädt (z.B. während der Batch-Ausführung bei der Audio-bearbeitung). (#7345)
...
* Während der Navigation mit den Schnellnavigationstasten in Nachrichten im neuen Gmail, liest Aslan nun nicht mehr den ganzen Text der Nachricht vor, sobald man ein entsprechendes Element fokusiert. (#8887)
* Browser wie Mozilla Firefox und Google Chrome dürften nicht mehr abstürzen, nachdem Aslan aktualisiert wurde. Das Lesemodus müsste nun weiterhin Aktualisierungen in aktuell geladenen Dokumenten richtig reflektieren. (#7641)

### Änderungen für Entwickler

* Die Funktion "gui.aslanControls" beinhaltet nun zwei Klassen zum Erstellen zugänglicher Listen mit Kontrollkästchen. (#7325)
 ...
 * Schauen Sie für weitere Informationen in den Implementierung von "gui.aslanControls.ListCtrlAccPropServer" nach.
...
Dies ist ein kleineres Update, in dem ein kritischer Fehler in Aslan behoben wurde, der zum Absturz von 32-Bit-Versionen von Mozilla Firefox führte. (#8759)
...
* Aslan meldet Grammatikfehler, wenn sie von Webseiten in Mozilla Firefox und Google Chrome angemessen dargestellt werden. (#8280)
* Inhalte, die in Webseiten als eingefügt oder gelöscht gekennzeichnet sind, werden nun in Google Chrome angezeigt. (#8558)
* Unterstützung für BrailleNote QT und das Scrollrad von Apex BT wurde hinzugefügt, wenn BrailleNote als Braillezeile mit Aslan verwendet wird. (#5992, #5993)
 ...
 * Sie können diese Funktion aktivieren, indem Sie die automatische Option aus der Liste der Braillezeilen im Braillezeilen-Auswahldialog von Aslan auswählen.
...
* Der Benutzer wird einmalig beim Start von Aslan gefragt, ob bei der Überprüfung auf Aslan-Aktualisierungen Nutzungsstatistiken an NV Access gesendet werden sollen. (#8217)
...
* Wenn der Benutzer zugestimmt hat, Nutzungsstatistiken an NV Access zu senden, sendet Aslan nun den Namen des aktuell verwendeten Sprachausgaben-Treibers und der Braillezeile, um eine bessere Priorisierung für zukünftige Arbeiten an diesen Treibern zu ermöglichen. (#8217)
...
* Aslan kündigt keine redundanten Änderungen am Tastaturlayout mehr an. (#7383, #8419)
...
* Aslan erkennt mehr Dialoge in Windows 10 und anderen modernen Anwendungen. (#8405)
* Unter Windows 10 Oktober 2018 Update und Server 2019 und neuer kann Aslan den System-Fokus nicht mehr verfolgen, wenn eine Anwendung das System einfriert oder mit Ereignissen überschwemmt. (#7345, #8535)
...
* In der Liste der Sprachen in den Allgemeinen Einstellungen von Aslan wird der Sprachname für Burmesisch unter Windows 7 korrekt angezeigt. (#8544)
* In Microsoft Edge meldet Aslan Benachrichtigungen wie die Verfügbarkeit der Leseansicht und den Fortschritt des Seitenladens. (#8423)
* Beim Navigieren in eine Liste im Web meldet Aslan nun dessen Beschriftung, wenn der Web-Autor eines angegeben hat. (#7652)
...
* iTunes 12.9 ist mit Aslan wieder bedienbar. (#8744)

### Änderungen für Entwickler

* scriptHandler.script hinzugefügt; kann als decorator für Scripts verwendet werden. (#6266)
* Für Aslan wurde ein Systemtest-Framework eingeführt. (#708)
...
* Da Aslan jetzt nur noch Windows 7 SP1 und neuer unterstützt, wurde der Schlüssel "minWindowsVersion" entfernt, mit dem geprüft wird, ob UIA für eine bestimmte Windows-Version aktiviert werden sollte. (#8422)
 ...
 * Das Skript "config.pre_configSave" wird verwendet, um benachrichtigt zu werden, wenn die Konfiguration von Aslan gespeichert werden soll und "config.post_configSave" wird nach dem Speichern der Konfiguration aufgerufen.
...
* In Windows 10 Fall Creators Update und neuer kann Aslan nun Benachrichtigungen von Apps wie etwa dem Rechner oder dem Windows-Store ausgeben. (#8045)
...
* Bei der Aktualisierung von Aslan auf eine neue Version ist es nun möglich, die Instalation der Aktualisierung auf einen späteren Zeitpunkt zu verschieben. (#4263)
...
* Datum und Uhrzeit für unterstützte Braillezeilen von HandyTech (wie Active Braille und Active Star) werden nun automatisch von Aslan synchronisiert, wenn sie länger als fünf Sekunden nicht synchronisiert sind. (#8016)
...
* Beim Eingeben von Kurzschrift fällt Aslan automatisch auf eine Basisschrift-Eingabetabelle zurück, wenn sich der Fokus z. B. im Lesemodus auf einem Steuerelement befindet, das keine Einfügemarke besitzt. (#7306)
* Aslan zeigt in Outlook weniger ausführliche Informationen an, wenn eine Besprechung oder ein Termin angezeigt wird, der/die einen ganzen Tag umfasst. (#7949)
* Alle Einstellungen von Aslan werden nun in einem einzigen Dialogfeld unter Optionen -> Einstellungen angezeigt. (#7302)
...
* Aslan gibt nun nicht mehr die letzte Ziffer der Pin beim Entsperren des Rechners im Klartext aus. (#7908)
...
* Aslan erkennt nun mehr Aufzählungsarten in Microsoft Word. (#6778)
...
* Aslan zeigt nicht mehr "Unbekannt" an, wenn Sie in Windows 8 oder neuer mit Windows-Taste+X das "Kleine Startmenü" aufrufen. (#8137)
...
* Aslan meldet nicht mehr (LTR- und RTL) Marken in Braille oder bei "zeichenweises Lesen", wenn in neueren Windows-Versionen auf die Uhr zugegriffen wird. (#5729)
...
* Unter Windows 10 (Redstone 5) meldet Aslan keine zusätzlichen redundanten Informationen mehr, wenn mit Alt + Tab zwischen ausgeführten Programmen gewechselt wird. (#8258)
...
Dies ist eine spezielle Aslan-Version, die einen Fehler im Windows-OneCore-Sprachausgabentreiber behebt. Zuvor sprach Aslan unter Windows 10 Redstone 4 (1803) mit höherer Stimme und höherer Geschwindigkeit. (#8082)
...
* Es ist nicht mehr nötig, die Braillezeile ausdrücklich an den Fokus oder den Aslan-Cursor zu koppeln. Standardmäßig wird die Braillezeile automatisch gekoppelt. (#2385)
 * Die automatische Kopplung der Braillezeile an den Aslan-Cursor funktioniert nur bei Befehlen, die ausdrücklich den Aslan-Cursor oder den Navigator betreffen (wie z. B. die Befehle zum Betrachten von Text). Scrollen auf der Braillezeile wird keine automatische Kopplung auslösen.

### Fehlerbehebungen

* Sollte der Pfad des Ordners, in dem Aslan installiert ist, nicht-ASCII-Zeichen enthalten, schlägt die Anzeige der Formatierungsinformationen im lesemodus nicht mehr fehl, wenn Sie Aslan+F zweimal drücken. (#7474)
* Der Fokus wird nun korrekt gesetzt, wenn Sie von einer anderen Anwendung zu Spotify zurückkehren. (#7689)
* Wenn unter Windows 10 Fall Creators Update der ordnerschutz im Windows Defender Security Center aktiviert ist, schlägt die Aktualisierung von Aslan nicht mehr fehl. (#7696)
...
* Beim Markieren von Text sagt Aslan die Sonderzeichen und Symbole jetzt in der richtigen Sprache an. (#7687)
...
* Aslan gibt beim Starten auf Windows 7 keine Fehlermeldung mehr aus, wenn bestimmte Versionen der Visual Studio 2017 Redistributable durch andere Anwendungen installiert wurden. Die Fehlermeldung betraf interne API-LS dll-Dateien. (#7975)

### Änderungen für Entwickler

* Neue versteckte bool'sche Einstellung im Abschnitt Braille in der Aslan-Konfiguration: "outputPass1Only". (#7301, #7693, #7702)
...
* Um zu erkennen, ob Aslan aus dem Windows Store heraus ausgeführt wurde, wurde dem Modul config eine neue Variable ISAPPX hinzugefügt. (#7851)
...
* Die Funktion "hwIo.Hid" erhält einen zusätzlichen exklusiven parameter, welcher standardmäßig auf True gesetzt ist. Wenn der Parameter auf False gesetzt wird, dann können andere Anwendunngen mit einem Gerät kommunizieren, während die Anwendungen von Aslan getriggert werden. (#7859)
...
Bitte beachten Sie, dass diese Aslan-Version unter Windows XP und Windows Vista nicht mehr läuft. Sie benötigen mindestens Windows 7 mit Service pack 1.
...
* Beim Aktualisieren von Aslan-Erweiterungen werden Versionsinformationen angegeben. (#5324)
* Neue Kommandozeilenparameter zum Erstellen einer portablen Aslan-Version wurden hinzugefügt. (#6329)
...
* Wenn Aslan auf einem Laptop oder einem Tablet ausgeführt wird, werden Sie benachrichtigt, wenn Sie ein Netzteil anschließen, abziehen oder wenn sich die Bildschirmausrichtung ändert. (#4574, #4612)
...
* Aslan benötigt mindestens Windows 7 Service Pack 1 oder Windows Server 2008 R2 Service Pack 1. (#7546)
...
* Beim Zuweisen von Aslan-Befehlen an Tasten der Braillezeilen von Baum kann die Zuweisung auf ein bestimmtes Braillezeilenmodell beschränkt werden (z. B. Baum Vario Ultra, Baum Pronto). (#7517)
...
* Versionierte Dateien (Benutzerkonfiguration, Aussprache-Wörterbücher) werden nicht gespeichert, wenn Aslan von Launcher aus ausgeführt wird. (#7688)
* Die Braillezeilenmodelle Braillino, Buchwurm und Modular (mit der alten Firmware) werden nicht mehr ohne weiteres unterstützt. Um diese Braillezeilenmodelle zu verwenden, müssen Sie den Handytech-Universaltreiber und die Aslan-Erweiterung installieren. (#7590)

### Fehlerbehebungen

* In Anwendungen, wie Microsoft Word, werden Links nun in Braille angezeigt. (#6780)
* Wenn in Firefox oder Google Chrome viele Registerkarten geöffnet sind, reagiert Aslan nun schneller. (#3138)
...
* Im Internet Explorer und anderen MSHTML-Dokumenten wird das Attribut Required korrekt unterstützt, sodass Aslan angiebt, dass ein Formularfeld zwingend ausgefüllt werden muss. (#7321)
...
* Unter Windows 10 Creators Update funktioniert die Aslan-Unterstützung von Firefox auch dann noch, wenn Sie Aslan neu starten. (#7269)
* Wenn Sie Aslan neu starten, während Firefox im Vordergrund ist, steht der Lesemodus wieder zur Verfügung, auch wenn Sie den Fokus mit alt+tab aus Firefox hinaus und wieder hineinbewegen müssen. (#5758)
* Unterstützung mathematischer Inhalte in Google Chrome auf Systemen, auf denen Firefox nicht installiert ist. (#7308)
* Das Betriebssystem und die Anwendungen sollten nun nach der Installation von Aslan stabiler laufen (verglichen mit vorherigen Aslan-Versionen). (#7563)
* Wenn das Navigator-Objekt beim Aufruf der Inhaltserkennung (z. B. mit Aslan+R) verschwunden sein sollte, zeigt Aslan eine Warnmeldung an. (#7567)
...
* Aslan wird nun mit Visual Studio 2017 und Windows 10 SDK compiliert. (#7568)
...
* Aslan unterstützt jetzt auch die neuen Windows OneCore-Stimmen, die in Windows 10 enthalten sind. Sie können diese Stimmen verwenden, indem Sie Windows OneCore-Stimmen unter Einstellungen --> Sprachausgabe auswählen. (#6159)
* Die Konfigurationsdateien von Aslan können jetzt auch im lokalen Konfigurationsverzeichnis gespeichert werden. Die betreffende Einstellung wird in der Registrierungsdatenbank von Windows vorgenommen. Weitere Informationen dazu finden Sie im Abschnitt "Systemparameter" im Benutzerhandbuch. (#6812)
* In Internet-Browsern werden Platzhalterwerte für Eingabefelder angezeigt, solange sie noch leer sind. Aslan unterstützt jetzt auch das Aria-Attribut "aria-placeholder". (#7004)
 ...
 * Um den Inhalt des Navigator-Objektes zu erkennen, drücken Sie Aslan+R.
...
* Mit Hilfe eines neuen, nicht zugewiesenen Befehls kann Aslan schnell neu gestartet werden. Dieser Befehl ist unter der Kategorie "Verschiedenes" zu finden. (#6396)
* Das Tastaturschema kann nun im Willkommensbildschirm von Aslan eingestellt werden. (#6863)
...
* Standardmäßig zeigt Aslan nur noch dann den Kontext eines Objektes an, wenn ein neues Objekt den Fokus erhält. (#217)
 * Zuvor wurde der Kontext auch dann angezeigt, wenn er sich nicht geändert hat.
 * Sie können das alte Verhalten erzwingen, indem Sie in den Braille-Einstellungen von Aslan die Option "Kontext anzeigen" auf "immer" umstellen.
...
* Das Logo von Aslan wurde geändert. Es zeigt nun die Buchstaben Aslan in weißer Schrift auf rotem Hintergrund. (#7446)
...
* Wenn ein Dialogfenster eine Fortschrittsanzeige enthält, wird bei Änderungen an der Fortschrittsanzeige auch der Text des Dialogfensters in Braille aktualisiert. Das bedeutet, dass z. B. das Fortschreiten der Zeit zum Herunterladen beim Aktualisieren von Aslan Sie verfolgen können. (#6862)
...
* In Java-Swing-Anwendungen stürzt Aslan beim Navigieren in Tabellen nicht mehr ab. (#6992)
...
* Wenn Sie auf twitter.com die Details zu einem Beitrag öffnen, zeigt Aslan in Firefox nicht mehr mehrfach "Abschnitt" an. (#5741)
...
* Aslan muss mit Visual Studio Community 2015 erzeugt werden. Visual Studio Express wird nicht mehr unterstützt. (#7110)
...
* In Microsoft Excel können nun Informationen über Zellrahmen mittels Aslan+F abgefragt werden. (#3044)
...
* Die Navigation mit dem Aslan-Cursor bei aktivierter Option ""Einfacher Darstellungsmodus" in UIA-Anwendungen wurde vereinfacht. (#6948, #6950)
...
* Aslan zeigt nun auch solche (UIA)-Steuerelemente korrekt an, deren Tastenkombination nicht (richtig) definiert wurde. (#6779)
...
* Die Befehlszeile von Aslan wird nun mit Hilfe des Moduls "Argparser" abgearbeitet. Dadurch werden Optionen wie "-R" oder "-Q" exklusiv abgearbeitet. (#6865)
* Die Funktion "core.calllater" fügt jetzt die Rückruffunktion nach einer angegebenen Zeitspanne in eine Warteschlange ein, anstatt sie direkt auszuführen. Dadurch wird verhindert, dass Aslan beim Anzeigen modaler Dialoge (wie Meldungsfenster) hängen bleibt. (#6797)
...
* Es gibt jetzt ein Unit Test Framework für Aslan. (#7026)
 ...
 * Wenn Sie einen Pull-Request für Aslan einreichen, sollten Sie zunächst die Tests durchführen und sicherstellen, dass diese bestanden werden.
...
* Der Aslan-Suchbefehl Aslan+Strg+F wird nun auch in Microsoft Edge unterstützt. (#6580)
...
* In Microsoft Excel gibt der Befehl zur Ausgabe des aktuellen Standorts (Aslan+Entf und Aslan+NUM-Entf) nun den Namen des Tabellenblatts und die aktuell hervorgehobene Zelle aus. (#6613)
* Im Dialogfeld zum Beenden von Aslan gibt es eine neue Option zum Neustart von Aslan im Debug-Modus. (#6689)
...
* Auf Grund von Änderungen in Windows 10 Creators Update kann die Einstellung der Lautstärkereduktion "nur beim Sprechen" weder sicherstellen, dass die lautstärke anderer Anwendungen rechtzeitig reduziert wird, noch bevor Aslan zu sprechen beginnt; noch wird die Lautstärke lange genug reduziert, bis Aslan ausgesprochen hat. Diese Änderungen wirken sich nicht auf älteren Windows 10-Versionen aus. (#6684)
...
* Das Drücken der Escape-Taste in Microsoft Excel schaltet nun nicht mehr unerwartet in den Lesemodus um. Dies geschieht nur noch, wenn der Lesemodus mittels Aslan+Leertaste eingeschaltet und anschließend die Fokusmodus mittels Eingabe aktiviert wurde. (#6569)
* Aslan stürzt nicht mehr in Microsoft Excel-Tabellenblättern ab, bei denen eine ganze Reihe/Spalte in eine Zelle zusammengeführt wurde. (#6216)
* Die Ansage von überlagertem Text in Microsoft Excel wurde verbessert. (#6472)
* Aslan zeigt nun schreibgeschützte Kontrollkästchen korrekt an. (#6563)
* Aslan zeigt nun keine Warnmeldungen mehr an, wenn der Logo-Klang nicht abgespielt werden konnte. (#6289)
* Nicht verfügbare Steuerelemente in Menübändern von Microsoft Excel werden als solche angezeigt. (#6430)
* Aslan zeigt nicht mehr "Feld" an, wenn Fenster minimiert werden. (#6671)
...
* Aslan verhält sich korrekt, wenn Sie Windows Media Player beenden, während ein Schieberegler den Fokus besitzt. (#5467)
...
Zu den Schwerpunkten dieser Version gehören Verbesserungen in der Unterstützung von Microsoft Edge; der Lesemodus in der Mail-App in Windows 10 sowie Verbesserungen in Dialogfeldern von Aslan.

### Neue Funktionen

* Aslan kann Zeileneinrückungen nun auch mit Hilfe von Signaltönen ausgeben. Um dies einzustellen, können Sie die neue Option "Zeileneinrückungen ausgeben durch..." in den Einstellungen für Dokumentformatierung verwenden. (#5906)
* Unterstützung für die Braillezeile Orbit Reader 20. (#6007)
* Eine neue Option zum Anzeigen des Sprachbetrachters beim Start von Aslan wurde hinzugefügt. Dies kann mittels eines Kontrollkästchens im Sprachbetrachter eingeschaltet werden. (#5050)
* Die Position und Größe des Sprachbetrachters wird beim erneuten Öffnen wiederhergestellt. (#5050)
* Felder mit Querverweisen in Microsoft Word werden von Aslan als Links angezeigt und können wie solche aktiviert werden. (#6102)
...
* Ein Fehler wurde behoben, wenn beim Beenden von Aslan der Sprachbetrachter angezeigt wurde. (#5050)
...
* Probleme beim erneuten Laden von Erweiterungen mit Hilfe der Tastenkombination Aslan+Strg+F3 wurden behoben. (#2892, #5380)
* In der Sprachenliste in den allgemeinen Einstellungen werden einige Sprachen (wie z. B. Aragonesisch) richtig angezeigt. (#6259)
* Im Dialogfeld "Einstellungen" -> "Eingaben" werden emulierte Tastendrücke und Tastenkombinationen in der in Aslan eingestellten Sprache angezeigt. (#6212)
* Das Umstellen der Sprache in Aslan wirkt sich nun erst nach dem Neustart von Aslan aus. (#4561)
...
* Sie können das Dialogfeld "Einstellungen" -> "Eingaben" verwenden, um Aslan-Befehle an Tasten bzw. Tastenkombinationen der Braillezeilen von HandyTech zuzuweisen. (#6461)
* Wenn Sie in Microsoft Excel innerhalb einer Tabelle die Eingabetaste drücken, erkennt Aslan korrekt, dass sich der Fokus in die nächste Zeile bewegt hat. (#6500)
...
* In Microsoft Word wird der Titel einer Tabelle korrekt erkannt. Wenn außerdem eine Beschreibung vorhanden ist, kann diese im Lesemodus mit dem Befehl Ausführliche Beschreibung öffnen (Aslan+D) angezeigt werden. (#5943)
...
* Aslan gibt Farben nun leichter verständlich mit 9 Farbnamen und in verschiedenen Schattierungen wieder, anstatt subjektivere aber dafür schwere verständliche Bezeichnungen zu verwenden. (#6029)
* Das Verhalten von Aslan+F9 und Aslan+F10 wurde geändert. Wird Aslan+F10 einmal gedrückt, so wird der Text zwischen der zuvor gesetzten Startmarke und der aktuellen Position des Aslan-Cursors markiert. Wird die Tastenkombination zweimal gedrückt, so wird er in die Zwischenablage kopiert. (#4636)
...
* In Dialogen von Microsoft Word wurde die Objektnavigation von Aslan verbessert. (#6036)
...
* Im Web-Conference-Plugin (z. B. auf [www.out-of-sight.net](http://www.out-of-sight.net)) spielt Aslan keine Signaltöne mehr ab, wenn sich die Aussteuerungsanzeige für das Mikrofon ändert. (#5888)
...
* Beim Bearbeiten von Wörterbucheinträgen wird bei fehlerhaften regulären Ausdrücken eine Fehlermeldung angezeigt. Außerdem stürtzt Aslan nicht mehr ab, wenn ein Wörterbuch fehlerhafte Einträge enthält. (#4834)
* Falls Aslan nicht (mehr) in der Lage sein sollte, mit einer Braillezeile zu kommunizieren (etwa weil sie abgezogen wurde), wird die Verwendung von Braillezeilen generell unterlassen. (#1555)
* Leistungsverbesserungen im Dialogfeld "Filtern nach" im Lesemodus. (#6126)
* Die von Aslan zurückgegebenen namen für Filtergrundmuster in Microsoft Excel entsprechen jetzt denen, die tatsächlich in Excel verwendet wurden. (#6092)
...
* Aslan unterstützt nun die zweiten Routing-Tasten an Alva BC640/680-Braillezeilen. (#5206)
* Aslan kann Windows 10-Benachrichtigungen nun wieder anzeigen. Dies betrifft vor allem aktuelle Versionen von Windows 10. (#6096)
* Aslan erkennt Tastendrücke an Baum-/Humanware-Braillezeilen zuverlässiger. (#6035)
...
* Wenn die Sprache abgeschaltet wurde, erscheinen Objektinformationen (z. B. beim Drücken von Aslan+Tab) trotzdem im Sprachbetrachter. (#6049)
...
* In Microsoft Excel 2010 und neuer werden nun Zellschattierungen und Rahmen angesagt. Die automatische Ansage hierzu wird von der Einstellung für die Dokumentformatierungen in Aslan gesteuert. (#3683)
...
* Die Ausgabe von Grammatikfehlern in Microsoft Word kann nun in den Einstellungen für Dokumentformatierung von Aslan ein- oder ausgeschaltet werden. (#5877)

### Änderungen

* Im Lesemodus und in Eingabefeldern werden beide Eingabetasten nun gleich behandelt. (#5385)
* Aslan verwendet nun eSpeeak NG. (#5651)
...
* Wenn Sie die Schnellnavigationstasten verwenden, um zu einem Element eines Typs zu navigieren, der im aktuellen Dokument nicht unterstützt wird, wird dies von Aslan ausdrücklich angezeigt. (#5691)
...
* Aslan gibt nun nicht länger mehr belanglose Informationen beim Fensterwechseln in Java-Anwendungen mit mehrfachen Fenstern wie z. B. in IntelliJ oder Android Studio wieder. (#5732)
...
* Aslan stürzt nun nicht mehr ab, wenn Sie die Brailleausgabe aktivieren. (#4457)
...
* In Google Chrome und Chrome-basierten Browsern können Sie Aslan in den Lesemodus zwingen, während Sie sich in Web-Anwendungen befinden. (#5818)
* Im Internet Explorer schaltet Aslan nicht mehr irrtümlich in den Lesemodus um, wenn der Fokus auf ein Element bewegt wird, dessen Eigenschaft Aria-ActivateDescendant verwendet wird. Dies betrifft beispielsweise die Vorschlagsliste im Adressfeld beim Erstellen einer Nachricht in Gmail. (#5676)
* In Microsoft Word-Dokumenten mit großen Tabellen wird Aslan nun nicht mehr abstürzen, wenn die Ausgabe von Reihen-/Spaltenüberschriften aktiviert ist. (#5878)
* In Microsoft Word wird Aslan Text nicht mehr länger als Überschrift anzeigen, wenn er zwar eine Gliederungsebene besitzt, nicht jedoch mit einer der integrierten Vorlagen für Überschriften formatiert wurde. (#5186)
...
* Die C++-Komponenten von Aslan werden nun mit Microsoft Visual Studio 2015 erzeugt. (#5592)
...
* Aslan kann nun mittels Bluetooth mit HIMS Smart Beetle Braillezeilen kommunizieren. (#5607)
* Ab Windows 8 und neuer kann Aslan die Lautstärke anderer Audio-Quellen optional reduzieren. Dies kann im Dialogfeld für die Sprachausgabe mit der Option "Lautstärke anderer Audio-Quellen reduzieren" oder Aslan+Umschalt+D eingestellt werden. (#3830, #5575)
...
* Das Wort "Text" wird nun nicht länger ausgegeben, wenn der Fokus oder der Aslan-Cursor zu Text-Objekten bewegt wird. (#5452)
...
* Aslan stürzt nicht mehr ab, wenn Sie eine Eingabeaufforderung in Windows 10 mit Alt+F4 schließen. (#5343)
...
* In Microsoft Word werden Änderungen am Dokument nur dann durch Aslan mitgeteilt, wenn sie auch tatsächlich im Dokument ausgezeichnet werden. Hierzu zählen beispielsweise Meldungen über eingefügten oder gelöschten Text. (#5566)
* Wenn ein Umschalter fokussiert wird, gibt Aslan aus, wenn dieser von gedrückt zu nicht gedrückt wechselt. (#5441)
...
* Wenn die Menübänder in Office 2013 und neuer so eingestellt werden, dass sie nur Registerkarten anzeigen, werden sie von Aslan wie erwartet dargestellt, wenn eine Registerkarte aktiviert wird. (#5504)
...
* Fehler behoben, wonach Aslan in sehr seltenen Fällen den Inhalt von Tabellenblättern in Excel nicht angezeigt hat, während die Option "Spalten- und Reihenüberschriften von Tabellen ansagen" aktiviert ist. (#5705)
...
* Unter Windows 8 und neuer taucht Aslan nun im Center für erleichterte Bedienungen auf. (#308)
...
* Sie können in Microsoft Excel nun die Elementliste (Aslan+F7) verwenden, um Tabellenblätter aufzulisten oder umzubenennen. (#4630, #4414)
...
* In Microsoft Excel meldet Aslan nun alle vom Blattautor gesetzten Eingabemeldungen auf Zellen. (#5051)
...
* Verbesserungen im Erscheinungsbild. Dies betrifft Schalter in einer Warnung, die beim Abwerten auf eine frühere Aslan-Version angezeigt wurde. (#5325)
* In Windows 8 und neuer startet Aslan nun wesentlich früher, wenn es so konfiguriert wird, dass es nach der Anmeldung ausgeführt wird. (#308)
 * Wenn die Option zum Start nach der Anmeldung mit einer vorherigen Version aktiviert wurde, müssen Sie sie aus- und wieder einschalten, damit die Änderungen wirksam werden. Folgen Sie dieser Anleitung:
  ...
  1. Deaktivieren Sie die Option "Aslan automatisch nach der Anmeldung starten".
  1. Betätigen Sie den Schalter "OK".
  1. Öffnen Sie die Allgemeinen Einstellungen erneut.
  1. Aktivieren Sie die Option "Aslan automatisch nach der Anmeldung starten".
  1. Betätigen Sie den Schalter "OK".
* Leistungsverbesserungen der UIA einschließlich Windows-Explorer und Task-Manager. (#5293)
* Aslan schaltet nun korrekt in den Fokusmodus um, wenn Sie sich mit Tab auf schreibgeschützte Aria-Elemente bewegen. Dies betrifft Firefox und andere Gecko-Anwendungen. (#5118)
* Wenn Sie mit einem Touchscreen arbeiten, zeigt Aslan nun korrekterweise "kein voriges Objekt" an, wenn sie vom ersten objekt auf dem Bildschirm aus nach links streichen.
* Problem behoben, wenn Sie im Dialogfeld "Einstellungen" -> "Eingaben" mehrere Wörter in das Suchfeld eingeben. (#5426)
* Aslan wird nun nicht mehr abstürzen, wenn Sie eine Humanware bi/b-Braillezeile über usb erneut anschließen. (#5406)
...
* Beim Aufruf des Startmenüs von Windows 10 sollte Aslan nun nicht mehr abstürzen. (#5417)
...
* Aslan behandelt nun das Schließen von Kontextmenüs in Anwendungen wie Jarte korrekt. (#5302)
...
* Im Lesemodus können Sie nun die Schnellnavigation mittels Aslan+Umschalt+Leertaste ein- und ausschalten. Bei ausgeschalteter Schnellnavigation werden Buchstaben an die aktive Anwendung weitergereicht. Dies ist in einigen Internet-Anwendungen wie GMail, Twitter und Facebook hilfreich. (#3203)
...
* Wenn Sie versuchen, eine ältere Aslan-Version über eine aktuelle Version zu installieren wird eine Warnung angezeigt. Dies wird nicht empfohlen. Aslan sollte vollständig deinstalliert werden, bevor Sie eine ältere Version installieren. (#5037)
...
* In Firefox erstellt Aslan keine Beschreibungen mehr für ARIA-Registerkarten, die den gesamten Inhalt der Registerkarte enthalten. (#4638)
...
* Wenn Sie den Fokus von einer Anwendung nehmen, die langsam reagiert, sollte Aslan nun in den meisten Fällen schneller reagieren. (#3831)
...
* Im Lesemodus verhält sich Aslan beim Navigieren wie erwartet, wenn Sie mit Umschalt+)Tab in einem Rahmen navigieren. (#5227)
...
* Im Sperrbildschirm von Windows 10 ist es nicht mehr möglich mittels des Aslan-Cursors auf laufende Anwendungen zuzugreifen oder die Konfiguration von Aslan zu ändern sowie die Zwischenablage auszulesen. (#5269)
...
* Aslan meldet in Excel überstehende oder abgeschnittene Zelleninhalte. (#3040)
* In Microsoft Excel können Sie sich mit Hilfe der Elementliste (Aslan+F7) eine Liste aller Formeln, Diagramme oder Kommentare anzeigen lassen. (#1987)
* Diagramme können nun in Microsoft Excel ausgelesen werden. Wählen Sie das Diagramm mit Hilfe der Elementliste (Aslan+F7) aus und bewegen Sie sich anschließend mit den Pfeiltasten zwischen den Datenpunkten. (#1987)
* Mit Hilfe von Mathplayer von Design Science kann Aslan in Webbrowsern, in Microsoft Word und in PowerPoint innerhalb mathematischer Inhalte navigieren. Siehe das kapitel "auslesen mathematischer Inhalte" für weitere informationen. (#4673)
* Im Dialogfeld "Eingaben" können Sie nun Tastenkombinationen, Tastenbefehle, Etc. aller Aslan-Einstellungen sowie Einstellungen zur Dokumentformatierung zuweisen. (#4898)
...
* In den Maus-Einstellungen von Aslan wurden Tastenkürzel für Audiokoordinaten bei Mausbewegungen wiedergeben sowie Lautstärke der Audiokoordinaten durch Helligkeit kontrollieren hinzugefügt. (#4916)
...
* Aslan-Erweiterungen können nun ihre eigene Darstellung für mathematische Formeln implementieren. Siehe hierzu das paket MathPres für weitere Informationen. (#4509)
...
* In Mintty gibt Aslan nun neuen Text aus. (#4588)
...
* In Microsoft Word kann nun mittels Aslan+Leertaste der Lesemodus verwendet werden, um mit Hilfe von Schnellnavigationstasten im Dokument zu navigieren. Die Elementliste ist ebenfalls verfügbar. (#2975)
* Das Lesen von HTML-Nachrichten in Outlook 2007 und neuer wurde verbessert, in dem der Lesemodus automatisch für diese Nachrichten aktiviert wird. Sollte dies in seltenen Fällen nicht geschehen, können Sie den Lesemodus mittels Aslan+Leertaste aktivieren. (#2975)
 ...
 * Wenn die Tabelle jedoch verbundene Zeilen enthält, funktioniert dies nicht automatisch. Sie können die Spaltenüberschrift manuell mit Aslan setzen. Verwenden Sie hierfür die Tastenkombination Aslan+Umschalt+c.
* In der Desktop-Version von Skype werden Benachrichtigungen angezeigt. (#4741)
* In Skype können Sie die letzten Nachrichten mit Aslan+Strg+1 bis Aslan+Strg+0 anzeigen Aslan+Strg+1 zeigt beispielsweise die letzte eingegebene Nachricht an. (#3210)
* In einer Skype-Konversation wird nun angesagt, wenn jemand tippt. (#3506)
* Bei der Installation von Aslan kann die Anzeige von Meldungen unterdrückt werden. Verwenden Sie hierfür die Kommandozeilenoption --install-silent. (#4206)
...
* Aslan verwendet die Sprache der Sprachausgabe bzw. der Stimme, um Namen von Symbolen und Sonderzeichen zu lesen; unabhängig davon, ob "Sprache automatisch wechseln" aktiviert ist. Um dieses Verhalten abzuschalten, sodass Aslan wieder die Standard-Sprache verwendet, deaktivieren Sie in den Stimmen-Einstellungen die neue Option "Beim Vorlesen von Symbolen und Sonderzeichen die Sprache der Stimme berücksichtigen". (#4210)
* Die Unterstützung für Newfon wurde entfernt; Newfon ist jetzt als Aslan-Erweiterung verfügbar. (#3184)
* Skype 7 oder neuer wird nun benötigt; ältere Skype-Versionen werden nicht mehr unterstützt. (#4218)
* Das Herunterladen von Aslan-Aktualisierungen ist nun sicherer. Es erfolgt nun über https. Zudem wird die Datei nach dem Herunterladen mit Hilfe ihres Hash-Wertes auf Korrektheit geprüft. (#4716)
...
* Alle Aslan-Dialoge werden zentriert angezeigt, um die Lesbarkeit zu verbessern. (#3148)
...
* Beim Entfernen einer Erweiterung wird das Verzeichnis der Erweiterung korrekt gelöscht. Zuvor musste Aslan hierfür zwei Mal neu gestartet werden. (#3461)
...
* In Skype sollte Aslan nicht mehr fälschlicherweise ganze Konversationen lesen. (#4644)
* Problem behoben, wonach Aslan beim Lesen des Datums und der Uhrzeit manchmal die Länder-Einstellungen des Benutzers nicht berücksichtigt hat. (#2987)
...
* Aslan sollte nun nicht mehr hängen bleiben, sobald eine Metro-App durch Windows 8 stillgelegt wird. (#4572)
...
* Im Internet Explorer und anderen MSHTML-Elementen ignoriert Aslan das fokusieren von Elementen nicht mehr, wenn dies durch das Atribut aria-activedescendant erfolgte. (#4667)

### Änderungen für Entwickler

* wxPython auf 3.0.2.0 aktualisiert. (#3763)
* Python auf Version 2.7.9 aktualisiert. (#4715)
* Wenn Sie eine Erweiterung entfernen oder aktualisieren, die während der Installation das Modul speechDictHandler importiert hat, wird Aslan nicht mehr abstürzen. (#4496)
...
* Sie können Aslan nun auch über den Dialogfeld "Beenden" aus (wahlweise mit deaktivierten Erweiterungen) neu starten. (#4057)
 * Sie können Aslan auch mit deaktivierten Erweiterungen neu starten, indem Sie die Kommandozeilenoption --disable-addons verwenden.
...
* Wenn Sie mit der linken Maustaste auf das Aslan-Symbol im Infobereich klicken, wird nun das Aslan-Menü geöffnet. (#4459)

### Fehlerbehebungen

* Wenn Sie mit Alt+Tab in ein bereits geöffnetes virtuelles Dokument wechseln, wird der Aslan-Cursor korrekt auf dem virtuellen Cursor und nicht wie bisher auf dem fokusierten Objekt (z. B. einem nahe gelegenen Link) positioniert. (#4369)
* Der Aslan-Cursor verfolgt nun den virtuellen Cursor richtig in PowerPoint-Präsentationen. (#4370)
...
* Aslan stürzt nun nicht mehr ab, wenn es mit einer Handytech-Braillezeile verwendet wird. (#3709)
* Problem in Windows Vista behoben, wonach Aslan mit dem Dialogfeld "Einsprungpunkt nicht gefunden" abstürzte, wenn man es über die Desktop-Verknüpfung oder über die Tastenkombination gestartet hat. (#4235)
...
* Die Klänge beim Starten und Beenden von Aslan können mit einer neuen Option in den allgemeinen Einstellungen ein- und ausgeschaltet werden. (#834)
...
* Microsoft Word: Sie können Aslan+Alt+C drücken, um sich den Kommentar in einem Word-Dokument ausgeben zu lassen. (#3528)
 ...
 * Die Befehle zum Setzen von Spalten- und Reihenbeschriftungen (Aslan+Umschalt+C und Aslan+Umschalt+R) speichern die Spalten- und Reihenbeschriftungen im Excel-Arbeitsblatt, sodass diese nach einem Neustart von Aslan oder Microsoft Excel wieder zur Verfügung stehen. Solche Reihen-/Spaltenüberschriften sollten von allen Bildschirmlesern ausgewertet werden können, die definierte Namen unterstützen
 ...
 * Wenn Sie sich in der ersten Zeile/Spalte einer Tabelle befinden, können Sie diese mit Aslan+Umschalt+C und Aslan+Umschalt+R als Spalten-/Zeilenbeschriftung festlegen. Diese Beschriftungen werden im Word-Dokument gespeichert und stehen allen Bildschirmlesern zur Verfügung, die derartige Lesezeichen unterstützen.
* Microsoft Word: Wenn Sie Tab drücken, wird der Abstand zum linken Seitenrand ausgegeben. (#1353)
* Für die meisten Befehle, die die Formatierungen in einem Word-Dokument verändern (Fett, Kursiv, unterstrichen; Gliederungsebenen usw.) meldet Aslan die Änderung der Formatierung per Sprache und Braille. (#1353)
* Wenn in einem Arbeitsblatt von Microsoft Excel die aktuelle Zelle einen Kommentar besitzt, können Sie diesen über die Tastenkombination Aslan+Alt+C sich anzeigen lassen. (#2920)
* Wenn Sie in Microsoft Excel den Modus zum Bearbeiten von Kommentaren mit der Tastenkombination Umschalt+F2 einschalten, wird ein Aslan-eigenes Dialogfeld zur Eingabe eines Kommentars angezeigt. (#2920)
...
* Microsoft Excel: Wenn die Ansage der Ausrichtung in den Einstellungen für Dokumentformatierungen aktiviert ist, wird die vertikale und horizontale Ausrichtung der aktuellen Zelle automatisch ausgegeben. Ausgabe derselben mittels Formatierung ausgeben (Aslan+F). (#4212)
* Microsoft Excel: Das Layout einer -Zelle wird ausgegeben, wenn Sie Aslan+F drücken. Dies geschieht automatisch, wenn die Option "Layout ansagen" in den Einstellungen für Dokumentformatierungen aktiviert ist. (#4213)
 ...
 * Verwenden Sie den Befehl "Position anzeigen" (Aslan+Nummernblock Entf), um diese Informationen abzurufen, ohne das objekt zu bewegen.
...
* Aslan funktioniert nun ordnungsgemäß auf serbischen Windows-Systemen. (#4203)
* Aslan verhält sich nun korrekt, wenn Sie bei eingeschalteter Eingabehilfe die Taste für den Nummernblock drücken, um den Nummernblock ein- oder auszuschalten. (#4226)
...
* Die Unterstützung für das Beilegen einer Dokumentation zu Aslan-Erweiterungen wurde vereinheitlicht. Sehen Sie sich den Abschnitt zu Dokumentation für Erweiterungen für weitere Informationen an. (#2694)
...
* Es ist nun möglich, die Tastenkombination zum Starten von Aslan zu ändern. In einigen Sprachräumen verursacht die Standard-Tastenkombination Probleme. (#2209)
...
* Im Internet Explorer werden Live-Regionen unterstützt. Dies erlaubt es Webentwicklern, bestimmte Teile einer Webseite automatisch von Aslan vorlesen zu lassen, sobald sie sich ändern. (#1846)
...
* Wenn Aslan-Meldungsfenster und -Dialoge erscheinen, wird die Sprache unterbrochen.
...
* Wenn Sie den Fokus von einer Anwendung wegschalten, die nicht mehr reagiert, wird sich Aslan zuverlässiger neu starten. (#3825)
...
* Wenn Sie in Microsoft Word oder Excel das Dialogfeld zum Einfügen von Sonderzeichen verwenden, kann Aslan nun das ausgewählte Symbol erkennen. (#3538)
* In den Einstellungen zur Dokumentformatierung können Sie nun festlegen, ob anklickbare Elemente als solche erkannt werden sollen. Diese Option ist standardmäßig aktiviert. Dies entspricht dem Verhalten älterer Aslan-Versionen. (#3556)
...
* Um ARIA-Dialoge und -Anwendungen im Lesemodus anzuzeigen, können Sie mit Aslan+Leertaste in den Lesemodus wechseln, während Sie eine ARIA-Anwendung verwenden. (#2023)
...
* Aslan benötigt nun mindestens VC Runtime Version 11. Das bedeutet, dass nun mindestens Windows XP SP2 oder Windows Server 2003 SP1 benötigt wird.
...
* Aslan verhält sich erwartungsgemäß, wenn Sie ein Kombinationsfeld für eine Zelle über das Kontextmenü öffnen. (#3586)
...
* Aslan wird keine Abstürze mehr verursachen, wenn Sie die Maus über die Dokumentfenster (TRichEdit) mancher Programme bewegen. Dies betrifft u. a. Jarte 5.1 und BRfácil. (#3693, #3603, #3581)
...
* Asiatische Sonderzeichen werden nun korrekt angesagt und angezeigt, wenn Sie sie innerhalb von Aslan eingeben (z. B. im Suchdialog im Lesemodus). (#3726)
...
* Problem behoben, wonach Aslan unter Windows 8 manchmal abstürzte, wenn man ein Erweitertes Eingabefeld (wie den Protokollbetrachter oder WinDbg) in den Fokus nimmt. (#3867)
* Auf Systemen mit modernen Monitoren wird nun die Maus nicht mehr an die falsche Stelle gesetzt. (#3758, #3703)
* Problem behoben, wonach Aslan beim Lesen einer Webseite nicht richtig funktioniert. (#3804)
...
* Aslan hängt sich nicht mehr auf, wenn Sie versuchen, den Treiber für ältere Papenmeier-Braillezeilen auszuwählen, obwohl keine Braillezeile angeschlossen ist.

### Änderungen für Entwickler

* Alle Anwendungsmodule enthalten nun die Eigenschaften productName und productVersion. Diese Informationen werden auch in der Entwicklerinfo angezeigt, die mit der Tastenkombination Aslan+F1 abgerufen werden kann. (#1625)
...
* Das Dialogfeld "Eingaben" erlaubt das Ändern von Eingabemöglichkeiten wie Tastenkombinationen für diverse Befehle innerhalb von Aslan. (#1532)
...
* Aslan erkennt nun den Status "Angeheftet" für angeheftete Steuerelemente, wie z. B. Registerkarten in Mozilla Firefox. (#3372)
...
* Beim Beenden von Aslan stürzt der Internet Explorer nicht mehr ab. (#3397)
* Physikalisches verschieben, Änderung der Helligkeit sowie andere Ereignisse werden nun nicht mehr fälschlicher weise als Tastendruck erkannt. Zuvor wurde die Sprache gestoppt oder manchmal wurden Befehle für Aslan ausgelößt. (#3468)
* Aslan verhält sich nun in Poedit 1.5.7 erwartungsgemäß. Nutzer von älteren Versionen müssen aktualisieren. (#3485)
* Aslan kann nun geschützte Dokumente in Word 2010 lesen und stürzt nun nicht mehr ab. (#1686)
* Wird ein Aslan-Distributionspaket mit einem ungültigen Kommandozeilenparameter aufgerufen, führt dies nicht mehr zu einer endlosen Kette von Fehlermeldungen. (#3463)
...
* Aslan kann nun XHTML-Dokumente in Microsoft Internet Explorer und anderen mshtml-Steuerelementen richtig auslesen. (#3542)
* In den Tastatur-Einstellungen muss mindestens eine Taste als Aslan-Taste eingestellt werden. Ist dies nicht der Fall, so wird eine Fehlermeldung angezeigt. (#2871)
...
* Wenn Aslan beim Wechsel in die Bildschirmdarstellung keine Objektposition erkennen kann, wird der Aslan-Cursor in die obere linke Ecke des Bildschirms gesetzt. (#3454)
...
* Im Dialogfeld "Erweiterungen verwalten" gibt es nun einen neuen Schalter "Erweiterungen herunterladen", über den Sie die Webseite für Aslan-Erweiterungen öffnen können. (#3209)
* Im Willkommensdialog, welcher immer beim ersten Start von Aslan geöffnet wird, gibt es ein neues Kontrollkästchen, mit dessen Hilfe Sie festlegen können, ob Aslan nach der Anmeldung automatisch geladen werden soll. (#2234)
 ...
 * Wenn Sie in die Modi Bildschirmdarstellung oder Dokumentdarstellung gewechselt haben und die Befehle zum Betrachten von Text verwenden, wird der Navigator den Aslan-Cursor verfolgen und sich auf das am meisten untergeordnete Objekt setzen.
 * Wenn Sie in den Modus Bildschirmdarstellung gewechselt haben, wird Aslan in diesem Modus bleiben, bis Sie zurück in den Modus Dokumentdarstellung oder Objektdarstellung wechseln.
 * Wenn Sie sich im Dokument- oder Objektmodus befinden, wird Aslan zwischen den beiden Modi automatisch wechseln, abhängig davon, ob Sie sich im Lesemodus befinden oder nicht.
...
* Aslan wird nun nicht mehr abstürzen, wenn die Systemzeit auf einen früheren Zeitpunkt eingestellt ist als der Zeitpunkt der letzten Prüfung nach aktualisierungen. (#3260)
...
* Wenn Sie in Windows 8 die Eingabesprache ändern, wird Aslan nun in der richtigen Sprache sprechen.
* Aslan meldet nun eine Änderung des IME-Konvertierungsmodus in Windows 8, sobald Sie ihn ändern.
* Aslan sagt auf dem Desktop keine unsinnigen Zeichen mehr an, wenn die Eingabemetoden Google Japanese oder Atok IME verwendet werden. (#3234)
* Wenn Sie in Windows 7 oder neuer einen Touchscreen oder die Spracherkennung zur Dateneingabe verwenden, wird Aslan keinen Wechsel der Tastatursprache mehr melden.
* Wenn die Ansage eingegebener Zeichen aktiviert ist und Sie drücken in bestimmten Eingabefeldern Strg+Rücktaste, wird Aslan nicht mehr irrtümlich das Steuerzeichen 0x7f ansagen. (#3315)
* Aslan wird nun nicht mehr irrtümlich die Lautstärke, Tonhöhe oder die Sprechgeschwindigkeit ändern, wenn der zu sprechende Text bestimmte Steuerzeichen oder xml-Strukturen enthält. (#3334) (regression von #437)
...
* Wenn der Erweiterungs-Manager bereits ausgeführt wird, führt ein erneutes Aktivieren des Erweiterungs-managers (entweder über extras --> Erweiterungen verwalten oder direkt aus dem Explorer heraus durch Öffnen einer ".aslan-addon"-Datei) nicht mehr zu einem Fehler. (#3351)
* Aslan wird nicht mehr abstürzen, wenn Sie eine japanische oder chinesische IME Version von Office 2010 verwenden. (#3064)
...
* Aslan bringt den Adobe Reader nun nicht mehr zum Absturz, wenn fehlerhaft gestaltete Pdf-Dokumente geöffnet sind, die Tabellenzeilen außerhalb von Tabellen enthalten. (#3399)
...
* Das Modul bisect ist veraltet und wird demnächst aus dem Binärpaket von Aslan entfernt. (#3368)
 * Erweiterungen, die Bisect (einschließlich urllib2) benötigen, sollten diese Module mitliefern.

## 2013.1.1

Neben anderen Fehlerbehebungen und aktualisierten Übersetzungen wird in dieser Aslan-Version ein Problem behoben, wonach Aslan abgestürzt ist, wenn irisch als Landessprache eingestellt war.

### Fehlerbehebungen

* Wenn in Aslan-Dialogen bei der Eingabe von Zeichen eingabemethoden für koreanisch oder japanisch verwendet werden, werden nun die richtigen Zeichen erzeugt. (#2909)
...
* Aslan stürzt nun nicht mehr beim Start ab, wenn Irisch als Landessprache eingestellt ist.
...
So werden z. B. die Pfeiltasten in Kombination mit der Aslan-Taste und anderen Umschaltern zum Betrachten von Text verwendet.

Folgende Änderungen wurden an häufig verwendeten Befehlen vorgenommen:

| Name |Tastenkombination|
|---|---|
|Alles ansagen |Aslan+A|
|Aktuelle Zeile lesen |Aslan+L|
|Aktuelle Textauswahl lesen |Aslan+Umschalt+S|
|Statuszeile vorlesen |Aslan+Umschalt+Ende|
...
* Im Lesemodus für MSHTML (z. B. Internet Explorer) und Gecko (z. B. Firefox) wird nun die Existenz langer Beschreibungen gemeldet. Sie können die lange Beschreibung jedoch auch durch Drücken von Aslan+D in einem neuen Fenster öffnen. (#809)
...
* Wenn es Aslan einmal nicht gelingen sollte, eine Sprachausgabe zu laden, sodass es auf eSpeak zurückfallen oder die Sprachausgabe gänzlich deaktivieren muss, wird die Konfiguration nun nicht mehr aktualisiert und Aslan wird erneut versuchen, die ursprünglich eingestellte Sprachausgabe zu laden, wenn es neu gestartet wird. (#2589)
* Wenn es Aslan einmal nicht gelingen sollte, eine Braillezeile anzusteuern, sodass es auf keine Braillezeile zurückfällt, wird die Konfiguration nun nicht mehr aktualisiert und Aslan wird erneut versuchen, die ursprünglich eingestellte Braillezeile anzusteuern, wenn es neu gestartet wird. (#2264)
...
* Wird die automatische Prüfung nach Aktualisierungen deaktiviert, muss Aslan nicht mehr neu gestartet werden, damit die Änderung wirksam wird.
* Aslan wird nun nicht mehr abstürzen, wenn bei der Deinstallation einer Erweiterung deren Ordner nicht gelöscht werden kann, weil er von einem anderen Programm verwendet wird. (#2860)
* Die Beschriftungen der Registerkarten im Einstellungsdialog von Dropbox werden nun im Flächenmodus angezeigt.
* Wenn die Eingabesprache gewechselt wurde, erkennt Aslan diese nun korrekt und kann sie für Befehle und der Eingabehilfe verwenden.
...
* Wenn Sie in Scintilla-Eingabefeldern die Befehle zum Betrachten von Text verwenden oder Zeichen löschen, wird Aslan Multibyte-Zeichen korrekt ausgeben. (#2855)
* Die Installation von Aslan wird nun nicht mehr fehlschlagen, wenn der Name des Benutzerprofilverzeichnisses bestimmte Multibyte-Zeichen enthält. (#2729)
...
* In IBM Lotus Symphony und Apache OpenOffice wird nun der Aslan-Cursor mitgezogen, wenn der System-Cursor bewegt wird.
...
* Bestimmte Microsoft SAPI-5-Stimmen wie z. B. Stimmen von Koba Speech konnten von Aslan nicht verwendet werden. Dies wurde nun behoben. (#2629).
...
* Aslan wird nun nicht mehr abstürzen, wenn asiatische Zeichen in .net-Anwendungen eingegeben werden. (#3005)
* Im Internet Explorer 10 ist es jetzt auch im Standardmodus möglich den Lesemodus von Aslan zu benutzen (Beispiel: Die Anmeldeseite auf [www.gmail.com](http://www.gmail.com)). (#3151)
...
* Sie können nun "comHelper.getActiveObject" verwenden, um ein Com-Objekt von einem normalen Prozess zu erstellen, wenn Aslan mit UIAccess-Berechtigungen ausgeführt wird. (#2483)
...
* Aslan unterstützt nun asiatische Zeichen eingabe mittels IME und Eingabemethoden im Textdienst in allen Anwendungen, einschließlich:
...
* Wenn die Einrastfunktion der Eingabehilfen in Windows aktiviert ist, verhält sich die Aslan-Taste nun wie andere Umschalt-Tasten. Das Heißt, dass Sie die Aslan-Taste nicht mehr gedrückt halten müssen, wenn Sie Aslan-Befehle ausführen möchten. (#230)
* In Microsoft Excel wird nun die automatische Angabe von Spalten- und Reihenüberschriften unterstützt. Drücken Sie Aslan+Umschalt+C, um die Reihe festzulegen, die Spaltenüberschriften enthält; oder Aslan+Umschalt+r um die Spalte festzulegen, die die Reihenbeschriftungen enthält. Drücken Sie die entsprechende Tastenkombination zweimal, um die Zuweisungen zu löschen. (#1519)
* Unterstützung der Braillezeilen Braille Sense, Braille EDGE und SyncBraille der Firma HIMS. (#1266, #1267)
* Wenn in Windows 8 Benachrichtigungen angezeigt werden, werden diese auch von Aslan ausgegeben, sofern die Option "Hilfe-Sprechblasen ansagen" in den Einstellungen zur Objektdarstellung aktiviert ist. (#2143)
 ...
 * Viele Gesten zur Objektnavigation, zum Betrachten von Text und für andere Aslan-Befehle
...
* Es ist nun möglich, Aslan auf die Standard-Einstellungen zurückzusetzen. Dies kann entweder durch dreimaliges Drücken von Aslan+Strg+R oder durch Wählen von "Auf Standard-Einstellungen zurücksetzen" im Aslan-Menü erledigt werden. (#2086)
...
* Wenn Aslan fest installiert ist, kann eine Erweiterung direkt geöffnet werden; entweder vom Explorer aus oder über eine entsprechende Funktion im Browser beim Herunterladen. (#2306)
...
* Im Dialogfeld für die Aslan-Cursor-Einstellungen wurde die Einstellung "Tastaturfokus verfolgen" in "System-Fokus verfolgen" umbenannt, um mit anderen Teilen von Aslan zu harmonieren, in denen diese Namensgebung verwendet wird.
...
* Beim Aktualisieren von Aslan wird die Tastenkombination der Desktop-Verknüpfung nun nicht mehr auf Strg+Alt+N zurückgesetzt, wenn der Anwender diese manuell geändert haben sollte. (#2572)
...
* Da die Aslan-Kurztasten- und Befehlsreferenz nun auch Gesten für Touchscreens enthält, wurde sie in "Befehlsreferenz" umbenannt.
* In der Elementliste merkt sich Aslan nun den eingestellten Elementtyp (z. B. Links, überschriften), solange Sie Aslan nicht neu starten. (#365)
...
* Vor der Ausführung des Aslan-Installationspakets wird nun dessen Integrität geprüft. (#2475)
* Wenn das Herunterladen der temporären Kopie von Aslan während einer Aktualisierung fehlschlägt, wird die temporäre Kopie entfernt. (#2477)
* Wird Aslan mit Administratorrechten ausgeführt, kommt es beim Kopieren der Benutzerkonfiguration zur Systemkonfiguration nun nicht mehr zu Abstürzen. (#2485)
...
* Verbesserungen an der Unterstützung für die Rechtschreibprüfung in Microsoft Word. Dies betrifft z. B. eine bessere Ausgabe von Rechtschreibfehlern und die Benutzung der Rechtschreibprüfung bei der Verwendung einer installierten Aslan-Version unter Windows Vista und neuer.
...
* Die Einstellungen für die Ausgabe von Fortschrittsbalken werden nun auch während der Installation von Aslan oder während der Erstellung einer portablen Version berücksichtigt. (#2574)
* Wenn ein Sicherer Desktop (wie etwa ein Sperrbildschirm) angezeigt wird, können nun keine Aslan-Befehle mehr mit einer Braillezeile ausgeführt werden. (#2449)
* Die Brailleausgabe wird nun im Lesemodus korrekt aktualisiert, wenn sich der anzuzeigende Text geändert hat. (#2074)
* In Sicheren Desktops wie etwa in der Benutzerkontensteuerung werden nun Meldungen ignoriert, die über den Aslan-Controller gesprochen werden oder in Braille angezeigt werden.
...
* Aslan bleibt nicht mehr in Microsoft Word hängen, wenn in einem Absatz numerische Listen- oder Aufzählungszeichen aufeinander folgen, und man versucht mit Strg+Pfeil links oder Pfeil links herauszunavigieren. (#2402)
...
* In Webbrowsern verbirgt Aslan nun Inhalte, die vor Bildschirmlesern versteckt gehalten werden sollen. Dies gilt insbesondere für solche Inhalte, die mit dem Attribut "aria-hidden" gekennzeichnet sind. (#2117)
...
* Wenn in Kombinationsfeldern im Internet Explorer Aslan+Pfeiltaste nach oben gedrückt wird, wird nun korrekterweise der aktuelle Eintrag gelesen, bisher wurden irrtümlich alle Einträge gelesen. (#2337)
...
* Aslan wird nun nicht mehr abstürzen, wenn der Ordnername des Benutzerprofils Multibyte-Zeichen enthält. (#2729)
...
* Aslan funktioniert nun in der Listenansicht der Yahoo Mails ordnungsgemäß, wenn Sie mit dem Internet Explorer arbeiten. (#2780)

### Änderungen für Entwickler

* Beim Start von Aslan wird nun das vorherige Protokoll nach "aslan-old.log" kopiert. Sollte Aslan also einmal abstürzen oder neu gestartet werden müssen, steht das Protokoll der vorigen Aslan-Sitzung zu Inspektionszwecken zur Verfügung. (#916)
* Wenn Sie innerhalb einer Methode chooseAslanObjectOverlayClasses den Objekttyp abrufen wollen, wird dieser nun nicht mehr falsch zurückgegeben, wenn Objekte wie Eingabeaufforderungen oder Scintilla-Steuerelemente den Fokus haben. (#2569)
* Die Untermenüs des Aslan-Menüs sind nun als Attribute von gui.mainFrame.sysTrayIcon verfügbar. Sie heißen preferencesMenu, toolsMenu und helpMenu. Hierdurch ist es für globale Plug-ins einfacher, Einträge in diese Menüs einzufügen.
 ...
 * Dies wird sowohl von Aslan selbst als auch von Erweiterungen unterstützt.
...
* Es ist nun möglich, auf Aslan-interne Module zuzugreifen, wenn diese von Drittanbietern überschrieben wurden. Sehen Sie sich das Modul aslanBuiltin für weitere Informationen an.
...
Schwerpunkte dieser Version sind u. a. eine eingebaute Installationsroutine sowie eine Funktion zum einfachen Erstellen portabler Versionen, eine automatische Aktualisierungsfunktion, die einfache Verwaltung neuer Erweiterungen für Aslan, die Erkennung von Grafiken in Microsoft Word, Die unterstützung von Windows-8-Metro-Anwendungen, sowie einige wichtige Fehlerbehebungen.

### Neue Funktionen

* Aslan kann nun nach Aktualisierungen suchen, diese herunterladen und installieren. (#73)
* Der Umgang mit eigenen Treibern und Plug-ins wurde durch die Einführung eines Erweiterungs-Managers stark vereinfacht. Der Erweiterungs-Manager erlaubt das einfache Installieren und deinstallieren von Erweiterungen (Dateien mit der Erweiterung ".aslan-addon"). Sie finden ihn im Menü Extras im Aslan-Menü. Ältere im Benutzer-Konfigurationsverzeichnis abgelegte Plug-ins und Treiber werden im Erweiterungs-Manager jedoch nicht angezeigt. (#213)
* Viele gebräuchliche Funktionen seitens Aslan funktionieren nun in den Metro-Anwendungen von Windows 8, in sofern sie eine installierte Aslan-Version nutzen. Dies schließt das Sprechen von eingegebenen Zeichen sowie den Lesemodus in der Metro-Version Internet Explorer 10 ein. Portable Versionen können nicht auf Metro-Anwendungen zugreifen. (#1801)
...
* Aslan wird nunmehr in einer einzigen Datei vertrieben. Wenn sie die heruntergeladene Datei ausführen, wird eine temporäre Kopie von Aslan ausgepackt und gestartet und sie werden gefragt, ob Sie Aslana installieren oder eine portable Version erstellen wollen. (#1715)
* Aslan wird nun immer im Ordner "Programme" installiert. Wenn eine frühere Aslan-Version in einem anderen Ordner installiert wurde, wird Aslan im Zuge der Aktualisierung nach "Programme" verschoben.
...
* Problem behoben, wonach sich Aslan in Microsoft Word aufhängt, wenn es länger als zwei Sekunden gebraucht hat, um Text aus einem Dokument anzufordern (z. B. bei sehr langen Zeilen oder Inhaltsverzeichnissen). (#2191)
...
* Das Symbol für Aslan, welches auf dem Desktop sowie im Startmenü angezeigt wird, wird nun in 64-Bit-Versionen von Windows richtig dargestellt. (#354)

### Änderungen für Entwickler

* Da NSIS durch eine Aslan-interne Installationsroutine ersetzt wurde, werden die Meldungen für die Installationsroutine nun nicht mehr in einer Datei namens "langstrings.txt" gespeichert. Vielmehr werden diese nun in der Sprachdatei von Aslan integriert.
...
* Aslan kann nun die Anzahl führender Tabs und Leerzeichen in der aktuellen Zeile in der Reihenfolge angeben, in der sie eingegeben wurden. Dies kann in den Einstellungen zur Dokumentformatierung mit der Option "Zeileneinrückungen ansagen" eingestellt werden. (#373)
* Aslan kann nun Tastendrücke erkennen, die mittels Eingabeemulationen wie etwa bildschirmtastaturen oder Spracherkennungssoftware erzeugt wurden.
...
* Aslan bricht nun standardmäßig Wörter in Braille um. Dies kann in den Braille-Einstellungen ausgeschaltet werden. (#1890, #1946)
...
* Wenn der System-Cursor oder der Aslan-Cursor bewegt wird, wird die Braillezeile so mitbewegt, wie wenn man die Braillezeile manuell weiterbewegen würde. Dies erleichtert das lesen, wenn die Einstellungen Absatzweise lesen und/oder der Wortumbruch aktiviert ist. (#1996)
...
* Im Windows-Explorer in Windows 8 wird der Fokus nun nicht mehr unerwartet aus dem Suchfeld springen. Dieser Fehler machte es unmöglich, mit Aslan mit dem Windows-Suchfeld zu arbeiten.
...
* Die Auswahl von "keine Sprache" im Sprachausgabendialog ist jetzt wieder möglich. Das ist nützlich, falls Aslan nur mit einer Braillezeile oder dem Sprachbetrachter als Ausgabemedium betrieben wird. (#1963)
...
* Wenn in Aslan eine andere Sprache als englisch eingestellt wurde, wird der Name der Tabulator-Taste nun in der richtigen Sprache angesagt.
...
* In Mozilla Firefox 11 funktioniert der Befehl "Zum Inhalt des Lesemodus zurückkehren" (Aslan+Steuerung+Leertaste) nun ordnungsgemäß und verlässt eingebettete Objekte wie Flasch-Inhalte.
* Aslan startet nun korrekt neu, wenn z. B. die eingestellte Sprache geändert und Aslan in einen Ordner installiert wurde, der keine ASCII-Zeichen enthält. (#2079)
...
* Aslan arbeitet nun wieder korrekt mit der Sprachausgabe Audioologic TTS3. (#2109)
...
* Wenn ein Anwender die Aussprache von Satzzeichen und/oder Symbolen ändert, werden die Änderungen ohne Neustart von Aslan bzw. das Deaktivieren des automatischen Sprachenwechsels übernommen.
...
* Es gibt nun eine Remote-Python-Konsole. Diese ist für Situationen gedacht, in denen es sinnvoll ist, Aslan über ein Netzwerk hinweg aus der Ferne zu warten. Weitere Informationen hierzu finden Sie im Aslan-Entwicklerhandbuch.
...
* "TreeInterceptors" und "AslanObjects", die nur eine Seite beinhalten, können automatischen Seitenwechsel während alles lesen unterstützen, wenn sie textInfos.DocumentWithPageTurns verwenden. (#1978)
...
* Aslan kann nun die Stimmen von eSpeak passend zur Dokumentsprache einstellen. Dies gilt für Web- und Pdf-Dokumente. Die Automatische Umschaltung von Sprachen und Dialekten kann über das Dialogfeld für die Stimmen-Einstellungen konfiguriert werden. (#845)
...
* Aslan wird sich selbst neu starten, wenn es abstürzt.
...
* Wenn Sie versuchen, die statuszeile auszulesen (beispielsweise mit der Tastenkombination Aslan+Ende) und es existiert keine Statuszeile, so wird die unterste Zeile des Hauptfensters der aktiven Anwendung ausgelesen. (#649)
* Wenn Sie im lesemodus ein virtuelles Dokument über die Funktion "alles lesen" lesen, wird Aslan nun nach Überschriften eine Pause einlegen, anstatt die Überschrift zusammen mit dem nachfolgenden Text als einen langen Satz zu lesen.
...
* Wenn Sie Aslan neu starten (beispielsweise durch Drücken von Strg+Alt+N wird die laufende Instanz ordnungsgemäß geschlossen, bevor eine neue gestartet wird.
...
* In nicht mehr reagierenden Anwendungen kann sich Aslan nun in viel mehr Fällen wiederherstellen, welches früher zum kompletten Absturz von Aslan führte. (#1408)
* Aslan wird nun nicht mehr im Firefox an Elementen scheitern, die als display:table ausgewiesen sind. (#1373)
* Aslan gibt nun keine Beschriftungen mehr aus, wenn diese den Fokus erhalten. dies verhindert die doppelte ausgabe der Beschriftungen mancher Steuerelemente in Firefox und Internet Explorer. (#1650)
* Aslan liest nun in Excel Zellen korrekt aus, die mit Strg+V eingefügt wurden. (#1781)
...
* Wenn Sie bei aktivierter mausverfolgung den Mauszeiger in ein Eingabefeld bewegen (wie z. B. in den Einstellungen für Synaptics Pointing Device oder SpeechLab SpeakText) bringt Aslan die Anwendung nicht mehr zum Absturz (#672)
* Aslan wertet nun die Dialogfelder für die Infos zahlreicher Anwendungen korrekt aus, die mit Windows XP mitgeliefert werden, wie z. B. das Info-Dialogfeld des Editors oder von Windows. (#1853, #1855)
* Wortweise Navigation in Eingabefeldern behoben. (#1877)
* Wenn Sie sich mit Pfeil nach links, Pfeil nach oben oder seite rauf aus einem Eingabefeld bewegen, während Sie sich im Fokus Modus befinden, wächselt Aslan nun richtig in den Lesemodus, wenn "Bei Bewegungen des System-Cursors automatisch den Fokusmodus einschalten" aktiviert ist. (#1733)

### Änderungen für Entwickler

* Aslan kann nun Sprachausgaben anweisen, die Sprache in bestimmten Textpassagen zu wechseln.
 ...
 * SynthDriver -Objekte sollten zudem das Attribut language als argument an VoiceInfo-objekte übergeben (oder das language-Attribut überschreiben, um die aktuelle Sprache abzufragen). Andernfalls wird die in Aslan eingestellte sprache für die Benutzeroberfläche verwendet.
...
* Aslan sagt nun an, wenn ettwas sortiert wurde (gemäß der "aria-sort-Richtlinie) in Internetbrowsern. (#1500)
* Braillezeichen im Unicode-Format werden jetzt auf der Braillezeile richtig angezeigt. (#1505)
* Wenn sich der Fokus im Internet Explorer und anderen MSHTML-Dokumenten über eine Gruppe von Elementen bewegt, welche von einem Attribut "fieldset" umgeben ist, sagt Aslan den Namen der Gruppe, also die Legende an. (#535)
...
* Beim Navigieren in Microsoft Word-Tabellen mit der Tabulator-Taste, wird Aslan nun die aktuelle Zelle ansagen, wenn sie sich bewegen. (#159)
...
* Aslan kann nun auf Farb- und Formatierungsinformationen von Texten zugreifen, welche direkt ausgegeben wurden.
* In der Nachrichtenansicht von Outlook Express/Windows Mail/Windows Live Mail sagt Aslan nun an, wenn eine Nachricht ungelesen und im Falle einer Nachrichtenkonversation ausgeklappt oder zusammengeschoben ist. (#868)
...
* Um den System-Cursor zum Aslan-Cursor zu bewegen, führen Sie die Funktion "Fokus zum Navigator ziehen" (desktop Aslan+Umschalt+Nummernblock Minus, Laptop Aslan+Umschalt+Rücktaste) zwei Mal aus. Dies gibt mehr Tastenkombinationen für die künftige Verwendung frei. (#837)
...
* In Dokumentation von Aslan und in der oberfläche ist nun nicht mehr die Rede von virtuellen Puffern oder virtuellen Ansichten, sondern allenfalls noch vom Lesemodus, da der Begriff virtuelle Puffer oder virtuelle ansichten für normale anwender bedeutungslos ist. (#1509)
...
* Wenn Sie sich anmelden, wird der Aslan-Dienst Aslan nun nicht mehr starten und beenden.
* Auch wenn die UIA in Windows XP und Windows Vista über eine Aktualisierung nachinstalliert werden kann, macht Aslan unter diesen Systemen keinen Gebrauch davon. Obwohl die Verwendung der UIA die Zugänglichkeit moderner Anwendungen verbessert, kommt es unter Windows XP und Windows Vista sehr oft zu Abstürzen oder Leistungseinbußen. (#1437)
...
* Aslan gibt nun den status eines Containers zurück, wenn sich der fokus auf ein Steuerelement innerhalb eines Containers bewegt. Wenn sie z. B. in ein html-Dokument springen, noch bevor es vollständig geladen wurde, wird dessen Status als "Beschäftigt" angezeigt.
* Die Benutzeroberfläche und die Dokumentation von Aslan verwenden nun nicht mehr die Begriffe "erstes Kind-Objekt" und "ElternObjekt", da diese im Zusammenhang mit objektnavigation verwirrend sein können.
* Der Status "reduziert" wird nun nicht mehr für Menüpunkte angezeigt, die ein Untermenü besitzen.
* Das Skript reportCurrentFormatting (Aslan+F) zeigt nun nicht mehr die Formatierungen am System-cursor, sondern die formatierungen am Aslan-Cursor an. Die meisten Anwender werden keinen Unterschied bemerken, weil der Aslan-Cursor standardmäßig dem System-Cursor folgt. Jetzt ist es jedoch möglich, z. B. im Flächenmodus die Formatierungen bestimmter Bildschirmbereiche abzufragen.

### Fehlerbehebungen

* Wenn Sie mit Aslan+Leertaste den Fokus-modus erzwungen haben und ein Kombinationsfeld schließen, wird nun nicht mehr zurück in den Lesemodus gewechselt (#1386)
...
* Bei Verwendung des Laptop-Tastaturschemas arbeitet die Tastenkombination Aslan+Entf jetzt wie dokumentiert und gibt die Größe und Position des aktuellen Navigator-Objekts zurück. (#1498)
* Aslan verarbeitet nun das "aria-selected"-Attribut korrekt.
...
* Im Lesemodus behandelt Aslan schreibgeschützte Eingabefelder nun korrekt.
* Im Lesemodus schaltet Aslan nun nicht mehr in den Fokusmodus um, wenn "aria-activedescendant" gesetzt ist; z. B. wenn die Liste mit den Einträgen zur autovervollständigung bei einem Eingabefeld erscheint, das dies unterstützt).
...
* Wenn Sie beim Lesen von Text auf eine Liste stoßen, sagt Aslan nun z. B. "Liste mit 5 Einträgen" statt "Listemit 5 Einträgen" (#1515)
...
* Wenn bei aktivierter Eingabehilfe eine Umschalttaste gedrückt gehalten wird, wird diese nicht mehr als Modifikator behandelt, der sich selbst modifiziert (wie z. B. Aslan+Aslan).
...
* Wenn ein Meldungsfenster angezeigt wird, können Sie nun keine Aslan-Einstellungsdialoge mehr aufrufen, weil sich die Einstellungsdialoge sonst festfahren. (#1451)
* In Microsoft Excel fährt sich Aslan nun nicht mehr fest, wenn sie die Tasten(kombinationen) zum Navigieren zwischen bzw. zum Markieren von Zellen in Dauerfunktion verwenden.
* Problem behoben, wonach sich der Aslan-Dienst in sicheren Desktops beendet hat.
...
* Das Fenster mit heruntergeladenen und herunterladenden Dateien im Internet explorer 9 kann nun mit Aslan gelesen werden. (#1280)
* Es ist nun nicht mehr möglich, versehentlich mehrere Aslan-Instanzen zu starten. (#507)
* Auf langsamen Systemen zeigt Aslan nun nicht mehr sein Hauptfenster ständig an. (#726)
* Beim Start von WPF-Anwendungen unter Windows XP stürzt Aslan nun nicht mehr ab. (#1437)
...
* Aslan zeigt nun die Empfänger in den Feldern "Kopie" und "Blindkopie" korrekt an. (#421)
...
* Aslan zeigt nun die neue ausgewählte Tabellenzelle korrekt an, nachdem Sie Inhalte über die Zwischenablage ausgeschnitten oder eingefügt haben. (#1567)
...
* In Microsoft Word 2010 liest Aslan nun Bestätigungsdialoge automatisch vor. (#1538)
...
* Wenn Sie Aslan auf einer Hongkong-Version von Windows Vista oder Windows 7 installieren, zeigt das Installationsprogramm nun keinen unsinnigen Text mehr an. (#1596)
...
* Aslan fährt sich im Internet explorer oder anderen mshtml-Dokumenten nun nicht mehr in Eingabefeldern fest, wenn Braille aktiviert ist.
...
* In den Dialogen zum Verschieben und kopieren fährt sich Aslan nun nicht mehr fest. (#574)
* Aslan wird nun den Fokus in der Nachrichtenliste von Outlook 2010 korrekt verfolgen. (#1285)
...
* Wenn Aslan nicht starten kann und die Wiedergabe des Klangs für kritische Fehler nicht möglich ist, wird keine Fehlermeldung mehr in die Protokolldatei geschrieben.
...
* In den unterschiedlichen Sprachräumen können jetzt Beschreibungen für Sonderzeichen bereitgestellt werden. Sehen Sie sich den Abschnitt "Beschreibungen der Sonderzeichen" im Aslan-Entwicklerhandbuch an. (#55)
* In den einzelnen Sprachräumen können nun Regeln für die aussprache von Satzzeichen und anderen Symbolen bereitgestellt werden. Sehen Sie sich den Abschnitt "Aussprache von Symbolen" im Aslan-Entwicklerhandbuch an. (#332)
* Der AslanHelper kann nun so erstellt werden, dass er zahlreiche Informationen zur Fehlerbehebung enthält. Verwenden Sie hierfür die SCons-variable aslanHelperDebugFlags. Weitere Informationen hierzu finden Sie in der Datei readme.txt im Stammverzeichnis des Quellcodes (#1390).
...
Dieses Release behebt wichtige Fehler und Sicherheitsprobleme, welche in Aslan 2011.1 gefunden wurden.
...
* Es ist nun nicht länger möglich die benutzerdefinierte Konfiguration von Aslan während dieser Sicherheitsebenen zu kopieren oder einzufügen. Dies stellt ebenso ein Sicherheitsrisiko dar. (#1421)
* In Firefox 4 funktioniert nun der Befehl "Aslan+Strg+Leertaste" für die eingebetteten Inhalte in der Virtuellen Ansicht. (#1429)
* Bei aktivierter Ansage der Funktionstasten werden nun die großgeschriebenen Zeichen nicht länger falsch angesagt. (#1422)
* Wenn bei aktivierter Ansage der Funktionstasten die Leertaste mit der Aslan-Taste gedrückt wird, wird nun nicht mehr die Taste umgeschaltet, sondern dies auch mitgeteilt. (#1424)
...
* Aslan zeigt nun neu erscheinenden Text in Terminalfenstern korrekt an. Dies betrifft u. a. mIRC, PuTTY, Tera Term und SecureCRT. (#936)
* Nun kann der Anwender neue Aslan-Tastenkombinationen hinzufügen oder vorhandene ersetzen, in dem er eine einfache Zuordnung von Benutzereingaben bereitstellt. (#194)
* Unterstützung für globale Plugins. Diese können verwendet werden, um Aslan mit neuen Funktionen anwendungsübergreifend zu erweitern. (#281)
...
* Um Drittentwicklern die Arbeit zu erleichtern, können jetzt sowohl Anwendungsmodule als auch globale Erweiterungen neu geladen werden, ohne dass Aslan neu gestartet werden muss. Verwenden Sie hierzu den Befehl "Plugins neu laden" aus dem Menü "Extras" oder drücken Sie die Tastenkombination Aslan+Strg+F3. (#544)
* Aslan merkt sich nun die aktuelle Position, wenn Sie in einem Web-Browser zur letzten besuchten Seite wechseln. Dies gilt so lange, bis Sie entweder Aslan oder den Browser beenden. (#132)
...
* Wenn Aslan erkennt, dass es "sich aufgehängt hat", werden weiterhin Aslan-Tasten verarbeitet, auch wenn alle anderen Tastenkombinationen an das System weitergereicht werden. Dies verhindert, dass der Anwender unabsichtlich z. B. die Dauergroßschreibung umschaltet. (#939)
...
* Wenn ein Aslan-Modifikator zwei Mal gedrückt (und beim zweiten Mal gedrückt gehalten) wird, werden alle Tastenanschläge einschließlich Wiederholungen ebenfalls an die anwendung durchgereicht.
...
* Im Aslan-Einstellungsmenü wurden die Kurztasten für die Einstellen für Braille und nVDA-Cursor geändert, um Konflikte auszuschließen.
...
* Wenn ein Aslan-Modifikator zwei Mal schnell hintereinander gedrückt wird, jedoch eine andere Taste zwischendurch gedrückt wird, wird beim 2. Tastendruck des Modifikators nicht die eigendliche Funktion der Taste ausgeführt.
...
* Der Schalter "Aktuell gespeicherte Einstellungen im Anmeldebildschirm und bei Sicherheitsmeldungen verwenden (erfordert Administrationsberechtigungen!)" im Dialogfeld "Allgemeine Einstellungen" funktioniert nun auch dann, wenn er gleich nach der Aslan-Installation, aber noch vor der anzeige eines Sicherheitshinweises benutzt wird. Vorher zeigte Aslan zwar an, das Kopieren der Einstellungen sei erfolgreich gewesen, tatsächlich hatte der Schalter jedoch keinerlei wirkung. (#1194)
...
* Wenn Ihr Benutzername ein Leerzeichen enthält, tritt bei aktivierter Benutzerkontensteuerung nach dem Schließen des Dialogfensters keinen Fehler mehr auf, wenn Sie in den Allgemeinen Einstellungen von Aslan den schalter "Momentan gespeicherte Einstellungen für Anmeldedialoge und Sicherheitshinweise verwenden" drücken. (#918)
* Anstatt leere Links anzuzeigen, benutzt Aslan im Internet Explorer und anderen MSHTML-Dokumenten nun die Zieladresse des Dokuments als Linkbeschriftung. (#633)
* In den Menüs vom AOL Instant Messenger 7 ignoriert Aslan nun nicht weiter den Fokus. (#655)
* Bei der Rechtschreibprüfung von Microsoft Word zeigt Aslan nun die korrekte Fehlerbeschreibung an (nicht im Wörterbuch, Gramatikfehler, Zeichensetzung). Bisher wurden alle Rechtschreibfehler als Gramatikfehler gekennzeichnet. (#883)
...
* Wenn in Gecko2-Dokumenten (Firefox 4) mehrere Registerkarten geöffnet sind, werden Änderungen auf dynamischen Webseiten nun korrekt von Aslan wiedergegeben. Bisher wurden nur Änderungen auf der ersten Registerkarte wiedergegeben. (Mozilla Fehlerbericht 610985)
* Aslan kann jetzt die Vorschläge bei der Grammatik- und Rechtschreibprüfung in Microsoft Word korrekt anzeigen. (#704)
* Im Internet Explorer und anderen MSHTML-Dokumenten zeigt Aslan nun nicht mehr die Anker als normale Links an. Ab sofort sind diese verborgen. (#1326)
* Der Umgang mit Gruppenfelder bei der Objektnavigation ist jetzt ohne Fehler möglich.
* In Mozilla Firefox und anderen Gecko-basierten Dokumenten bleibt Aslan nun nicht mehr hängen, falls ein Rahmen eher geladen wurde als das restliche Dokument.
* Wenn Sie in Zeichen mit der Entf-Taste des Nummernblocks löschen, sagt Aslan nun das nächste zeichen korrekt an. (#286)
...
* Die Tasten der Braillezeilen der ALVA-Serien BC640 / BC680 werden von Aslan beim nachfolgenden Tastendrücken nicht länger ignoriert.
...
* Aslan schaltet nun auf den vorher eingestellten Treiber der Braillezeile um, wenn Sie die gespeicherte Konfiguration zurücksetzen. (#1346)
* Die Projektverwaltung von Visual Studio 2008 wird wieder korrekt erkannt. (#974)
* Aslan wird nun nicht mehr abstürzen, wenn Sie mit Anwendungen arbeiten, in deren Dateinamen nicht-Ascii-Zeichen vorkommen. (#1352)
* Bei der zeilenweisen Navigation in AkelPad wird Aslan nicht mehr das erste Zeichen der folgenden Zeile lesen, wenn der Wortumbruch aktiviert ist.
...
* Problem behoben, wonach einige Braillezeilen nicht richtig geleert wurden, wenn Aslan beendet oder die Braillezeile gewechselt wurde.
* das fokussierte objekt wird beim Start von Aslan nicht mehr zweimal gelesen. (#1359)
...
* In den einzelnen Gebietsschemen können jetzt individuelle Zuweisungen der Eingabemethoden erstellt werden, die neue Eingabemethoden definieren oder vorhandene überschreiben können. Diese so erstelten Eingabemethodenzuweisungen sind überall in Aslan wirksam. (#810)
...
* Das neue "LiveText und Terminal AslanObject" erleichtert das automatische Ansagen von neuen Texten. Lesen Sie im Abschnitt "AslanObjects.behaviors" zu diesen Klassen für Details. (#936)
* Die Overlay-Klasse "AslanObjects.window.DisplayModelLiveText" kann für Objekte verwendet werden, die den anzuzeigenden Text direkt vom Bildschirm abfangen müssen.
...
* Unterstützung für globale Erweiterungen hinzugefügt, die Anwendungsübergreifend Skripte zuweisen, "AslanObject"-Ereignisse verarbeiten und "AslanObject"-Overlay-Klassen auswählen können. (#281)
...
* Braillezeilentreiber wurden so erweitert, dass Tasten, Rädchen und andere Steuerelemente an Aslan-Skripte zugewiesen werden können:
 * Treiber können eine globale Eingabemethodenzuweisung bereitstellen, um Zuweisungen für Skripte an beliebiger Stelle inAslan hinzuzufügen.
...
* Die Navigation durch Objekte mit dem Aslan-Cursor wurde enorm vereinfacht. Der Aslan-Cursor lässt Objekte aus, die dem Anwender nichts bringen. So werden Objekte, die zu Strukturierungszwecken verwendet werden oder nicht verfügbar sind, ausgeblendet.
...
* Wenn Sie die Maus über Zellen in Microsoft Excel bewegen, wird Aslan diese nun ansagen.
...
* Den meisten Text am Bildschirm kann nun Aslan vorlesen. (#40, #643)
 * Dies ermöglicht Aslan, Objektinhalte auch dann auszulesen, wenn sie nicht auf direktem Wege gewonnen werden können.
...
* Frühe, experimentelle Unterstützung von Google Crome. Bitte bedenken Sie, dass Google Chromes Unterstützung für Bildschirmleser noch in der Entwicklungsphase steckt. Auch Aslan-seitig ist noch Arbeit nötig, um Google Crome zu unterstützen. Sie benötigen die neueste Entwicklerversion von Google Chrome.
 ...
 * Aslan+Nummerntaste 7 schaltet in den Flächenmodus um und positioniert den Aslan-Cursor an die Stelle des aktuellen Objekts. Dies ermöglicht bildschirmorientierte Navigation im gesamten Bildschirm oder im aktuellen Dokument mit Hilfe der Kommandos für die Textnavigation.
 * Aslan+Nummerntaste 1 zieht den Aslan-Cursor zum objektbasierten Text der Position vom Anzeige-Cursor, um so das Navigieren des Objekts an dieser Stelle zu ermöglichen.
...
* Die Funktionen "Alle Objekte vorlesen" (Aslan+Nummernblocktaste), rekursiv zum nächsten Navigator-Objekt springen (Aslan+Umschalt+Nummerntaste 6) und rekursiv zum vorigen Objekt springen (Aslan+Umschalt+Nummerntaste 4) wurden entfernt, da sie fehlerhaft arbeiteten und um die Tastenkombinationen für andere Funktionen freizugeben.
...
* In eingebetteten Anwendungen oder Virtuellen Ansichten (z. B. Adobe Flash) können Sie jetzt Aslan+Strg+Leertaste drücken, um aus der eingebetteten Anwendung oder der Virtuellen Ansicht in das übergeordnete Dokument zu wechseln. bisher wurde Aslan+Leertaste hierfür verwende. Jetzt dient Aslan+Leertaste nur noch dazu, zwischen Fokus- und Lesemodus umzuschalten.
...
* Wenn Sie in Microsoft Excel auf ein Arbeitsblatt wechseln, auf dem mehr als eine Zelle markiert ist, wird nun von Aslan der gesamte markierte Bereich angezeigt und nicht nur die aktive Zelle. (#763)
...
* Wenn Aslan bereits läuft, wird es beim Anklicken des Desktop-Symbols oder Drücken der Tastenkombination Strg+Alt+N neu gestartet.
* Das Kontrollfeld "Objekt unter Mauszeiger ansagen" (Aslan+M) wurde in "Mausverfolgung einschalten" umbenannt.
...
* Wenn Skype bereits läuft, während Aslan gestartet wird, ist es nun nicht mehr nötig, Skype neu zu starten, um die Barrierefreiheits-Optionen zu aktivieren. Dies könnte auch auf andere Anwendungen zutreffen, die abfragen können, ob ein bildschirmleser läuft.
* Wenn Sie in Office-Anwendungen die Tastenkombination Aslan+B drücken oder durch Symbolleisten navigieren, wird Aslan nun nicht mehr abstürzen. (#616)
...
* Wenn Sie sich In Scintilla-Steuerelementen wie notepad++ den gesamten Text vorlesen lassen und Aslan den System-Cursor bewegt, wird nun kein Text mehr markiert. (#746)
* Es ist nun wieder möglich, sich mit dem Aslan-internen Cursor den Inhalt von Tabellenzellen in Microsoft Excel anzeigen zu lassen.
* In einigen problematischen mehrzeiligen Eingabefeldern in Internet Explorer 8 kann Aslan nun den Text zeilenweise auslesen. (#467)
* Windows Live Messenger 2009 stürzt nun nicht mehr nach dem Start ab, wenn Aslan läuft. (#677)
...
* Aslan funktioniert nun im System korrekt, wo das Arbeitsverzeichnis des Suchpfades für Programmbibliotheken entfernt wurde (). (einzustellen im Registrierungseintrag "CWDIllegalInDllSearch" auf "0xFFFFFFFF"). Beachten Sie, dass dies nicht für die meisten Nutzer relevant ist. (#907)
...
* Wenn die Befehle zur Tabellennavigation nicht ausgeführt werden können, weil sich der Cursor an einer Ecke der Tabelle in Microsoft Word befindet, sagt Aslan "Ecke der Tabelle" in der eingestellten Sprache und nicht mehr in englisch an. (#921)
...
* Auf Systemen ohne jegliche Audio-Ausgabe schlägt der Start von Aslan jetzt nicht mehr fehl. In solchen Fällen benötigen Sie entweder eine Braillezeile oder den Dummy-Synthesizer "Keine Sprachausgabe" im Zusammenspiel mit dem Sprachbetrachter, um Aslan effektiv nutzen zu können. (#425)
...
* Wenn die Ansage von Funktionstasten aktiviert ist, erkennt Aslan jetzt auch Multimedia-Tasten wie Abspielen, Anhalten, Nächster bzw. Vorheriger Track, etc. (#472)
* In Eingabefeldern, die dies unterstützen, sagt Aslan beim Löschen mit Strg+Rücktaste das gelöschte Wort an. (#491)
...
* Ein neues Skript (Aslan+Umschalt+Nummerntaste Minus) kann verwendet werden, um den System-Cursor zum aktuellen Navigator-Objekt zu ziehen.
...
* Es ist nun möglich, bei der Installation von Aslan die Erstellung der Desktop-Verknüpfung zu verhindern. (#518)
* Aslan kann nun iAccessible2 auch auf 64-Bit-Systemen verwenden, sofern es dort verfügbar ist. (#479)
* Unterstützung für Live-Regionen in Mozilla-Anwendungen erweitert. (#246)
* Die Aslan-API wird nun bereitgestellt, was es Programmierern erlaubt, Aslan zu Steuern. Hierzu gehören z. B.: Das Sprechen von Text, das Stummschalten der Sprache, das Anzeigen von Blitzmeldungen, etc.
...
* Wenn die Anzeige dynamischer Inhalte aktiviert ist, werden nun auch in Miranda IM neu eintreffende Nachrichten in Unterhaltungsfenstern angezeigt. Zudem können die drei letzten Nachrichten abgerufen werden (Aslan+Strg+Ziffer). (#546)
...
* Die Kopplung der Braillezeile (Aslan+Strg+T) kann nun auch in den Braille-Einstellung eingestellt werden. Diese Einstellung wird nun auch in der benutzerspezifischen Konfiguration gespeichert.
...
* Der Start von Aslan wird nun nicht mehr fehlschlagen, wenn UI automation zwar vorhanden ist, dessen Iinitialisierung jedoch aus irgendeinem Grund fehlschlug. (#483)
...
* Aslan wird sich nun nicht mehr aufhängen, wenn sie einen Baumknoten erweitern, der sehr viele Unterknoten enthält.
...
* Mehrere Sicherheitsprobleme beim Ausführen von Aslan in Anmeldefenstern oder sicheren Desktops behoben. (#515)
...
* Wenn Sie in Firefox 3.6 Links aktivieren, deren Ziel sich auf der selben Seite befindet (z. B. Links zum Überspringen von Navigationsleisten), platziert Aslan nun den Cursor an die richtige Stelle im virtuellen Dokument.
...
* Wenn Sie in Windows Update unter Windows XP Kontrollkästchen aktivieren, bleibt Aslan nun nicht mehr hängen. (#477)
...
* Aslan sagt nun nicht mehr an, wenn firefox beschäftigt ist (z. B. wenn eine Seite oder der Browser aktualisiert wird) während sich das betreffende Dokument im Hintergrund befindet. Dies bewirkte eine irrtümliche Ansage der Statuszeile der aktiven Anwendung.
* Wenn Sie mit Alt+Umschalt bzw. Strg+Umschalt das Tastaturschema ändern, zeigt Aslan das neu eingestellte schema nun auch in Braille an. Bisher wurde das Schema nur angesagt.
...
* Der Dialogfeld "Über Aslan" erscheint nun in der benutzerspezifischen Sprache. (#586)
...
* Fehler in einigen fehlerhaften SAPI4-Treibern umgangen, wonach Aslan regelmäßig abstürzte. (#597)
...
* Aslan unterbricht nun nicht mehr die Sprache, wenn Sie die Tasten zur lautstärkeregelung drücken. Dies erlaubt Ihnen, die lautstärke einzustellen und das eigentliche Ergebnis zu hören. (#287)
...
* Wenn Sie sich im Windows Vista auf einem sicheren Desktop befinden (wie z. B. die Benutzerkontensteuerung oder wenn Sie Strg+Alt+Entf drücken), wird Aslan dies nun melden.
* In der Eingabeaufforderung kann Aslan nun den Text unter dem Mauszeiger ansagen.
* Unterstützung der UIA in Windows 7.
* Aslan kann nun so konfiguriert werden, dass Aslan nach der Anmeldung automatisch startet. Die entsprechende Option befindet sich im Einstellungsdialog "Allgemein".
* Aslan kann nun auch geschützte Desktops auslesen (Anmeldebildschirm, Benutzerkontensteuerung usw). Die Optionen zum Aktivieren von Aslan für die Anmeldung sind im Einstellungsdialog "Allgemein" zu finden. (#97)
...
* Portable Aslan-Versionen suchen nun im Unterordner "userconfig" nach ihrer benutzerspezifischen Konfiguration. Die trennt - wie bei der fest installierten Version - die Benutzerdefinierte Konfiguration von Aslan.
...
* Virtuelle Dokumente werden nun im Hintergrund verarbeitet. dies erlaubt es dem Benutzer, mit dem Dokument zu arbeiten, während es noch geladen wird. Wenn das Laden des Dokuments länger als eine Sekunde dauert, wird dies von Aslan gemeldet.
* Fährt sich Aslan aus irgendeinem Grund fest, werden alle Tastenanschläge an die anwendung durchgereicht, sodass der Anwender die Möglichkeit hat, das System wiederherzustellen.
* Unterstützung für "Ziehen und Ablegen" in ARIA-Dokumenten in Firefox (#239)
* Wenn Sie in ein virtuelles Dokument wechseln, werden dokumenttitel und aktuell markierter Text gesprochen. Hiermit stimmt das Verhalten von Aslan in virtuellen Dokumenten mit dem in normalen Dokumentobjekten überein. (#210)
* Wenn Sie in einem virtuellen Dokument die Eingabetaste drücken, während sich der Fokus auf einem Objekt befindet, können Sie nun mit eingebetteten Objekten interagieren. Falls das Objekt zugänglich ist, können Sie sich mit Tab und umschalt+tab hindurchbewegen. Um zurück in das virtuelle Dokument zu wechseln, drücken Sie Aslan+Leertaste. (#431)
* In virtuellen Dokumenten können Sie nun mit O und Umschalt+O zwischen eingebetteten Objekten wechseln.
* In Windows Vista und neuer kann Aslan nun auch auf Anwendungen zugreifen, die Sie als Administrator ausführen. Sie müssen eine offizielle Aslan-Version installieren, damit dies funktioniert. Es funktioniert nicht mit tragbaren Versionen oder Entwicklerversionen. (#397)

### Änderungen

* Wenn Aslan startet, sagt er nicht mehr "Aslan ist bereit".
* Zum Abspielen der Klänge beim starten und beenden von Aslan wird nun das eingestellte audio-Ausgabegerät verwendet, und nicht mehr das Windows-Standardaudiogerät. (#164)
* Die Ansage von Fortschrittsbalken wurde verbessert. Sie können nun Aslan so konfigurieren, dass Fortschrittsbalken sowohl durch Signaltöne als auch durch Sprache angekündigt werden.
...
* Der Befehl Aslan+F10 schließt nun die aktuelle Position des Aslan-Cursors beim Kopieren mit ein. Hierdurch ist es nun nach möglich, das letzte Zeichen einer Zeile mitzukopieren, was vorher nicht möglich war. (#430)
* Das Skript navigatorObject_where (Strg+Aslan+Nummernblock 5) wurde entfernt. Die Tastenkonbination hat auf einigen Tastaturen nicht funktioniert. Außerdem war dieses Skript auch nicht besonders nützlich.
* Das Skript "navigatorObject_currentDimentions" wurde an Aslan+Nummernblock Entf zugewiesen. die alte Tastenkombinationen funktionierte auf einigen Tastaturen nicht. Das Skript gibt nun die Breite und Höhe des aktuellen Objekts anstelle der Koordinaten für die rechte untere Ecke zurück.
...
* Der Klang für Fehler wird in offiziellen Aslan-Versionen nicht mehr abgespielt, Fehler werden aber dennoch protokolliert.

### Fehlerbehebungen

* Wenn Aslan von einem Pfad mit kurzem Dateinamen asgeführt, jedoch in einen Pfad mit langem Dateinamen installiert wurde (z. B. PROGRA~1 vs. Programme) findet er nun seine benutzerspezifischen Einstellungen.
* Wenn Sie sich in einem Menü befinden, funktioniert Aslan+T (Titel lesen) nun korrekt.
...
* Bessere Unterstützung die Eingabeaufforderung. Wenn Sie Strg+Pause drücken, wird Aslan nicht mehr beendet.
* In Windows Vista und neuer wird Aslan nun mit normalen Benutzerrechten gestartet, wenn Sie angewiesen haben, Aslan nach der Installation zu starten.
...
* Wenn Sie auf Webseiten wie https://tigerdirect.com/ der Fokus bewegen und ein Eingabefeld dadurch seinen Wert ändert, schaltet Aslan nun nicht mehr irrtümlich in den Fokusmodus um. (#220)
* Aslan versucht nun, sich aus manchen Situationen zu retten, in denen er sich früher komplett festgefahren hat. Eine derartige Rettungsaktion kann bis zu 10 Sekunden dauern.
* Wenn Sie die Aslan-Sprache auf "Benutzerspezifische Standard-Einstellung" gesetzt haben, wird nun die anzeigesprache anstelle der Länder-Einstellung verwendet. (#353)
* Aslan erkennt nun auch die Steuerelemente in AIM 7.
* Beim ausführen des Befehls zum Speichern von Tastenanschlägen bleiben nun keine Umschalttasten mehr hängen. Früher musste Aslan in einer solchen Situation neu gestartet werden. (#413)
* Wenn die Taskleiste den Fokus erhält, wird dies nicht mehr länger igoriert. dies geschieht oft, wenn sie eine Anwendung beenden. Früher verhielt sich Aslan so, als hätte sich der Fokus überhaupt nicht bewegt.
* Wenn die ansage von Zeilennummern aktiviert ist, arbeitet Aslan nun auch in Eingabefeldern von Java-Anwendungen wie OpenOffice.org korrekt.
* Der Befehl Aslan+F10 behandelt nun auch den Fall korrekt, wenn die endmarke vor der Startmarke gesetzt wird. Früher führte dies in Anwendungen wie Notepad++ zu Abstürzen.
...
* Der Befehl Aslan+Umschalt+Pfeil hoch (markierten Text vorlesen) behandelt nun auch den Fall korrekt, wenn er auf Objekte angewendet wird, die das Markieren von Text nicht unterstützen.
* Problem behoben, wonach Aslan abstürzte, wenn man im Miranda-im bestimmte Schaltflächen drückt. (#440)
...
* Da die Formularleiste von Microsoft Excel unzugänglich ist, stellt Aslan ein eigenes Dialogfeld zum Bearbeiten von Zell-Inhalten bereit, wenn Sie F2 drücken.
...
* Aslan kann nun so konfiguriert werden, dass entweder alle oder nur sichtbare Fortschrittsanzeigen durch Signaltöne angezeigt werden. Alternativ kann auch eingestellt werden, dass der Fortschritt alle 10% angesagt wird.
* In Richedit-steuerelementen können nun auch Links erkannt werden.
* Die maus kann nun auf das Zeichen unter dem Aslan-Cursor bewegt werden. Zuvor konnte die Maus lediglich in die Mitte eines Objekts bewegt werden.
* In virtuellen Puffern bezieht sich nun die Aslan-Corsor-Navigation auf den Inhalt des virtuellen Puffers und nicht mehr auf den Inhalt des aktuellen Navigator-Objekts. Das bedeutet, dass Sie die Tasten(Konbinationen) zur Navigation innerhalb eines Objekts auch innerhalb virtueller Dokumente benutzen können.
* Einige neue Zustände von Java-Steuerelementen werden nun erkannt.
* Wird der Befehl "Titel ansagen" Aslan+T zweimal ausgeführt, wird der Titel buchstabiert. Wird er dreimal ausgeführt, wird der Titel in die Zwischenablage kopiert.
...
* Aslan+C hinzugefügt, das den Inhalt der Zwischenablage zurückgibt. (#193)
* Wenn Aslan in virtuellen Puffern automatisch in den Fokusmodus wechselt, können Sie durch Drücken von ESC wieder in den Lesemodus wechseln. Die Tastenkombination Aslan+Leertaste existiert aber trotzdem noch.
* Je nachdem, auf welche Art von Steuerelement der Fokus beim Navigieren in virtuellen Puffern trifft, schaltet Aslan nun in den Fokus- bzw. Lesemodus um. Dies kann in den Einstellungen für virtuelle Puffer konfiguriert werden. (#157)
* Der Treiber für SAPI4 wurde neu geschrieben und beseitigt damit Probleme, die den alten Treibern noch inne wohnten.
* Die Anwendung Aslan enthält nun ein Manifest, d. h., sie wird unter Windows Vista nicht mehr im Kompatibilitätsmodus ausgeführt.
* Die Konfigurationsdateien und Wörterbücher werden nun im benutzerspezifischen Konfigurationsverzeichnis gespeichert, wenn Aslan installiert wird. Dies ist einerseits unter Windows Vista nötig geworden, ermöglicht andererseits mehreren Benutzern, eine individuelle Aslan-Konfiguration zu unterhalten.
* Unterstützung für Positionsinformationen von iAccessible2-Steuerelementen.
* Möglichkeit des Kopierens von Text mit Hilfe des Aslan-Cursors hinzugefügt: Aslan+F9 setzt an der aktuellen Position eine Startmarke, Aslan+F10 kopiert den Text zwischen der Startmarke und der aktuellen Position des Aslan-Cursors in die Zwischenablage. (#240)
* Unterstützung einiger Eingabefelder in der TV-Software von Pinnacle.
* Wurden mehr als 512 Zeichen markiert, sagt Aslan die Anzahl der markierten Zeichen an, anstatt den gesamten Text vorzulesen. (#249)

### Änderungen

* Wenn Aslan so eingestellt wird, dass er das Windows-Standardaudiogerät benutzen soll (Microsoft Windows sound mapper), schaltet er automatisch auf das neue Ausgabegerät um, wenn es sich ändert (wenn Sie z. B. ein USB-Audiogerät anschließen).
...
* Wenn Aslan auf einem Remote desktop-Server benutzt wird, wird das Ende der Audioausgabe nun nicht mehr abgeschnitten.
...
* Der Aslan-Protokollbetrachter benutzt nun ein RichEdit-Eingabefeld anstelle eines normalen Eingabefeldes. dies erleichtert das wortweise lesen.
* Probleme mit eingebetteten Objekten in RichEdit-Objekten behoben.
* Aslan liest nun die Seitenzahlen in Microsoft word. (#120)
...
* Problem behoben, wonach das Lesen von markiertem Text in Eingabefeldern unmittelbar nach dem Start von Aslan nicht funktionierte.
...
* Aslan ignoriert nun Fokusänderungen im Hintergrund. Früher wurden diese so behandelt, als hätte sich der echte Fokus geändert.
* Fokuserkennung beim Verlassen von Kontextmenüs verbessert. Früher hat Aslan überhaupt nicht mehr reagiert, wenn Sie ein Kontextmenü verlassen haben.
* Aslan erkennt nun, wenn Sie innerhalb des Startmenüs ein Kontextmenü öffnen.
...
* Aslan sagt nun nicht mehr das Demarkieren von Text an (z. B. in der Adressleiste von Internet Explorer oder in Adressfeldern von Thunderbird 3).
...
* Aslan liest nun Eingabefelder mit mehr als 65535 Zeichen korrekt (z. B. große Dateien im Editor).
* Das Lesen von MSHTML-Eingabefeldern wurde verbessert (z. B. Eingabefelder in html-Dokumenten im Internet Explorer).
* Aslan stürzt nun nicht mehr ab, wenn Sie Text in OpenOffice.org bearbeiten. (#148, #180)
...
* Tastenkombinationen für viele Konfigurationsdialoge von Aslan hinzugefügt.
* Unterstützung von iAccessible2-Anwendungen wie Firefox hinzugefügt, wenn Aslan von einem tragbaren Medium aus gestartet wird, ohne DLL-Dateien registrieren zu müssen.
* Problem behoben, wonach in virtuellen Gecko-Dokumenten (Firefox) die Linkliste abstürzte. (#48)
* Aslan sollte im Zusammenhang mit Gecko-Anwendungen wie Firefox oder Thunderbird nicht mehr abstürzen, wenn es mit anderen Rechten ausgeführt wird als die Anwendung selbst.
...
* Im Dialogfeld für die virtuelle Ansicht kann nun eingestellt werden, ob Aslan ein Bildschirmlayout verwenden soll oder nicht.
* Anker ohne Referenz werden in Gecko-Dokumenten nicht mehr als Links erkannt. (#47)
* Der Aslan-Suchbefehl Aslan+Strg+f merkt sich nun den letzten Suchbegriff anwendungsübergreifend. (#53)
...
* Interaktive Aslan-Python-Konsole hinzugefügt, die es Entwicklern ermöglicht, Aslan's Interna zu manipulieren.
...
* Die Skripte "increaserate" und "decreaserate" wurden entfernt. Anwender sollten den Einstellungsring (Strg+Aslan+Pfeiltasten) oder das Dialogfeld für Stimmen-Einstellungen verwenden.
...
* Das Skript "findPrevious" (Aslan+Umschalt+F3) für die Verwendung in virtuellen Dokumenten hinzugefügt.
* Probleme behoben, wonach Aslan in Gecko-Dialogen (Firefox und Thunderbird) träge reagierte. (#66)
* Aslan-Protokollbetrachter zum menü Extras hinzugefügt.
...
* Problem beim Öffnen von Aslan-Dialogen af einigen Systemen behoben. (#65)
...
* Das Skript moveMouseToNavigator (Aslan+Nummernblock Schrägstrich) bewegt den Mauszeiger nun zur Mitte des Objekts und nicht mehr zur oberen linken Ecke.
...
* Leistung von Aslan in Eingabefeldern verbessert: Aslan sagt nun keinen zusätzlichen Text mehr an, wenn der cursor beim Navigieren in eingabefeldern am Ende angekommen ist.
...
* Willkommensbildschirm hinzugefügt. Der Willkommensbildschirm stellt grundlegende Informationen für neue Aslan-Nutzer zur Verfügung und erlaubt es außerdem, die Dauergroßschreibtaste als Aslan-Taste zu verwenden. Er wir beim ersten Start angezeigt und blebt so lange aktiv, bis er abgeschaltet wird.
...
* Probleme behoben, wonach es zu Fehlern beim Niederhalten mancher Tasten kam, noch bevor Aslan ordnungsgemäß gestartet war.
* Wenn Aslan so konfiguriert wird, dass die Konfiguration beim Beenden gespeichert wird, wird jetzt sichergestellt, dass die Konfiguration beim Beenden von Windows oder beim Abmelden eines Benutzers ordnungsgemäß gespeichert wird.
* Akustisches Aslan-Logo beim Start der Installation hinzugefügt. Dank an Victor Tsaran.
* Aslan sollte jetzt das Symbol im Infobereich beim Beenden ordnungsgemäß entfernen.
* Die Beschriftungen von Standard-Steuerelementen wie "OK" oder "Abbrechen" erscheinen jetzt in der in Aslan eingestellten Sprache - nicht mehr nur in Englisch.
* Das Aslan-Programmsymbol wird jetzt anstelle eines Standardsymbols für die Verknüpfungen im Startmenü und auf dem Desktop verwendet.
...
* Verfolgung des System-Cursors in iAccessible2- und Java-Steuerelementen (wie OpenOffice.org und Lotus Symphony) behoben. Aslan wartet nun ab, bis sich der System-Cursor verschoben hat, anstatt versehentlich falsche Wörter, Zeilen oder Absätze zu lesen. (#119)
* Unterstützung für Akeledit-Steuerelemente hinzgefügt, die z. B. in akelpad 4.0 zu finden sind.
* Aslan hängt sich nun nicht mehr auf, wenn Sie in Lotus Symphony vom Dokument in die Menüleiste wechseln.
* Aslan fährt sich nun nicht mehr im dialog "Programme ändern/entfernen" fest, während eine Deinstallation ausgeführt wird. (#30)
* Aslan fährt sich nun nicht mehr beim Öffnen von Spybot Search & Destroy fest.
...
* Linkliste hinzugefügt (Aslan+F7).
* Verbesserungen am Aslan-Suchdialog (Aslan+Strg+F). Er führt jetzt eine Suche ohne Beachtung der Groß-/Kleinscheibung durch; einige Fokus-relevante Probleme wurden ebenfalls behoben.
* Es ist nun möglich, Text innerhalb virtueller Puffer zu markieren und zu kopieren.
* Die neuen virtuellen Puffer zeigen die Elemente jetzt so an, wie sie auf dem Bildschirm angeordnet sind d. h. sie werden nicht auf separaten Zeilen angeordnet, es sei denn, sie sind auch visuell auf getrennten Zeilen dargestellt. Dieses Verhalten kann mit Aslan+V zur Laufzeit geändert werden.
...
* Es ist nun möglich, Akzentzeichen einzugeben, während Aslan ausgeführt wird.
* Aslan meldet nun einen Wechsel des Tastaturschemas, wenn Sie Alt+Rechte Umschalttaste drücken.
...
* Aslan wird nun mit eSpeak 1.33 ausgeliefert, welches u. a. benannte Varianten zur verfügung stellt. Außerdem wurde das Sprechtempo heraufgesetzt.
...
* Aslan kann nun einen Signalton ausgeben, wenn Sie einen Großbuchstaben eingeben. Ebenso wurde eine Option für das Anheben der Stimme bei Großbuchstaben hinzugefügt.
...
* Aslan beansprucht jetzt nicht mehr so viel speicher, wenn Sie in MSHTML-Sterelemente Text eingeben.
...
* Aslan sollte jetzt nicht mehr ohne Grund Systemspeicher verbrauchen.
* Problem behoben, wonach Aslan dazu neigte, abzustürzen, wenn ein Dos-Fenster mehr als dreimal fokussiert wurde.

### Tastaturbefehle

* Aslan+Umschalt+Nummernblock6 und Aslan+Umschalt+Nummernblock4 bewegen den Navigator rekursiv zum nächsten/vorigen Objekt. Dies macht es möglich, allein mit diesen zwei Tastaturbefehlen eine Anwendung zu erkunden, ohne in der Objekthierarchie auf- und abzusteigen.
* Sie können die Stimmen-Einstellungen nun ändern, ohne das Dialogfeld öffnen zu müssen. Hierzu dient der Synthesizer-Einstellungsring, mit Aslan+Strg+Pfeil links/rechts können Sie zwischen den Einstellungen wechseln. Die Einstellungen werden mit Aslan+Strg+Pfeil auf/ab geändert.
* Befehl zum Ansagen des markierten Textes in Eingabefeldern (Aslan+Umschalt+Pfeil auf) hinzugefügt
* Einige Befehle, die Text sprechen, können nun zweimal ausgeführt werden, um den entsprechenden Text zu buchstabieren.
* Die Dauergroßschreibtaste, die Einfügetaste der erweiterten Tastatur sowie die Einfügetaste des Nummernblocks können jetzt als Aslan-Tasten eingestellt werden. Wenn eine dieser Tasten zweimal gedrückt wird, wird der Tastendruck an das Betriebssystem weitergereicht, sodass die Taste die normale Funktion ausführt. Die Aslan-Taste kann im Dialogfeld für Tastatur-Einstellungen geändert werden.

### Unterstützung von Anwendungen

* Die Unterstützung für Firefox3 und Thunderbird 3 wurde verbessert. Die Ladezeiten wurden um den Faktor 30 verkürzt, ein Bildschirm-Layout wird jetzt standardmäßig benutzt, das jedoch mit Aslan+V deaktiviert werden kann. Eine Linkliste (Aslan+F7) wurde hinzugefügt, der Suchdialog (Strg+Aslan+F) arbeitet nun ohne Berücksichtigung der Groß- und Kleinschreibung, die Unterstützung für dynamische Inhalte wurde verbessert, Das Markieren und Kopieren von Text ist nun möglich.
...
* Aslan sagt nun den markierten Text in Eingabefeldern an, so bald diese den Fokus erhalten. Falls kein Text markiert ist, wird - wie gewohnt - die aktuelle Zeile angesagt.
* Beim Abspielen von Signaltönen für Fortschrittsbalken ist Aslan jetzt wesentlich umsichtiger.

### Benutzeroberfläche

* Das Aslan-Hauptfenster wurde durch ein Popup-Menü ersetzt.
* Die Einstellungen zur Benutzeroberfläche wurden in "Allgemeine Einstellungen" umbenannt. Das Dialogfeld enthält außerdem ein Kombinationsfeld für den Protokollumfang, mit dem eingestellt werden kann, welche Informationen im Aslan-Protokoll festgehalten werden soll. Die Protokoll-Datei wurde zudem von "debug.log" in "aslan.log" umbenannt.
...
* Aslan bringt jetzt den integrierten Synthesizer eSpeak mit, der von Jonathan Duddington entwickelt wurde. Espeak ist Schnell, klein und unterstützt viele unterschiedliche Sprachen. SAPI-Sprachen werden zwar immer noch unterstützt, eSpeak wird jedoch standardmäßig verwendet.
...
* Skript zum Ein-/Ausschalten der Signaltöne für Fortschrittsbalken hinzugefügt (Aslan+U).
...
* Wichtige Teile des Quellcodes wurden neu geschrieben, um etliche Probleme (u. a. mit den Aslan-Dialogen und der Benutzeroberfläche) zu lösen.
...
* Wenn eine Aslan-Instanz gestartet wird, während eine andere bereits läuft, wird die neu hinzugekommene Aslan-Instanz sofort wieder beendet. Dies behebt Probleme, wonach das System sehr instabil wird, sobald mehrere Instanzen von Aslan laufen.
* Der Titel der Aslan-Benutzeroberfläche wurde von "Aslan-Benutzeroberfläche" in Aslan geändert.
* Problem behoben, wonach das Drücken der rücktaste am Anfang einer Zeile Fehler verursacht hat.
* Skript zur ansage des akkustatus bei Notebooks hinzugefügt (Aslan+Umschalt+B)
* Neuer Synthesizer "Keine Sprache" hinzugefügt. Dieser Treiber bringt Aslan komplett zum Schweigen. Der Treibe könnte zusammen mit Braillezeilen sinnvoll sein, sobald die Braille-Unterstützung funktioniert.
...
* Problem behoben, wonach entweder die Sprache abbrach oder Aslan abstürzte, sobald eine Eingabeaufforderung geöffnet wurde.
* Wenn unter Windows eine bestimmte Sprache eingestellt ist, die Aslan unterstützt, wird er diese auch automatisch benutzen. Die Sprache kann aber trotzdem noch manuell geändert werden.
* Skript "toggleReportDynamicContentChanges" (Aslan+5) hinzugefügt. Dies schaltet die ansage von neu erscheinendem Text oder dynamischen Inhalten um. Momentan funktioniert das nur in dos-Fenstern.
* Skript "toggleCaretMovesReviewCursor" (Aslan+6) hinzugefügt. Hiermit wird festgelegt, ob der Aslan-Cursor automatisch dem System-Cursor folgen soll. Dies kann beim Lesen von DOS-Fenstern sinnvoll sein, deren Inhalt schnell aktualisiert wird.
* Skript "toggleFocusMovesNavigatorObject" (Aslan+7) hinzugefügt. Hermit legen Sie fest, ob der Navigator automatisch nachgezogen werden soll, wenn sich der Fokus ändert.
* Dokumentation in einigen Sprachen hinzugefügt (u. a. Französisch, Spanisch und Finnisch.
* Entwicklerdokumentation aus den Binären Distributionen von Aslan entfernt, im Quellcode ist sie aber immer noch enthalten.
...
* Skript "passNextKeyThrough" (Aslan+F2) hinzugefügt. Wenn Sie diese Tastenkombination drücken, können Sie danach eine Tastenkombination drücken, die durch Aslan durch an Windows weitergereicht werden soll. Dies ist sinnvoll, Wenn Sie eine Tastenkombination an das Betriebssystem durchreichen wollen, die normalerweise durch Aslan reserviert wird.
* Aslan fährt sich nun nicht mehr eine Minute und länger fest, wenn sehr große Dokumente in ms word geöffnet werden.
...
* Wenn Aslan mit einer Sprachausgabe gestartet wird, die nicht existiert, wird zunächst versucht, SAPI5 zu laden. Schlägt auch dies fehl, so wird keine sprache benutzt.
...
* Aslan fragt nun nach, ob nach einem Sprachenwechsel die Konfiguration gespeichert und neu gestartet werden soll. Ein Neustart von Aslan ist nach einem sprachenwechsel notwendig.
